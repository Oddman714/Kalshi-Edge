...
async function startBot() {
  ...
  _botStarted = true;
  S.isRunning = true;
  log('🚀 KalshiBot v10 started — running pre-flight sync...');
  ...
  try{
    ...
    es.addEventListener('notify',function(e){
      try{
        ...
        var m=d.msg||'';
        var type=/🟢|WIN|profit/i.test(m)?'win':/🔴|LOSS|error/i.test(m)?'loss':/🧠|Brain|signal/i.test(m)?'brain':/✅|Order|trade/i.test(m)?'trade':/warn/i.test(m)?'warn':'info';
        pushFeed(m,type);
        if(/🟢|🔴|✅|❌|🎯|🚀|🧠|WIN|LOSS|Order|Brain/i.test(m))toast(m.slice(0,80),type);
      }catch(ex){}
    });
    es.onopen=function(){
...
function ico(t){
  ...
  if(/NBA|KXNBA/i.test(t))return{e:'🏀',c:'ico-nba'};
  if(/CPI|KXCPI|FED|FEDRATE|INFL/i.test(t))return{e:'📊',c:'ico-cpi'};
  if(/NFL|KXNFL/i.test(t))return{e:'🏈',c:'ico-nba'};
  if(/MLB|KXMLB/i.test(t))return{e:'⚾',c:'ico-nba'};
  if(/NHL|KXNHL/i.test(t))return{e:'🏒',c:'ico-nba'};
  if(/SOL|KXSOL/i.test(t))return{e:'◎',c:'ico-eth'};
  return{e:'~',c:''};
}
    ...
    if(lw&&s.logs&&s.logs.length){
      ...
      if(s.logs[0]){
        ...
        if(newKey!==_prevLogKey){
          ...
          var ftype=lvl0==='error'?'error':lvl0==='warn'?'warn':/WIN|🟢/.test(l0.msg)?'win':/LOSS|🔴/.test(l0.msg)?'loss':/Brain|🧠/.test(l0.msg)?'brain':/Order|✅/.test(l0.msg)?'trade':'info';
...
function doPurge(){
  ...
  fetch(API+'/api/purge-positions',{method:'POST'})
    ...
    .then(function(d){
      toast('🗑 Purged '+d.removed+' stale positions — '+d.remaining+' remain','trade');

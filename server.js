...
const CFG = {
  ...
  kellyFrac:  0.40,        // aggressive kelly — faster compounding
  minEdge:    0.03,        // minimum 3% edge — more trades, more opportunities
  minProb:    0.52,        // minimum 52% confidence — catches more edges
  scanInterval:  20000,   // math scanner: every 20s (faster)
  brainInterval: 120000,  // claude brain: every 2min (more frequent signals)
...
function dynamicMaxPos() {
  ...
  // Tuned for doubling speed: enough diversification to survive bad luck,
  // few enough slots that each bet is sized to move the needle.
  // Math: at $16, 4 slots = $3.30/slot avg, 6 straight losses to floor (ruin risk <2.6%)
  //        at $32, 5 slots = $5.80/slot avg — same ruin protection at higher balance
  if (bal < 8)   return 2;  // <$8:    2 max — concentrated survival mode
  if (bal < 25)  return 4;  // $8-25:  4 slots — SWEET SPOT for doubling $16→$32
  if (bal < 50)  return 5;  // $25-50: 5 slots — scaling up after first double
  if (bal < 100) return 6;  // $50-100: 6 slots
  if (bal < 200) return 7;  // $100-200: 7 slots
  return 8;                 // $200+:  8 slots, fully diversified
}
...
async function syncPositions() {
  try {
    ...
    // Purge ghost positions: bot-placed positions no longer open on Kalshi
    const liveTickerSet = new Set(livePositions.map(p => p.ticker));
    const botPositions = S.openPositions.filter(p => !p.fromKalshi);
    const validBotPositions = botPositions.filter(p => {
      const stillOpen = liveTickerSet.has(p.ticker);
      if (!stillOpen) log('Ghost purged: ' + p.ticker + ' no longer open on Kalshi');
      return stillOpen;
    });
    const validBotTickers = new Set(validBotPositions.map(p => p.ticker));
    const freshKalshi = livePositions.filter(p => !validBotTickers.has(p.ticker));
    S.openPositions = [...validBotPositions, ...freshKalshi];
    const ghostsRemoved = botPositions.length - validBotPositions.length;
    if (ghostsRemoved > 0) log('Purged ' + ghostsRemoved + ' ghost positions — slots freed');
...
async function getTopMarkets() {
  try {
    ...
    const seriesList = [
      // ── CRYPTO (daily + weekly) ──────────────────────────────────────
      'KXBTCD','KXBTCW',           // Bitcoin daily/weekly
      'KXETHUSD','KXETH',          // Ethereum
      'KXSOL','KXSOLUSDT',         // Solana — high volatility = edge
      'KXXRP','KXXRPUSDT',         // XRP — massive Kalshi volume
      'KXDOGE',                    // Dogecoin — volatile, short-duration
      'KXBNB',                     // BNB
      // ── SPORTS ───────────────────────────────────────────────────────
      'KXNBAGAME','KXNBAPLAYOFFS', // NBA games + playoff series
      'KXMLBGAME',                 // MLB games
      'KXNHLGAME',                 // NHL games
      'KXNFLGAME',                 // NFL games
      'KXMMA',                     // UFC/MMA — resolves same night, great edge
      'KXSOCCER','KXMLS',          // Soccer/MLS
      'KXGOLF',                    // PGA — Claude knows player stats well
      'KXTENNIS',                  // Grand slams
      // ── ECONOMICS ────────────────────────────────────────────────────
      'FED','FEDRATE',             // Fed rate decisions
      'INFL','KXCPI','KXPCE',      // Inflation: CPI + PCE
      'KXUNEMPLOYMENT',            // Jobs report — high volume day-of
      'KXGDP',                     // GDP growth
      'KXOIL','KXGOLD',            // Commodities — moving hard right now
      // ── POLITICS (HOT right now) ─────────────────────────────────────
      'KXPREZ','KXSENATE','KXHOUSE',
      'KXTARIFF','KXTRADE',        // Tariff/trade war — most active market
      'KXAPPROVAL',                // Presidential approval
      'KXIMMIGRATION',             // Immigration policy
      'KXSC',                      // Supreme Court decisions
      // ── EQUITIES ─────────────────────────────────────────────────────
      ...(isMarketHours ? [
        'INXD','NASDAQ100D',       // S&P + Nasdaq daily
        'KXSPX500',                // S&P 500 level markets
        'KXEARNINGS','KXTECH',     // Earnings beats — AI edge here
        'HIGHNY','HIGHMIA','HIGHCHI','HIGHLA', // Weather (temp) markets
      ] : []),
      ...(!isMarketHours ? [
        'INXW','NASDAQ100W',       // Weekly equity markets
        'KXSPX500',                // S&P level (open 24/7)
      ] : []),
    ];
    ...
    const markets = allMarkets.filter(m => {
      ...
      // STRICT EDGE FILTER: exclude near-certain outcomes (1-5c or 95-99c) — zero edge, zero return
      if (yesAsk <= 0.15 || yesAsk >= 0.85) return false; // 15¢-85¢ only — penny contracts pay $0
      // Exclude illiquid markets with no real bid
      if (yesBid <= 0.01) return false;
      // Exclude markets where spread > 25c — too costly to enter/exit
      if ((yesAsk - yesBid) > 0.25) return false;
    ...
    log(`After filter: ${markets.length} tradeable markets (15c-85c, vol>=50, spread<=25c)`);
...
async function runBrain(topMarkets) {
  ...
  const prompt = `{"task":"generate_trade_signals","rules":["respond with ONLY the JSON object below","no explanation","no preamble","start with {","end with }"],"context":{"MISSION":"DOUBLE ${(S.realBalance||S.balance).toFixed(2)} to ${((S.realBalance||S.balance)*2).toFixed(2)} as fast as safely possible. RULES: 1) Only trade YES contracts priced 15¢-85¢ — penny contracts pay nothing. 2) Prioritize YES at 20-40¢ = 2.5-5x payout ratio. 3) Size minimum $1.50 per trade. 4) Prioritize markets resolving within 24h. 5) Be aggressive on any edge above 52% confidence","mode":"${CFG.dryRun?'paper':'live'}","balance":${(S.realBalance||S.balance).toFixed(2)},"peak":${S.peakBalance.toFixed(2)},"todayPnl":${S.todayPnl.toFixed(2)},"totalPnl":${S.totalPnl.toFixed(2)},"wins":${S.wins},"losses":${S.losses},"winRate":"${S.wins+S.losses>0?((S.wins/(S.wins+S.losses))*100).toFixed(0)+'%':'new'}","openSlots":${dynamicMaxPos() - S.openPositions.length},"maxPos":${dynamicMaxPos()},"openPositions":${JSON.stringify(S.openPositions.map(p=>({ticker:p.ticker,side:p.side,entryPrice:p.entryPrice,hoursOpen:Math.round((Date.now()-p.openedAt)/3600000)})))},"recentMemory":${JSON.stringify(S.brainMemory.slice(-5))}},"markets":${JSON.stringify(topMarkets.slice(0,10).map(m=>({ticker:m.ticker,title:m.title,yesAsk:Math.round(m.yesAsk*100),yesBid:Math.round((m.yesBid||0)*100),volume:Math.round(m.volume),score:parseFloat(m.score.toFixed(3)),hoursLeft:m.hoursLeft,closesIn:m.closeTime?Math.round((new Date(m.closeTime)-Date.now())/3600000)+'h':'unknown'})))},"strategy":{"priority1":"SHORT-DURATION: Markets closing in 1-24h score highest — fast resolution = fast compounding","priority2":"HIGH-EDGE: Search web for breaking news that market hasn't priced in yet — mispricing = edge","priority3":"ASYMMETRIC: YES at 20-40¢ = 2.5-4x payout if wins — seek these when confidence is >55%","priority4":"MOMENTUM: Live sports in 2nd half, crypto at key levels, political events with new data","avoid":"Markets >7 days out with no clear catalyst — slow money","sizing":"Be aggressive: use 30-40% of available balance per high-conviction trade (Kelly recommends it)"},"instructions":"1) Use web search to find TODAY's breaking news, sports scores, crypto prices for each ticker. 2) Identify where the market price is WRONG vs reality. 3) Favor markets with hoursLeft < 24 heavily. 4) If a YES is at 30¢ but you're 60% confident it wins, that's a massive edge — take it. 5) Apply Kelly criterion sizing. Output required_json_format only.","required_json_format":{"signals":[{"ticker":"string","side":"YES or NO","confidence":0.62,"edge":0.10,"reasoning":"one sentence citing specific news/data/score","kellySize":4.50,"hoursToClose":6}],"marketSummary":"one sentence on best opportunity","skipReason":"only if truly no edge found anywhere"}}`;
...
function kellySize(prob, price) {
  ...
  const bal = S.realBalance > 0 ? S.realBalance : S.balance;
  // At small balances, concentrate more per trade for faster compounding
  // At $16 bal: allow up to 40% per slot | At $100+: cap at 20% per slot
  // Playable = balance above $3 floor — size against what we can actually lose
  const playable = Math.max(0, bal - 3.00);
  const maxPct = bal < 20 ? 0.45 : bal < 35 ? 0.35 : bal < 60 ? 0.28 : Math.min(0.22, 1 / slots);
  const maxBetFloor = Math.min(bal * maxPct, playable * 0.60); // never risk more than 60% of playable
  const maxBet = maxBetFloor;
  const minBet = Math.max(1.50, Math.min(2.00, bal * 0.08)); // min $1.50 — every trade must move the needle
...
async function executeTrade(sig) {
  ...
  // HARD GUARD: reject any signal outside 5c-95c — no edge, no return
  const sigPrice = sig.side === 'YES' ? sig.confidence : (1 - sig.confidence);
  if (sigPrice <= 0.05 || sigPrice >= 0.95) {
    log(`Skip ${sig.ticker}: price ${(sigPrice*100).toFixed(0)}c outside edge zone (5c-95c)`);
    return;
  }
  // HARD GUARD: minimum /bin/sh.50 expected return — avoid penny plays
  const expectedReturn = (1 - sigPrice) / sigPrice * sigPrice; // simplified EV
  if (sig.kellySize && sig.kellySize < 1.50) {
    log(`Skip ${sig.ticker}: kelly size $${sig.kellySize.toFixed(2)} too small (<$1.50) — no meaningful return`);
    return;
  }
...
function applyState(s){
  ...
  document.getElementById('cfgBrainInt').textContent=s.cfg?((s.cfg.brainInterval||120000)/60000)+'min':'2min';
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/api/purge-ghosts' && req.method === 'POST') {
    kalshi('GET', '/portfolio/positions', null, { limit: '100' })
      .then(r => {
        const positions = r.market_positions || r.positions || [];
        ...
        S.openPositions = S.openPositions.filter(p => {
          const keep = liveTickers.has(p.ticker);
          if (!keep) log('Ghost purged via API: ' + p.ticker);
          return keep;
        });
        const purged = before - S.openPositions.length;
        saveState();
        log('Ghost purge complete: removed ' + purged + ', ' + S.openPositions.length + ' remain');
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ purged, remaining: S.openPositions.length }));
      })
      .catch(e => {
        res.writeHead(500);

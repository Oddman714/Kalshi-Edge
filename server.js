...
async function getTopMarkets() {
  try {
    ...
    const scored = markets.map(m => {
      ...
      const edgeScore   = Math.abs(yesAsk - 0.5) * 1.5;
      // Upside multiplier: cheaper YES = bigger payout if wins
      const upsideScore = yesAsk < 0.40 ? 1.2 : yesAsk < 0.60 ? 1.0 : 0.8;
...
function kellySize(prob, price) {
  ...
  const maxPct = bal < 25 ? 0.40 : bal < 50 ? 0.30 : Math.min(0.25, 1 / slots);
  const maxBet = bal * maxPct;
  const minBet = Math.max(0.50, Math.min(1.00, bal * 0.05)); // min 5% of balance
...
async function executeTrade(sig) {
  ...
  if (sig.kellySize && sig.kellySize < 0.50) {
    log(`Skip ${sig.ticker}: kelly size $${sig.kellySize.toFixed(2)} too small — no meaningful return`);
    return;
  }

...
async function validate() {
  ...
  try {
    ...
    if (r.status === 200) {
      ...
      if (!C.paper && b > 0) { S.bal = b; S.peak = Math.max(S.peak, b); }
      authOk();
      log(`Auth OK — balance $${(b / 100).toFixed(2)}`);
      return true;
    }
...
async function monitor() {
  if (C.paper || !S.positions.length) return;
  try {
    ...
    const live = new Set((r.data.market_positions || []).map(p => p.ticker_name));
    for (const pos of [...S.positions]) {
      if (live.has(pos.ticker)) continue;
      const pnl = -pos.size; // conservative until Kalshi reports realized
      ...
      log(`Settled ${pos.ticker}: ${fmt(pnl)}`);
      await tg(`${pnl > 0 ? '✅' : '❌'} <b>Settled: ${pos.ticker}</b>\nP&L: ${fmt(pnl)} | All-time: ${fmt(S.totPnl)}`);
    }
    await syncBal();
    save();
...
function startBot() {
  ...
  log('Bot started');
  scan();
  _sc = setInterval(async () => { midnight(); await scan(); await monitor(); if (breakers()) await execute(); }, C.scanSec * 1000);
  _br = setTimeout(() => { brain(); _br = setInterval(brain, C.brainSec * 1000); }, 20000);
  _pl = setInterval(() => { S.pnlHist.push({ ts: Date.now(), bal: usd(), pnl: S.totPnl }); if (S.pnlHist.length > 500) S.pnlHist.shift(); }, 300000);
  save();
}
function stopBot() {
  S.on = false;
  clearInterval(_sc); clearTimeout(_br); clearInterval(_br); clearInterval(_pl);
  _sc = _br = _pl = null;
...
app.listen(PORT, async () => {
  log('════════════════════════════════');
  log(' KALSHI EDGE v7');
  log(` ${C.paper ? 'PAPER' : '⚡ LIVE'} | Kelly:${C.kelly} | Edge:${(C.edgeLive*100).toFixed(0)}%`);
  log(` maxBet:$${C.maxBet} | dailyStop:$${C.dailyStop} | dd:${(C.ddLimit*100).toFixed(0)}%`);
  log(` scan:${C.scanSec}s | brain:${C.brainSec}s`);
  log(` TG: ${C.tgTok ? '✓ token set' : '✗ NO TOKEN'} | chat: ${C.tgChat ? '✓ set' : '✗ NO CHAT'}`);
  log(` Claude: ${C.claude ? '✓' : '✗'} | Kalshi: ${C.keyId ? '✓' : '✗'}`);
  log('════════════════════════════════');

  // Test Telegram immediately on boot if configured
  if (C.tgTok && C.tgChat) {
    log('Testing Telegram...');
    await tg('🔧 Kalshi Edge v7 booting — testing Telegram connection');
  }

  if (!C.keyId || !C.pem) {
    log('STANDBY — add KALSHI_API_KEY_ID + KALSHI_PRIVATE_KEY and redeploy');
    return;
  }

  const ok = await validate();
  if (!ok) { log('STANDBY — fix credentials and redeploy'); return; }

  const msg = `🟢 <b>Kalshi Edge v7 online</b>\nMode: ${C.paper ? 'Paper' : '⚡ LIVE'}\nBalance: $${usd().toFixed(2)}\nKelly: ${C.kelly} | Edge: ${(C.edgeLive*100).toFixed(0)}%\nDaily stop: $${C.dailyStop} | Drawdown: ${(C.ddLimit*100).toFixed(0)}%`;
  await tg(msg);

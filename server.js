...
async function runCryptoFastPath(topMarkets) {
  ...
  for (const m of cryptoMarkets) {
    ...
    const isDirectionMarket = (
      ...
      !m.title.match(/\$[\d]/)  // no dollar strike = direction market
    );
    if (isDirectionMarket) {
      const isBTCdir = /kxbtc/i.test(m.ticker);
      const isETHdir = /kxeth/i.test(m.ticker);
      const livePrice2 = isBTCdir ? prices.btc : isETHdir ? prices.eth : null;
      if (!livePrice2) { log(`FastPath dir: no price for ${m.ticker} — skip`); continue; }

      // Detect "up" or "down" from title
      const titleLower = (m.title || '').toLowerCase();
      const isUpMarket = /price up/i.test(m.title);   // YES = price goes up
      const isDownMarket = /price down/i.test(m.title); // YES = price goes down

      // Use Binance 5m kline to get trend — fetch last 3 candles
      let trendConf = 0, trendDir = null;
      try {
        const coinSymbol = isBTCdir ? 'BTCUSDT' : 'ETHUSDT';
        const kr = await req(
          `https://api.binance.com/api/v3/klines?symbol=${coinSymbol}&interval=1m&limit=5`,
        ...
        if (kr.status === 200 && Array.isArray(kr.body) && kr.body.length >= 3) {
          ...
          // Strong uptrend: last 3 candles trending up AND current price above last close
          if (trendPct > 0.001 && currentVsClose >= 0) {
            trendDir = 'up';
            trendConf = trendPct > 0.003 ? 0.82 : 0.76; // stronger trend = higher conf
          } else if (trendPct < -0.001 && currentVsClose <= 0) {
            trendDir = 'down';
      ...
      const dirSig = {
        ...
        hoursToClose: m.hoursLeft,
        source: 'fastpath',
        reasoning: `⚡ FastPath [DIR] ${coinLabelD} trend=${trendDir} | ${dirSide} @${(dirPrice*100).toFixed(1)}c = ${dirPayout.toFixed(2)}x | conf=${(trendConf*100).toFixed(0)}%`,
      };
...
STEP 0 - DIRECTION MARKETS (title says "price up in next X mins?" or "price down"):
- These have NO dollar strike. Check if the 1-minute trend of BTC/ETH is clearly up or down.
- "BTC price up in next 15 mins?" → YES if BTC has been trending UP last 3-5 minutes
- "ETH price down in next 15 mins?" → YES if ETH has been trending DOWN
- Use LIVE PRICES provided. If current price > price 5min ago = uptrend. Opposite = downtrend.
- Only signal if trend is CLEAR and consistent (not mixed). Confidence: clear=0.78, strong=0.82
- These are SHORT windows (<30min) so trend must be obvious. If uncertain → SKIP.
- netPayout must be >= 1.5x for direction trades. Signal up to 2 direction trades.

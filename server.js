...
async function scan(){
  ...
  try{
    ...
    if(!markets.length){log('Scan: 0 markets returned');return;}

    // Log raw field names on first scan with 0 candidates (diagnostic)
    if(S.cands.length===0&&markets.length>0){
      log('RAW FIELDS: '+Object.keys(markets[0]).join(', '));
      log('RAW SAMPLE: '+JSON.stringify(markets[0]).slice(0,300));
    }

    const held=new Set(S.positions.map(p=>p.ticker));
    const now=Date.now();
    let rej={ticker:0,time:0,price:0,pass:0};
    ...
    const scored=markets.filter(m=>{
      ...
      // Try all close time field names Kalshi uses across API versions
      ...
      if((closeMs-now)/3.6e6>720){rej.time++;return false;}
      ...
      const hrs=((new Date(closeRaw).getTime()-now)/3.6e6).toFixed(1);
      ...
      return{ticker,title,yes:y,no:100-y,vol:m.volume||0,hrs,cat:m.category||m.event_category||'market',mom,score,hasVol:(m.volume||0)>0?1:0};
    });

    scored.sort((a,b)=>b.hasVol-a.hasVol||b.score-a.score);
    S.cands=scored.slice(0,25);
    log('Scan: '+markets.length+' | pass='+rej.pass+' rej_time='+rej.time+' rej_price='+rej.price+' | candidates='+S.cands.length);
    if(S.cands.length>0){const t=S.cands[0];log('Top: '+t.ticker+' yes='+t.yes+'¢ '+t.hrs+'h vol='+t.vol);}
    else if(markets.length>0)log('All rejected — check RAW SAMPLE above for correct field names');
...
✓ PREFER: Fed/rates, elections, economic data, crypto, single-event markets
✓ No duplicate topics

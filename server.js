...
const CFG = {
  ...
  version: 'v20.4.0-directional-fastpath',
};
...
async function runCryptoFastPath(topMarkets) {
  ...
  for (const m of cryptoMarkets) {
    ...
    if (isDirectionMarket) {
      ...
      log(`⚡ FastPath [DIR]: ${m.ticker} ${side} | conf ${(confidence*100).toFixed(0)}% | edge ${(edge*100).toFixed(1)}% | momentum ${mom.direction}/${(mom.strength*100).toFixed(0)}%`);
...
async function executeTrade(sig) {
  ...
  let actualCost = (priceCents * contracts) / 100;
  ...
  if (!CFG.dryRun) {
    ...
    if (executableContracts < contracts) {
      const resizedCost = (priceCents * executableContracts) / 100;
      if (resizedCost < 0.50) {

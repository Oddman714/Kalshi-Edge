...
const PHASES = [
  { name: 'SEED',     min: 0,      max: 50,     kelly: 0.40, minEdge: 0.08, minProb: 0.70, maxPos: 4,  label: '🌱 Seed',     desc: '4 slots (3 safe + 1 hourly). 70%+ conf, 2.0x+ crypto / 2.5x+ sports, <36h resolution.' },
...
async function getTopMarkets() {
  try {
    ...
    // ── PRIMARY HOURLY FETCH — all markets closing in next 4h ────────────────
    // This is the key to finding "Bitcoin price today at 9pm EDT" type markets.
    // Uses min/max close_ts to get only fillable, short-window markets.
    try {
      const _nowTs = Math.floor(Date.now() / 1000);
      const hourlyRes = await kalshi('GET', '/markets', null, {
        status: 'open', limit: '500',
        max_close_ts: (_nowTs + 4 * 3600).toString(),  // closes within 4h
        min_close_ts: (_nowTs + 480).toString()          // at least 8 min away
      });
      const hourlyMkts = (hourlyRes.markets || []).filter(m => !m.mve_collection_ticker);
      if (hourlyMkts.length > 0) {
        const _existSet = new Set(allMarkets.map(m => m.ticker));
        const newHourly = hourlyMkts.filter(m => !_existSet.has(m.ticker));
        allMarkets.push(...newHourly);
        log(`Hourly fetch: +${newHourly.length} new markets closing within 4h (${hourlyMkts.length} total found)`);
      } else {
        log('Hourly fetch: 0 markets in 4h window — Kalshi may not support min_close_ts', 'WARN');
      }
    } catch(e) { log('Hourly fetch error: ' + e.message.slice(0,80), 'WARN'); }

    // ── TARGETED BTC/ETH HOURLY SERIES FETCH ────────────────────────────────
    // Fetch the specific BTC/ETH daily series with a close_time cap
    // This guarantees "Bitcoin price today at 9pm EDT" type markets are included
    const nowTs2 = Math.floor(Date.now() / 1000);
    const hourlySeriesFetches = await Promise.allSettled([
      'KXBTCD','KXETHUSD'
    ].map(s => kalshi('GET', '/markets', null, {
      series_ticker: s, status: 'open', limit: '50',
      max_close_ts: (nowTs2 + 4 * 3600).toString(),
      min_close_ts: (nowTs2 + 480).toString()
    })));
    for (const f of hourlySeriesFetches) {
      if (f.status === 'fulfilled') {
        const mkts = f.value.markets || [];
        if (mkts.length > 0) log(`KXBTCD/KXETHUSD 4h window: ${mkts.length} markets`);
        allMarkets.push(...mkts);
      }
    }
    ...
    // CRITICAL: After scoring, re-sort so ALL hourly markets (hoursLeft<2) appear first
    // This guarantees brain sees tonight's BTC/ETH hourly markets before tomorrow's daily
    // Brain picks the first qualifying signal, so order matters enormously
    const hourlyMarkets = scored.filter(m => m.hoursLeft < 2.0 && !_heldSet.has(m.ticker));
    const otherMarkets  = scored.filter(m => m.hoursLeft >= 2.0 || _heldSet.has(m.ticker));
    const reordered = [...hourlyMarkets, ...otherMarkets];
    S.topMarkets = reordered.slice(0, 20);
    if (hourlyMarkets.length > 0) {
      log(`Hourly markets surfaced: ${hourlyMarkets.length} markets with hoursLeft<2 sent to brain first`);
    }
...
4. Restart — position caps now hard-enforced`);
    ...
    const _currentBal = S.realBalance || S.balance;
    const _availCash = S.availableCash > 0 ? S.availableCash : _currentBal;
    // 4th slot: safe if cash after typical bet stays above floor
    const _estBet = Math.min(_availCash * 0.30, 5.00);
    const _cashAfter = _availCash - _estBet;
    const _floorSafe = _cashAfter > 5.50;
    const _effectiveMaxSlots = _floorSafe ? maxPos : Math.max(3, maxPos - 1);
    const hasSlots = S.openPositions.length < _effectiveMaxSlots;

...
const CFG = {
  ...
  version: 'v20.0.0-edge-calibrated-ios',
};
...
async function executeTrade(sig) {
  ...
  if(effectiveEdge < phase.minEdge && !(isCryptoMath && payoutJustifiesEdge)){
    recordSkip(sig, `Calibrated edge ${(effectiveEdge*100).toFixed(1)}% below ${(phase.minEdge*100).toFixed(0)}%`);
  ...
  if(sig.confidence<phase.minProb){recordSkip(sig, `Calibrated confidence ${(sig.confidence*100).toFixed(0)}% below phase min`);log('Skip '+sig.ticker+': conf '+(sig.confidence*100).toFixed(0)+'% < min '+(phase.minProb*100)+'%');return;}
...
<div id="pg-home" class="pg on">
  ...
  <div style="text-align:center;font-size:9px;color:rgba(238,238,245,.16);padding:8px;font-family:monospace">KalshiBot v20 edge calibrated</div>
  <div class="gap"></div>
</div>

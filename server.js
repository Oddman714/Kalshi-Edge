...
async function tg(text) {
  ...
  try { await http(`https://api.telegram.org/bot${C.tgToken}/sendMessage`, { method: 'POST', headers: { 'Content-Type': 'application/json' } }, JSON.stringify({ chat_id: C.tgChat, text, parse_mode: 'HTML' })); } catch (e) {}
}
...
async function scan() {
  ...
  try {
    ...
    const scored = mkts.filter(m => m.status === 'open' || m.status === 'active')
      .map(m => ({ ...m, _s: score(m) })).sort((a, b) => b._s - a._s);
...
async function brain() {
  ...
  const body = JSON.stringify({
    ...
    system: `You are an autonomous Kalshi prediction market trader managing $${Math.max(S.balance/100,C.bankroll).toFixed(2)}.

GOAL: Double the portfolio. Target 55-65% win rate.

RULES:
- Binary: YES=$1, NO=$0. Prices 1-99¢. Buy the underpriced side.
- Min edge: ${C.dryRun ? '3' : (C.edgeMin*100).toFixed(0)}¢
- PREFER: markets closing <48h, high volume, economy/politics/crypto
- EXPLOIT: favourite-longshot bias (>85¢ favourites underpriced, <15¢ longshots overpriced)
- Momentum: rising price + fundamentals = ride it
- AVOID: parlays, sports without edge, weeks-out markets, low-info guesses
- HIGH confidence = big bet. MEDIUM = normal. LOW = skip unless 15%+ edge.
${paperBoost}

YOUR MEMORY:
${mem}

RESPOND: Valid JSON array ONLY. No text, no markdown.
Format: [{"ticker":"X","title":"name","side":"yes"|"no","marketPrice":0.XX,"trueProb":0.XX,"edge":0.XX,"confidence":"high"|"medium"|"low","reasoning":"max 15 words"}]
Empty = []`,
    messages: [{role:'user',content:`TIME: ${new Date().toISOString().slice(0,16)}Z\n\nCANDIDATES:\n${mBlock}\n\nSignal all genuine edge.`}],
  });
  ...
  try {
    ...
    let sigs = [];
    try { sigs = JSON.parse(raw.replace(/```json|```/g, '').trim()); } catch (e1) {
      const match = raw.match(/\[[\s\S]*?\]/);
      if (match) { try { sigs = JSON.parse(match[0]); } catch (e2) {} }
    }
...
async function fallback() {
  ...
  const plays = S.candidates
    ...
    .filter(c => {
      if (c.yes <= 12 && c.score >= 30) return true;   // sell longshot
      if (c.yes >= 88 && c.score >= 30) return true;   // buy favourite
      if (c.mom.includes('↑') && c.yes >= 35 && c.yes <= 65) return true; // momentum
      if (c.mom.includes('↓') && c.yes >= 35 && c.yes <= 65) return true;
      // In paper mode, be more aggressive
      if (C.dryRun && c.score >= 40 && c.yes >= 25 && c.yes <= 75) return true;
      return false;
    })
    .slice(0, C.dryRun ? 2 : 1);

  for (const c of plays) {
    let side, edge;
    if (c.yes <= 12) { side = 'no'; edge = 0.08; }
    else if (c.yes >= 88) { side = 'yes'; edge = 0.06; }
    else if (c.mom.includes('↑')) { side = 'yes'; edge = 0.05; }
    else if (c.mom.includes('↓')) { side = 'no'; edge = 0.05; }
    else { side = c.yes < 50 ? 'no' : 'yes'; edge = 0.04; }

    await trade({ ticker: c.ticker, title: c.title, side, marketPrice: c.yes/100, edge, confidence: 'medium', reasoning: 'Fallback: ' + (c.yes<=12?'longshot bias':c.yes>=88?'favourite bias':c.mom!=='flat'?'momentum':'scanner pick') });
  }
}
...
async function runScan() {
  try {
    ...
    if (S.dailyPnL <= -(C.maxLoss*100)) { log('🛑 Loss limit'); return; }
...
function start() {
  ...
  tg(`🚀 <b>Kalshi Edge v5</b>\n${C.dryRun?'📋 Paper':'⚡ LIVE'}\nScan:${SCAN_SEC}s | Brain:${BRAIN_SEC}s | HB:${HEARTBEAT_SEC}s\nEdge:${C.edgeMin*100}% | Kelly:${C.kelly} | Max:$${C.maxPos}\n${portfolioLine()}`).catch(()=>{});

  runScan();
  scanT = setInterval(() => runScan().catch(e => log('s:' + e.message)), SCAN_SEC * 1000);

  setTimeout(() => {
    runBrain().catch(e => log('b:' + e.message));
    brainT = setInterval(() => runBrain().catch(e => log('b:' + e.message)), BRAIN_SEC * 1000);
  }, 45000); // first brain after 45s

  heartT = setInterval(() => heartbeat().catch(() => {}), HEARTBEAT_SEC * 1000);
  // First heartbeat after 5min
  setTimeout(() => heartbeat().catch(() => {}), 300000);
...
function stop() {
  S.isRunning = false;
  [scanT, brainT, heartT].forEach(t => { if (t) clearInterval(t); });
...
app.get('/api/status', (_, r) => {
  const tw=S.wins+S.paperWins, tl=S.losses+S.paperLosses, tot=tw+tl;
  r.json({
    v: 5, isRunning: S.isRunning, dryRun: C.dryRun, balance: S.balance, bankroll: C.bankroll,

...
async function executeTrade(sig) {
  ...
  if (CFG.dryRun) {
    ...
    try {
      ...
      log(`✅ LIVE ORDER CONFIRMED: ${sig.ticker} ${sig.side} x${contracts} @ ${price}c | order_id=${confirmedOrderId} | payout ${netPayoutRatio.toFixed(2)}x | src:${sig.source||'brain'}`);
...
async function startKalshiWS() {
  ...
  kalshiWS.on('open', () => {
    ...
    // Subscribe to orderbook deltas for all tracked crypto series
    // and user fill events (no extra auth needed — already signed the connection)
    const subscribeMsg = JSON.stringify({
      id: 1,
      cmd: 'subscribe',
      params: {
        channels: ['orderbook_delta', 'user', 'fill'],
      }
    });
  ...
  kalshiWS.on('message', async (data) => {
    try {
      ...
      // Real-time orderbook delta on crypto markets → fire FastPath immediately
      if (type === 'orderbook_delta') {
        const ticker = parsed.market_ticker || parsed.ticker || '';

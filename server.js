...
async function syncPositions() {
  try {
    ...
    log(`Synced ${livePositions.length} positions from Kalshi (${freshKalshi.length} new) | state=${S.openPositions.length}/${maxAllowed}`);
...
💰 $${(S.realBalance||S.balance).toFixed(2)} | ${S.openPositions.length}/${maxPos} slots | ${markets.length} mkts
🧠 Brain ${brainReady ? 'READY' : 'in ' + secsToNext + 's'}${S.openPositions.length > 0 ? '\n📋 ' + S.openPositions.map(p => p.ticker.split('-').slice(-1)[0] + ' ' + p.side).join(' | ') : ''}`);
    }
...
function renderPos(positions) {
  ...
  w.innerHTML = positions.map(function(p) {
    ...
    var payoutRatio = ep > 0 ? ((100/ep)-1).toFixed(2) : '--';
    return '<div class="pos-card"><div class="pos-inner">'
      ...
      +'<span class="pos-meta">'+ct+' ct @ '+ep+'c</span>'
      +'<span class="pos-meta" style="color:var(--gold)">'+payoutRatio+'x payout</span></div></div>'
...
function renderMkts() {
  ...
  w.innerHTML = KB.state.topMarkets.slice(0, 15).map(function(m) {
    ...
    var payoutX = yp > 0 ? ((100/yp)-1).toFixed(1) : '--';
    return '<div class="mkt-card">'
      ...
      +'<div class="mkt-vol">'+payoutX+'x payout | vol '+(m.volume||0).toLocaleString()+'</div></div></div>'
...
function applyState(s) {
  ...
  try {
    ...
    st('brainBadge','Brain #'+(s.brainCount||0)+' | API: $'+_n(s.estimatedApiSpend).toFixed(2));
    ...
    st('cfgBrain',_n(s.brainCount).toLocaleString()+(_n(s.brainCount)===0?' (resets on redeploy)':''));

...
async function scan(){
  ...
  try{
    ...
    // Kalshi support confirmed: one key covers ALL markets, no access restrictions.
    // Issue is query filtering. Fix: fetch multiple pages via cursor pagination
    // to get past the KXMVE markets that appear first.
    // Also: do NOT filter by event_ticker — that accidentally limits to KXMVE.
    ...
    // Paginate through ALL markets until we have enough or run out
    while(page<maxPages){
      ...
      if(r.status!==200){log('Scan page '+page+' HTTP '+r.status);break;}
      authOk();
      const mkts=r.data.markets||[];
      if(!mkts.length)break;
      allMarkets.push(...mkts);
      log('Page '+page+': '+mkts.length+' markets (total: '+allMarkets.length+')');
    ...
    if(!allMarkets.length){log('Scan: 0 markets from all pages');return;}
    ...
    log('TOTAL: '+allMarkets.length+' markets | '+withVol+' with volume>0');

    // Top 5 by volume — critical diagnostic
    const byVol=[...allMarkets].sort((a,b)=>getVol(b)-getVol(a)).slice(0,5);
    byVol.forEach((m,i)=>{
      const t=m.ticker||m.ticker_name||'?';
      log('VOL'+i+': $'+getVol(m).toFixed(0)+' | '+t.slice(0,40)+' | price='+getPrice(m)+'¢');
    });

    const held=new Set(S.positions.map(p=>p.ticker));
    const now=Date.now();

    // Only skip KXMVE multivariate composites — allow all other market types
    const scored=allMarkets.filter(m=>{
      const ticker=m.ticker||m.ticker_name||'';
      if(!ticker||held.has(ticker))return false;
      if(ticker.startsWith('KXMVE'))return false; // skip multivariate composites only
    ...
    log('Candidates: '+S.cands.length+' liquid markets found');
    if(S.cands.length>0){
      const t=S.cands[0];
      log('Best: "'+t.title+'" yes='+t.yes+'¢ vol=$'+t.vol+' '+t.hrs+'h ['+t.ticker+']');
    }else{
      log('Zero candidates after filtering. withVol='+withVol+' total markets='+allMarkets.length);
      if(withVol>0){
        const sample=allMarkets.filter(m=>getVol(m)>0)[0];
        log('Sample liquid market: '+JSON.stringify({ticker:sample.ticker,vol:getVol(sample),title:String(sample.title||'').slice(0,50)}));
      }
    }

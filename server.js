...
const PHASES = [
  { name: 'SEED',     min: 0,      max: 50,     kelly: 0.55, minEdge: 0.05, minProb: 0.56, maxPos: 5,  label: '🌱 Seed',     desc: 'Aggressive compounding -- target high-confidence fast markets' },
  { name: 'SPROUT',   min: 50,     max: 200,    kelly: 0.45, minEdge: 0.05, minProb: 0.57, maxPos: 6,  label: '🌿 Sprout',   desc: 'Building -- stay aggressive, favor 55-80c YES contracts' },
...
async function syncPositions() {
  try {
    ...
    // Respect maxPos — don't count Kalshi-synced positions beyond our limit
    // This prevents 6/5 overcount from double-tracking
    const maxAllowed = dynamicMaxPos();
    const combined = [...validBot, ...freshKalshi];
    S.openPositions = combined.slice(0, maxAllowed);
    if (combined.length > maxAllowed) {
      log('Position cap: trimmed ' + (combined.length-maxAllowed) + ' excess synced positions to respect maxPos='+maxAllowed);
    }
...
async function getTopMarkets() {
  try {
    ...
    const markets = allMarkets.filter(m => {
      ...
      if (yesAsk <= 0.14 || yesAsk >= 0.91) return false;
    ...
    log(`After filter: ${markets.length} tradeable (14c-91c, vol>=25, spread<=28c)`);
    ...
    const scored = markets.map(m => {
      ...
      // Duration scoring — fast resolution = faster compounding = priority
      // Long-dated (>7d) positions lock capital and slow doubling speed
      const durationScore = hoursLeft < 0.5  ? 0.05  // too close, skip
        : hoursLeft < 2   ? 0.90              // excellent: resolves in hours
        : hoursLeft < 6   ? 1.20              // BEST: today's game/data
        : hoursLeft < 12  ? 1.15              // great: tonight
        : hoursLeft < 24  ? 1.10              // good: tomorrow
        : hoursLeft < 48  ? 0.60              // ok: 2 days
        : hoursLeft < 96  ? 0.20              // slow
        : hoursLeft < 168 ? 0.08              // bad: >4 days
        : 0.02;                                // terrible: >7 days, blocks compounding
      ...
      // For small account doubling: favor markets where payout ratio is 2x+
      // High price (70c+) = small payout, avoid. Sweet spot = 25-55c with edge.
      const priceScore = yesAsk >= 0.30 && yesAsk <= 0.55 ? 1.4  // best: 2x+ payout potential
        : yesAsk >= 0.55 && yesAsk <= 0.72 ? 1.1  // ok: modest payout
        : yesAsk >= 0.20 && yesAsk < 0.30 ? 1.0   // risky but high payout
        : yesAsk > 0.72 ? 0.6                       // bad: tiny payout, avoid
        : 0.5; // < 20c -- longshot bias, avoid
...
async function runBrain(topMarkets) {
  ...
  const prompt = `{"task":"generate_trade_signals","rules":["respond ONLY with valid JSON","no preamble","start with {","end with }"],"context":{"MISSION":"Double $${_bal.toFixed(2)} to $500K as fast as safely possible. We have ${dynamicMaxPos()-S.openPositions.length} OPEN SLOTS. Fill them with the BEST available edge — NOT markets we already hold.","ALREADY_HELD":"${_heldStr} — DO NOT signal any of these tickers. Find DIFFERENT markets.","CURRENT_STATE":"Balance: $${_bal.toFixed(2)} | Cash: $${S.availableCash.toFixed(2)} | Open: ${S.openPositions.length}/${dynamicMaxPos()} slots | Phase: ${phase.name} | ${doublingsNeeded()} doublings to $500K","PAYOUT_STRATEGY":"For doubling a small portfolio FAST, PAYOFF RATIO matters as much as edge%. Target: (a) Markets where you have 60%+ confidence but market prices 25-50c = 1x-3x payout on confidence gap. (b) Avoid 65-80c bets - too little upside. (c) Best bets: confidence 65%, market 35c = net edge 27%, payout $1.86 per $1 risked. (d) Fee: 4.5% on wins. Net edge formula: (confidence - marketPrice) - (confidence * 0.045). MINIMUM: net edge >= ${(phase.minEdge*100).toFixed(0)}% AND payout_ratio >= 1.5x (prefer 2x+)","DIVERSITY_RULE":"Actively seek variety: sports (live scores), crypto (price vs strike), econ data, weather, politics. Do NOT keep re-analyzing markets we already hold."},"markets":${JSON.stringify(topMarkets.slice(0,10).map(m=>({ticker:m.ticker,title:m.title,yesAsk:+(m.yesAsk*100).toFixed(1),yesBid:+((m.yesBid||0)*100).toFixed(1),volume:Math.round(m.volume),hoursLeft:+m.hoursLeft.toFixed(1),alreadyHeld:_heldTickers.includes(m.ticker)})))},"search_strategy":{"searches":10,"PRIORITY_ORDER":["1. Any market NOT in alreadyHeld=true list — search those FIRST","2. Live sports scores RIGHT NOW — check scores for any NBA/NHL/MLB game in progress","3. Crypto prices vs strike — BTC/ETH current price vs market threshold","4. Economic data released today — compare actual vs consensus","5. Weather/politics markets closing within 24h","6. High volume markets with tight spreads"],"MANDATE":"Use all 10 searches. Be decisive. If net edge >= ${(phase.minEdge*100).toFixed(0)}% exists, signal it. Empty slots = missed compounding."},"execution":{"MIN_NET_EDGE":"${(phase.minEdge*100).toFixed(0)}% after 4.5% fee","MIN_CONFIDENCE":"${(phase.minProb*100).toFixed(0)}%","SKIP_HELD":"Never signal a ticker in ALREADY_HELD list — server will reject it anyway","FILL_ALL_SLOTS":"Signal up to ${dynamicMaxPos()-S.openPositions.length} different markets if edge exists","MIN_BET_SIZE":"$1.00 minimum — server handles Kelly sizing"},"required_output":{"signals":[{"ticker":"string — must NOT be in ALREADY_HELD","side":"YES or NO","confidence":0.68,"edge":0.10,"marketPrice":0.58,"feeAdjustedEdge":0.07,"reasoning":"SPECIFIC data found: [exact score/price/stat]. Edge = (conf - mktPrice) - fees = X%.","hoursToClose":6}],"marketSummary":"What you searched and what edge you found","skipReason":"Only if ALL non-held markets searched and zero edge found"}}`;
  ...
  try {
    const r = await req('https://api.anthropic.com/v1/messages', {
      ...
      body: {
        ...
        max_tokens: 3000,
        system: 'You are an elite autonomous trading signal generator. ONE goal: double a small portfolio to $500K as fast as safely possible through aggressive compounding. RULES: (1) Prioritize markets resolving within 24h - fast resolution = fast compounding. (2) Target 25-55c YES markets where your research shows 65%+ probability - these pay 2x+ profit. (3) Avoid 65c+ bets - tiny upside, slow compounding. (4) Search for sharp money signals: unusual volume, line movement, consensus from top traders. (5) If you find edge >= 5% net after 4.5% fee, SIGNAL IT immediately - empty slots are wasted compounding. (6) Output ONLY valid JSON. No markdown. Start with { end with }.',
...
async function executeTrade(sig) {
  ...
  // Block long-dated positions from using ALL slots — reserve 1 slot for <24h trades
  const longDated = S.openPositions.filter(p => {
    if (!p.openedAt) return false;
    const hrs = (Date.now() - p.openedAt) / 3600000;
    return hrs > 0 && p.reasoning && p.reasoning.includes && !p.ticker.match(/26APR(0[0-9]|10)/);
  });
  const hoursToClose = sig.hoursToClose || 999;
  const isLongDated = hoursToClose > 168; // >7 days
  if (isLongDated && S.openPositions.length >= maxPos - 1) {
    log('Skip ' + sig.ticker + ': long-dated ('+hoursToClose.toFixed(0)+'h) — reserving slots for fast-resolving markets');
    return;
  }
  ...
  if(serverKelly<1.00){log('Skip '+sig.ticker+': Kelly $'+serverKelly.toFixed(2)+' < $1.00');return;}
  ...
  if (CFG.dryRun) {
    ...
    log(`📝 PAPER TRADE: ${sig.ticker} ${sig.side} x${contracts} @ ${price}c = $${position.cost.toFixed(2)}`);
    ...
    try {
      ...
      S.openPositions.push(position);
      log(`✅ LIVE ORDER: ${sig.ticker} ${sig.side} x${contracts} @ ${price}c`);
...
async function scan() {
  ...
  try {
    ...
    if (S.scanCount === 1 || S.scanCount % 10 === 0) await syncPositions();
...
Deposit funds and restart manually when ready.`);
    ...
    if (S.scanCount % 5 === 0) {
      tg(`📡 <b>Scan #${S.scanCount}</b> | $${(S.realBalance||S.balance).toFixed(2)} | ${S.openPositions.length}/${maxPos} pos | ${markets.length} mkts | Brain ${brainReady ? 'READY' : 'in ' + secsToNext + 's'}`);
    }

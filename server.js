...
const PHASES = [
  { name: 'SEED',     min: 0,      max: 50,     kelly: 0.40, minEdge: 0.06, minProb: 0.65, maxPos: 4,  label: '🌱 Seed',     desc: '4 slots. 65%+ conf, 2.0x+ crypto / 2.5x+ sports, sub-3h resolution. FastPath + Sports + Brain.' },
...
async function getTopMarkets() {
  try {
    ...
    const markets = allMarkets.filter(m => {
      ...
      const isFastClose = m.close_time && ((new Date(m.close_time) - Date.now()) < 3 * 3600000);
...
async function runBrain(topMarkets) {
  ...
  // Pre-fetch live crypto prices — always fetch, brain uses these directly for crypto math
  let _priceCtx = '';
  try {
    const _pr = await req('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,solana,ripple&vs_currencies=usd', { method: 'GET', headers: {} }, 6000);
    if (_pr.status === 200 && _pr.body && typeof _pr.body === 'object') {
      const _b = _pr.body?.bitcoin?.usd, _e = _pr.body?.ethereum?.usd, _s = _pr.body?.solana?.usd, _x = _pr.body?.ripple?.usd;
      _priceCtx = `LIVE PRICES @ ${new Date().toLocaleString('en-US',{timeZone:'America/New_York',hour:'2-digit',minute:'2-digit'})} ET: BTC=${_b ? '$'+_b.toLocaleString() : 'UNAVAIL'} ETH=${_e ? '$'+_e.toLocaleString() : 'UNAVAIL'} SOL=${_s ? '$'+_s.toFixed(2) : 'UNAVAIL'} XRP=${_x ? '$'+_x.toFixed(4) : 'UNAVAIL'} — USE THESE FOR CRYPTO MATH, NO SEARCH NEEDED`;
      log(`Pre-fetched prices: BTC=$${_b} ETH=$${_e} SOL=$${_s}`);
    }
  } catch(_err) { _priceCtx = 'PRICES UNAVAILABLE — search for current BTC/ETH price if needed'; }

  const prompt = `{"task":"trade_signals","rules":["JSON only","no markdown","start { end }"],"NOW":"${_priceCtx||'PRICES UNAVAILABLE'}","TIME":"${new Date().toLocaleString('en-US',{timeZone:'America/New_York',hour:'2-digit',minute:'2-digit',weekday:'short'})} ET","HELD":"${_heldStr}","BAL":"$${_bal.toFixed(2)} cash:$${(S.availableCash||0).toFixed(2)} ${S.openPositions.length}/${dynamicMaxPos()} phase:${phase.name}","GOAL":"Fill ${_slots} slot(s). Sub-3h trades only. Fastest compounding = fastest path to $500K.","STEPS":["STEP1 — CRYPTO MATH (no search needed): For each crypto market, read title to find STRIKE. Compare to LIVE price above. YES wins if price ABOVE strike at close. NO wins if BELOW. Compute: gap%=abs(live-strike)/live. If gap>=1% and hoursLeft<2: conf=85%+ signal it. If gap>=2% and hoursLeft<3: conf=80%+ signal it. payout floor: 2.0x.","STEP2 — SPORTS (1 search per game): Search '[team] score now'. If team losing 15+ pts NBA Q4, 3+ runs MLB inn8+, 2+ goals NHL P3: signal the winning side. payout floor: 2.5x.","STEP3 — WEATHER (1 search): Search 'NWS [city] hourly forecast'. Signal exact band if NWS shows >=65% probability. payout floor: 2.5x.","STEP4 — If STILL no signal after steps 1-3: use 1 more search to find ANY sub-3h market with clear edge. DO NOT leave slots empty without exhausting all options."],"FLOORS":"price<12c=85%conf 12-20c=78% 20-30c=72% 30c+=${(phase.minProb*100).toFixed(0)}% | payout: crypto>=2.0x sports/wx>=2.5x","MARKETS":${JSON.stringify(topMarkets.slice(0,15).map(m=>({t:m.ticker,title:(m.title||'').slice(0,60),ask:+(m.yesAsk*100).toFixed(0),bid:+((m.yesBid||0)*100).toFixed(0),payout:+(m.netPayoutRatio||((1-m.yesAsk)/m.yesAsk*0.955)).toFixed(2),side:m.suggestedSide||'YES',h:+m.hoursLeft.toFixed(2),held:_heldTickers.includes(m.ticker)})))},"OUT":{"signals":[{"ticker":"KXBTCD-26APR-T85000","side":"NO","confidence":0.88,"edge":0.73,"marketPrice":0.15,"feeAdjustedEdge":0.70,"netPayoutRatio":5.4,"hoursToClose":0.8,"reasoning":"BTC=$84500 vs strike $85000. $500 below. NO wins. gap=0.59%. conf=88%."}],"summary":"brief","skip":""}}`;

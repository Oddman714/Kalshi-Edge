...
async function scan(){
  ...
  try{
    ...
    if(r.status!==200){log(`Scan ${r.status}`);return;}
    authOk();
    const held=new Set(S.positions.map(p=>p.ticker)),now=Date.now(),markets=r.data.markets||[];
    const scored=markets.filter(m=>{
      if(held.has(m.ticker_name))return false;if((m.volume||0)<C.minVol)return false;
      const hrs=(new Date(m.close_time||0)-now)/3.6e6;if(hrs<1||hrs>168)return false;
      const y=m.last_price||50;return y>=5&&y<=95;
    }).map(m=>{
      const y=m.last_price||50,hrs=((new Date(m.close_time||0)-now)/3.6e6).toFixed(1),t=m.ticker_name;
      const ph=S._ph[t]||[],prev=ph.length?ph[ph.length-1]:y,mom=y>prev+2?'↑':y<prev-2?'↓':'→';
      S._ph[t]=[...ph.slice(-19),y];
      const score=(1-Math.abs(y-50)/50)*0.4+Math.min((m.volume||0)/10000,1)*0.3+Math.max(0,1-parseFloat(hrs)/48)*0.3;
      return{ticker:t,title:m.title||t,yes:y,no:100-y,vol:m.volume||0,hrs,cat:m.category||'—',mom,score};
    });
    scored.sort((a,b)=>b.score-a.score);S.cands=scored.slice(0,20);
    log(`Scan: ${markets.length} → ${scored.length} → ${S.cands.length} top`);
  }catch(e){log('Scan err: '+e.message);S.lastErr=e.message;}
}

async function brain(){
  if(!C.claude){log('No Claude key');return;}if(!S.cands.length){log('No cands');return;}
...
let _sc,_br,_hb;
function startBot(){
  if(S.on)return;S.on=true;S.haltMsg=null;S.startedAt=new Date().toISOString();log('Bot started');
  scan().then(()=>syncBal());

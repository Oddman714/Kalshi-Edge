...
const PHASES = [
  // name        min      max    kelly  minEdge minProb maxPos maxPct maxExposure maxBetDollars
  { name: 'SEED',     min: 0,      max: 50,     kelly: 0.30, minEdge: 0.06, minProb: 0.72, maxPos: 4, maxPct: 0.18, maxExposure: 0.50, maxBetDollars: 8,    label: '🌱 Seed',     desc: 'Max $8/bet. 72%+ conf, 3x+ payout. 15min/1hr crypto FastPath only.' },
  { name: 'SPROUT',   min: 50,     max: 200,    kelly: 0.28, minEdge: 0.07, minProb: 0.70, maxPos: 4, maxPct: 0.12, maxExposure: 0.35, maxBetDollars: 15,   label: '🌿 Sprout',   desc: 'Max $15/bet. 70%+ conf, 3x+ payout. FastPath primary.' },
  { name: 'GROWTH',   min: 200,    max: 1000,   kelly: 0.25, minEdge: 0.07, minProb: 0.68, maxPos: 5, maxPct: 0.08, maxExposure: 0.28, maxBetDollars: 50,   label: '📈 Growth',   desc: 'Max $50/bet. 68%+ conf. Diversify. Never >8% on one trade.' },
  { name: 'MOMENTUM', min: 1000,   max: 5000,   kelly: 0.20, minEdge: 0.06, minProb: 0.66, maxPos: 5, maxPct: 0.06, maxExposure: 0.22, maxBetDollars: 120,  label: '🚀 Momentum', desc: 'Max $120/bet. 66%+ conf. Protect capital first.' },
  { name: 'SCALE',    min: 5000,   max: 25000,  kelly: 0.16, minEdge: 0.06, minProb: 0.65, maxPos: 6, maxPct: 0.04, maxExposure: 0.18, maxBetDollars: 350,  label: '⚡ Scale',    desc: 'Max $350/bet. Steady compounding. No moonshots.' },
  { name: 'HARVEST',  min: 25000,  max: 100000, kelly: 0.12, minEdge: 0.06, minProb: 0.65, maxPos: 8, maxPct: 0.03, maxExposure: 0.15, maxBetDollars: 1000, label: '💰 Harvest',  desc: 'Max $1000/bet. Capital preservation mode.' },
  { name: 'ENDGAME',  min: 100000, max: 500000, kelly: 0.10, minEdge: 0.06, minProb: 0.65, maxPos: 10,maxPct: 0.02, maxExposure: 0.12, maxBetDollars: 2000, label: '🏆 Endgame',  desc: 'Max $2000/bet. Controlled march to $500K.' },
];
...
async function getTopMarkets() {
  ...
  // -- PAYOUT-FIRST FILTER --
    ...
    const markets = allMarkets.filter(m => {
      ...
      // YES side: 5c-35c = 2x+ payout range (was 8c-45c which let in 1.2x garbage)
      const noAsk = Math.max(0.01, 1 - yesBid);
      const yesSideOk = yesAsk >= 0.05 && yesAsk <= 0.35;
      // NO side: 5c-35c equivalent (YES ask 65c-95c)
      const noSideOk  = noAsk >= 0.05 && noAsk <= 0.35 && yesAsk >= 0.65;
...
async function runCryptoFastPath(topMarkets) {
  ...
  for (const m of cryptoMarkets) {
    ...
    // Strategy 1: Ticker suffix T<number> — handles integers AND decimals
    // e.g. KXBTCH-26APR1700-T94999, KXBTCD-26APR-T84.9999, KXETHUSD-T57.99
    let strike = null;
    const tMatch = m.ticker.match(/[-_]T([\d]+\.?[\d]*)(?:[_-]|$)/i);
    if (tMatch) strike = parseFloat(tMatch[1]);

    // Strategy 2: Title dollar amount — "$84,999.99" or "$84000" or "$1.82" or "$57.99"
    if (!strike && m.title) {
      const clean = m.title.replace(/,/g, '');
      // Match any dollar amount: integers or decimals
      const dollarMatch = clean.match(/\$([\d]+\.?[\d]*)/);
      if (dollarMatch) strike = parseFloat(dollarMatch[1]);
    }

    // Strategy 3: Last numeric segment of ticker (handles decimals too)
    if (!strike) {
      const numMatch = m.ticker.match(/([\d]+\.[\d]+|[\d]{4,7})(?:[_-]|$|[A-Z])/i);
    ...
    const maxStrike = isBTC ? 200000 : isETH ? 10000 : isSOL ? 1000 : isXRP ? 100 : 200000;
    const minStrike = isBTC ? 10000 : isETH ? 100 : isSOL ? 1 : isXRP ? 0.01 : 0.01;
    ...
    let confidence;
    if (is15Min) {
      // 15-min: volatile, need bigger gap
      if (gapPct >= 0.030) confidence = 0.91;
      else if (gapPct >= 0.018) confidence = 0.83;
      else if (gapPct >= 0.010) confidence = 0.75;
      else if (gapPct >= 0.006) confidence = 0.67;
      else { log(`FastPath 15min: ${m.ticker} gap ${(gapPct*100).toFixed(2)}% too small — skip`); continue; }
    } else if (m.hoursLeft < 1.5) {
      // 1-hour window
      if (gapPct >= 0.020) confidence = 0.92;
      else if (gapPct >= 0.012) confidence = 0.85;
      else if (gapPct >= 0.006) confidence = 0.76;
      else if (gapPct >= 0.003) confidence = 0.68;
      else { log(`FastPath hourly: ${m.ticker} gap ${(gapPct*100).toFixed(2)}% too small — skip`); continue; }
    } else {
      // 1.5h-3h window
      if (gapPct >= 0.030) confidence = 0.90;
      else if (gapPct >= 0.018) confidence = 0.82;
      else if (gapPct >= 0.010) confidence = 0.74;
      else if (gapPct >= 0.005) confidence = 0.67;
      else { log(`FastPath sub3h: ${m.ticker} gap ${(gapPct*100).toFixed(2)}% too small — skip`); continue; }
    }

    // Large gap bonus — BTC $10k+ from strike is extremely reliable
    if (gapPct >= 0.10) confidence = Math.min(0.96, confidence + 0.05);
    else if (gapPct >= 0.05) confidence = Math.min(0.95, confidence + 0.03);
    ...
    // --- GATE CHECKS ---
    // FastPath hard minimum: 72% confidence regardless of phase.
    // Below 72% the math edge isn't reliable enough — skip and wait for better setup.
    const fastPathMinConf = Math.max(phase.minProb, 0.72);
    ...
    const minPayout = is15Min ? 2.0 : 2.5; // raised floors — only clean opportunities
...
1. NEVER bet YES on crypto if current price is BELOW the strike (that is a moon shot — it loses)
2. NEVER bet NO on crypto if current price is ABOVE the strike
3. NEVER trade anything with market-implied probability <10% (price <10c = DO NOT TRADE)
4. NEVER trade weather bands with <15% market probability — market maker has NWS data edge
5. FASTEST markets win: 15min > 1hr > 3hr. Never tomorrow's markets.
6. Gap direction MUST match your side: price ABOVE strike = YES, price BELOW = NO

STEP 1 - CRYPTO (no search needed): For each crypto market not held, find the strike in the title. Compare live price vs strike. YES ONLY if price>strike with gap>=3%. NO ONLY if price<strike with gap>=3%. Signal if gap>=3% AND netPayout>=2.0. Use confidence: gap<3%=skip, gap 3-5%=80%, gap 5-10%=87%, gap>10%=93%.

STEP 2 - SPORTS (1 search per game): Search "[team] live score". Signal winner if leading in final period. netPayout>=2.5 only. 65%+ confidence only.

STEP 3 - WEATHER (1 search): Search "NWS [city] forecast". ONLY signal if >=70% probability on exact band AND market price is 10-15c. Skip all <10% probability weather.

STEP 4 - FALLBACK: If still under ${_slots} signals, take best crypto with gap>=2% and netPayout>=2.0. NO moon shots.

Confidence floors: price<=15c needs 80%+, price 16-25c needs 75%+, price>25c needs ${(phase.minProb*100).toFixed(0)}%+.
...
function kellySize(prob, price) {
  ...
  // TOTAL PORTFOLIO BALANCE — always use this for sizing, never just available cash.
  // Sizing off availableCash caused runaway bets when cash was briefly high.
  const totalBal = Math.max(S.realBalance || 0, S.balance || 0);
  if (totalBal < 3.00) return 0;

  // HARD CAP 1: single bet never exceeds phase maxPct of TOTAL portfolio
  const maxFromPct = totalBal * phase.maxPct;

  // HARD CAP 2: single bet never exceeds phase maxBetDollars (absolute dollar cap)
  const maxFromDollars = phase.maxBetDollars;

  // HARD CAP 3: check existing exposure — never exceed maxExposure across all positions
  const currentExposure = S.openPositions.reduce((sum, p) => sum + (p.cost || 0), 0);
  const maxTotalExposure = totalBal * phase.maxExposure;
  const remainingExposureBudget = Math.max(0, maxTotalExposure - currentExposure);

  // Kelly raw bet from available cash
  ...
  const rawKellyBet = availCash * fracKelly;

  // Take the MINIMUM of all three caps — the most restrictive wins
  const safeBet = Math.min(rawKellyBet, maxFromPct, maxFromDollars, remainingExposureBudget);

  // Floor: $1 minimum, but never bet more than we can actually afford
  return Math.min(safeBet, Math.max(1.00, Math.min(availCash * 0.85, safeBet)));
}
...
async function executeTrade(sig) {
  ...
  if (sidePriceActual < 0.10) {
    log("Hard gate: " + sig.ticker + " " + sig.side + " @" + (sidePriceActual*100).toFixed(0) + "c market prob <10% -- BANNED");
    return;
  }
...
async function scan() {
  ...
  try {
    ...
    if ((S.drawdownHaltUntil || 0) > Date.now()) {
      ...
      log(`🛡️ Drawdown guard ACTIVE — ${minsLeft}min left | peak=$${(S.sessionPeakBalance||0).toFixed(2)} cur=$${curBalForDD.toFixed(2)}`);
      try { await syncBalance(); } catch(e) {}
      try { await resolvePositions(); } catch(e) {}
      if (S.isRunning) scanTimer = setTimeout(scan, CFG.scanInterval);
      return;
    }
    // Trigger: 20% drop from session peak → halt 30min
    if ((S.sessionPeakBalance || 0) > 10 && curBalForDD < S.sessionPeakBalance * 0.80) {
      const ddPct = ((S.sessionPeakBalance - curBalForDD) / S.sessionPeakBalance * 100).toFixed(1);
      S.drawdownHaltUntil = Date.now() + 30 * 60000;
      log(`🛡️ DRAWDOWN GUARD: ${ddPct}% drop from $${S.sessionPeakBalance.toFixed(2)} peak — pausing 30min`, 'WARN');
      tg(`🛡️ <b>Drawdown Guard Triggered</b>
Peak: $${S.sessionPeakBalance.toFixed(2)} → Current: $${curBalForDD.toFixed(2)} (${ddPct}% drop)
Trading paused 30 minutes to prevent further drawdown.
Open positions will continue to resolve normally.
Resumes: ${new Date(S.drawdownHaltUntil).toLocaleTimeString('en-US',{timeZone:'America/New_York'})} ET`);
      if (S.isRunning) scanTimer = setTimeout(scan, CFG.scanInterval);
      return;
    }
...
4. Bot auto-restarts when cash is available`);
    ...
    // They don't need brainReady — they run every 20 seconds if there are slots
    ...
    // 🧠 BRAIN: fires on its timer, fills any remaining open slots
    const slotsAfterFastPath = dynamicMaxPos() - S.openPositions.length;
    if (brainReady && slotsAfterFastPath > 0 && hasMarkets) {
      log(`🧠 Brain firing: ${slotsAfterFastPath} slots open, $${effectiveCash.toFixed(2)} cash`);
...
server.listen(CFG.port, () => {
  log(`KalshiBot v11 listening on port ${CFG.port}`);

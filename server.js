...
async function runCryptoFastPath(topMarkets) {
  ...
  // Target: ALL crypto threshold markets closing within 4h
  // Use full S.topMarkets (up to 20) but ALSO scan any additional crypto markets
  // that scored well but may have been pushed down by sports/other categories
  const allAvailableMarkets = [...topMarkets]; // topMarkets already sorted best-first
  const cryptoMarkets = allAvailableMarkets
    .filter(m => m.hoursLeft < 4.0 && m.hoursLeft > 0.08 && !heldSet.has(m.ticker) && /kxbtc|kxeth|kxsol|kxxrp/i.test(m.ticker))
  ...
  for (const m of cryptoMarkets) {
    ...
    // Strategy 3: Numeric segment from ticker — 4-7 digit numbers only
    // Skip 2-digit suffixes like "-00", "-45" (these are sequence numbers not strikes)
    if (!strike) {
      const numMatch = m.ticker.match(/[-_]([\d]{4,7})(?:[-_]|$)/i);
    ...
    // (A) Underdog: price 5-35c, payout 2x+, confidence 72%+ → full Kelly sizing
    // (B) Favorite: price 55-88c, payout 0.2-0.8x, confidence 85%+ → 50% Kelly sizing
    //     Favorite trades have lower payout but near-certain wins = consistent flywheel
    //     BTC at $83k vs strike $48k: YES @80c = 92% conf = +14% EV per trade
    const isUnderdog = sidePrice <= 0.38;
    const isFavorite = sidePrice >= 0.55 && sidePrice <= 0.88 && confidence >= 0.85;
    ...
    const sig = {
      ...
      hoursToClose: m.hoursLeft,
      source: 'fastpath',
      isFavorite: isFavorite && !isUnderdog, // favorite = smaller Kelly sizing
...
function kellySize(prob, price, source, sig) {
  ...
  // Favorite trades (high-conf, low-payout): size at 30% of normal
  // They win often but return less — smaller consistent bets are optimal
  const isFavoriteTrade = sig && sig.isFavorite;
  const kellyFrac = isFavoriteTrade ? phase.kelly * 0.30
...
async function startBot() {
  ...
  _botStarted = true;
  S.isRunning = true;
  log('[M] KalshiBot v12.4 started -- running pre-flight sync...');
...
server.listen(CFG.port, () => {
  log(`KalshiBot v12.4 listening on port ${CFG.port}`);

...
async function runCryptoFastPath(topMarkets) {
  ...
  for (const m of cryptoMarkets) {
    ...
    if (is15Min) {
      if (gapPct >= 0.15) confidence = 0.96;       // 10x vol buffer: near-certain
      else if (gapPct >= 0.10) confidence = 0.93;  // 6.7x: very high
      else if (gapPct >= 0.07) confidence = 0.88;  // 4.7x: solid (was 0.89 at 5%)
      else if (gapPct >= 0.05) confidence = 0.82;  // 3.3x: real edge, lean size
      else { log(`FastPath 15min: ${m.ticker} gap ${(gapPct*100).toFixed(2)}% < 5% — skip`); continue; }
    } else if (m.hoursLeft < 1.5) {
      if (gapPct >= 0.12) confidence = 0.94;       // 4.8x buffer: near-certain for 1hr
      else if (gapPct >= 0.08) confidence = 0.90;  // 3.2x: very high (was 0.94 at 5%)
      else if (gapPct >= 0.05) confidence = 0.85;  // 2x: good edge (was 0.89)
      else if (gapPct >= 0.03) confidence = 0.78;  // 1.2x: marginal, small size
      else { log(`FastPath hourly: ${m.ticker} gap ${(gapPct*100).toFixed(2)}% < 3% — skip`); continue; }
    } else {
      if (gapPct >= 0.10) confidence = 0.93;
      else if (gapPct >= 0.06) confidence = 0.87;
      else if (gapPct >= 0.035) confidence = 0.81;
      else if (gapPct >= 0.020) confidence = 0.75;
      else { log(`FastPath sub4h: ${m.ticker} gap ${(gapPct*100).toFixed(2)}% < 2% — skip`); continue; }
    }
    ...
    if (isUpDownTargetMkt) {
      ...
      // Confidence = market probability + momentum bonus if signals agreed
      const udConf = Math.min(0.97, betProb + 0.01 + udMomentumBonus); // systematic + momentum bonus

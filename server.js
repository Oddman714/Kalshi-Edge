...
async function getTopMarkets() {
  try {
    ...
    const markets = allMarkets.filter(m => {
      ...
      const isHourly = /H$|15M$|HOURLY/i.test(m.ticker) || (m.series_ticker && /H$|15M/i.test(m.series_ticker));
      // Hourly markets are newer — lower vol floor to catch them
      if (vol < (isHourly ? 10 : 50)) return false;
      const spread = yesAsk - yesBid;
      // Tighter spread requirement for hourly (liquidity check)
      if (spread > (isHourly ? 0.18 : 0.22)) return false;
      // Accept if YES side is in range (8c-42c) OR NO side is in range (8c-42c = YES 58c-92c)
      // This doubles the opportunity set by scanning both sides of every market
      const noAsk = Math.max(0.01, 1 - yesBid);
      const yesSideOk = yesAsk > 0.08 && yesAsk < 0.42;
      const noSideOk  = noAsk  > 0.08 && noAsk  < 0.42 && yesBid > 0.58;
      return yesSideOk || noSideOk;
    });

    log(`After filter: ${markets.length} tradeable (8c-42c YES, vol>=50, spread<=18c)`);

    // Weather pre-filter: mid-probability weather bands (20-35c) are losers —
    // market maker has live NWS data. Only keep high-prob bands (35c+) or
    // extreme-outlier plays (≤20c) for weather markets.
    const preFilterCount = markets.length;
    const filteredMarkets = markets.filter(m => {
      const isWeatherTicker = /kxhigh|kxlow/i.test(m.ticker);
      if (isWeatherTicker) {
        const yesAsk = parsePrice(m.yes_ask_dollars);
        if (yesAsk > 0.20 && yesAsk < 0.35) return false; // mid-prob weather: worst risk/reward
      }
      return true;
    });
    if (filteredMarkets.length < preFilterCount) {
      log(`Weather pre-filter: removed ${preFilterCount - filteredMarkets.length} mid-prob weather markets`);
    }
...
async function runBrain(topMarkets) {
  ...
  const prompt = `{"task":"generate_trade_signals","rules":["respond ONLY with valid JSON","no preamble","start with {","end with }"],"context":{"MISSION":"Double $${_bal.toFixed(2)} to $500K. ${_slots} OPEN SLOT(S). ${_priceCtx || ''} | TIME: ${new Date().toLocaleString('en-US',{timeZone:'America/New_York',hour:'2-digit',minute:'2-digit',weekday:'short'})}","ALREADY_HELD":"${_heldStr} — DO NOT signal any of these. Find DIFFERENT markets.","BALANCE":"$${_bal.toFixed(2)} | Cash: $${S.availableCash.toFixed(2)} | Open: ${S.openPositions.length}/${dynamicMaxPos()} | Phase: ${phase.name}","PAYOUT_MATH":"Net payout=(1-price)/price*0.955. At 28c=2.5x. At 22c=3.5x. At 17c=4.6x. At 12c=7.0x. EDGE FORMULA: For YES: edge=confidence-yesAsk. For NO: edge=confidence-noAsk where noAsk=(1-yesBid). Example: BTC NO at 12c with 90% conf -> edge=0.90-0.12=0.78 (78%). Always report feeAdjustedEdge as edge for the SIDE you are betting.","HARD_RULES":["1. PAYOUT FLOOR: >=2.5x in SEED/SPROUT, >=2.0x otherwise. Hard reject below floor.","2. HOURLY CRYPTO FIRST (hoursLeft<1): If a market resolves THIS HOUR, check BTC/ETH price from LIVE_PRICES_NOW vs strike. If price is clearly ABOVE or BELOW strike with >5% margin AND payout>=2.5x: this is your highest-priority signal. These are pure math — no research needed beyond the live price.","3. DAILY CRYPTO: For hoursLeft 1-8h, same logic — price vs strike from LIVE_PRICES_NOW.","4. NO-SIDE: Markets with suggestedSide=NO — calculate NO payout. If confident event won't happen and payout>=2.5x: signal NO.","5. SPORTS: Only with official injury/lineup data shifting win prob >20%. No narrative signals.","6. WEATHER: SKIP unless NWS gives >=65% to that specific band.","7. CONFIDENCE: <12c=85%+. 12-20c=78%+. 20-30c=72%+. 30c+=${(phase.minProb*100).toFixed(0)}%+. EXCEPTION: hourly crypto where math clearly shows price far from strike — confidence can be 90%+ from the math alone.","8. Signal EXACTLY ${_slots} trade(s). Zero is correct when nothing qualifies."]},"markets":${JSON.stringify(topMarkets.slice(0,12).map(m=>({ticker:m.ticker,title:m.title,yesAsk:+(m.yesAsk*100).toFixed(1),yesBid:+((m.yesBid||0)*100).toFixed(1),netPayoutRatio:+(m.netPayoutRatio||((1-m.yesAsk)/m.yesAsk*0.955)).toFixed(2),suggestedSide:m.suggestedSide||'YES',volume:Math.round(m.volume),hoursLeft:+m.hoursLeft.toFixed(2),isHourly:m.hoursLeft<1.5,alreadyHeld:_heldTickers.includes(m.ticker)})))},"search_strategy":{"SEARCHES":10,"ORDER":["1. HOURLY CRYPTO (hoursLeft<1.5): Scan for markets where hoursLeft<1.5. For each, use LIVE_PRICES_NOW. Calculate: is BTC/ETH clearly above or below the strike? Edge = (price-strike)/strike. If |edge|>5% and payout>=2.5x: signal. These need NO web search — pure math from LIVE_PRICES_NOW.","2. DAILY CRYPTO (hoursLeft 1.5-8h): Same logic for BTC/ETH daily markets. Use LIVE_PRICES_NOW, compute edge, signal if qualifies.","3. NO-SIDE SCAN: Markets with suggestedSide=NO where you have 65%+ conf event won't happen. Calculate NO payout.","4. SPORTS: Search '[team] injury report [date]'. Only signal with hard evidence.","5. WEATHER: Search 'NWS [city] forecast [date]'. Only signal if >=65% on that band.","6. ZERO SIGNALS is correct if nothing passes the bar — never fill slots with weak trades."]},"execution":{"MIN_NET_PAYOUT":"2.5x in SEED/SPROUT phase, 2.0x otherwise — hard reject below","MIN_EDGE":"${(phase.minEdge*100).toFixed(0)}% after 4.5% fee","MIN_CONF":"${(phase.minProb*100).toFixed(0)}%","SLOTS":"Signal EXACTLY ${_slots} trade(s) max — the number of open slots available right now"},"required_output":{"signals":[{"ticker":"NOT in ALREADY_HELD","side":"YES or NO","confidence":0.75,"edge":0.63,"marketPrice":0.127,"feeAdjustedEdge":0.60,"netPayoutRatio":6.56,"reasoning":"SPECIFIC DATA: BTC=$72900 vs strike $73999 — BTC is $1100 BELOW strike. NO wins if BTC stays below $74000 at 7pm. Conf=90%. NO at 12.7c = 6.56x net payout. Edge=conf(0.75)-NOPrice(0.127)=0.623","hoursToClose":1}],"marketSummary":"what searched, what found","skipReason":"only if zero edge found after all searches"}}`;
...
async function executeTrade(sig) {
  ...
  // Phase-scaled payout floor: SEED/SPROUT need 2.5x — can't waste shots at low balance.
  const minPayout = (phase.name === 'SEED' || phase.name === 'SPROUT') ? 2.5 : 1.8;
  if (netPayoutRatio < minPayout) {
    log(`Skip ${sig.ticker}: payout ${netPayoutRatio.toFixed(2)}x < ${minPayout}x floor for ${phase.name} phase (${sig.side}@${(sidePriceForPayout*100).toFixed(0)}c)`);
    return;
  }

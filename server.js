...
function loadState(){
  ...
  if(_firstLoad&&cb){cb.style.display='block';cb.textContent='⏳ Connecting...';}
  // 15s timeout — Railway cold starts can be slow
  ...
  fetch(url,{cache:'no-store',signal:ctrl?ctrl.signal:undefined})
    ...
    .then(function(s){
      ...
      try{applyState(s);}catch(e2){
        console.error('applyState crash:',e2);
        if(eb){eb.textContent='Render error: '+e2.message;eb.style.display='block';}
      }
    })
    .catch(function(e){
      if(timer)clearTimeout(timer);
      _retries++;
      var delay=Math.min(3000*_retries,15000);
      var msg=e.name==='AbortError'?'Timeout — server slow (15s)':'Error: '+e.message;
      console.error('loadState #'+_retries+':',msg,url);
      if(_firstLoad){
        if(cb){cb.style.display='block';cb.textContent='⏳ Attempt '+_retries+': '+msg;}
      } else {
        if(eb){eb.style.display='block';eb.textContent='⚠️ Reconnecting... '+msg;}
        if(cb)cb.style.display='none';
      }
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/api/state') {
    ...
    return res.end(JSON.stringify(slim));
      ...
      return res.end(JSON.stringify({ error: stateErr.message, isRunning: S.isRunning, balance: S.balance, realBalance: S.realBalance }));
    }
  }

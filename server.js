...
const CFG = {
  ...
  heartbeatInterval: 900000,  // telegram heartbeat: every 15minin
...
const PHASES = [
  // SEED: Preservation mode. 3 slots max — if we lose 2 we still have capital.
  // High conviction only: 70%+ confidence, 10%+ edge, 2.5x+ payout preferred.
  // Focus: same-day crypto thresholds + sports with verified injury/lineup data.
  { name: 'SEED',     min: 0,      max: 50,     kelly: 0.40, minEdge: 0.10, minProb: 0.70, maxPos: 3,  label: '🌱 Seed',     desc: 'High-conviction only: 70%+ conf, 2.5x+ payout, same-day resolution. 3 slots max.' },
  // SPROUT: Building phase. 4 slots, still strict. Proven wins unlock more aggression.
  { name: 'SPROUT',   min: 50,     max: 200,    kelly: 0.38, minEdge: 0.09, minProb: 0.68, maxPos: 4,  label: '🌿 Sprout',   desc: 'Building: 68%+ conf, 2.0x+ payout, favor same-day + next-day resolution.' },
  // GROWTH: Compound mode. 5 slots, slightly looser on payout floor.
  { name: 'GROWTH',   min: 200,    max: 1000,   kelly: 0.35, minEdge: 0.08, minProb: 0.65, maxPos: 5,  label: '📈 Growth',   desc: 'Compounding: 65%+ conf, 1.8x+ payout, diversify sports + crypto + macro.' },
  { name: 'MOMENTUM', min: 1000,   max: 5000,   kelly: 0.32, minEdge: 0.07, minProb: 0.63, maxPos: 6,  label: '🚀 Momentum', desc: 'Momentum: press clean edges, 1.8x+ payout, protect capital.' },
  { name: 'SCALE',    min: 5000,   max: 25000,  kelly: 0.28, minEdge: 0.07, minProb: 0.62, maxPos: 8,  label: '⚡ Scale',    desc: 'Scaling: larger positions, 1.5x+ payout acceptable.' },
  { name: 'HARVEST',  min: 25000,  max: 100000, kelly: 0.22, minEdge: 0.07, minProb: 0.61, maxPos: 10, label: '💰 Harvest',  desc: 'Harvest: steady compounding to six figures.' },
  { name: 'ENDGAME',  min: 100000, max: 500000, kelly: 0.18, minEdge: 0.08, minProb: 0.62, maxPos: 12, label: '🏆 Endgame',  desc: 'Endgame: controlled march to $500K.' },
];
...
async function getTopMarkets() {
  try {
    ...
    const seriesList = [
      // Crypto -- 24/7, fast resolution, math-based edge
      'KXBTCD','KXBTCW','KXETHUSD','KXETH','KXSOL','KXXRP','KXDOGE',
      'KXBNB','KXAVAX','KXADA','KXLINK','KXMATIC',
      // Sports -- dominant volume, clear live scores
      'KXNBAGAME','KXNBAPLAYOFFS','KXMLBGAME','KXNHLGAME','KXNFLGAME',
      'KXMMA','KXGOLF','KXTENNIS','KXSOCCER','KXNCAAMB',
      // Economics -- CPI, Fed, macro -- inefficient early, big edge
      'FED','FEDRATE','INFL','KXCPI','KXPCE','KXUNEMPLOYMENT','KXJOBS',
      'KXGDP','KXRETAIL','KXHOUSING',
      // Commodities
      'KXOIL','KXGOLD','KXNG','KXSILVER',
      // Politics/events -- high volume around news
      'KXPREZ','KXSENATE','KXHOUSE','KXTARIFF','KXTRADE','KXAPPROVAL',
      'KXCONGRESS','KXSUPREME',
      // Weather -- often inefficient, good for short-term edge
      'KXHIGHNY','KXHIGHMIA','KXHIGHLA','KXHIGHCHI',
      // Indices -- market hours
      ...(isMarketHours ? ['INXD','NASDAQ100D','KXSPX500','KXEARNINGS','KXVIX','SP500'] : []),
    ...
    const markets = allMarkets.filter(m => {
      ...
      // HARD PAYOUT CAP: max 48c YES price = minimum ~1.1x net payout after fees
      // Anything above 48c pays too little to meaningfully double a small portfolio
      if (yesAsk <= 0.08 || yesAsk >= 0.42) return false; // max 42c = min ~1.6x payout
      // Also consider NO side: if YES is 52c+, betting NO at 48c- is the same trade
      // NO side handled via brain signal side='NO', yesAsk here is the YES ask
      if (yesBid <= 0.02) return false;
      if ((yesAsk - yesBid) > 0.18) return false; // tight spread = better fill, less slippage
      const vol = parseFloat(m.volume_fp || m.volume || 0);
      if (vol < 50) return false; // higher volume floor = more liquid markets
      return true;
    });

    log(`After filter: ${markets.length} tradeable (8c-48c YES, vol>=50, spread<=22c — payout-first filter)`);

    // Weather pre-filter: low-probability weather bands are nearly always losers.
    // Market maker has live NWS data. If a weather band is priced at 8-25c, the
    // forecast is NOT in that band. Only keep weather markets priced 35c+ (high-prob band).
    const preFilterCount = markets.length;
    const filteredMarkets = markets.filter(m => {
      const isWeatherTicker = /kxhigh|kxlow/i.test(m.ticker);
      if (isWeatherTicker) {
        const yesAsk = parsePrice(m.yes_ask_dollars);
        // Keep weather only if it's a high-probability band (35c+ = likely the forecast band)
        // OR if it's a very high payout play (<=20c — extreme edge if correct)
        if (yesAsk > 0.20 && yesAsk < 0.35) {
          return false; // 20-35c weather = mid-tier uncertainty, worst risk/reward
        }
      }
      return true;
    });
    if (filteredMarkets.length < preFilterCount) {
      log(`Weather pre-filter: removed ${preFilterCount - filteredMarkets.length} mid-prob weather markets (20-35c range)`);
    }
    ...
    const scored = filteredMarkets.map(m => {
      ...
      // Duration scoring — faster resolution = faster compounding = higher priority.
      // Sweet spot: 3-12 hours (tonight's games, same-day data, crypto thresholds).
      // Next-day gets modest weight. Multi-day nearly excluded — too much can change.
      const durationScore = hoursLeft < 0.5  ? 0.05  // too imminent — can't fill
        : hoursLeft < 1   ? 0.70              // very soon — ok
        : hoursLeft < 3   ? 1.10              // excellent: resolves within hours
        : hoursLeft < 8   ? 1.40              // BEST: tonight/today — fast compounding
        : hoursLeft < 16  ? 1.20              // great: tonight late
        : hoursLeft < 24  ? 0.85              // good: tomorrow
        : hoursLeft < 36  ? 0.45              // ok: next day extended
        : hoursLeft < 72  ? 0.15              // slow — skip unless exceptional
        : hoursLeft < 168 ? 0.04              // bad: multi-day
        : 0.01;                               // terrible: >7 days
      ...
      // Payout scoring: strongly bias toward 2.5x-5x range.
      // 2.5x+ (<=28c) is the sweet spot for doubling — each win moves the needle.
      // Anything under 2x payout is deprioritized — takes too many wins to double.
      const payoutScore = netPayoutRatio >= 5.0 ? 1.15  // ~16c: extreme payout, needs very high conf
        : netPayoutRatio >= 3.5 ? 1.50                   // ~21c: outstanding — target this range
        : netPayoutRatio >= 2.5 ? 1.60                   // ~27c: SWEET SPOT — doubles fast
        : netPayoutRatio >= 2.0 ? 1.35                   // ~32c: good — still meaningful
        : netPayoutRatio >= 1.8 ? 1.00                   // ~35c: acceptable minimum
        : 0.30;                                           // <1.8x: deprioritized (below floor)
      ...
      // Category bonus/penalty: crypto thresholds are most verifiable, weather uncertain
      const category = (m.category || m.series_ticker || '').toLowerCase();
      const isCrypto = /btc|eth|crypto|bitcoin|ethereum|inx|spx|ndx/.test(category + m.ticker.toLowerCase());
      const isWeather = /kxhigh|kxlow|weather|temp/.test(m.ticker.toLowerCase());
      const isSports = /game|nba|nfl|mlb|nhl|mma|soccer|epl/.test(category + m.ticker.toLowerCase());
      // Crypto: binary, verifiable in real time — no research error risk
      // Weather: high research error (which band? exact temp?) — penalize
      // Sports: good if evidence-backed, neutral otherwise
      const categoryMult = isCrypto ? 1.25 : isWeather ? 0.65 : isSports ? 1.05 : 1.0;
      ...
      return {
        ...
        netPayoutRatio: parseFloat(netPayoutRatio.toFixed(2)),
        edge: 0, // populated by brain after web search
      };
    });

    // Deprioritize markets already held - push to bottom so brain sees fresh markets
...
async function runBrain(topMarkets) {
  ...
  const prompt = `{"task":"generate_trade_signals","rules":["respond ONLY with valid JSON","no preamble","start with {","end with }"],"context":{"MISSION":"Double $${_bal.toFixed(2)} to $500K FAST. We have ${_slots} OPEN SLOT(S). Each bet must have a payout ratio that can actually double money.","ALREADY_HELD":"${_heldStr} — DO NOT signal any of these. Find DIFFERENT markets.","BALANCE":"$${_bal.toFixed(2)} | Cash: $${S.availableCash.toFixed(2)} | Open: ${S.openPositions.length}/${dynamicMaxPos()} | Phase: ${phase.name}","PAYOUT_MATH":"Net payout=(1-price)/price*0.955. At 28c=2.5x(TARGET). At 22c=3.5x(GREAT). At 17c=4.6x(EXCELLENT). MINIMUM ${phase.name==='SEED'||phase.name==='SPROUT'?'2.5':'2.0'}x — anything below this is deprioritized.","HARD_RULES":["1. PAYOUT FLOOR: Net payout MUST be >=2.5x in SEED/SPROUT phase, >=2.0x otherwise. HARD REJECT below floor.","2. CRYPTO FIRST: Any BTC/ETH/crypto threshold market resolving <8h: check live price immediately. If price is within 2% of strike, skip (too uncertain). If price diverges >5% from strike and payout >=2.5x: signal it.","3. WEATHER: SKIP all weather markets UNLESS you find the NWS forecast lands IN that specific band with >=65% probability. High-temp 82-83 at 16% means NWS says forecast is ~82F — the crowd is RIGHT. Do not bet against verified forecasts.","4. SPORTS: Only signal if you find OFFICIAL confirmation of key injuries or lineup changes that shift win probability >20% from market price. Search ESPN/official team pages, not just standings.","5. CONFIDENCE TIERS: <12c = need 85%+ conf. 12-20c = need 78%+ conf. 20-30c = need 72%+ conf. 30c+ = need ${(phase.minProb*100).toFixed(0)}%+ conf.","6. SIGNAL EXACTLY ${_slots} trade(s) — no more. Zero is correct if nothing meets the bar.","7. Prefer resolution within 8 hours. Reject anything >36h unless payout is exceptional (4x+)."]},"markets":${JSON.stringify(topMarkets.slice(0,10).map(m=>({ticker:m.ticker,title:m.title,yesAsk:+(m.yesAsk*100).toFixed(1),yesBid:+((m.yesBid||0)*100).toFixed(1),netPayoutRatio:+(((1-m.yesAsk)/m.yesAsk*0.955)).toFixed(2),volume:Math.round(m.volume),hoursLeft:+m.hoursLeft.toFixed(1),alreadyHeld:_heldTickers.includes(m.ticker)})))},"search_strategy":{"SEARCHES":10,"ORDER":["1. CRYPTO FIRST: For each crypto/BTC/ETH market in the list: search current price RIGHT NOW. Compare to strike. If current price is far from strike and payout>=2.5x: strong signal.","2. SPORTS INJURIES: For each NBA/MLB/NHL game market at 8c-30c: search '[team] injury report today' and '[team] starting lineup tonight'. Only signal if you find hard evidence of key player out.","3. WEATHER VERIFICATION: For each KXHIGH/temp market: search '[city] NWS high temperature forecast [date]'. Find the OFFICIAL NWS forecast. Only signal if NWS forecast places >=65% probability on one specific band.","4. ECONOMIC RELEASES: Check if any data releases today (CPI/jobs/GDP). Search '[indicator] forecast consensus'. Only signal before release if there is a clear directional skew.","5. FINAL CHECK: Only include signals where you found SPECIFIC DATA. No signals based on reasoning alone."]},"execution":{"MIN_NET_PAYOUT":"${phase.name==='SEED'||phase.name==='SPROUT'?'2.5x (SEED/SPROUT phase minimum)':'2.0x minimum'}","MIN_EDGE":"${(phase.minEdge*100).toFixed(0)}% after 4.5% fee","MIN_CONF":"${(phase.minProb*100).toFixed(0)}%","SLOTS":"Signal EXACTLY ${_slots} trade(s) max — the number of open slots available right now"},"required_output":{"signals":[{"ticker":"NOT in ALREADY_HELD","side":"YES or NO","confidence":0.68,"edge":0.12,"marketPrice":0.30,"feeAdjustedEdge":0.09,"netPayoutRatio":2.19,"reasoning":"SPECIFIC: [exact score/price/data found]. Payout=(1-0.30)/0.30*0.955=2.2x. Conf=68%>30% price=38% edge.","hoursToClose":8}],"marketSummary":"what searched, what found","skipReason":"only if zero edge found after all searches"}}`;
  ...
  try {
    const r = await req('https://api.anthropic.com/v1/messages', {
      ...
      body: {
        ...
        max_tokens: 3000,
        system: `You are a disciplined prediction market analyst. Your ONLY goal: compound a small portfolio to $500K through high-conviction, fast-resolving trades.

WHAT ACTUALLY WORKS on Kalshi:
1. CRYPTO THRESHOLDS (most reliable): BTC/ETH above/below a price resolving within 2-8 hours. Pure math — check current price vs strike. No opinion error possible.
2. SPORTS MISPRICING (good when verified): Kalshi implied probability diverges 20%+ from actual win probability. REQUIRES hard evidence: confirmed injury reports, official starting lineup, current season form. NEVER bet on 'motivation' or 'playoff implications' alone.
3. WEATHER (only high-probability bands): ONLY bet the exact band NWS forecast lands on. If NWS says 82F high and the market is '82-83 YES at 16%' — DO NOT BET. That means the crowd already knows the forecast. Only signal weather when the NWS forecast gives ≥65% probability to one specific band — then bet that band.
4. ECONOMIC DATA: Only before release. Skip any market priced >55c — payout is too small.

WHAT KILLS PORTFOLIOS:
- Betting 15-25c weather longshots. The market maker has live NWS data. If it's 15c, there's a reason.
- Sports underdogs without confirmed injury/lineup data from official sources.
- Any trade under 2.5x net payout at low balance — need each win to move the needle.

HARD RULES:
1. Net payout MUST be >=${phase.name === 'SEED' || phase.name === 'SPROUT' ? '2.5' : '2.0'}x: ((1-price)/price)*0.955.
2. Minimum confidence: ${(getPhase().minProb*100).toFixed(0)}%. No guessing.
3. ONLY signal ${_slots} trade(s) maximum — the exact number of open slots.
4. Prefer markets resolving within 8 hours. Deprioritize anything >36 hours out.
5. Output ONLY valid JSON. Start with { end with }. No markdown.`,
    ...
    if (S.signals.length > 0) {
      const sigText = S.signals.map(s => {
        ...
        const payoutRatio = mktP > 0 ? ((1/mktP - 1)).toFixed(2) : '?';
        return `📊 ${s.ticker} ${s.side} | ${(s.confidence*100).toFixed(0)}% conf | edge ${(s.edge*100).toFixed(1)}% | $${s.kellySize?.toFixed(2)} | ${payoutRatio}x payout | ${s.reasoning}`;
      }).join('\n');
...
function kellySize(prob, price) {
  ...
  // Max bet per trade: conservative in early phases — preserve runway for compounding.
  // SEED (<$50): max 30% per trade. SPROUT (<$200): max 35%. Growth+: standard Kelly.
  // Rationale: at $22 balance, a 60% single bet is ruin if it loses. 3 x 30% = 90% exposure max.
  const maxPct = bal < 20  ? 0.28        // SEED micro: 28% — 3 bets max before floor
    : bal < 50   ? 0.32                   // SEED: 32% cap — 3 comfortable positions
    : bal < 200  ? 0.35                   // SPROUT: 35% cap
    : bal < 1000 ? 0.32                   // GROWTH: 32% cap
    : bal < 5000 ? 0.26                   // MOMENTUM
    : 0.20;                               // SCALE+
...
async function executeTrade(sig) {
  ...
  if (totalPosCount >= maxPos) {
    log(`Skip ${sig.ticker}: at position cap (${totalPosCount}/${maxPos})`);
    return;
  }
  ...
  if(tradePrice<=0.08||tradePrice>=0.92){log('Skip '+sig.ticker+': '+(tradePrice*100).toFixed(0)+'c outside range');return;}
  ...
  const netPayoutRatio = ((1 - tradePrice) / tradePrice) * (1 - KALSHI_FEE);
  // Phase-scaled payout floor: SEED needs 2.5x (can't waste shots at this balance),
  // SPROUT needs 2.0x, GROWTH+ needs 1.8x. The smaller the bankroll, the higher bar.
  const minPayout = phase.name === 'SEED' ? 2.5
    : phase.name === 'SPROUT' ? 2.0
    : 1.8;
  if (netPayoutRatio < minPayout) {
    log(`Skip ${sig.ticker}: payout ${netPayoutRatio.toFixed(2)}x < ${minPayout}x floor for ${phase.name} phase (price=${(tradePrice*100).toFixed(0)}c)`);
    return;
  }
  // Longshot guards — tiered by price. Market price reflects crowd wisdom.
  // The lower the price, the more evidence we need to justify betting against market.
  if (tradePrice < 0.12 && sig.confidence < 0.85) {
    log(`Skip ${sig.ticker}: extreme longshot (${(tradePrice*100).toFixed(0)}c) needs 85%+ conf, got ${(sig.confidence*100).toFixed(0)}%`);
    return;
  }
  if (tradePrice < 0.20 && sig.confidence < 0.78) {
    log(`Skip ${sig.ticker}: longshot (${(tradePrice*100).toFixed(0)}c) needs 78%+ conf, got ${(sig.confidence*100).toFixed(0)}%`);
    return;
  }
  if (tradePrice < 0.30 && sig.confidence < 0.72) {
    log(`Skip ${sig.ticker}: underdog (${(tradePrice*100).toFixed(0)}c) needs 72%+ conf, got ${(sig.confidence*100).toFixed(0)}%`);
    return;
  }
  // Trust brain's edge calculation (it has web search data we don't have server-side)
  // Use feeAdjustedEdge if provided, else sig.edge, else recalculate
  const brainEdge = sig.feeAdjustedEdge||sig.edge||0;
  const serverEdge = sig.confidence-tradePrice;
  const effectiveEdge = brainEdge>0 ? brainEdge : serverEdge;
  if(effectiveEdge<phase.minEdge){
    log('Skip '+sig.ticker+': edge '+(effectiveEdge*100).toFixed(1)+'% < min '+(phase.minEdge*100)+'%');return;
  }
  ...
  const minBet = Math.max(0.75, (S.realBalance||S.balance) * 0.05);
  ...
  // Order at market price (what Kalshi offers), capped for safety
  const orderP=sig.marketPrice||(sig.side==='YES'?sig.confidence:1-sig.confidence);
  const priceCents=Math.round(Math.min(0.92,Math.max(0.08,orderP))*100);
  ...
  if (CFG.dryRun) {
    ...
    try {
      ...
      S.openPositions.push(position);
      log(`✅ LIVE ORDER: ${sig.ticker} ${sig.side} x${contracts} @ ${price}c | payout ${netPayoutRatio.toFixed(2)}x | max return $${(contracts*(100-price)/100).toFixed(2)}`);
      tg(`✅ <b>Live Order Placed</b>\n${sig.ticker} ${sig.side}\n${contracts} contracts @ ${price}c\nCost: $${((price*contracts)/100).toFixed(2)} → max $${(contracts*(100-price)/100).toFixed(2)} (${netPayoutRatio.toFixed(2)}x payout)\nOrder ID: ${order.order?.order_id}`);
      saveState();
    } catch(e) {
...
Remaining capital preserved. Next steps:
1. Cancel any unfilled resting orders on Kalshi to recover reserved cash
2. Let open positions resolve naturally
3. Deposit to at least $25 before restarting
4. Restart bot — new guards prevent repeat overfilling`);
        stopBot();
      }
      return;
    }
    ...
    if (S.scanCount % 5 === 0) {
      const posStr = S.openPositions.length > 0
        ? '\n📋 ' + S.openPositions.map(p => p.ticker.split('-').slice(-1)[0] + ' ' + p.side + (p.cost ? ' $' + p.cost.toFixed(2) : '')).join(' | ')
        : '';
      const overLimit = S.openPositions.length > maxPos ? ` ⚠️ ${S.openPositions.length - maxPos} over limit` : '';
      const meaningful = S.openPositions.length; // raw count
      const phase = getPhase();
      ...
      tg('📡 <b>Scan #' + S.scanCount + '</b> | ' + phase.label + '\n💰 $' + (S.realBalance||S.balance).toFixed(2) + ' | Cash: $' + effectiveCash.toFixed(2) + '\n📊 ' + meaningful + '/' + maxPos + ' positions' + overLimit + ' | ' + markets.length + ' mkts' + unrealLine + '\n🧠 Brain ' + (brainReady ? 'READY 🔥' : 'in ' + secsToNext + 's') + posStr);
    }

    // hasSlots: count meaningful positions only (cost >= $0.50) — tiny penny positions
    // should NOT block the brain from finding real trades
    const hasSlots = S.openPositions.length < maxPos;
    if (brainReady && hasSlots && hasMarkets) {
      log(`🧠 Brain firing: ${S.openPositions.length}/${maxPos} positions, $${effectiveCash.toFixed(2)} cash avail`);
      await runBrain(markets);
    } else if (brainReady && !hasSlots) {
      log(`Brain: at cap (${S.openPositions.length}/${maxPos}) -- position monitor`);
      S.lastBrainAt = Date.now();
...
Brain: ${CFG.brainInterval/60000}min | Scan: ${CFG.scanInterval/1000}s${_startPosList}`);
...
function drawChart() {
  ...
  // If only 1 point (flat balance, fresh deploy), synthesize start point so chart always draws
  if (d.length === 1) { d = [{t: d[0].t - 300000, v: d[0].v}, d[0]]; }
  if (d.length < 2) {
    svg.innerHTML = '<text x="215" y="75" text-anchor="middle" fill="rgba(238,238,245,0.15)" font-size="12" font-family="DM Mono">Waiting for balance sync...</text>';
    return;
  }
...
function applyState(s) {
  ...
  try {
    ...
    // Seed chart from persisted server history on first load
    if (!KB.chartSeeded && s.portfolioHistory && s.portfolioHistory.length) {

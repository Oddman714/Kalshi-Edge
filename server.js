...
STEP 1 - CRYPTO (no search needed): For each crypto market not held, find the strike in the title. Compare live price vs strike. YES wins if price>strike, NO wins if price<strike. Gap = abs(price-strike)/price. Signal if gap>=0.5% AND netPayout>=2.0. Use confidence: gap<1%=72%, gap 1-2%=80%, gap 2-5%=87%, gap>5%=93%.

STEP 2 - SPORTS (1 search per game): Search "[team] live score". Signal winner if blowout in final period. netPayout>=2.5 only.

STEP 3 - WEATHER (1 search): Search "NWS [city] forecast". Signal if >=65% on exact band. netPayout>=2.5 only.

STEP 4 - FALLBACK: If still under ${_slots} signals, lower crypto gap threshold to 0.3% or take best available market with netPayout>=2.0.

Confidence floors: price<=15c needs 78%+, price 16-25c needs 72%+, price>25c needs ${(phase.minProb*100).toFixed(0)}%+.

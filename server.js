...
const CFG = {
  ...
  minEdge:    0.06,        // 6% minimum edge — real mispricings only
  minProb:    0.58,        // minimum 58% confidence — quality over quantity
...
function kellySize(prob, price) {
  ...
  const maxPct = bal < 25 ? 0.50 : bal < 50 ? 0.40 : bal < 150 ? 0.28 : bal < 500 ? 0.20 : 0.15;
  const maxBetFloor = Math.min(bal * maxPct, playable * 0.65);
  const maxBet = maxBetFloor;
  const minBet = Math.max(2.00, Math.min(4.00, bal * 0.10)); // $2 hard floor
...
async function executeTrade(sig) {
  ...
  // $500K GUARDS — all must pass
  const bal = S.realBalance>0?S.realBalance:S.balance;
  const mktP = sig.marketPrice||(sig.side==='YES'?sig.confidence:1-sig.confidence);
  const tradePrice = sig.side==='YES'?mktP:(1-mktP);
  if(tradePrice<=0.15||tradePrice>=0.85){log('Skip '+sig.ticker+': '+( tradePrice*100).toFixed(0)+'c outside 15c-85c');return;}
  const edgeVsMarket = sig.confidence-tradePrice;
  if(edgeVsMarket<CFG.minEdge){log('Skip '+sig.ticker+': edge '+(edgeVsMarket*100).toFixed(1)+'% < min brain='+(sig.confidence*100).toFixed(0)+'% mkt='+(tradePrice*100).toFixed(0)+'%');return;}
  const serverKelly=kellySize(sig.confidence,tradePrice);
  if(serverKelly<2.00){log('Skip '+sig.ticker+': Kelly $'+serverKelly.toFixed(2)+' < $2');return;}
  if(tradePrice>0.72&&sig.confidence<0.82){log('Skip '+sig.ticker+': market strong need 82%+');return;}
  ...
  const priceCents=Math.round(Math.min(0.84,Math.max(0.16,orderP))*100);
  ...
  if(actualCost<2.00){log("Skip "+sig.ticker+": cost $"+actualCost.toFixed(2)+" < $2 min");return;}

...
const PHASES = [
  { name: 'SEED',     min: 0,      max: 50,     kelly: 0.35, minEdge: 0.05, minProb: 0.72, maxPos: 3, maxPct: 0.25, maxExposure: 0.65, maxBetDollars: 12,    label: '🌱 Seed',     desc: '3 slots. FastPath $12 max. Brain $5 max.' },
  { name: 'SPROUT',   min: 50,     max: 200,    kelly: 0.35, minEdge: 0.05, minProb: 0.70, maxPos: 3, maxPct: 0.22, maxExposure: 0.60, maxBetDollars: 35,    label: '🌿 Sprout',   desc: '3 slots. FastPath $35 max. Brain $14 max.' },
...
function kellySize(prob, price, source) {
  const phase = getPhase();
  const q = 1 - prob;
  ...
  const kellyFrac = isHighConfFastPath ? phase.kelly : phase.kelly * 0.40;
...
async function executeTrade(sig) {
  ...
  const serverKelly = kellySize(sig.confidence, sidePriceActual, sig.source || 'brain');
  ...
  const size = kellySize(sig.confidence, priceFloat, sig.source || 'brain');
...
4. Bot auto-restarts when cash is available`);
    ...
    // This cuts API cost 60-80% and prevents brain from placing bad weather/moon-shot trades
    ...
    if (brainReady && slotsAfterFastPath > 0 && hasMarkets && !fastPathFoundTrade && !inDDCooldown) {
      log(`🧠 Brain firing: FastPath=0 trades, ${slotsAfterFastPath} slots open`);
      await runBrain(markets);
    } else if (brainReady && fastPathFoundTrade) {
      log(`Brain skip — FastPath filled slot(s), saving API cost`);
      S.lastBrainAt = Date.now();
...
async function startBot() {
  ...
  _botStarted = true;
  S.isRunning = true;
  log('[M] KalshiBot v12 started -- running pre-flight sync...');
...
server.listen(CFG.port, () => {
  log(`KalshiBot v12 listening on port ${CFG.port}`);

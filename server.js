...
async function getTopMarkets() {
  try {
    ...
    // Only series that have proven to return markets — trimmed from 40 to 18
    const sportsMacroSeries = [
      // Sports (current season)
      'KXNBAGAME', 'KXMLBGAME', 'KXNHLGAME',
      // Crypto daily (for sub-12h daily BTC/ETH markets)
      'KXBTCD', 'KXETHUSD',
...
4. Restart — position caps now hard-enforced`);
    ...
    const [markets, topTraders] = await Promise.all([
      Promise.race([getTopMarkets(), new Promise(r => setTimeout(() => r([]), 25000))]), // 25s timeout
      getTopTraders(),
    ]);

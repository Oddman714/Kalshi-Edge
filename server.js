...
const PHASES = [
  { name: 'SEED',     min: 0,      max: 50,     kelly: 0.50, minEdge: 0.06, minProb: 0.58, maxPos: 5,  label: '[S] Seed',     desc: 'Aggressive compounding -- target high-confidence fast markets' },
  { name: 'SPROUT',   min: 50,     max: 200,    kelly: 0.45, minEdge: 0.06, minProb: 0.59, maxPos: 6,  label: '[SP] Sprout',   desc: 'Building -- stay aggressive, favor 55-80c YES contracts' },
  { name: 'GROWTH',   min: 200,    max: 1000,   kelly: 0.40, minEdge: 0.07, minProb: 0.60, maxPos: 7,  label: '[G] Growth',   desc: 'Scaling -- diversify, edge bar rising, fee awareness critical' },
  { name: 'MOMENTUM', min: 1000,   max: 5000,   kelly: 0.35, minEdge: 0.07, minProb: 0.61, maxPos: 8,  label: '[M] Momentum', desc: 'Momentum -- protect gains, press only clear edges' },
  { name: 'SCALE',    min: 5000,   max: 25000,  kelly: 0.30, minEdge: 0.08, minProb: 0.62, maxPos: 9,  label: '[SC] Scale',    desc: 'Scaling -- larger positions, only cleanest signals' },
  { name: 'HARVEST',  min: 25000,  max: 100000, kelly: 0.25, minEdge: 0.08, minProb: 0.63, maxPos: 10, label: '[H] Harvest',  desc: 'Harvest -- preservation + steady compounding' },
  { name: 'ENDGAME',  min: 100000, max: 500000, kelly: 0.20, minEdge: 0.09, minProb: 0.64, maxPos: 12, label: '[E] Endgame',  desc: 'Endgame -- controlled march to $500K' },
];
...
function checkMilestone(prevBal, newBal) {
  ...
  if (newD > prevD && newBal > CFG.bankroll) {
    ...
    tg(`[TARGET] <b>DOUBLING #${newD} COMPLETE!</b>\n[H] Balance: $${newBal.toFixed(2)}\n[AT] Phase: ${phase.label}\n[DONE] ${doublingsNeeded()} more doublings to $500K\n[DATA] Next target: $${nextTarget().toFixed(2)}\n\n${phase.desc}`);
    log(`[TARGET] MILESTONE: Doubling #${newD} -- $${newBal.toFixed(2)} | ${doublingsNeeded()} to $500K`);
  }
}
...
async function syncPositions() {
  try {
    ...
    // Only notify Telegram if positions actually changed (avoid spam every 5 scans)
    if (freshKalshi.length > 0 || S.openPositions.length !== prevCount) {
      tg(`[LIST] <b>Positions synced from Kalshi</b>\n${livePositions.map(p => `${p.ticker} ${p.side} x${p.contracts}`).join('\n')}`);
    }
    saveState();
  } catch(e) {
...
async function getTopMarkets() {
  try {
    ...
    // Expanded series list -- research shows sports is 75% of Kalshi volume
    const seriesList = [
      // Crypto -- high volume, 24/7, fast resolution
      'KXBTCD','KXBTCW','KXETHUSD','KXETH','KXSOL','KXXRP','KXDOGE',
      // Sports -- dominant volume, clear data sources for brain
      'KXNBAGAME','KXNBAPLAYOFFS','KXMLBGAME','KXNHLGAME','KXNFLGAME',
      'KXMMA','KXGOLF','KXTENNIS','KXSOCCER','KXNCAAMB',
      // Economics -- CPI, Fed, macro -- inefficient early, big edge possible
      'FED','FEDRATE','INFL','KXCPI','KXPCE','KXUNEMPLOYMENT','KXJOBS',
      // Commodities
      'KXOIL','KXGOLD','KXNG',
      // Politics/current events -- high volume around news cycles
      'KXPREZ','KXSENATE','KXHOUSE','KXTARIFF','KXTRADE','KXAPPROVAL','KXCONGRESS',
      // Indices -- only during market hours
      ...(isMarketHours ? ['INXD','NASDAQ100D','KXSPX500','KXEARNINGS','HIGHNY','HIGHMIA','KXVIX'] : []),
      ...(isMarketHours ? [] : ['INXW','NASDAQ100W','KXSPX500W']),
    ];
    ...
    log(`Series fetch: ${successCount}/${seriesList.length} returned markets`);

    if (allMarkets.length === 0) {
      log('Series fetch empty -- falling back to generic endpoint', 'WARN');
      try {
    ...
    log(`Raw: ${allMarkets.length} markets from ${seriesList.length} series`);

    // -- RESEARCH-BACKED FILTER --
    // Burgi et al. 2026: High-price contracts (50-85c) have best expected return.
    // Low-price (<40c) shows favorite-longshot bias -- avoid as primary targets.
    // Fee-adjusted floor: need gross edge > KALSHI_FEE to have positive net EV.
    const markets = allMarkets.filter(m => {
      ...
      // SWEET SPOT: 18c-88c -- avoid extreme longshots (fee kills edge)
      if (yesAsk <= 0.18 || yesAsk >= 0.88) return false;
      if (yesBid <= 0.02) return false;
      if ((yesAsk - yesBid) > 0.28) return false;
      const vol = parseFloat(m.volume_fp || m.volume || 0);
      if (vol < 25) return false;
      return true;
    });

    log(`After filter: ${markets.length} tradeable (18c-88c, vol>=25, spread<=28c)`);
    ...
    const scored = markets.map(m => {
      ...
      // Duration scoring -- fast resolution = faster compounding
      const durationScore = hoursLeft < 0.5 ? 0.05
        : hoursLeft < 2   ? 0.80
        : hoursLeft < 6   ? 1.0
        : hoursLeft < 12  ? 0.95
        : hoursLeft < 24  ? 0.85
      ...
      // Research-backed: high-price contracts outperform -- favor 55-80c
      // Fee-adjusted edge score: must overcome 4.5% fee drag
      const priceScore = yesAsk >= 0.55 && yesAsk <= 0.82 ? 1.3
        : yesAsk >= 0.45 && yesAsk < 0.55 ? 1.15
        : yesAsk >= 0.35 && yesAsk < 0.45 ? 1.0
        : yesAsk >= 0.25 && yesAsk < 0.35 ? 0.85
        : 0.65; // < 25c -- longshot bias penalty

      const volWeight = Math.min(1, Math.log10(Math.max(1, vol)) / 4);
      const edgeScore = Math.abs(yesAsk - 0.5) * 1.2 * (0.5 + 0.5 * volWeight);

      // Fee-adjusted: score penalized by the fee cost at this price
      const feeAdj = 1 - (KALSHI_FEE * (yesAsk >= 0.5 ? yesAsk : 1 - yesAsk));
      ...
      return {
        ...
        score: totalScore, hoursLeft: parseFloat(hoursLeft.toFixed(1)),
    ...
    if (S.topMarkets[0]) {
      log(`Top: ${S.topMarkets[0].ticker} YES=${(S.topMarkets[0].yesAsk*100).toFixed(1)}c vol=${S.topMarkets[0].volume} score=${S.topMarkets[0].score.toFixed(3)}`);
    }
    return S.topMarkets;
  } catch(e) {
...
async function runBrain(topMarkets) {
  ...
  const marketList = topMarkets.slice(0, 6).map(m =>
    `- ${m.ticker} | "${m.title}" | YES=${(m.yesAsk*100).toFixed(0)}c | Vol=${m.volume} | Score=${m.score.toFixed(3)} | Closes: ${m.closeTime ? new Date(m.closeTime).toLocaleDateString() : 'unknown'}`
  ).join('\n');
  ...
  const prompt = `{"task":"generate_trade_signals","rules":["respond ONLY with the JSON object","no explanation","no preamble","start with {","end with }"],"context":{"MISSION":"Double portfolio to $500K. Balance: $${_bal.toFixed(2)} | Cash: $${S.availableCash.toFixed(2)} | Resting: ${S.restingOrders.length} orders | Next target: $${_next.toFixed(2)} (${_progress}% there) | ${_doublings} doublings done, ${_needed} to go | Phase: ${phase.name}","STRATEGY_RESEARCH":"Academic analysis of 300K+ Kalshi contracts shows: 1) HIGH-PRICE contracts (55-85c) win MORE than their price implies -- positive expected return. 2) LOW-PRICE contracts (<35c) display favorite-longshot BIAS -- lose more than priced, avoid as primary bets. 3) Markets become more accurate near close -- late entry = better prices. 4) Fee drag is 4.5% on wins -- need gross edge of ${(phase.minEdge*100).toFixed(0)}%+ AFTER fees to be profitable.","FEE_RULE":"Kalshi fee ~4.5% on winning trades. Your true edge = (confidence - marketPrice) - (confidence * 0.045). Only trade if true_edge >= ${(phase.minEdge*100).toFixed(0)}%.","mode":"${CFG.dryRun?'paper':'live'}","balance":${_bal.toFixed(2)},"availableCash":${S.availableCash.toFixed(2)},"phase":"${phase.name}","openSlots":${dynamicMaxPos()-S.openPositions.length},"openPositions":${JSON.stringify(S.openPositions.map(p=>({ticker:p.ticker,side:p.side,entryPrice:p.entryPrice,hoursOpen:Math.round((Date.now()-p.openedAt)/3600000)})))},"recentMemory":${JSON.stringify(S.brainMemory.slice(-5))}},"markets":${JSON.stringify(topMarkets.slice(0,8).map(m=>({ticker:m.ticker,title:m.title,yesAsk:+(m.yesAsk*100).toFixed(1),yesBid:+((m.yesBid||0)*100).toFixed(1),volume:Math.round(m.volume),score:+m.score.toFixed(3),hoursLeft:m.hoursLeft,closesIn:m.closeTime?Math.round((new Date(m.closeTime)-Date.now())/3600000)+'h':'?'})))},"searchStrategy":{"searches":8,"instruction":"You have 8 web searches. Use ALL of them. Search TOP 4-5 markets, 1-2 searches each. Find: current score/price/data vs what the market expects. Search for breaking news, final scores, current prices, analyst consensus. The more you search the better your edge.","priority_search_topics":["live game score or final result","current crypto price vs strike price","latest economic data vs consensus","breaking news affecting outcome","weather forecast vs market","recent poll vs election market"]},"strategy":{"RESEARCH_INSIGHT_1":"HIGH-PRICE YES (55-85c) = best risk-adjusted return per academic research. Favor these.","RESEARCH_INSIGHT_2":"Markets near close have most accurate pricing -- but fast-moving data (scores, prices) can still create edges even minutes before close.","RESEARCH_INSIGHT_3":"Sports markets: check LIVE scores. If game is in progress and market hasn't updated, massive edge possible.","RESEARCH_INSIGHT_4":"Economic markets: compare actual released data to market expectation. If CPI just released below consensus and market hasn't moved -- instant edge.","RESEARCH_INSIGHT_5":"Crypto markets: use current price vs strike. BTC at $84K, market for >$83K at 45c? That should be 90c. Search first.","priority1":"SPORTS LIVE SCORES -- if any game is in progress, search immediately. Score + time remaining = edge.","priority2":"CRYPTO STRIKES -- current price vs threshold. Math is objective.","priority3":"ECON DATA -- check if any data released today vs market consensus.","priority4":"SHORT DURATION -- prefer markets closing <=24h for fast compounding.","MIN_CONFIDENCE":"${(phase.minProb*100).toFixed(0)}%","MIN_EDGE_AFTER_FEES":"${(phase.minEdge*100).toFixed(0)}% (gross edge minus 4.5% fee)","OUTPUT_RULE":"Signal at least 1 trade if ANY market has net edge >=${(phase.minEdge*100).toFixed(0)}% after fees and cash is available."},"required_json_format":{"signals":[{"ticker":"string","side":"YES or NO","confidence":0.65,"edge":0.09,"marketPrice":0.56,"feeAdjustedEdge":0.06,"reasoning":"SPECIFIC data: [exact score/price/data] vs market expectation [X]. Market is mispriced by [Y]c.","hoursToClose":6}],"marketSummary":"Best edge found and why","skipReason":"ONLY if ALL markets searched and zero net edge found after fees"}}`;
  ...
  try {
    ...
    if (S.signals.length > 0) {
      const sigText = S.signals.map(s =>
        `[DATA] ${s.ticker} ${s.side} | ${(s.confidence*100).toFixed(0)}% confidence | edge ${(s.edge*100).toFixed(1)}% | $${s.kellySize?.toFixed(2)} | ${s.reasoning}`
      ).join('\n');

      tg(`[BRAIN] <b>Brain #${S.brainCount}</b>\n${sigText}\n\n<i>${parsed.marketSummary || ''}</i>`);
  ...
  try {
    ...
    if (parsed.exitNow && parsed.exitNow.length > 0) {
      for (const exit of parsed.exitNow) {
        log(`[ALERT] Monitor: EXIT recommended for ${exit.ticker} -- ${exit.reason}`);
        tg(`[ALERT] <b>Position Monitor Alert</b>\nExit: ${exit.ticker}\n${exit.reason}`);
      ...
      log(`[SIG] Monitor: all ${S.openPositions.length} positions holding -- ${parsed.summary || ''}`);
    }
  } catch(e) {
...
function kellySize(prob, price) {
  ...
  const kelly = Math.max(0, (netWin * prob - q) / netWin);
  const fracKelly = kelly * phase.kelly;
  const rawBal = S.realBalance > 0 ? S.realBalance : S.balance;
  ...
  const maxPct = bal < 10 ? 0.75 : bal < 25 ? 0.60 : bal < 50 ? 0.48 : bal < 200 ? 0.36 : bal < 1000 ? 0.26 : 0.18;
...
async function executeTrade(sig) {
  ...
  if (CFG.dryRun) {
    ...
    log(`[NOTE] PAPER TRADE: ${sig.ticker} ${sig.side} x${contracts} @ ${price}c = $${position.cost.toFixed(2)}`);
    tg(`[NOTE] <b>Paper Trade</b>\n${sig.ticker} ${sig.side}\n${contracts} contracts @ ${price}c\nCost: $${position.cost.toFixed(2)}\n[WHY] ${sig.reasoning}`);
    saveState();
  } else {
    // Live trade
    try {
      ...
      S.openPositions.push(position);
      log(`[OK] LIVE ORDER: ${sig.ticker} ${sig.side} x${contracts} @ ${price}c`);
      tg(`[OK] <b>Live Order Placed</b>\n${sig.ticker} ${sig.side}\n${contracts} contracts @ ${price}c\nOrder ID: ${order.order?.order_id}`);
      saveState();
    } catch(e) {
      log(`Order failed ${sig.ticker}: ${e.message}`, 'ERROR');
      tg(`[ERR] <b>Order Failed</b>\n${sig.ticker}: ${e.message}`);
    }
  }
}
  ...
  for (const pos of [...S.openPositions]) {
    try {
      ...
      if (settled) {
        ...
        log(`${won ? '[WIN] WIN' : '[LOSS] LOSS'}: ${pos.ticker} ${pos.side} | PnL: ${pnl >= 0 ? '+' : ''}$${pnl.toFixed(2)}`);
        tg(`${won ? '[WIN] <b>WIN</b>' : '[LOSS] <b>LOSS</b>'}
${pos.ticker} ${pos.side}
...
Total: ${S.wins}W/${S.losses}L | All-time: ${S.totalPnl >= 0 ? '+' : ''}$${S.totalPnl.toFixed(2)}`);
      ...
      if (market.status === 'finalized' || market.status === 'settled' || market.result) {
        ...
        log(`${won ? '[WIN] WIN' : '[LOSS] LOSS'}: ${pos.ticker} | PnL: ${pnl >= 0 ? '+' : ''}$${pnl.toFixed(2)}`);
        tg(`${won ? '[WIN] <b>WIN</b>' : '[LOSS] <b>LOSS</b>'}
${pos.ticker} ${pos.side}
PnL: ${pnl >= 0 ? '+' : ''}$${pnl.toFixed(2)} | Balance: $${S.balance.toFixed(2)}`);
        saveState();
...
async function scan() {
  ...
  try {
    ...
    if (effectiveBal <= FLOOR && !CFG.dryRun) {
      if (S.isRunning) {
        log('[STOP] FLOOR HIT: balance $' + S.balance.toFixed(2) + ' <= $' + FLOOR + ' -- halting live trading', 'ERROR');
        tg(`[STOP] <b>Floor Hit -- Trading Halted</b>
...
Deposit funds and restart manually when ready.`);
    ...
    if (brainReady && hasSlots && hasMarkets) {
      log(`[BRAIN] Brain firing: ${S.openPositions.length}/${maxPos} positions, ${maxPos - S.openPositions.length} open slot(s), $${effectiveCash.toFixed(2)} cash`);
...
function sendHeartbeat() {
  ...
  const sigAge = S.lastBrainAt > 0
    ...
    : '--';
  const mode = CFG.dryRun ? '[NOTE] Paper' : '[LOSS] LIVE';
  ...
  tg(`[DATA] <b>Heartbeat -- ${phase.label}</b>
${mode} | Scans: ${S.scanCount} | Brain: #${S.brainCount}
[H] Balance: $${bal.toFixed(2)} -> Next: $${nextTarget().toFixed(2)}
...
[DONE] ${doublingsNeeded()} doublings to $500K | DD: ${drawdown}%
[COST] API: $${apiSpend.toFixed(2)} total (~$${dailyBurn.toFixed(2)}/day) | ROI vs spend: ${roiVsSpend}%`);
}

// --- FULL PLUMBING VALIDATION ----------------------------------------------
async function runPlumbingTest() {
  log('[TEST] Starting full plumbing validation...');
  const results = [];
  const check = (name, ok, detail) => {
    results.push({ name, ok, detail });
    log(`${ok ? '[OK]' : '[ERR]'} ${name}: ${detail}`);
  };
  ...
  // 4. Telegram
  try {
    const body = JSON.stringify({ chat_id: CFG.tgChat, text: '[TEST] Plumbing test: Telegram [OK]' });
  ...
  const lines = results.map(r => `${r.ok ? '[OK]' : '[ERR]'} <b>${r.name}</b>: ${r.detail}`).join('\n');
  const launchReady = failed === 0;

  tg(`[TEST] <b>Plumbing Test Complete</b>
${passed}/${results.length} checks passed

${lines}

${launchReady
  ? '[WIN] <b>LAUNCH READY</b> -- All systems operational'
  : `[LOSS] <b>NOT READY</b> -- ${failed} system(s) need attention`}

Mode: ${CFG.dryRun ? '[NOTE] Paper (set DRY_RUN=false to go live)' : '[LOSS] LIVE'}`);
...
async function startBot() {
  ...
  tg(`[M] <b>KalshiBot v10 Online</b>
Mode: ${CFG.dryRun ? '[NOTE] Paper' : '[LOSS] LIVE'}
...
<div id="pg-home" class="pg on">
  ...
  <div class="act-row">
    ...
    <button class="abtn abtn-purge" id="purgeBtn" onclick="doPurge()" title="Purge stale positions">[DEL]</button>
  </div>
  <div class="card">
...
<script>
/* KalshiBot v10 -- SSE live push + balance fix + research-backed strategy */
'use strict';
var API='',state=null,chartData=[],allTx=[],tf='1D',txf='all';
var _loaded=false,_retries=0,_sse=null,_pollTimer=null;
var _feedItems=[],_prevLogKey='',_lastBrain=0,_lastScan=0;
...
function fp(n){var x=isFinite(+n)?+n:0;return(x>=0?'+':'')+fm(Math.abs(x));}
function fpp(n){var x=isFinite(+n)?+n:0;return(x>=0?'+':'-')+fm(x);}

/* -- TOASTS -- */
function toast(msg,type){
  var t=document.createElement('div');
  t.className='toast toast-'+(type||'info');
  t.textContent=(msg||'').slice(0,90);
  document.body.appendChild(t);
  requestAnimationFrame(function(){t.classList.add('show');});
  setTimeout(function(){t.classList.remove('show');setTimeout(function(){t.parentNode&&t.parentNode.removeChild(t);},350);},3800);
}
...
function pushFeed(msg,type){
  ...
  _feedItems.unshift({msg:String(msg).slice(0,180),type:type||'info',ts:ts});
  if(_feedItems.length>80)_feedItems.pop();
  var w=g('liveFeed');
  if(w&&document.getElementById('pg-activity')&&document.getElementById('pg-activity').classList.contains('on'))renderFeed();
}
function renderFeed(){
  var w=g('liveFeed');if(!w)return;
  if(!_feedItems.length){w.innerHTML='<div class="lf-empty">No events yet -- start the bot</div>';return;}
  w.innerHTML=_feedItems.slice(0,30).map(function(f){
    var dc=f.type==='win'?'lf-g':f.type==='loss'||f.type==='error'?'lf-r':f.type==='brain'||f.type==='signal'?'lf-g':f.type==='trade'?'lf-gold':'lf-m';
    return '<div class="lf-row"><span class="lf-dot '+dc+'"></span><span class="lf-msg">'+escHtml(f.msg)+'</span><span class="lf-ts">'+f.ts+'</span></div>';
  }).join('');
}
...
function sseConnect(){
  ...
  try{
    ...
    _sse=es;
    es.addEventListener('update',function(){loadState();});
    es.addEventListener('notify',function(e){
      try{
        var d=JSON.parse(e.data);
        var m=d.msg||'';
        var type=/[WIN]|WIN|profit/i.test(m)?'win':/[LOSS]|LOSS|error/i.test(m)?'loss':/[BRAIN]|Brain|signal/i.test(m)?'brain':/[OK]|Order|trade/i.test(m)?'trade':/warn/i.test(m)?'warn':'info';
        pushFeed(m,type);
        if(/[WIN]|[LOSS]|[OK]|[ERR]|[TARGET]|[M]|[BRAIN]|WIN|LOSS|Order|Brain/i.test(m))toast(m.slice(0,80),type);
      }catch(ex){}
    });
    es.onopen=function(){
...
function _poll(ms){
  ...
  _pollTimer=setInterval(loadState,ms);
}
...
/* -- ICO -- */
function ico(t){
  if(/BTC|KXBTCD/i.test(t))return{e:'[BTC]',c:'ico-btc'};
  if(/ETH|KXETH/i.test(t))return{e:'[ETH]',c:'ico-eth'};
  if(/NBA|KXNBA/i.test(t))return{e:'[NBA]',c:'ico-nba'};
  if(/CPI|KXCPI|FED|FEDRATE|INFL/i.test(t))return{e:'[DATA]',c:'ico-cpi'};
  if(/NFL|KXNFL/i.test(t))return{e:'[NFL]',c:'ico-nba'};
  if(/MLB|KXMLB/i.test(t))return{e:'[MLB]',c:'ico-nba'};
  if(/NHL|KXNHL/i.test(t))return{e:'[NHL]',c:'ico-nba'};
  if(/SOL|KXSOL/i.test(t))return{e:'[SOL]',c:'ico-eth'};
  return{e:'~',c:''};
}
...
function renderPos(positions){
  ...
  w.innerHTML=positions.map(function(p){
    ...
    return '<div class="pos-card"><div class="pos-inner"><div class="pos-ico '+ic.c+'">'+ic.e+'</div>'
      ...
      +'<div class="pos-r"><div class="pos-price">'+ep+'c</div><div class="pos-cost">'+fm(cost)+'</div>'
      +'<div class="pos-pnl '+pc+'">'+(pnl>=0?'+':'')+fm(Math.abs(pnl))+'</div></div></div>'
      +'<div class="pos-foot">'
      ...
      +'<div class="pf"><div class="pf-lbl">Value</div><div class="pf-val">'+fm(ep*ct/100)+'</div></div>'
      +'</div></div>';
  }).join('');
}
...
function renderSigs(sigs){
  ...
  w.innerHTML=sigs.slice(0,5).map(function(s){
    ...
    var feeAdj=s.feeAdjustedEdge?'Fee-adj edge '+Math.round(s.feeAdjustedEdge*100)+'%':('Edge '+Math.round((s.edge||0)*100)+'%');
...
function renderMkts(){
  ...
  if(!state||!state.topMarkets||!state.topMarkets.length){
    w.innerHTML='<div class="empty">'+(state?'Scanner running...':'Loading...')+'</div>';return;
  }
  w.innerHTML=state.topMarkets.slice(0,12).map(function(m){
    ...
    var edge=+(m.edge||0);
    var hlStr=hl<999?(hl<1?'<1h':Math.round(hl)+'h'):'--';
    return '<div class="mkt-card"><div class="mkt-top"><div class="mkt-ttl">'+(m.title||m.ticker)+'</div>'
      ...
      +'<div class="mkt-row"><div class="mkt-yes">'+yp+'c</div>'
      +'<div class="mkt-nr"><div class="mkt-no">'+np+'c</div><div class="mkt-vol">vol '+(m.volume||0).toLocaleString()+'</div></div></div>'
...
function renderTx(){
  ...
  w.innerHTML=Object.keys(groups).map(function(mon){
    return '<div class="tx-mon">'+mon+'</div>'+groups[mon].map(function(tx,i){
      ...
      return '<div class="tx-row"><div class="tx-ico '+ic+'">'+(tx.type==='win'?'W':tx.type==='loss'?'L':'~')+'</div>'
        +'<div class="tx-info"><div class="tx-ttl">'+lbl+' . '+escHtml(tx.ticker)+'</div>'
        +'<div class="tx-sub">'+tx.side+' . '+(tx.contracts||1)+' ct . '+new Date(tx.ts).toLocaleDateString('en-US',{month:'short',day:'numeric'})+'</div></div>'
        ...
        +'</div>'+(i<groups[mon].length-1?'<div class="tx-div"></div>':'');
    }).join('');
  }).join('');
}
...
function applyState(s){
  ...
  /* -- safe helpers local to applyState -- */
  ...
  var wr=(s.winRate!=null&&s.winRate!==''&&isFinite(+s.winRate))?+s.winRate:null;
  ...
  try{
    var hb=g('heroBal');
    if(hb){hb.style.fontSize='';hb.style.color='';}
    st('heroBal', bal>0 ? fm(bal) : (_loaded ? '$0.00' : 'Connecting...'));
    st('heroReal', real>0 ? fm(real) : '$0.00');
    st('heroPeak', peak>0 ? fm(peak) : '$0.00');
    var pnlEl=g('heroPnl');
    if(pnlEl){pnlEl.textContent=(total>=0?'+':'-')+fm(Math.abs(total));pnlEl.style.color=total>=0?'var(--g)':'var(--r)';}
    st('heroArr',today>=0?'^':'v');
    st('heroChg',fm(Math.abs(today))+' today');
    var chip=g('heroChip');
    if(chip)chip.className='hero-chip '+(today>0?'chip-up':today<0?'chip-dn':'');
    pushPt(bal);
  }catch(e){console.warn('hero crash',e.message);}
  ...
  try{
    ...
    if(tdEl){tdEl.textContent=(today>=0?'+':'')+fm(Math.abs(today));tdEl.style.color=today>0?'var(--g)':today<0?'var(--r)':'var(--m)';}
    ss('todayBar','background',today>0?'var(--g)':today<0?'var(--r)':'var(--m2)');
    st('statTodaySub',(total>=0?'+':'-')+fm(Math.abs(total))+' all-time');
    st('statPos',openCnt);
    st('statPosSub','of '+mp+' max');
  }catch(e){console.warn('stats crash',e.message);}
  ...
  try{
    ...
    var availStr=avail>0?'$'+avail.toFixed(2):'$'+bal.toFixed(2);
  ...
  }catch(e){console.warn('engine crash',e.message);}

  /* -- BRAIN -- */
  try{
    var sig0=s.signals&&s.signals[0];
    if(sig0)st('brainTxt',_s(sig0.reasoning)||'Analyzing...');
    else if(running)st('brainTxt','Scanner active'+age+' . hunting for edge...');
  ...
  try{
    if(s.phaseLabel){
      ...
      if(s.cfg){
        ...
        st('jEdge',Math.round((_n(s.cfg.minEdge)||0.06)*100)+'%');
      }
    }
  }catch(e){console.warn('journey crash',e.message);}
  ...
  try{
    ...
    var wrCfg=g('cfgWR');if(wrCfg){wrCfg.textContent=wr!=null?wr.toFixed(1)+'%':'--';wrCfg.className='sv '+(wr!=null?(wr>=65?'sv-g':wr>=50?'sv-gold':'sv-r'):'');}
    var wlCfg=g('cfgWL');if(wlCfg){wlCfg.textContent=w+'W / '+l+'L';wlCfg.className='sv '+(w>l?'sv-g':w<l?'sv-r':'');}
    var tdCfg=g('cfgToday');if(tdCfg){tdCfg.textContent=(today>=0?'+':'-')+fm(Math.abs(today));tdCfg.className='sv '+(today>=0?'sv-g':'sv-r');}
    st('cfgScans',(_n(s.scanCount)).toLocaleString());
    st('cfgBrain',(_n(s.brainCount)).toLocaleString());
    st('cfgSpend','$'+_n(s.estimatedApiSpend).toFixed(2));
    st('cfgMode',dryRun?'Paper':'LIVE');st('cfgPhase',_s(s.phaseLabel)||'--');
    st('cfgMax',mp);st('cfgOpen',openCnt+' / '+mp);
    st('cfgBrainInt',s.cfg?((_n(s.cfg.brainInterval)||120000)/60000).toFixed(1)+'min':'2.0min');
  }catch(e){console.warn('settings crash',e.message);}
  ...
  try{
    ...
    if(lw&&s.logs&&s.logs.length){
      lw.innerHTML=s.logs.slice(0,20).map(function(l){
        ...
        return '<div'+(cls?' class="'+cls+'"':'')+'><span class="log-ts">'+(l.ts||'')+'</span>'+escHtml(l.msg||'')+'</div>';
      }).join('');
      if(s.logs[0]){
        var newKey=(s.logs[0].ts||'')+'|'+(s.logs[0].msg||'');
        if(newKey!==_prevLogKey){
          _prevLogKey=newKey;
          var l0=s.logs[0];
          var lvl0=(l0.level||'').toLowerCase();
          var ftype=lvl0==='error'?'error':lvl0==='warn'?'warn':/WIN|[WIN]/.test(l0.msg)?'win':/LOSS|[LOSS]/.test(l0.msg)?'loss':/Brain|[BRAIN]/.test(l0.msg)?'brain':/Order|[OK]/.test(l0.msg)?'trade':'info';
          pushFeed(l0.msg,ftype);
        }
      }
    }
  }catch(e){console.warn('log crash',e.message);}

  /* -- BRAIN TOASTS -- */
  try{
    if(s.brainCount&&s.brainCount>_lastBrain&&_lastBrain>0){
      var bs=s.signals&&s.signals[0];
      toast('[BRAIN] Brain #'+s.brainCount+(bs?' -> '+bs.ticker+' '+bs.side:''),bs?'signal':'info');
    }
    _lastBrain=_n(s.brainCount);
  }catch(e){}

  /* -- POSITIONS + SIGNALS + TRADES + MARKETS -- */
  try{renderPos(s.openPositions||[]);}catch(e){console.warn('renderPos crash',e.message);}
  try{renderSigs(s.signals||[]);}catch(e){console.warn('renderSigs crash',e.message);}
  try{
  ...
  }catch(e){allTx=[];console.warn('trades crash',e.message);}
  try{renderMkts();}catch(e){console.warn('renderMkts crash',e.message);}
...
function loadState(){
  fetch(API+'/api/state?_='+Date.now(),{cache:'no-store'})
    ...
    .then(function(txt){
      ...
      catch(e){console.error('JSON:',txt.slice(0,120));throw new Error('Bad JSON');}
    ...
    .catch(function(e){
      ...
      if(!_loaded){
        var hb=g('heroBal');
        if(hb){hb.textContent=_retries>3?'Reconnecting...':'Connecting...';hb.style.fontSize='26px';hb.style.color='var(--m)';}
        var hc=g('heroChg');if(hc)hc.textContent='retry '+_retries;
      }
    });
}
function doPurge(){
  if(!confirm('Purge all Kalshi-synced positions from bot tracker?\n\nThis removes stale positions that are blocking new trades.\nYour actual Kalshi positions are NOT affected.'))return;
  fetch(API+'/api/purge-positions',{method:'POST'})
    .then(function(r){return r.json();})
    .then(function(d){
      toast('[DEL] Purged '+d.removed+' stale positions -- '+d.remaining+' remain','trade');
      setTimeout(loadState,500);
    }).catch(function(){toast('Purge failed -- try sync','error');});
}
function doSync(){
  ...
  fetch(API+'/api/sync',{method:'POST'})
    ...
    .then(function(t){
      try{var s=JSON.parse(t);state=s;applyState(s);}
      catch(e){loadState();}
      if(btn)btn.style.opacity='1';
    }).catch(function(){loadState();if(btn)btn.style.opacity='1';});
}
function toggleBot(){
  fetch(API+'/api/toggle',{method:'POST'}).then(function(){setTimeout(loadState,500);});
}

/* -- BOOT -- */
loadState();          /* immediate first fetch */
sseConnect();         /* SSE for instant push */
_poll(3000);          /* fallback poll -- SSE will slow to 8s on connect */
</script>
</body>
</html>
`;
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/' || url.pathname === '/app' || url.pathname === '/dashboard') {
    ...
    const _html = DASHBOARD
      ...
      .replace('__INIT_PHASE__', (getPhase().label || '[S] Seed'));
    res.writeHead(200, {
    ...
    try {
      ...
      const slim = {
        ...
        logs: (S.logs||[]).slice(0,20).map(l => ({
          ts: _ss(l.ts,10), level: _ss(l.level,5), msg: _ss(l.msg, 120),
        })),

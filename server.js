...
function dynamicMaxPos() {
  ...
  if (bal < 10)  return 2;  // $0-10:   2 concentrated positions — max impact per dollar
  if (bal < 20)  return 3;  // $10-20:  3 positions
  if (bal < 40)  return 4;  // $20-40:  4 positions
  if (bal < 75)  return 5;  // $40-75:  5 positions
  if (bal < 150) return 6;  // $75-150: 6 positions
  if (bal < 300) return 7;  // $150-300: 7 positions
  return 8;                 // $300+:   8 positions, fully diversified
}
...
server.listen(CFG.port, () => {
  log(`KalshiBot v9.1 listening on port ${CFG.port}`);

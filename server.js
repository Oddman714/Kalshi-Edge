...
async function getTopMarkets() {
  try {
    ...
    // Boost markets we don't already hold to encourage diversity
    const heldSet = new Set(S.openPositions.map(p => p.ticker));
    scored.forEach(m => {
      if (heldSet.has(m.ticker)) m.score *= 0.1; // Deprioritize held markets
    });
...
async function executeTrade(sig) {
  ...
  if(actualCost<1.00){log("Skip "+sig.ticker+": cost $"+actualCost.toFixed(2)+" < $1.00 min");return;}

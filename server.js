...
async function runBrain(topMarkets) {
  ...
  // COST GUARD: skip if we have fresh unexecuted signals from last 60s
  // (brain fired but trades haven't resolved yet — no new info to act on)
  const freshSignals = (S.signals || []).filter(s => s.ts && (Date.now() - s.ts) < 60000);
  if (freshSignals.length > 0 && S.openPositions.length >= dynamicMaxPos()) {
    log(`Brain skipped: ${freshSignals.length} fresh signals + slots full — saving API credits`);
    return;
  }
...
<head>
<script>
// CACHE BUSTER - runs before anything else
(function(){
  var V='20260408154831';
  // Kill any service workers
  ...
  // Force reload if cached version detected (old JS shows $-- and version mismatch)
  var stored = sessionStorage.getItem('kbv');
  if(stored !== V){
    sessionStorage.setItem('kbv',V);
    if(stored !== null){
      // Old version detected - hard reload
...
<!-- PWA mode disabled to prevent iOS shell caching -->
<meta http-equiv="cache-control" content="no-cache, no-store, must-revalidate">
<meta http-equiv="pragma" content="no-cache">
<meta http-equiv="expires" content="0">
<title>KalshiBot 20260408154831</title>
...
.j-fill{height:100%;background:linear-gradient(90deg,var(--green),#00ff88);border-radius:4px;transition:width 0.9s cubic-bezier(.4,0,.2,1)}
...
.j-pips{display:flex;gap:0;overflow-x:auto;scrollbar-width:none;padding-bottom:2px}
...
.pip-node{width:10px;height:10px;border-radius:50%;background:var(--s3);border:2px solid var(--border);transition:all 0.4s}
.pip-node.done{background:var(--green);border-color:var(--green)}
.pip-node.now{background:transparent;border-color:var(--gold);animation:nowpip 1.8s infinite}
...
.pos-ticker{font-size:13px;font-weight:700;font-family:var(--mono);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
<body>
<div id="errBanner" style="display:none;position:fixed;top:0;left:0;right:0;background:#ff3d5a;color:#fff;font-size:12px;font-weight:600;padding:10px 16px;text-align:center;z-index:9999;letter-spacing:0.02em"></div>
<div class="app">
...
<div id="page-home" class="page active">
  ...
  <div class="hero">
    ...
    <div class="hero-row">
      ...
      <div class="hero-cell"><div class="hc-label">All-time P&L</div><div class="hc-val sv-green" id="heroTotal">$--</div></div>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div class="journey-body">
      ...
      <div class="j-stats">
        ...
        <div class="j-stat"><div class="j-stat-lbl">Kelly Frac</div><div class="j-stat-val sv-gold" id="jKelly">--</div></div>
  ...
  <div style="text-align:center;font-size:9px;color:rgba(238,238,245,0.18);padding:4px;font-family:monospace">v20260408154831</div>
  <div class="gap"></div>
</div>
...
<div id="page-predict" class="page">
  <div class="page-hdr">
    ...
    <div class="page-sub">Ranked by edge</div>
  </div>
  ...
  <div class="settings-wrap">
    ...
    <div class="s-section">
      ...
      <div class="s-group">
        ...
        <div class="s-row"><div><div class="s-name">Starting Bankroll</div><div class="s-sub">Actual initial capital</div></div><div class="s-val" id="cfgBankroll">$--</div></div>
        <div class="s-row"><div><div class="s-name">Phase Kelly Fraction</div></div><div class="s-val sv-gold" id="cfgKelly">--</div></div>
      </div>
    </div>
    <div class="s-section">
      ...
      <div class="s-group">
        <div class="s-row">
          <div>
            ...
            <div class="wr-note s-sub" id="wrNote">-- resolved trades</div>
          </div>
          <div class="s-val sv-green" id="cfgWR">--</div>
        </div>
        <div class="s-row"><div><div class="s-name">Win / Loss Record</div></div><div class="s-val" id="cfgWL">--</div></div>
    ...
    <div class="s-section">
      ...
      <div class="s-group">
        ...
        <div class="s-row"><div><div class="s-name">Est. Daily Burn</div><div class="s-sub">at current rate</div></div><div class="s-val sv-gold" id="cfgDailyBurn">$0.00/day</div></div>
      </div>
    </div>
    <div class="s-section">
...
<script>
var API='',POLL=12000,state=null,chartData=[],allTx=[],currentTF='1D',txFilter='all';
...
function setTF(tf,el){currentTF=tf;document.querySelectorAll('.tf-btn').forEach(function(b){b.classList.remove('active');});el.classList.add('active');drawChart();}

function pushChartPoint(v){chartData.push({t:Date.now(),v:v});if(chartData.length>600)chartData.shift();drawChart();}
...
function drawChart(){
  ...
  var W=430,H=164,pad=10;
  var vals=data.map(function(p){return p.v;}),mn=Math.min.apply(null,vals),mx=Math.max.apply(null,vals),rng=mx-mn||0.01;
...
function renderWLBars(trades){
  ...
  var mx=Math.max.apply(null,buckets.map(function(b){return b.w+b.l;}));mx=Math.max(mx,1);
  wrap.innerHTML=buckets.map(function(b){
    if(!b.w&&!b.l) return '<div class="wl-bar wl-e" style="height:5px"></div>';
    var h=Math.max(7,((b.w+b.l)/mx)*50);
    return '<div class="wl-bar '+(b.w>=b.l?'wl-w':'wl-l')+'" style="height:'+h+'px"></div>';
  }).join('');
  ...
  el.style.color=wins>=losses?'var(--green)':'var(--red)';
}

function mktIcon(ticker){
  if(/BTC|KXBTCD/i.test(ticker)) return {e:'B',cls:'btc'};
  if(/ETH|KXETH/i.test(ticker)) return {e:'E',cls:'eth'};
  if(/NBA|KXNBA/i.test(ticker)) return {e:'B',cls:'nba'};
  if(/NFL|KXNFL/i.test(ticker)) return {e:'F',cls:'nfl'};
  if(/CPI|KXCPI/i.test(ticker)) return {e:'C',cls:'cpi'};
  if(/FED|KXFED/i.test(ticker)) return {e:'F',cls:'fed'};
  return {e:'~',cls:'gen'};
}
...
function renderPositions(positions){
  ...
  wrap.innerHTML=positions.map(function(p){
    ...
    var emojiMap={btc:'&#8383;',eth:'&#926;',nba:'&#127936;',nfl:'&#127944;',cpi:'&#128202;',fed:'&#127963;',gen:'&#128200;'};
    var em=emojiMap[icon.cls]||'~';
    var catMap={btc:'BTC',eth:'ETH',nba:'NBA',nfl:'NFL',cpi:'CPI',fed:'FED',gen:'MKTX'};
    var cat=catMap[icon.cls]||'MKTX';
    var pnlCls=pnl>0?'pnl-pos':pnl<0?'pnl-neg':'pnl-zero';
    var pnlStr=(pnl>=0?'+':'')+formatMoney(pnl);
    return '<div class="pos-card">'+
      '<div class="pos-card-inner">'+
        '<div class="pos-icon '+icon.cls+'">'+em+'</div>'+
        '<div class="pos-info">'+
          '<div class="pos-ticker">'+p.ticker+'</div>'+
          '<div class="pos-tags">'+
            '<span class="side-tag '+(isYes?'tag-yes':'tag-no')+'">'+(isYes?'YES':'NO')+'</span>'+
            '<span class="pos-meta-tag">'+contracts+' contract'+(contracts!==1?'s':'')+' &middot; '+cat+'</span>'+
          '</div>'+
        '</div>'+
        '<div class="pos-right">'+
          '<div class="pos-price-big">'+ep+'&cent;</div>'+
          '<div class="pos-cost-line">Cost '+formatMoney(cost)+'</div>'+
          '<div class="pos-pnl-line '+pnlCls+'">'+pnlStr+'</div>'+
        '</div>'+
      '</div>'+
      '<div class="pos-footer">'+
        '<div class="pf-item"><div class="pf-lbl">Confidence</div><div class="pf-val">'+conf+'</div></div>'+
        '<div class="pf-item" style="align-items:center"><div class="pf-lbl">Open</div><div class="pf-val">'+hoursOpen+'h</div></div>'+
        '<div class="pf-item" style="align-items:flex-end"><div class="pf-lbl">Value</div><div class="pf-val">'+formatMoney(ep*contracts/100)+'</div></div>'+
      '</div>'+
    '</div>';
  }).join('');
}
  ...
  wrap.innerHTML=signals.slice(0,5).map(function(s){
    ...
    return '<div class="sig-card '+(isYes?'':'sig-no')+'">'+
      ...
      '<div class="sig-stats-row">'+
        ...
        '<div class="sig-stat">Size <strong>'+formatMoney(s.kellySize||0)+'</strong></div>'+
...
function renderTxFeed(txList,filter){
  ...
  wrap.innerHTML=Object.keys(groups).map(function(month){
    ...
    return '<div class="tx-month">'+month+'</div>'+
      txs.map(function(tx,i){
        var lbl=tx.type==='win'?'Win':tx.type==='loss'?'Loss':'Open';
        var iconCls='ti-'+(tx.type==='win'?'win':tx.type==='loss'?'loss':'open');
        var em=tx.type==='win'?'&#10003;':tx.type==='loss'?'&#10007;':'~';
        var pnlDisp=tx.type==='win'&&tx.pnl>0?'+'+formatMoney(tx.pnl):
                    tx.type==='win'?'+$0.00':
                    tx.type==='loss'?'-'+formatMoney(Math.abs(tx.pnl||0)):
                    '-'+formatMoney(Math.abs(tx.cost||0));
        var pnlCls=tx.type==='win'?'tx-amt-pos':tx.type==='loss'?'tx-amt-neg':'tx-amt-zero';
        return '<div class="tx-row">'+
          '<div class="tx-icon '+iconCls+'">'+em+'</div>'+
          '<div class="tx-info">'+
            '<div class="tx-title">'+lbl+' &middot; '+tx.ticker+'</div>'+
            '<div class="tx-sub">'+tx.side+' &middot; '+(tx.contracts||1)+' contract &middot; '+new Date(tx.ts).toLocaleDateString('en-US',{month:'short',day:'numeric'})+'</div>'+
          '</div>'+
          '<div class="tx-right">'+
            '<div class="tx-amt '+pnlCls+'">'+pnlDisp+'</div>'+
...
function renderMarkets(){
  ...
  wrap.innerHTML=state.topMarkets.slice(0,15).map(function(m){
    ...
    var edge=m.edge||0,badgeCls=edge>0.08?'mb-high':'mb-med';
    return '<div class="mkt-card">'+
      '<div class="mkt-top"><div class="mkt-title">'+(m.title||m.ticker)+'</div><div class="mkt-exp'+(soon?' soon':'')+'">'+
      (hl<999?(hl<1?'<1h':Math.round(hl)+'h'):'--')+'</div></div>'+
      '<div class="mkt-row"><div class="mkt-yes">'+yp+'&cent; YES</div>'+
      '<div class="mkt-no-block"><div class="mkt-no">'+np+'&cent; NO</div><div class="mkt-vol">vol '+(m.volume||0).toLocaleString()+'</div></div></div>'+
...
function renderPips(done,needed,bankroll){
  ...
  var total=Math.min(done+(needed||16),16);
  var html='';
  for(var i=0;i<total;i++){
    var amt=bankroll*Math.pow(2,i+1);
    var lbl=amt>=1000?'$'+(amt/1000).toFixed(0)+'K':'$'+Math.round(amt);
    var isDone=i<done,isCurrent=i===done;
    html+='<div class="j-pip"><div class="pip-node'+(isDone?' done':isCurrent?' now':'')+'"></div>'+
          '<div class="pip-lbl'+(isDone?' done':isCurrent?' now':'')+'">'+lbl+'</div></div>';
  }
  wrap.innerHTML=html;
}

function formatMoney(n){return '$'+(Math.abs(n)).toFixed(2);}
function fmtSigned(n){return(n>=0?'+':'-')+formatMoney(n);}
...
function loadState(){
  fetch(API+'/api/state?_='+Date.now(),{cache:'no-store'})
    ...
    .catch(function(e){
      _retries++;
      var delay=Math.min(2000*_retries,15000);
      console.error('loadState failed (retry '+_retries+'):',e.message);
      var eb=document.getElementById('errBanner');
      if(eb){eb.textContent='Connecting to bot... (attempt '+_retries+')';eb.style.display='block';}
      setTimeout(loadState,delay);
    });
}

function applyState(s){
  var real=s.realBalance||0;
  var bal=real>0?real:(s.balance||0);
  var avail=s.availableCash||0;
  var today=s.todayPnl||0;
  var total=s.totalPnl||0;
  var peak=s.peakBalance||bal;
  var w=s.wins||0,l=s.losses||0,totalTrades=w+l;
  var openCnt=(s.openPositions||[]).length;
  var mp=s.maxPos||5;
  var wr=s.winRate!==null&&s.winRate!==undefined?parseFloat(s.winRate):null;
  var bankroll=s.startingBankroll||50;
  var dryRun=s.dryRun!==false;

  // HERO
  document.getElementById('heroBalance').textContent=formatMoney(bal);
  document.getElementById('heroReal').textContent=formatMoney(real);
  document.getElementById('heroPeak').textContent=formatMoney(peak);
  var totEl=document.getElementById('heroTotal');
  totEl.textContent=fmtSigned(total);
  ...
  document.getElementById('heroChangeText').textContent=(today>=0?'+':'-')+formatMoney(today)+' today';
  ...
  if(dryRun){pill.className='mode-pill pill-paper';dot.style.background='var(--gold)';}
  else{pill.className='mode-pill pill-live';dot.style.background='var(--green)';}

  // STATS
  var wrEl=document.getElementById('statWR');
  var wrBar=document.getElementById('wrBar');
  if(wr!==null){
  ...
  document.getElementById('statWRsub').textContent=totalTrades+' resolved';

  var td=document.getElementById('statToday');
  td.textContent=(today>=0?'+':'')+formatMoney(today);
  ...
  document.getElementById('statTodaySub').textContent=fmtSigned(total)+' all-time';
  ...
  document.getElementById('engineSub').textContent=running?
    'Scan #'+(s.scanCount||0)+' - '+openCnt+' open - '+formatMoney(avail)+' avail':
    'Tap Start to begin';
  ...
  if(s.phaseLabel){
    ...
    var pct=parseFloat(s.doublingProgress||0);
    var nxt=parseFloat(s.nextTarget||0);
    ...
    document.getElementById('jDone').textContent=dm+'x';
    if(s.cfg){
    ...
    document.getElementById('cfgCurBal').textContent=formatMoney(bal);
    document.getElementById('cfgNextTarget').textContent='-> $'+nxt.toLocaleString('en-US',{maximumFractionDigits:0});
    document.getElementById('cfgDoublings').textContent=dm+'x done';
  ...
  document.getElementById('cfgBankroll').textContent=formatMoney(bankroll);
  ...
  tpEl.textContent=fmtSigned(total);tpEl.className='s-val '+(total>=0?'sv-green':'sv-red');
  var tdEl=document.getElementById('cfgTodayPnl');
  tdEl.textContent=fmtSigned(today);tdEl.className='s-val '+(today>=0?'sv-green':'sv-red');
  ...
  // Settings portfolio
  document.getElementById('cfgReal').textContent=formatMoney(real);
  document.getElementById('cfgAvail').textContent=formatMoney(avail);
  document.getElementById('cfgPeak').textContent=formatMoney(peak);
  ...
  document.getElementById('cfgBrainInt').textContent=s.cfg?((s.cfg.brainInterval||120000)/60000).toFixed(1)+'min':'2.0min';
  document.getElementById('cfgOpen').textContent=openCnt+' of '+mp;

  // API spend tracker
  var apiSpend=parseFloat(s.estimatedApiSpend||0);
  var botUptimeHours=s.startedAt?(Date.now()-s.startedAt)/3600000:0;
  var dailyBurn=botUptimeHours>0.5?(apiSpend/botUptimeHours)*24:0;
  var apiEl=document.getElementById('cfgApiSpend');
  var burnEl=document.getElementById('cfgDailyBurn');
  if(apiEl) apiEl.textContent='$'+apiSpend.toFixed(2);
  if(burnEl){
    burnEl.textContent='$'+dailyBurn.toFixed(2)+'/day';
    // Color-code: green if burn < balance, red if burn > balance
    burnEl.style.color=dailyBurn>parseFloat(bal)?'var(--red)':'var(--gold)';
  }
  ...
  if(s.logs&&s.logs.length){
    ...
    lw.innerHTML=s.logs.slice(0,60).map(function(l){
...
function toggleBot(){
  fetch(API+'/api/toggle',{method:'POST'}).then(function(){setTimeout(loadState,600);}).catch(function(e){console.error(e);});
}
...
drawChart();
loadState();
setInterval(loadState,POLL); // each call uses Date.now() cache bust
</script>
</body>
</html>
`;
...
const server = http.createServer((req, res) => {
  ...
  // Versioned dashboard - iOS can never cache a changing URL
  const DASH_VERSION = '20260408154848';
  if (url.pathname === '/') {
    // Redirect to versioned URL - this forces iOS to fetch fresh every deploy
    res.writeHead(302, {
      'Location': '/app?v=' + DASH_VERSION,
      'Cache-Control': 'no-store, no-cache, must-revalidate',
    });
    return res.end();
  }
  if (url.pathname === '/app' || url.pathname === '/dashboard') {
    res.writeHead(200, {

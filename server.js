...
function applyState(s){
  ...
  try{
    ...
    st('statTodaySub',fpp(total)+' all-time');
    st('statPos',openCnt);
    ...
    st('engSub',running?'Scan #'+(s.scanCount||0)+' · '+openCnt+'/'+mp+' · $'+avail.toFixed(2)+age:'Tap Start to begin');
    ...
    if(s.phaseLabel){
      ...
      st('jDone',dm+'x');
      if(s.cfg){
        st('jKelly',Math.round((s.cfg.kellyFrac||0.5)*100)+'%');
        st('jEdge',Math.round((s.cfg.minEdge||0.06)*100)+'%');
      }
    }

    /* ── SETTINGS ── */
    st('cfgReal',fm(real));st('cfgAvail',fm(avail));st('cfgPeak',fm(peak));
    var pnEl=g('cfgPnl');if(pnEl){pnEl.textContent=fpp(total);pnEl.className='sv '+(total>=0?'sv-g':'sv-r');}
    var wrCfg=g('cfgWR');if(wrCfg){wrCfg.textContent=wr!=null?wr.toFixed(1)+'%':'--';wrCfg.className='sv '+(wr>=65?'sv-g':wr>=50?'sv-gold':wr!=null?'sv-r':'');}
    var wlCfg=g('cfgWL');if(wlCfg){wlCfg.textContent=w+'W / '+l+'L';wlCfg.className='sv '+(w>l?'sv-g':w<l?'sv-r':'');}
    var tdCfg=g('cfgToday');if(tdCfg){tdCfg.textContent=fpp(today);tdCfg.className='sv '+(today>=0?'sv-g':'sv-r');}
    st('cfgScans',(s.scanCount||0).toLocaleString());
    st('cfgBrain',(s.brainCount||0).toLocaleString());
    ...
    if(lw&&s.logs&&s.logs.length){
      ...
      /* Sync live feed from new log entries */
      if(s.logs[0]){
    ...
    /* ── BRAIN / SCAN CHANGE TOASTS ── */
    if(s.brainCount&&s.brainCount>_lastBrain&&_lastBrain>0){
      var bs=s.signals&&s.signals[0];toast('🧠 Brain #'+s.brainCount+(bs?' → '+bs.ticker+' '+bs.side:''),bs?'signal':'info');
    }
    _lastBrain=s.brainCount||0;
    ...
    var hb2=g('heroBal');
    if(hb2&&!_loaded)hb2.textContent='Error — retrying...';
  }
}

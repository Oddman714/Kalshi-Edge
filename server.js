...
async function resolvePositions() {
  ...
  let settlements = [];
  try {
    const sr = await kalshi('GET', '/portfolio/settlements', null, { limit: '20' });
    settlements = sr.settlements || [];
  } catch(e) {
    log('Settlement fetch failed: ' + e.message, 'WARN');
  }
...
function render(s) {
  ...
  changeEl.textContent = (totalChange >= 0 ? '↗ +' : '↘ ') + '$' + Math.abs(totalChange).toFixed(2) + ' (' + pct + '%)';
  ...
  const avgWin = closedTrades.length > 0
    ...
    : null;
  document.getElementById('bdAvgWin').textContent = avgWin !== null ? '$'+avgWin.toFixed(2) : '—';
  const drawdown = s.peakBalance > 0

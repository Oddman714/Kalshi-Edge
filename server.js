...
async function runCryptoFastPath(topMarkets) {
  ...
  for (const m of cryptoMarkets) {
    ...
    // --- DIRECTION MARKET HANDLER (no strike needed) ---
    // Handles: "BTC price up in next 15 mins?", "ETH price up in next 15 mins?"
    // These markets resolve YES if price goes UP over the window — pure momentum trade.
    // Strategy: use recent price momentum (compare current price vs 30s-cached price)
    // and Binance 5m kline trend to determine direction with high confidence.
    // Direction markets: ONLY pure "price up/down?" with NO dollar target.
    // Up/Down markets that HAVE a dollar strike (e.g. "BTC 15 min · $74,895 target")
    // are handled by the regular gap/strike logic below — they don't need trend analysis.
    // Only route to direction handler if there is genuinely no numeric target.
    ...
    if (isDirectionMarket) {
      const isBTCdir  = /kxbtc/i.test(m.ticker);
      const isETHdir  = /kxeth/i.test(m.ticker);
      const isSOLdir  = /kxsol/i.test(m.ticker);
      const isXRPdir  = /kxxrp/i.test(m.ticker);
      const isDOGEdir = /kxdoge/i.test(m.ticker);
      const isBNBdir  = /kxbnb/i.test(m.ticker);
      const isHYPEdir = /kxhype/i.test(m.ticker);
      const livePrice2 = isBTCdir ? prices.btc
        : isETHdir  ? prices.eth
        : isSOLdir  ? prices.sol
        : isXRPdir  ? prices.xrp
        : isDOGEdir ? prices.doge
        : isBNBdir  ? prices.bnb
        : null;
      if (!livePrice2) { log(`FastPath dir: no price for ${m.ticker} — skip`); continue; }

      // Detect "up" or "down" — from title OR from Up/Down market flag
      // Up/Down markets: YES = Up happened, NO = Down happened
      const isUpMarket = m._isUpDownMarket ? true : /price up/i.test(m.title);
      const isDownMarket = m._isUpDownMarket ? false : /price down/i.test(m.title);
      const coinLabelDir = isBTCdir?'BTC':isETHdir?'ETH':isSOLdir?'SOL':isXRPdir?'XRP':isDOGEdir?'DOGE':isBNBdir?'BNB':'ALT';
      const klineSymbol = isBTCdir?'BTCUSDT':isETHdir?'ETHUSDT':isSOLdir?'SOLUSDT':isXRPdir?'XRPUSDT':isDOGEdir?'DOGEUSDT':isBNBdir?'BNBUSDT':null;
      if (!klineSymbol) { log(`FastPath dir: no kline symbol for ${m.ticker} — skip`); continue; }

      // Use Binance 1m kline to get trend — fetch last 5 candles
      let trendConf = 0, trendDir = null;
      try {
        const kr = await req(
          `https://api.binance.com/api/v3/klines?symbol=${klineSymbol}&interval=1m&limit=5`,
          { method: 'GET', headers: {} }, 5000
        );
        if (kr.status === 200 && Array.isArray(kr.body) && kr.body.length >= 3) {
          // Each kline: [openTime, open, high, low, close, volume, ...]
          const candles = kr.body;
          const closes = candles.map(c => parseFloat(c[4]));
          // Compare last 3 closes for trend
          const recent = closes.slice(-3);
          const firstClose = recent[0], lastClose = recent[recent.length - 1];
          const trendPct = (lastClose - firstClose) / firstClose;
          const currentVsClose = (livePrice2 - lastClose) / lastClose;

          // Threshold calibrated to real crypto 1min volatility:
          // BTC/ETH 1min candles: typical move 0.02-0.06%. Old threshold 0.1% = always skipped.
          // New: 0.01% minimum — any clear directional move qualifies.
          // Confidence scales with strength: weak trend (0.01-0.05%) = 0.76, strong (0.05%+) = 0.82
          // currentVsClose: ±0.05% noise band — live price can lag last candle close by this much
          if (trendPct > 0.0001 && currentVsClose >= -0.0005) {
            trendDir = 'up';
            trendConf = trendPct > 0.0005 ? 0.82 : 0.76;
          } else if (trendPct < -0.0001 && currentVsClose <= 0.0005) {
            trendDir = 'down';
            trendConf = trendPct < -0.0005 ? 0.82 : 0.76;
          } else {
            // Flat / genuinely conflicting — no edge
            log(`FastPath dir: ${m.ticker} flat/mixed trend (trend=${(trendPct*100).toFixed(3)}% cur=${(currentVsClose*100).toFixed(3)}%) — skip`);
            continue;
          }
        }
      } catch(ke) {
        log(`FastPath dir: kline fetch failed for ${m.ticker}: ${ke.message}`);
        continue;
      }

      if (!trendDir || trendConf < phase.minProb) {
        log(`FastPath dir: ${m.ticker} no clear trend or conf ${(trendConf*100).toFixed(0)}% < ${(phase.minProb*100).toFixed(0)}% — skip`);
        continue;
      }

      // Map trend to trade side
      // isUpMarket: YES = price goes up → bet YES if trend is up
      // isDownMarket: YES = price goes down → bet YES if trend is down
      let dirSide = null;
      if (isUpMarket && trendDir === 'up') dirSide = 'YES';
      else if (isUpMarket && trendDir === 'down') dirSide = 'NO';
      else if (isDownMarket && trendDir === 'down') dirSide = 'YES';
      else if (isDownMarket && trendDir === 'up') dirSide = 'NO';

      if (!dirSide) { log(`FastPath dir: ${m.ticker} no matching side — skip`); continue; }

      const dirPrice = dirSide === 'YES' ? m.yesAsk : Math.max(0.05, 1 - (m.yesBid || (1 - m.yesAsk)));
      if (dirPrice <= 0.05 || dirPrice >= 0.95) continue;
      const dirPayout = ((1 - dirPrice) / dirPrice) * (1 - KALSHI_FEE);
      const dirEdge = trendConf - dirPrice;
      if (dirEdge < phase.minEdge) {
        log(`FastPath dir: ${m.ticker} edge ${(dirEdge*100).toFixed(1)}% < ${(phase.minEdge*100).toFixed(0)}% — skip`);
        continue;
      }
      // Pure direction markets (no dollar strike) are priced near 50c = ~0.95x payout.
      // Edge comes from trend accuracy (76-82% conf), not payout size.
      // Kelly math: 76% conf @ 50c = strong positive EV even at 0.95x payout.
      if (dirPayout < 0.85) {
        log(`FastPath dir: ${m.ticker} payout ${dirPayout.toFixed(2)}x < 0.85x floor — skip`);
        continue;
      }

      const coinLabelD = isBTCdir ? 'BTC' : 'ETH';
      const dirSig = {
        ticker: m.ticker,
        side: dirSide,
        confidence: trendConf,
        edge: dirEdge,
        marketPrice: dirPrice,
        feeAdjustedEdge: dirEdge - (dirPrice * KALSHI_FEE),
        netPayoutRatio: dirPayout,
        hoursToClose: m.hoursLeft,
        source: 'fastpath',
        reasoning: `⚡ FastPath [DIR] ${coinLabelDir} trend=${trendDir} | ${dirSide} @${(dirPrice*100).toFixed(1)}c = ${dirPayout.toFixed(2)}x | conf=${(trendConf*100).toFixed(0)}%`,
      };
      log(`⚡ FastPath [DIR]: ${m.ticker} ${dirSide} | ${(trendConf*100).toFixed(0)}% | ${dirPayout.toFixed(2)}x | trend=${trendDir}`);
      tg(`⚡ <b>FastPath [DIR] — $0 AI cost</b>\n${m.ticker} ${dirSide}\nTrend: ${trendDir} | ${(trendConf*100).toFixed(0)}% conf | ${dirPayout.toFixed(2)}x payout`);
      await executeTrade(dirSig);
      filled++;
    ...
    if (is15Min) {
      // 15-min: need significant gap — BTC volatility ~1-2% per 15min
      if (gapPct >= 0.15) confidence = 0.96;       // 15%+ gap: near-certain
      else if (gapPct >= 0.08) confidence = 0.93;  // 8-15%: very high conf
      else if (gapPct >= 0.05) confidence = 0.89;  // 5-8%: good conf, real edge
      else { log(`FastPath 15min: ${m.ticker} gap ${(gapPct*100).toFixed(2)}% < 5% minimum — skip`); continue; }
    } else if (m.hoursLeft < 1.5) {
      // 1-hour window: BTC can move 2-3% in an hour
      if (gapPct >= 0.10) confidence = 0.94;
      else if (gapPct >= 0.05) confidence = 0.89;
      else if (gapPct >= 0.03) confidence = 0.83;
      else if (gapPct >= 0.015) confidence = 0.75;
      else { log(`FastPath hourly: ${m.ticker} gap ${(gapPct*100).toFixed(2)}% < 1.5% minimum — skip`); continue; }
    } else {
      // 1.5h-4h window: more time = smaller gap acceptable
      if (gapPct >= 0.08) confidence = 0.93;
      else if (gapPct >= 0.04) confidence = 0.87;
      else if (gapPct >= 0.020) confidence = 0.80;
      else if (gapPct >= 0.010) confidence = 0.73;
      else { log(`FastPath sub4h: ${m.ticker} gap ${(gapPct*100).toFixed(2)}% < 1% minimum — skip`); continue; }
    }

    // Large gap bonus — BTC far from strike = near-certain outcome
    // 13.8% gap like the $986 win: starts at 0.93 + 0.05 = 0.96 → isFavorite15min qualifies
    if (gapPct >= 0.15) confidence = Math.min(0.97, confidence + 0.05);
    else if (gapPct >= 0.08) confidence = Math.min(0.96, confidence + 0.04);
    else if (gapPct >= 0.05) confidence = Math.min(0.94, confidence + 0.02);
    ...
    // SANITY: Never bet YES when BTC is significantly BELOW the strike.
    // A 3% gap the wrong way is a moon shot — not a FastPath trade.
    // FastPath only has edge when price is CLEARLY on one side of strike.
    // This is the #1 cause of losses: bot betting BTC will rally to a strike it's 53% below.
    ...
    // TWO VALID TRADE TYPES:
    // (A) Underdog: price 5-38c, payout 2x+, confidence 89%+ → full Kelly sizing
    //     These are the core FastPath trades (NO when BTC far below strike)
    //     Example: BTC at $70k, NO on strike $80k (gap=12.9%) @ 20c = 4x payout
    // (B) Favorite: price 55-92c, payout 0.1-0.8x, confidence >= market price
    //     For gap-based markets: BTC $83k vs strike $71.5k (13.8% gap), YES @88c = 96% conf
    //     For Up/Down target markets: market price IS the confidence signal.
    //     ETH Down@72c = market says 72% confident Down → use that directly.
    //     BNB Down@82c = 82% win rate at 82c = breakeven+ with any edge above zero.
    //     Gap check RELAXED for Up/Down target markets: small gap is OK because
    //     the market's own pricing already reflects the directional probability.
    const isUpDownTargetMkt = m._isUpDownMarket === true;
    const isUnderdog = sidePrice <= 0.38;
    // For Up/Down target markets: use market price as confidence (market does the math for us)
    // Boost confidence to match the market price — these markets are efficiently priced
    if (isUpDownTargetMkt && confidence < sidePrice) {
      // Market is pricing at sidePrice — use that as our confidence floor
      // e.g. Down@72c: market says 72% → confidence = max(gapConf, 0.72)
      confidence = Math.max(confidence, Math.min(0.92, sidePrice + 0.02)); // small edge bonus
    }
    const isFavorite15min = is15Min && sidePrice >= 0.55 && sidePrice <= 0.92 && confidence >= 0.88 &&
      (gapPct >= 0.08 || (isUpDownTargetMkt && sidePrice >= 0.60)); // Up/Down: relax gap req
    ...
    // --- GATE CHECKS ---
    // For Up/Down target markets: confidence is derived from the market price itself.
    // Market at 72c means 72% win probability — gate should reflect that not require 85%.
    // For gap-based favorites: still need 85% (gap math must be convincing).
    const fastPathMinConf = isUnderdog ? Math.max(phase.minProb, 0.72)
      : (isUpDownTargetMkt && isFavorite) ? Math.max(phase.minProb, sidePrice - 0.02) // market-derived conf
      : 0.85;
...
- For Up/Down markets with a dollar target: Up = YES if current price > target (above = stays above = Up wins)
- Use LIVE PRICES provided (BTC, ETH, SOL, XRP, DOGE, BNB). Compare price vs target from title.
- Confidence: price 0.5-2% from target=0.74, 2-5%=0.80, 5%+=0.87
- netPayout must be >= 1.5x. Signal up to 2 direction/Up-Down trades per cycle.
...
STEP 2 - SPORTS (1 search per game): Search "[team] vs [team] live score". Signal team leading by 10+ points in 4th quarter or 2 goals in final period. netPayout>=2.5. 70%+ confidence.

STEP 3 - FALLBACK: If still under ${_slots} signals, best crypto with gap>=4% and hoursLeft>=0.75h. NO 15min markets ever from brain.
...
async function executeTrade(sig) {
  ...
  if(sidePriceActual <= 0.08 || sidePriceActual >= 0.92){
  ...
  const _minBrainMins = _isBrainDirection ? 12 : 25; // direction: 12min ok, normal: 25min
...
4. Bot auto-restarts when cash is available`);
    ...
    if (brainReady && slotsAfterFastPath > 0 && hasMarkets && !inDDCooldown) {
...
async function startBot() {
  ...
  _botStarted = true;
  S.isRunning = true;
  log('[M] KalshiBot v13 started -- running pre-flight sync...');
...
server.listen(CFG.port, () => {
  log(`KalshiBot v13 listening on port ${CFG.port}`);

  ...
  try {
    ...
    try {
      ...
      const hourlyRes = await kalshi('GET', '/markets', null, {
        ...
        max_close_ts: (_nowTs + 3 * 3600).toString(),  // closes within 3h — our target window
        min_close_ts: (_nowTs + 300).toString()          // at least 5 min away — avoid un-fillable
      });
    ...
    // ── TARGETED BTC/ETH/15MIN SERIES FETCH ─────────────────────────────────
    // Fetch the specific BTC/ETH daily+hourly+15min series with a close_time cap
    // This guarantees sub-3h markets are always in the scanner
    ...
    const hourlySeriesFetches = await Promise.allSettled([
      ...
      min_close_ts: (nowTs2 + 300).toString()  // at least 5 min away
    })));
    ...
    const filteredMarkets = markets.filter(m => {
      ...
      const phase = getPhase();
      // SEED/SPROUT: only <12h markets — faster compounding, avoids slot-blocking
      const maxHours = (phase.name === 'SEED' || phase.name === 'SPROUT') ? 12 : 72;
...
4. Restart — position caps now hard-enforced`);
    ...
    if (brainReady && hasSlots && hasMarkets) {
      // ⚡ FAST PATHS FIRST — zero API cost signal engines run before Claude brain
      // 1. Crypto fast path: pure math on BTC/ETH/SOL/XRP threshold markets (15min + 1hr + sub-3h)
      const cryptoFilled = await runCryptoFastPath(markets);
      // 2. Sports fast path: live ESPN scores for NBA/MLB/NHL in-progress games
      const sportsFilled = await runSportsFastPath(markets);
      const totalFastFilled = cryptoFilled + sportsFilled;
      const slotsAfterFastPaths = dynamicMaxPos() - S.openPositions.length;

      if (slotsAfterFastPaths > 0) {
        // Brain only fires if there are remaining slots AND non-crypto/sports markets
        // or if fast paths found nothing and brain might see something they missed
        if (totalFastFilled === 0 || slotsAfterFastPaths > 0) {
          log(`🧠 Brain firing (${slotsAfterFastPaths} slots after fast paths filled ${totalFastFilled}): $${effectiveCash.toFixed(2)} cash`);
          await runBrain(markets);
        }
      } else {
        log(`Fast paths filled all ${dynamicMaxPos()} slots (crypto:${cryptoFilled} sports:${sportsFilled}) — brain skipped`);
        S.lastBrainAt = Date.now();
      }

...
function loadState() {
  ...
  // v18.1: also reset sessionPeakBalance so drawdown guard uses real balance, not stale env default
  if (S.peakBalance >= 50 && S.balance < 30) {
    S.peakBalance = S.balance;
    S.sessionPeakBalance = S.balance;
    S.drawdownHaltUntil = 0; // clear any false-positive drawdown pause carried across restart
    log('v18.1: peak + sessionPeak reset -- was stale BANKROLL default');
  }
...
async function scan() {
  ...
  try {
    ...
    // v18.1: peak must be at least $3 ABOVE current balance AND not within $1 of BANKROLL env
    // (prevents false-positive from stale $50 default peak when real balance is $28)
    const peakLooksReal = (S.sessionPeakBalance || 0) > (curBalForDD + 3) &&
                           Math.abs((S.sessionPeakBalance || 0) - CFG.bankroll) > 1.0;
    if (peakLooksReal && (S.sessionPeakBalance || 0) > 10 && curBalForDD < S.sessionPeakBalance * 0.75) {
...
async function startBot() {
  ...
  _botStarted = true;
  S.isRunning = true;
  log('[M] KalshiBot v18.1 started -- running pre-flight sync...');
...
server.listen(CFG.port, () => {
  log(`KalshiBot v18.1 listening on port ${CFG.port}`);

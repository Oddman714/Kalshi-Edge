...
async function getTopMarkets() {
  try {
    // Fetch high-volume markets sorted by volume
    const r = await kalshi('GET', '/markets', null, {
      status: 'open',
      limit: '100',
    });
    const markets = (r.markets || []).filter(m => {
      // Filter out composite/multivariate markets
      if (m.mve_collection_ticker) return false;
      // Must have real volume
      if ((m.volume || 0) < 10) return false; // low threshold to capture more markets
      // Handle both old (integer cents) and new (dollar string _dollars) price formats
      const yesAsk = parsePrice(m.yes_ask_dollars || m.yes_ask);
      if (yesAsk <= 0.01 || yesAsk >= 0.99) return false;
      return true;
    });

    // Score each market for trading opportunity
    const scored = markets.map(m => {
      const yesAsk = parsePrice(m.yes_ask_dollars || m.yes_ask);
      const yesBid = parsePrice(m.yes_bid_dollars || m.yes_bid);
      const spread = yesAsk - yesBid;
      const vol = m.volume || 0;
      const liquidity = m.open_interest || 0;

      // Scoring: prefer tight spreads, high volume, mid-range probabilities
      const spreadScore = Math.max(0, 1 - spread / 0.10);
      const volScore = Math.min(1, Math.log10(vol + 1) / 5);
      const probScore = 1 - Math.abs(yesAsk - 0.5) * 2; // prefer near 50%
    ...
    log(`Scanner: ${markets.length} tradeable markets found, top score: ${(scored[0]?.score || 0).toFixed(3)}`);
    return S.topMarkets;
  } catch(e) {
...
async function runPlumbingTest() {
  ...
  // 2. Kalshi Markets
  try {
    const r = await kalshi('GET', '/markets', null, { status: 'open', limit: '20' });
    const all = r.markets || [];
    // Debug: log first market's fields to see actual price field names
    if (all.length > 0) {
      const m0 = all[0];
      log(`DEBUG market fields: yes_ask=${m0.yes_ask} yes_ask_dollars=${m0.yes_ask_dollars} volume=${m0.volume} mve=${m0.mve_collection_ticker}`);
    }
    const tradeable = all.filter(m => {
      if (m.mve_collection_ticker) return false;
      const p = parsePrice(m.yes_ask_dollars || m.yes_ask);
      return p > 0.01 && p < 0.99;
    });
    check('Market Access', all.length > 0, `${all.length} open, ${tradeable.length} tradeable`);
    check('MVE Filter', tradeable.length > 0, `${tradeable.length} real markets pass filter`);
  } catch(e) {
    check('Market Access', false, e.message.slice(0, 60));
    check('MVE Filter', false, 'Cannot test — market fetch failed');
  }

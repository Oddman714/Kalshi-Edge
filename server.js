...
function loadState() {
  ...
  // ── ALWAYS reset on boot — Kalshi is source of truth, never state.json ──
  S.isRunning = false;
  S.availableCash = 0;
  S.restingOrders = [];
  // Force balance to resync from Kalshi — never trust persisted balance
  // Pre-flight will set these correctly within 5 seconds of boot
  S.balance = 0;
  S.realBalance = 0;
  S.peakBalance = 0; // will be set to real balance after first sync

  // On boot: purge ALL fromKalshi trades so they get re-imported fresh
  if (S.trades && S.trades.length > 0) {
    S.trades = S.trades.filter(t => !t.fromKalshi);
  ...
  log('State loaded — balance/realBalance reset to 0, awaiting Kalshi sync');
}
...
async function syncBalance() {
  try {
    ...
    if (bal <= 0) { log('Balance sync returned $0 — keeping previous', 'WARN'); return; }
    S.realBalance = bal;
    // Always use Kalshi as source of truth in live mode
    S.balance = bal;
    // Update peak — only go up, never down
    if (bal > S.peakBalance) S.peakBalance = bal;
    // If peak is still 0 (fresh boot), set it now
    if (S.peakBalance === 0) S.peakBalance = bal;
    log(`Balance synced: $${bal.toFixed(2)} | Peak: $${S.peakBalance.toFixed(2)}`);
  } catch(e) {
...
// We must subtract this from available balance before sizing any new trade.
async function syncRestingOrders() {
  try {
    ...
    S.restingOrders = orders.map(o => {
      const remaining = parseFloat(o.remaining_count || o.count || 0);
      // Price comes as cents int (13) OR dollar decimal (0.13) — normalize to dollars
    ...
    log(`Cash: total=$${rawBal.toFixed(2)} reserved=$${totalReserved.toFixed(2)} available=$${S.availableCash.toFixed(2)} (${S.restingOrders.length} resting)`);
  } catch(e) {
    // If endpoint fails, conservatively assume half balance is reserved
    const rawBal = S.realBalance > 0 ? S.realBalance : S.balance;
    S.availableCash = rawBal * 0.5;
    log('Resting order sync failed — conservatively using 50% of balance: ' + e.message, 'WARN');
  }
}
...
function kellySize(prob, price) {
  ...
  // Always size against availableCash — the true spendable amount
  const bal = S.availableCash > 1 ? S.availableCash : (S.realBalance > 0 ? S.realBalance : S.balance);
  if (bal < 2.00) return 0; // not enough to trade — signal executeTrade to skip
  const playable = Math.max(0, bal - 1.50); // keep $1.50 buffer
  const maxPct = bal < 10 ? 0.70 : bal < 25 ? 0.65 : bal < 50 ? 0.50 : bal < 200 ? 0.38 : bal < 1000 ? 0.28 : bal < 5000 ? 0.22 : 0.15;
  const maxBet = Math.min(bal * maxPct, playable * 0.80);
  const minBet = 1.50; // hard floor — never try to compute a sub-$1.50 bet
  const sized = Math.min(maxBet, Math.max(minBet, bal * fracKelly));
  return Math.min(sized, bal * 0.90); // never risk more than 90% of available in one trade
}
...
async function executeTrade(sig) {
  ...
  // ── AVAILABLE CASH GUARD — hard stop before any Kalshi order ──
  // Re-sync resting orders immediately before each trade to get freshest picture
  await syncRestingOrders();
  const availCash = S.availableCash;
  const TRADE_FLOOR = 2.00; // never attempt a trade if less than this is available
  if (availCash < TRADE_FLOOR) {
    log(`Skip ${sig.ticker}: only $${availCash.toFixed(2)} available ($${S.restingOrders.reduce((s,o)=>s+o.reservedCash,0).toFixed(2)} locked in ${S.restingOrders.length} resting orders) — waiting for orders to fill or expire`);
    return;
  }
...
function applyState(s){
  // Always prefer realBalance — synced directly from Kalshi, never stale
  ...
  document.getElementById('engineSub').textContent=running
    ?'Scan #'+(s.scanCount||0)+' · '+(s.openPositions||[]).length+' open · $'+avail.toFixed(2)+' available'
    :'Tap Start to begin';

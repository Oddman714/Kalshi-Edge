...
async function cycle() {
  try {
    ...
    if (S.dailyDate !== today) {
      ...
      tg(`📅 <b>Daily Reset</b>\nYesterday P&L: $${(yPnl / 100).toFixed(2)}\nRecord: ${S.wins + S.paperWins}W/${S.losses + S.paperLosses}L`).catch(() => {});
    }
...
// Crash-proof cycle wrapper — NEVER lets an error kill the interval
function safeCycle() {
  cycle().catch(e => {
    log('💥 safeCycle caught: ' + (e.message || e));
...
function startBot() {
  ...
  safeCycle();
  pollTimer = setInterval(safeCycle, C.poll * 1000);
  saveDB();
}

// Process-level safety nets
process.on('unhandledRejection', (err) => {
  log('⚠️ Unhandled rejection: ' + (err?.message || err));
});
process.on('uncaughtException', (err) => {
  log('⚠️ Uncaught exception: ' + (err?.message || err));
});

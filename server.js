...
async function runBrain(topMarkets) {
  ...
  const prompt = `{"task":"generate_trade_signals","rules":["respond ONLY with valid JSON","no explanation","no preamble","start with {","end with }"],"context":{"MISSION":"Double portfolio from $${_bal.toFixed(2)} to $500K as fast as safely possible. We have ${dynamicMaxPos()-S.openPositions.length} OPEN SLOTS that MUST be filled. Do NOT return empty signals if ANY tradeable edge exists.","CURRENT_STATE":"Balance: $${_bal.toFixed(2)} | Cash: $${S.availableCash.toFixed(2)} | Open: ${S.openPositions.length}/${dynamicMaxPos()} | Phase: ${phase.name} | Doublings needed: ${doublingsNeeded()} | Next target: $${nextTarget().toFixed(0)}","OPEN_POSITIONS":${JSON.stringify(S.openPositions.map(p=>({ticker:p.ticker,side:p.side,entry:p.entryPrice})))},"RESEARCH":"Academic study of 300K+ Kalshi contracts: HIGH-price contracts (55-85c) win more than priced — positive EV. LOW-price (<35c) show longshot bias — avoid. Fee drag: 4.5% on wins. Minimum GROSS edge needed: ${(phase.minEdge*100+4.5).toFixed(0)}% to net ${(phase.minEdge*100).toFixed(0)}% after fees.","FEE_FORMULA":"net_edge = (confidence - marketPrice) - (confidence * 0.045). Only signal if net_edge >= ${(phase.minEdge*100).toFixed(1)}%"},"markets":${JSON.stringify(topMarkets.slice(0,8).map(m=>({ticker:m.ticker,title:m.title,yesAsk:+(m.yesAsk*100).toFixed(1),yesBid:+((m.yesBid||0)*100).toFixed(1),volume:Math.round(m.volume),hoursLeft:m.hoursLeft,closesIn:m.closeTime?Math.round((new Date(m.closeTime)-Date.now())/3600000)+'h':'?'})))},"search_strategy":{"searches":10,"MANDATE":"Use ALL 10 searches. We have open slots that need filling. Search EVERY promising market. Be decisive.","priority":["1. SPORTS: search live scores RIGHT NOW for any game in progress — if a game is live and market hasnt repriced thats FREE MONEY","2. CRYPTO: current BTC/ETH price vs strike — math is objective, search coinmarketcap or coingecko","3. ECON: any data released today? CPI/jobs/Fed — compare actual vs consensus","4. SHORT DURATION: markets closing within 24h get priority — faster compounding","5. HIGH VOLUME: >10K volume means liquid, tight spreads, real edge possible"]},"execution_rules":{"FILL_SLOTS":"We have ${dynamicMaxPos()-S.openPositions.length} empty slots. Fill them ALL if edge exists. Do not leave slots empty out of excessive caution.","MIN_CONFIDENCE":"${(phase.minProb*100).toFixed(0)}% after web search research","MIN_NET_EDGE":"${(phase.minEdge*100).toFixed(1)}% after 4.5% fee deduction","KELLY_SIZING":"Server handles sizing — just give the signal","NO_DUPLICATE_GAMES":"Don't signal both sides of same game","AGGRESSIVE_STANCE":"When in doubt between signaling and skipping — SIGNAL. We need to compound. A missed edge is a missed doubling."},"required_output":{"signals":[{"ticker":"string","side":"YES or NO","confidence":0.68,"edge":0.10,"marketPrice":0.58,"feeAdjustedEdge":0.07,"reasoning":"SPECIFIC: [exact score/price/stat] vs market price [X]. Edge exists because [Y].","hoursToClose":6}],"marketSummary":"Best edge found and why — be specific about what data you found","skipReason":"ONLY include if you searched all 8+ markets and ZERO net edge found after fees — be honest"}}`;

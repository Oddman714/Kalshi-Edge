...
function kellySize(prob, price) {
  ...
  const fracKelly = Math.max(0, kelly * phase.kelly); // phase-driven Kelly fraction
  const bal = S.realBalance > 0 ? S.realBalance : S.balance;
  // Playable = balance above $3 floor
  const playable = Math.max(0, bal - 3.00);
  // Phase-aware max bet percentage
  ...
  const minBet = Math.max(1.50, Math.min(3.00, bal * 0.08)); // $1.50 hard floor

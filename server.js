...
const CFG = {
  ...
  tgToken:    process.env.TELEGRAM_TOKEN || process.env.TELEGRAM_BOT_TOKEN || process.env.TG_TOKEN || process.env.BOT_TOKEN || '',
  tgChat:     process.env.TELEGRAM_CHAT_ID || process.env.TG_CHAT_ID || process.env.CHAT_ID || '',
  ...
  directionFastPathEnabled: process.env.DIRECTION_FASTPATH_ENABLED !== 'false',
  directionMinMinutes: parseFloat(process.env.DIRECTION_MIN_MINUTES || '5'),
  directionMaxSpread: parseFloat(process.env.DIRECTION_MAX_SPREAD || '0.14'),
  premiumEvGateEnabled: process.env.PREMIUM_EV_GATE_ENABLED !== 'false',
  premiumMinExpectedRoi: parseFloat(process.env.PREMIUM_MIN_EXPECTED_ROI || '0.04'),
  premiumMinEvPerHour: parseFloat(process.env.PREMIUM_MIN_EV_PER_HOUR || '0.08'),
  premiumMinExpectedProfitDollars: parseFloat(process.env.PREMIUM_MIN_EXPECTED_PROFIT_DOLLARS || '0.03'),
  favoriteMinExpectedRoi: parseFloat(process.env.FAVORITE_MIN_EXPECTED_ROI || '0.06'),
  favoriteMinEdgeOverBreakeven: parseFloat(process.env.FAVORITE_MIN_EDGE_OVER_BREAKEVEN || '0.10'),
  favoriteHardMaxPrice: parseFloat(process.env.FAVORITE_HARD_MAX_PRICE || '0.72'),
  directionMinExpectedRoi: parseFloat(process.env.DIRECTION_MIN_EXPECTED_ROI || '0.05'),
  underdogMinExpectedRoi: parseFloat(process.env.UNDERDOG_MIN_EXPECTED_ROI || '0.08'),
  auditIntervalMinutes: parseInt(process.env.AUDIT_INTERVAL_MINUTES || '60'),
  auditMinVerifiedTrades: parseInt(process.env.AUDIT_MIN_VERIFIED_TRADES || '25'),
  telegramHeartbeatEnabled: process.env.TELEGRAM_HEARTBEAT_ENABLED !== 'false',
  dailyTargetDollars: parseFloat(process.env.DAILY_TARGET_DOLLARS || '100'),
  dailyTargetLockEnabled: process.env.DAILY_TARGET_LOCK_ENABLED === 'true',
  dailyLossLimitPct: parseFloat(process.env.DAILY_LOSS_LIMIT_PCT || '0.18'),
  dailyLossLimitDollars: parseFloat(process.env.DAILY_LOSS_LIMIT_DOLLARS || '0'),
  version: 'v20.7.0-survival-target-command-ui',
};
...
// Kalshi fee helpers. Official standard taker fee:
// round up(0.07 * contracts * price * (1 - price)); maker = 0.0175 multiplier.
// The old KALSHI_FEE constant remains for legacy ranking display only; execution
// uses exact rounded fee math below.
const KALSHI_FEE = 0.045;
const KALSHI_TAKER_FEE_RATE = 0.07;
const KALSHI_MAKER_FEE_RATE = 0.0175;
...
let S = {
  ...
  auditSnapshots: [],     // compact coach snapshots for continuous improvement
  telegramStatus: { configured: false, ok: false, lastOkAt: 0, lastErrorAt: 0, lastError: '', sent: 0, failed: 0 },
  lastAuditAt: 0,
};
...
function saveState() {
  try {
    ...
    if (slim.auditSnapshots && slim.auditSnapshots.length > 80) slim.auditSnapshots = slim.auditSnapshots.slice(0, 80);
...
function loadState() {
  ...
  // On boot: purge synced/manual Kalshi history and rebuild public P&L from
  // verified real bot trades only. This prevents stale paper/demo values from
  // showing up as headline app performance.
  S.trades = Array.isArray(S.trades) ? S.trades.filter(t => !t.fromKalshi) : [];
  S.auditSnapshots = Array.isArray(S.auditSnapshots) ? S.auditSnapshots : [];
  S.telegramStatus = S.telegramStatus || { configured: false, ok: false, lastOkAt: 0, lastErrorAt: 0, lastError: '', sent: 0, failed: 0 };
  applyVerifiedTradeStats();
...
function telegramConfigured() {
  return !!(CFG.tgToken && CFG.tgChat);
}

async function sendTelegramRaw(msg, opts = {}) {
  if (!S.telegramStatus) S.telegramStatus = { configured: false, ok: false, lastOkAt: 0, lastErrorAt: 0, lastError: '', sent: 0, failed: 0 };
  S.telegramStatus.configured = telegramConfigured();
  if (!telegramConfigured()) {
    const reason = 'TELEGRAM_TOKEN/TELEGRAM_CHAT_ID not configured';
    S.telegramStatus.ok = false;
    S.telegramStatus.lastError = reason;
    S.telegramStatus.lastErrorAt = Date.now();
    if (!S._lastTgConfigWarnAt || Date.now() - S._lastTgConfigWarnAt > 5 * 60000) {
      S._lastTgConfigWarnAt = Date.now();
      log('Telegram disabled: ' + reason, 'WARN');
    }
    return { ok: false, reason };
  }
  const text = String(msg || '').slice(0, 3900);
  const makeBody = (parseMode = 'HTML', textOverride = text) => JSON.stringify({
    chat_id: CFG.tgChat,
    text: textOverride,
    ...(parseMode ? { parse_mode: parseMode } : {}),
    disable_web_page_preview: true,
  });
  let body = makeBody('HTML');
  let r = await req(`https://api.telegram.org/bot${CFG.tgToken}/sendMessage`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) },
    body,
  }, opts.timeoutMs || 8000);
  if (r.status === 400 && /parse|entity|HTML/i.test(JSON.stringify(r.body || ''))) {
    const plain = text.replace(/<[^>]+>/g, '').replace(/[<>]/g, '').slice(0, 3900);
    body = makeBody(null, plain);
    r = await req(`https://api.telegram.org/bot${CFG.tgToken}/sendMessage`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) },
      body,
    }, opts.timeoutMs || 8000);
  }
  const ok = r.status === 200 && (typeof r.body !== 'object' || r.body.ok !== false);
  if (ok) {
    S.telegramStatus.ok = true;
    S.telegramStatus.lastOkAt = Date.now();
    S.telegramStatus.lastError = '';
    S.telegramStatus.sent = (S.telegramStatus.sent || 0) + 1;
    return { ok: true, status: r.status };
  }
  const desc = (r.body && (r.body.description || r.body.error_code || JSON.stringify(r.body).slice(0, 160))) || `HTTP ${r.status}`;
  throw new Error(String(desc).slice(0, 180));
}

function tg(msg) {
  const plain = String(msg || '').replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim().slice(0, 300);
  ssePush('notify', { msg: plain, ts: Date.now() });
  sendTelegramRaw(msg).catch(e => {
    if (!S.telegramStatus) S.telegramStatus = {};
    S.telegramStatus.ok = false;
    S.telegramStatus.lastError = String(e.message || e).slice(0, 180);
    S.telegramStatus.lastErrorAt = Date.now();
    S.telegramStatus.failed = (S.telegramStatus.failed || 0) + 1;
    if (!S._lastTgErrorLogAt || Date.now() - S._lastTgErrorLogAt > 3 * 60000) {
      S._lastTgErrorLogAt = Date.now();
      log('Telegram send failed: ' + S.telegramStatus.lastError, 'WARN');
    }
    ssePush('notify', { msg: 'Telegram failed: ' + S.telegramStatus.lastError, ts: Date.now() });
  });
}
...
function ceilCents(value) {
  ...
  return Math.ceil((n - 1e-12) * 100) / 100;
}

function kalshiFeeDollars(price, contracts = 1, opts = {}) {
  const p = Math.max(0.01, Math.min(0.99, Number(price || 0)));
  const c = Math.max(1, Math.floor(Number(contracts || 1)));
  const rate = opts.maker ? KALSHI_MAKER_FEE_RATE : KALSHI_TAKER_FEE_RATE;
  return ceilCents(rate * c * p * (1 - p));
}

function feeAwareTradeMath({ price, contracts = 1, confidence = 0, hoursToClose = 1, maker = false }) {
  const p = Math.max(0.01, Math.min(0.99, Number(price || 0)));
  const c = Math.max(1, Math.floor(Number(contracts || 1)));
  const conf = Math.max(0.01, Math.min(0.99, Number(confidence || 0)));
  const fee = kalshiFeeDollars(p, c, { maker });
  const contractCost = p * c;
  const totalCost = contractCost + fee;
  const winProfit = c - totalCost;
  const expectedDollars = (conf * c) - totalCost;
  const expectedRoi = totalCost > 0 ? expectedDollars / totalCost : -1;
  const hours = Math.max(1 / 12, Number(hoursToClose || 1));
  return {
    price: p,
    contracts: c,
    fee,
    feePerContract: fee / c,
    contractCost,
    cost: totalCost,
    winProfit,
    lossDollars: -totalCost,
    expectedDollars,
    expectedRoi,
    evPerHour: expectedRoi / hours,
    breakEvenConfidence: totalCost / c,
    edgeOverBreakeven: conf - (totalCost / c),
    netPayoutRatio: totalCost > 0 ? winProfit / totalCost : 0,
  };
}

function premiumEvGate(sig, math) {
  if (!CFG.premiumEvGateEnabled || sig?.isArb) return { ok: true, reason: '' };
  const price = Number(math.price || sig?.marketPrice || 0);
  const isFavorite = !!sig?.isFavorite || price >= 0.55;
  const isDirectional = !!sig?.isDirectionalFastPath;
  const isUnderdog = price <= 0.38;
  let minRoi = CFG.premiumMinExpectedRoi;
  if (isDirectional) minRoi = Math.max(minRoi, CFG.directionMinExpectedRoi);
  if (isUnderdog) minRoi = Math.max(minRoi, CFG.underdogMinExpectedRoi);
  if (isFavorite) minRoi = Math.max(minRoi, CFG.favoriteMinExpectedRoi);

  if (isFavorite && price > CFG.favoriteHardMaxPrice && Number(sig?.confidence || 0) < 0.88) {
    return { ok: false, reason: `Favorite ${(price*100).toFixed(0)}c needs 88%+ confidence` };
  }
  if (isFavorite && math.edgeOverBreakeven < CFG.favoriteMinEdgeOverBreakeven) {
    return { ok: false, reason: `Favorite edge ${(math.edgeOverBreakeven*100).toFixed(1)}% over breakeven below ${(CFG.favoriteMinEdgeOverBreakeven*100).toFixed(0)}%` };
  }
  if (math.expectedDollars < CFG.premiumMinExpectedProfitDollars) {
    return { ok: false, reason: `Expected profit $${math.expectedDollars.toFixed(2)} below $${CFG.premiumMinExpectedProfitDollars.toFixed(2)}` };
  }
  if (math.expectedRoi < minRoi) {
    return { ok: false, reason: `Expected ROI ${(math.expectedRoi*100).toFixed(1)}% below ${(minRoi*100).toFixed(0)}%` };
  }
  if (math.evPerHour < CFG.premiumMinEvPerHour) {
    return { ok: false, reason: `EV/hour ${(math.evPerHour*100).toFixed(1)}% below ${(CFG.premiumMinEvPerHour*100).toFixed(0)}%` };
  }
...
function verifiedUnrealizedPnl() {
  return (S.openPositions || []).reduce((sum, p) => {
    if (!p || p.fromKalshi || !p.costBasisVerified) return sum;
    const cur = Math.max(0, Math.min(1, Number(p.currentPrice || 0) / 100));
    const contracts = Math.max(0, Number(p.contracts || 0));
    const cost = Math.max(0, Number(p.cost || 0));
    if (cur <= 0 || contracts <= 0 || cost <= 0) return sum;
    const marketValue = cur * contracts;
    const pnl = marketValue - cost;
    const maxGain = Math.max(0, contracts - cost);
    const maxLoss = cost;
    return sum + Math.max(-maxLoss, Math.min(maxGain, pnl));
  }, 0);
}

function isPaperTradeRecord(t) {
  if (!t) return false;
  const oid = String(t.orderId || t.exitOrderId || '');
  return !!(t.paperTrade || t.dryRun || /^PAPER/i.test(oid));
}

function isVerifiedRealTrade(t) {
  if (!t || t.status !== 'resolved' || t.fromKalshi || isPaperTradeRecord(t)) return false;
  const cost = Number(t.cost || 0);
  if (!Number.isFinite(cost) || cost <= 0.01) return false;
  return t.liveTrade === true || !!t.orderId || !!t.exitOrderId;
}

function verifiedRealTrades() {
  return (S.trades || []).filter(isVerifiedRealTrade);
}

function verifiedTradeStats() {
  const trades = verifiedRealTrades();
  const todayStr = new Date().toDateString();
  const pnl = trades.reduce((sum, t) => sum + (Number(t.pnl) || 0), 0);
  const todayPnl = trades
    .filter(t => t.resolvedAt && new Date(t.resolvedAt).toDateString() === todayStr)
    .reduce((sum, t) => sum + (Number(t.pnl) || 0), 0);
  return {
    trades,
    wins: trades.filter(t => t.won === true).length,
    losses: trades.filter(t => t.won === false).length,
    totalPnl: pnl,
    todayPnl,
  };
}

function applyVerifiedTradeStats() {
  const stats = verifiedTradeStats();
  S.wins = stats.wins;
  S.losses = stats.losses;
  S.totalPnl = stats.totalPnl;
  S.todayPnl = stats.todayPnl;
  S.todayDate = new Date().toDateString();
  return stats;
}

function dailyTargetSnapshot(stats = verifiedTradeStats()) {
  const bal = Math.max(0, Number(S.realBalance || S.balance || CFG.bankroll || 0));
  const target = Math.max(0, Number(CFG.dailyTargetDollars || 0));
  const verifiedOpen = verifiedUnrealizedPnl();
  const realized = Number(stats.todayPnl || 0);
  const liveProgress = realized + verifiedOpen;
  const shortfall = Math.max(0, target - liveProgress);
  const progressPct = target > 0 ? Math.max(0, Math.min(999, (liveProgress / target) * 100)) : 0;
  const requiredRoi = bal > 0 && target > 0 ? target / bal : 0;
  const dailyLossLimit = CFG.dailyLossLimitDollars > 0
    ? CFG.dailyLossLimitDollars
    : Math.max(1, bal * CFG.dailyLossLimitPct);
  const status = target <= 0 ? 'Off'
    : liveProgress >= target ? 'Target locked'
    : requiredRoi > 1.0 ? 'Bankroll too small'
    : requiredRoi > 0.25 ? 'Stretch target'
    : 'Plausible target';
  return {
    target,
    realized: parseFloat(realized.toFixed(2)),
    verifiedOpen: parseFloat(verifiedOpen.toFixed(2)),
    liveProgress: parseFloat(liveProgress.toFixed(2)),
    shortfall: parseFloat(shortfall.toFixed(2)),
    progressPct: parseFloat(progressPct.toFixed(1)),
    requiredRoi: parseFloat(requiredRoi.toFixed(3)),
    requiredRoiPct: parseFloat((requiredRoi * 100).toFixed(1)),
    dailyLossLimit: parseFloat(dailyLossLimit.toFixed(2)),
    lockEnabled: !!CFG.dailyTargetLockEnabled,
    status,
  };
}
...
function marketGroup(ticker = '') {
  ...
  if (/kxsol|kxxrp|kxdoge|kxbnb|kxada|kxhype|kxavax/.test(t)) return 'crypto:alts';
...
function scoreTradeSetup(sig, ctx = {}) {
  ...
  const spread = Number(ctx.spread != null ? ctx.spread : sig?.spread != null ? sig.spread : CFG.maxSpread);
  ...
  score += isDirectional && hours <= 0.35 ? 10 : hours <= 0.25 ? 2 : hours <= 1.5 ? 8 : hours <= 3 ? 5 : -10;
  ...
  if (isDirectional) score += 5;
  if (isDirectional && Number(sig?.momentumStrength || 0) >= 0.65) score += 3;
...
function botHealthSnapshot() {
  ...
  const wr = total ? stats.wins / total : null;
...
function shadowStatsSnapshot() {
  const rows = S.missedTrades || [];
  const finalMissedWins = rows.filter(x => x.status === 'final_missed_win').length;
  const finalGoodSkips = rows.filter(x => x.status === 'final_good_skip').length;
  const pendingGreen = rows.filter(x => x.status === 'pending_green').length;
  const pendingRed = rows.filter(x => x.status === 'pending_red').length;
  const byReason = {};
  for (const x of rows.slice(0, 80)) {
    const key = String(x.reason || 'unknown').slice(0, 54);
    byReason[key] = byReason[key] || { reason: key, samples: 0, finalMissedWins: 0, finalGoodSkips: 0, pendingGreen: 0, pendingRed: 0 };
    byReason[key].samples += 1;
    if (x.status === 'final_missed_win') byReason[key].finalMissedWins += 1;
    if (x.status === 'final_good_skip') byReason[key].finalGoodSkips += 1;
    if (x.status === 'pending_green') byReason[key].pendingGreen += 1;
    if (x.status === 'pending_red') byReason[key].pendingRed += 1;
  }
  const reasonSummary = Object.values(byReason)
    .sort((a, b) => (b.finalMissedWins + b.pendingGreen) - (a.finalMissedWins + a.pendingGreen) || b.samples - a.samples)
    .slice(0, 8);
  return { total: rows.length, finalMissedWins, finalGoodSkips, pendingGreen, pendingRed, reasonSummary };
}

function auditSnapshot() {
  const tradeStats = verifiedTradeStats();
  const total = tradeStats.wins + tradeStats.losses;
  const target = dailyTargetSnapshot(tradeStats);
  const wr = total ? tradeStats.wins / total : null;
  const health = botHealthSnapshot();
  const risk = riskModeSnapshot();
  const shadow = shadowStatsSnapshot();
  const sources = sourceHealthSummary();
  const openSlots = Math.max(0, dynamicMaxPos() - (S.openPositions || []).length);
  const recentSkips = (S.skippedSetups || []).slice(0, 20);
  const premiumBlocks = recentSkips.filter(x => /premium ev gate/i.test(x.reason || '')).length;
  const liquidityBlocks = recentSkips.filter(x => /liquidity|resting/i.test(x.reason || '')).length;
  const scoreBlocks = recentSkips.filter(x => /quality score/i.test(x.reason || '')).length;
  const recommendations = [];

  if (!telegramConfigured()) recommendations.push('Configure TELEGRAM_TOKEN and TELEGRAM_CHAT_ID in Railway env vars.');
  else if (S.telegramStatus && S.telegramStatus.ok === false && S.telegramStatus.lastError) recommendations.push('Telegram configured but failing: ' + S.telegramStatus.lastError);
  if (target.requiredRoi > 1.0) recommendations.push(`$${target.target.toFixed(0)}/day needs ${target.requiredRoiPct.toFixed(0)}% daily ROI at this bankroll; grow bankroll first or the bot must take ruin-level risk.`);
  else if (target.requiredRoi > 0.25) recommendations.push(`$${target.target.toFixed(0)}/day is still a stretch at ${target.requiredRoiPct.toFixed(0)}% daily ROI; only scale after verified edge is proven.`);
  if (target.liveProgress >= target.target && target.target > 0) recommendations.push('Daily target reached; protect gains and avoid forcing extra trades.');
  if (total < CFG.auditMinVerifiedTrades) recommendations.push(`Collect ${CFG.auditMinVerifiedTrades - total} more verified settled trades before trusting win-rate claims.`);
  if (openSlots > 0 && premiumBlocks >= 3) recommendations.push('Premium EV gate is blocking several setups; keep it strict unless shadow results show repeated final missed winners.');
  if (openSlots > 0 && liquidityBlocks >= 2) recommendations.push('Liquidity is limiting fills; continue right-sizing and prefer markets with deeper executable book.');
  if (scoreBlocks >= 2) recommendations.push('Quality score is acting as the main throttle; review scorecards before lowering thresholds.');
  if (shadow.finalMissedWins > shadow.finalGoodSkips + 2) recommendations.push('Shadow ledger shows missed winners; consider easing the most common blocked reason by a small amount.');
  if (shadow.finalGoodSkips >= shadow.finalMissedWins + 2) recommendations.push('Shadow ledger supports current strictness; do not loosen yet.');
  for (const s of sources) {
    if (s.total >= CFG.sourceKillMinTrades && s.winRate != null && s.winRate < CFG.sourceKillWinRateFloor) {
      recommendations.push(`${s.source} is underperforming and should stay throttled until new evidence improves.`);
    }
  }
  if (!recommendations.length) recommendations.push('Continue collecting verified fills, exits, and shadow outcomes; current gates look coherent.');

  const evidenceCap = total >= CFG.auditMinVerifiedTrades ? 10 : total >= 10 ? 8.8 : total >= 5 ? 8.2 : 7.6;
  const rawRating = Math.min(evidenceCap, Math.max(5.5, health.score / 10));
  const snapshot = {
    ts: Date.now(),
    version: CFG.version,
    ...
    verified: {
      ...
      winRate: wr == null ? null : parseFloat((wr * 100).toFixed(1)),
      totalPnl: parseFloat(tradeStats.totalPnl.toFixed(2)),
      todayPnl: parseFloat(tradeStats.todayPnl.toFixed(2)),
      winRateReliable: total >= 5,
    },
    dailyTarget: target,
    open: {
      positions: (S.openPositions || []).length,
      maxPositions: dynamicMaxPos(),
      openSlots,
      verifiedUnrealizedPnl: parseFloat(verifiedUnrealizedPnl().toFixed(2)),
    },
    gates: { premiumBlocks, liquidityBlocks, scoreBlocks, recentBlocked: recentSkips.length },
    shadow,
    sources,
...
function recordAuditSnapshot(snapshot = auditSnapshot()) {
  if (!S.auditSnapshots) S.auditSnapshots = [];
  S.auditSnapshots.unshift(snapshot);
  if (S.auditSnapshots.length > 80) S.auditSnapshots.length = 80;
  S.lastAuditAt = snapshot.ts || Date.now();
  return snapshot;
}

function formatAuditForTelegram(a) {
  const wr = a.verified.winRate == null ? 'learning' : `${a.verified.winRate.toFixed(1)}%`;
  const dt = a.dailyTarget || { target: 0, progressPct: 0, liveProgress: 0, shortfall: 0, requiredRoiPct: 0, status: 'Off' };
  const recs = (a.recommendations || []).slice(0, 4).map((r, i) => `${i + 1}. ${r}`).join('\n');
  const sourceLine = (a.sources || []).map(s => {
    const w = s.winRate == null ? '--' : `${(s.winRate * 100).toFixed(0)}%`;
    return `${s.source}:${w}/${s.total}`;
  }).join(' | ');
  return `🧭 <b>Coach Audit — ${a.rating}/10</b>
Verified: ${a.verified.wins}W/${a.verified.losses}L (${wr}) | P&L ${a.verified.totalPnl >= 0 ? '+' : ''}$${a.verified.totalPnl.toFixed(2)}
Daily target: ${dt.progressPct.toFixed(1)}% | ${dt.liveProgress >= 0 ? '+' : ''}$${dt.liveProgress.toFixed(2)} / $${dt.target.toFixed(0)} | need ${dt.requiredRoiPct.toFixed(0)}% ROI
Open: ${a.open.positions}/${a.open.maxPositions} | slots ${a.open.openSlots} | open P&L ${a.open.verifiedUnrealizedPnl >= 0 ? '+' : ''}$${a.open.verifiedUnrealizedPnl.toFixed(2)}
Shadow: missed ${a.shadow.finalMissedWins} | good skips ${a.shadow.finalGoodSkips} | pending ${a.shadow.pendingGreen}/${a.shadow.pendingRed}
Blocks: premium ${a.gates.premiumBlocks} | liquidity ${a.gates.liquidityBlocks} | score ${a.gates.scoreBlocks}
Sources: ${sourceLine || 'learning'}

${recs}`;
}

function maybeSendCoachAudit(force = false) {
  const interval = Math.max(10, CFG.auditIntervalMinutes) * 60000;
  if (!force && (Date.now() - Number(S.lastAuditAt || 0)) < interval) return null;
  const snapshot = recordAuditSnapshot();
  tg(formatAuditForTelegram(snapshot));
  saveState();
  return snapshot;
}
...
async function getExecutableLiquidity(ticker, side, price, desiredContracts) {
  try {
    ...
    return {
      ok: executable >= desiredContracts,
      executable,
...
async function getExitQuote(ticker, side, desiredContracts = 1) {
  try {
    ...
    return {
      ok: bestBid > 0 && executable >= desiredContracts,
      bid: bestBid,
  ...
  try {
    ...
    const livePositions = positions
      ...
      .map(p => {
        ...
        // Kalshi returns current side prices as decimals (0.01 = 1c, 0.50 = 50c).
        // Use the side we actually hold; mixing YES bid into a NO position creates
        // wildly wrong dashboard P&L.
        const currentPriceDollars = side === 'YES'
          ? parsePrice(p.current_yes_bid_dollars || p.yes_price_dollars || p.yes_bid_dollars || '0')
          : parsePrice(p.current_no_bid_dollars || p.no_price_dollars || p.no_bid_dollars || '0');
        ...
        const currentPriceCents = currentPriceDollars > 0 ? Math.round(currentPriceDollars * 100) : entryPriceCents;
        return {
...
async function getTopMarkets() {
  try {
    ...
    const sportsMacroSeries = [
      ...
      'KXBTC15M', 'KXETH15M', 'KXSOL15M', 'KXXRP15M', 'KXDOGE15M', 'KXBNB15M', 'KXADA15M', 'KXHYPE15M',
    ...
    const cryptoSeriesFetches = await Promise.allSettled([
      ...
      'KXSOL15M','KXXRP15M','KXDOGE15M','KXBNB15M','KXADA15M','KXHYPE15M','KXSOLH','KXXRPH','KXSOLD','KXXRPD',
  ...
  // -- PAYOUT-FIRST FILTER --
    ...
    const markets = allMarkets.filter(m => {
      ...
      const is15mCrypto = /kxbtc15m|kxeth15m|kxsol15m|kxxrp15m|kxdoge15m|kxbnb15m|kxada15m|kxhype15m/i.test(m.ticker);
      ...
      if (vol < (isHourly || isFastClose ? 5 : 25) && !isDirectionMkt && !is15mCrypto) return false;
    ...
    const scored = filteredMarkets.map(m => {
      ...
      const isCrypto = /btc|eth|sol|xrp|doge|bnb|ada|hype|avax|crypto|inxd|inxw|spx|ndx/i.test(m.ticker);
    ...
    const _cryptoTop = scored.filter(m => /kxbtc|kxeth|kxsol|kxxrp|kxdoge|kxbnb|kxada|kxhype|kxavax/i.test(m.ticker) && !_heldSet.has(m.ticker) && m.hoursLeft > 0.08).sort((a,b)=>a.hoursLeft-b.hoursLeft).slice(0, 8);
    const _nonCrypto = reordered.filter(m => !/kxbtc|kxeth|kxsol|kxxrp|kxdoge|kxbnb|kxhype|kxavax/i.test(m.ticker)).slice(0, 12);
    const _cryptoInReordered = reordered.filter(m => /kxbtc|kxeth|kxsol|kxxrp|kxdoge|kxbnb|kxada|kxhype|kxavax/i.test(m.ticker));
    ...
    S._allScoredCrypto = scored.filter(m =>
      /kxbtc|kxeth|kxsol|kxxrp|kxdoge|kxbnb|kxada|kxhype|kxavax/i.test(m.ticker) && !_heldSet.has(m.ticker) && m.hoursLeft > 0.08
    );
...
let _priceCache = { btc: null, eth: null, sol: null, xrp: null, doge: null, bnb: null, ada: null, hype: null, fetchedAt: 0 };
let _klineCache = {};
...
async function fetchLivePrices() {
  ...
  try {
    const symbols = ['BTCUSDT','ETHUSDT','SOLUSDT','XRPUSDT','DOGEUSDT','BNBUSDT','ADAUSDT','HYPEUSDT'];
    ...
    const prices = {};
    const map = { BTCUSDT:'btc', ETHUSDT:'eth', SOLUSDT:'sol', XRPUSDT:'xrp', DOGEUSDT:'doge', BNBUSDT:'bnb', ADAUSDT:'ada', HYPEUSDT:'hype' };
    results.forEach((r, i) => {
    ...
    if (prices.btc && prices.eth) {
      ...
      log(`Prices (Binance): BTC=$${prices.btc?.toLocaleString()} ETH=$${prices.eth?.toLocaleString()} SOL=$${prices.sol?.toFixed(2)} DOGE=$${prices.doge?.toFixed(4)} BNB=$${prices.bnb?.toFixed(2)} ADA=$${prices.ada?.toFixed(4)} HYPE=$${prices.hype?.toFixed(2)}`);
      return _priceCache;
    }
  ...
  try {
    const r = await req('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,solana,ripple,cardano,hyperliquid&vs_currencies=usd', { method:'GET', headers:{} }, 8000);
    if (r.status === 200 && r.body?.bitcoin?.usd) {
      const prices = { btc: r.body.bitcoin.usd, eth: r.body.ethereum?.usd, sol: r.body.solana?.usd, xrp: r.body.ripple?.usd, ada: r.body.cardano?.usd, hype: r.body.hyperliquid?.usd };
      _priceCache = { ...prices, fetchedAt: now };
...
function cryptoSymbolForTicker(ticker = '') {
  const t = String(ticker).toLowerCase();
  if (/kxbtc/.test(t)) return { key: 'btc', symbol: 'BTCUSDT', label: 'BTC' };
  if (/kxeth/.test(t)) return { key: 'eth', symbol: 'ETHUSDT', label: 'ETH' };
  if (/kxsol/.test(t)) return { key: 'sol', symbol: 'SOLUSDT', label: 'SOL' };
  if (/kxxrp/.test(t)) return { key: 'xrp', symbol: 'XRPUSDT', label: 'XRP' };
  if (/kxdoge/.test(t)) return { key: 'doge', symbol: 'DOGEUSDT', label: 'DOGE' };
  if (/kxbnb/.test(t)) return { key: 'bnb', symbol: 'BNBUSDT', label: 'BNB' };
  if (/kxada/.test(t)) return { key: 'ada', symbol: 'ADAUSDT', label: 'ADA' };
  if (/kxhype/.test(t)) return { key: 'hype', symbol: 'HYPEUSDT', label: 'HYPE' };
  return null;
}
...
async function fetchShortMomentum(symbol) {
  const now = Date.now();
  const cached = _klineCache[symbol];
  if (cached && now - cached.fetchedAt < 20000) return cached;
  let rows = null;
  try {
    const r = await req(`https://api.binance.com/api/v3/klines?symbol=${symbol}&interval=1m&limit=8`, { method:'GET', headers:{} }, 4500);
    if (r.status === 200 && Array.isArray(r.body) && r.body.length >= 6) rows = r.body;
  } catch(e) {}
  if (!rows) {
    const pair = krakenPairForSymbol(symbol);
    if (!pair) throw new Error(`no OHLC pair for ${symbol}`);
    const r = await req(`https://api.kraken.com/0/public/OHLC?pair=${pair}&interval=1`, { method:'GET', headers:{} }, 5500);
    const key = Object.keys(r.body?.result || {}).find(k => k !== 'last');
    const data = key ? r.body.result[key] : null;
    if (r.status === 200 && Array.isArray(data) && data.length >= 6) rows = data;
  }
  if (!rows || rows.length < 6) throw new Error(`kline ${symbol} unavailable`);
  const kl = rows.slice(-6).map(k => ({
    open: parseFloat(k[1]),
    high: parseFloat(k[2]),
    low: parseFloat(k[3]),
    close: parseFloat(k[4]),
  })).filter(k => isFinite(k.open) && isFinite(k.close) && k.open > 0);
  if (kl.length < 5) throw new Error(`kline ${symbol} sparse`);
  const first = kl[0].close;
  const last = kl[kl.length - 1].close;
  const ret5 = (last - first) / first;
  const last3 = kl.slice(-3);
  const green3 = last3.filter(k => k.close > k.open).length;
  const red3 = last3.filter(k => k.close < k.open).length;
  const absMoves = [];
  for (let i = 1; i < kl.length; i++) absMoves.push(Math.abs((kl[i].close - kl[i-1].close) / kl[i-1].close));
  const avgAbsMove = absMoves.reduce((a,b)=>a+b,0) / Math.max(1, absMoves.length);
  const dir = ret5 > 0 ? 'UP' : ret5 < 0 ? 'DOWN' : 'FLAT';
  const candleAgree = dir === 'UP' ? green3 : dir === 'DOWN' ? red3 : 0;
  const moveScore = Math.min(1, Math.abs(ret5) / 0.0025);
  const candleScore = Math.min(1, candleAgree / 3);
  const volScore = Math.min(1, avgAbsMove / 0.0012);
  const strength = Math.max(0, Math.min(1, moveScore * 0.55 + candleScore * 0.30 + volScore * 0.15));
  const item = { fetchedAt: now, symbol, direction: dir, ret5, green3, red3, avgAbsMove, strength };
  _klineCache[symbol] = item;
  return item;
}

function directionalConfidence(marketProb, momentum) {
  const p = Math.max(0.01, Math.min(0.97, Number(marketProb || 0)));
  const strength = Math.max(0, Math.min(1, Number(momentum?.strength || 0)));
  const bonus = strength >= 0.85 ? 0.10 : strength >= 0.65 ? 0.075 : strength >= 0.48 ? 0.045 : 0.02;
  return Math.min(0.91, p + bonus);
}
...
async function updateOpenPositionPrices(livePrices) {
  ...
  for (const pos of S.openPositions) {
    try {
      ...
      const isADAp = /kxada/i.test(pos.ticker);
      const isHYPEp= /kxhype/i.test(pos.ticker);

      if ((isBTC || isETH || isSOL || isXRPp || isDOGEp || isBNBp || isADAp || isHYPEp) && livePrices) {
        ...
        const liveP = isBTC ? livePrices.btc : isETH ? livePrices.eth : isSOL ? livePrices.sol
          : isXRPp ? livePrices.xrp : isDOGEp ? livePrices.doge : isBNBp ? livePrices.bnb
          : isADAp ? livePrices.ada : livePrices.hype;
...
async function runCryptoFastPath(topMarkets) {
  ...
  for (const m of cryptoMarkets) {
    ...
    // --- DIRECTION MARKET HANDLER ---
    // Pure "price up/down in next X min" markets require an independent momentum
    // confirmation. We handle them after coin detection so we can cross-check
    // Kalshi's implied side with live exchange candles.
    ...
    const isADA  = /kxada/i.test(m.ticker);
    ...
    const is15Min = /15M|15MIN|KXBTC15|KXETH15|KXSOL15|KXXRP15|KXDOGE15|KXBNB15|KXADA15|KXHYPE15/i.test(m.ticker) || m.hoursLeft < 0.30;

    if (isDirectionMarket) {
      if (!CFG.directionFastPathEnabled) {
        log(`FastPath DIR: ${m.ticker} disabled by config`);
        continue;
      }
      const minutesLeft = (m.hoursLeft || 0) * 60;
      if (minutesLeft < CFG.directionMinMinutes || minutesLeft > 32) {
        log(`FastPath DIR: ${m.ticker} ${minutesLeft.toFixed(0)}m left outside ${CFG.directionMinMinutes}-32m window`);
        continue;
      }
      const spread = Math.max(0, Number(m.spread || 0));
      if (spread > CFG.directionMaxSpread) {
        log(`FastPath DIR: ${m.ticker} spread ${(spread*100).toFixed(0)}c > ${(CFG.directionMaxSpread*100).toFixed(0)}c max`);
        continue;
      }

      const yesPrice = Math.max(0.01, Math.min(0.97, Number(m.yesAsk || 0)));
      const noPrice = Math.max(0.01, Math.min(0.97, 1 - Number(m.yesBid || (1 - yesPrice))));
      const betUp = yesPrice >= noPrice;
      const side = betUp ? 'YES' : 'NO';
      const sidePrice = betUp ? yesPrice : noPrice;
      const marketProb = sidePrice;
      const dirMinMarketProb = riskModeSnapshot().active ? 0.64 : 0.58;
      if (marketProb < dirMinMarketProb || marketProb > 0.86) {
        log(`FastPath DIR: ${m.ticker} market side ${(marketProb*100).toFixed(0)}c outside ${(dirMinMarketProb*100).toFixed(0)}-86c target`);
        continue;
      }

      const coin = cryptoSymbolForTicker(m.ticker);
      if (!coin) { log(`FastPath DIR: ${m.ticker} no coin symbol`); continue; }
      let mom;
      try {
        mom = await fetchShortMomentum(coin.symbol);
      } catch(e) {
        log(`FastPath DIR: ${m.ticker} momentum unavailable (${e.message}) — skip`);
        continue;
      }
      const wantsUp = side === 'YES';
      const momentumAgrees = (wantsUp && mom.direction === 'UP') || (!wantsUp && mom.direction === 'DOWN');
      if (!momentumAgrees || mom.strength < 0.48) {
        log(`FastPath DIR: ${m.ticker} momentum ${mom.direction} strength ${(mom.strength*100).toFixed(0)}% disagrees/weak for ${side}`);
        continue;
      }

      const confidence = directionalConfidence(marketProb, mom);
      const edge = confidence - sidePrice;
      const minDirEdge = riskModeSnapshot().active ? Math.max(0.08, phase.minEdge) : Math.max(0.055, phase.minEdge * 0.75);
      if (edge < minDirEdge) {
        log(`FastPath DIR: ${m.ticker} edge ${(edge*100).toFixed(1)}% < ${(minDirEdge*100).toFixed(1)}%`);
        continue;
      }
      const previewMath = feeAwareTradeMath({ price: sidePrice, contracts: 1, confidence, hoursToClose: m.hoursLeft });
      const netPayout = previewMath.netPayoutRatio;
      const dirSig = {
        ticker: m.ticker,
        side,
        confidence,
        edge,
        marketPrice: sidePrice,
        feeAdjustedEdge: previewMath.edgeOverBreakeven,
        netPayoutRatio: netPayout,
        hoursToClose: m.hoursLeft,
        spread,
        source: 'fastpath',
        isFavorite: sidePrice >= 0.55,
        isDirectionalFastPath: true,
        momentumStrength: mom.strength,
        reasoning: `⚡ FastPath [DIR] ${coin.label} ${side === 'YES' ? 'UP' : 'DOWN'} | Kalshi ${(marketProb*100).toFixed(0)}c + momentum ${mom.direction} ${(mom.strength*100).toFixed(0)}% | ret5=${(mom.ret5*100).toFixed(3)}% | ${minutesLeft.toFixed(0)}m left`,
      };
      log(`⚡ FastPath [DIR]: ${m.ticker} ${side} | conf ${(confidence*100).toFixed(0)}% | fee-edge ${(previewMath.edgeOverBreakeven*100).toFixed(1)}% | momentum ${mom.direction}/${(mom.strength*100).toFixed(0)}%`);
      tg(`⚡ <b>FastPath [15m Direction]</b>\n${m.ticker} ${side} (${side === 'YES' ? 'UP' : 'DOWN'})\nKalshi: ${(marketProb*100).toFixed(0)}c | Conf: ${(confidence*100).toFixed(0)}% | Edge: ${(edge*100).toFixed(1)}%\nMomentum: ${mom.direction} ${(mom.strength*100).toFixed(0)}% | ${minutesLeft.toFixed(0)}m left`);
      const _okDir = await executeTrade(dirSig);
      if (_okDir) filled++;
      continue;
    }
    ...
    const maxStrike = isBTC ? 200000 : isETH ? 10000 : isSOL ? 2000 : isXRP ? 10
      : isDOGE ? 5 : isBNB ? 5000 : isADA ? 10 : isHYPE ? 500 : 200000;
    const minStrike = isBTC ? 10000 : isETH ? 100 : isSOL ? 0.50 : isXRP ? 0.001
      : isDOGE ? 0.001 : isBNB ? 50 : isADA ? 0.01 : isHYPE ? 0.50 : 0.001;
    ...
    if (isUpDownTargetMkt) {
      ...
      try {
        const _coin = isBTC?'BTCUSDT':isETH?'ETHUSDT':isSOL?'SOLUSDT':isXRP?'XRPUSDT':isDOGE?'DOGEUSDT':isADA?'ADAUSDT':isHYPE?'HYPEUSDT':'BNBUSDT';
      ...
      const udCoinLabel = isBTC?'BTC':isETH?'ETH':isSOL?'SOL':isXRP?'XRP':isDOGE?'DOGE':isBNB?'BNB':isADA?'ADA':isHYPE?'HYPE':'ALT';
      const udSig = {
...
${udCoinLabel}=$${livePrice.toLocaleString()} vs $${strike}`);
    ...
    const coinLabel = isBTC?'BTC':isETH?'ETH':isSOL?'SOL':isXRP?'XRP':isDOGE?'DOGE':isBNB?'BNB':isADA?'ADA':isHYPE?'HYPE':'ALT';
...
async function runBrain(topMarkets) {
  ...
  try {
    ...
    if (_b || _e) {
      const _dg = _prices.doge, _bn = _prices.bnb, _ad = _prices.ada, _hy = _prices.hype;
      _priceCtx = `LIVE PRICES @ ${new Date().toLocaleString('en-US',{timeZone:'America/New_York',hour:'2-digit',minute:'2-digit'})} ET: BTC=${_b ? '$'+_b.toLocaleString() : 'N/A'} ETH=${_e ? '$'+_e.toLocaleString() : 'N/A'} SOL=${_s ? '$'+_s.toFixed(2) : 'N/A'} XRP=${_x ? '$'+_x.toFixed(4) : 'N/A'} DOGE=${_dg ? '$'+_dg.toFixed(4) : 'N/A'} BNB=${_bn ? '$'+_bn.toFixed(2) : 'N/A'} ADA=${_ad ? '$'+_ad.toFixed(4) : 'N/A'} HYPE=${_hy ? '$'+_hy.toFixed(2) : 'N/A'} — USE THESE FOR CRYPTO MATH`;
...
function kellySize(prob, price, source, sig) {
  const phase = getPhase();
  const q = 1 - prob;
  const feePerContract = kalshiFeeDollars(price, 1, { maker: false });
  const costPerContract = Math.min(0.99, price + feePerContract);
  const b = Math.max(0.01, (1 - costPerContract) / costPerContract);
...
async function executeTrade(sig) {
  ...
  sig.source = srcName;
  const riskMode = riskModeSnapshot();
  const targetSnap = dailyTargetSnapshot();
  if (targetSnap.target > 0 && CFG.dailyTargetLockEnabled && targetSnap.liveProgress >= targetSnap.target) {
    recordSkip(sig, `Daily target reached $${targetSnap.liveProgress.toFixed(2)}/$${targetSnap.target.toFixed(0)}`);
    recordRisk('Daily target lock', `New entries paused after hitting $${targetSnap.target.toFixed(0)} target`);
    return;
  }
  if (targetSnap.dailyLossLimit > 0 && targetSnap.liveProgress <= -targetSnap.dailyLossLimit) {
    recordSkip(sig, `Daily loss limit hit -$${targetSnap.dailyLossLimit.toFixed(2)}`);
    recordRisk('Daily loss limit', `New entries paused for today: $${targetSnap.liveProgress.toFixed(2)} P&L`);
    return;
  }
  ...
  const phase = getPhase();
  const directionalLane = !!sig.isDirectionalFastPath;
  const baseMinEdge = directionalLane && !riskMode.active ? Math.min(phase.minEdge, 0.055) : phase.minEdge;
  const baseMinProb = directionalLane && !riskMode.active ? Math.min(phase.minProb, 0.72) : phase.minProb;
  const effectiveMinEdge = Math.min(0.20, baseMinEdge + riskMode.minEdgeBump);
  const effectiveMinProb = Math.min(0.90, baseMinProb + riskMode.minProbBump);
  const bal = S.realBalance>0?S.realBalance:S.balance;
  ...
  // Preview uses exact rounded Kalshi taker fees for 1 contract; final EV is
  // rechecked after contract sizing below.
  const sidePriceForPayout = sidePriceActual;
  let premiumMath = feeAwareTradeMath({ price: sidePriceForPayout, contracts: 1, confidence: sig.confidence, hoursToClose: sig.hoursToClose || 1 });
  let netPayoutRatio = premiumMath.netPayoutRatio;
  const isCryptoTicker = /kxbtc|kxeth|kxsol|kxxrp|kxdoge|kxbnb|kxada|kxhype|kxavax/i.test(sig.ticker);
  ...
  const effectiveEdge = premiumMath.edgeOverBreakeven;
  ...
  const isCryptoMath = /kxbtc|kxeth|kxsol|kxxrp|kxdoge|kxbnb|kxada|kxhype/i.test(sig.ticker);
  ...
  let contracts = Math.max(1, Math.floor(size / priceFloat));
  let tradeMath = feeAwareTradeMath({ price: priceFloat, contracts, confidence: sig.confidence, hoursToClose: sig.hoursToClose || 1 });
  let contractCost = tradeMath.contractCost;
  let actualCost = tradeMath.cost;
  netPayoutRatio = tradeMath.netPayoutRatio;
  ...
  if (!CFG.dryRun) {
    ...
    const executableContracts = Math.floor(Number(liq.executable || 0));
    if (executableContracts < 1) {
      recordSkip(sig, `No executable resting liquidity at ${price}c`, { liquidity: liq.executable, edge: effectiveEdge });
      recordRisk('Liquidity blocked', `${sig.ticker} ${sig.side}: 0 available for ${contracts} contracts @ ${price}c`);
      log(`Skip ${sig.ticker}: no executable resting volume for ${sig.side} @ ${price}c`);
      return;
    }
    if (executableContracts < contracts) {
      const resizedMath = feeAwareTradeMath({ price: priceFloat, contracts: executableContracts, confidence: sig.confidence, hoursToClose: sig.hoursToClose || 1 });
      if (resizedMath.cost < 0.50) {
        recordSkip(sig, `Executable liquidity only ${executableContracts} contract(s)`, { liquidity: liq.executable, edge: effectiveEdge });
        recordRisk('Liquidity blocked', `${sig.ticker} ${sig.side}: ${executableContracts}/${contracts} available, below $0.50 order minimum`);
        log(`Skip ${sig.ticker}: executable ${executableContracts}/${contracts} contracts @ ${price}c is below $0.50 minimum`);
        return;
      }
      recordRisk('Order right-sized to liquidity', `${sig.ticker} ${sig.side}: ${contracts} -> ${executableContracts} contracts @ ${price}c`);
      log(`Right-size ${sig.ticker}: ${contracts} -> ${executableContracts} contracts to match executable resting volume @ ${price}c`);
      contracts = executableContracts;
      tradeMath = resizedMath;
      contractCost = tradeMath.contractCost;
      actualCost = tradeMath.cost;
      netPayoutRatio = tradeMath.netPayoutRatio;
    }
  }

  const evGate = premiumEvGate(sig, tradeMath);
  sig.expectedValueDollars = tradeMath.expectedDollars;
  sig.expectedRoi = tradeMath.expectedRoi;
  sig.evPerHour = tradeMath.evPerHour;
  sig.feeDollars = tradeMath.fee;
  sig.breakEvenConfidence = tradeMath.breakEvenConfidence;
  sig.netPayoutRatio = netPayoutRatio;
  if (!evGate.ok) {
    recordSkip(sig, `Premium EV gate: ${evGate.reason}`, { liquidity: sig.liquidityContracts || contracts, edge: effectiveEdge });
    recordRisk('Premium EV blocked', `${sig.ticker} ${sig.side}: ${evGate.reason}`);
    log(`Skip ${sig.ticker}: Premium EV gate blocked — ${evGate.reason} | EV=$${tradeMath.expectedDollars.toFixed(2)} ROI=${(tradeMath.expectedRoi*100).toFixed(1)}% fee=$${tradeMath.fee.toFixed(2)}`);
    return;
  }
  ...
  if (CFG.dryRun) {
    ...
    const position = {
      ...
      contracts,
      cost: actualCost,
      feeDollars: tradeMath.fee,
      expectedValueDollars: tradeMath.expectedDollars,
      expectedRoi: tradeMath.expectedRoi,
      breakEvenConfidence: tradeMath.breakEvenConfidence,
    ...
    try {
      const order = await kalshi('POST', '/portfolio/orders', {
        ...
        time_in_force: 'immediate_or_cancel',
        buy_max_cost: Math.ceil(contractCost * 100),
        cancel_order_on_pause: true,
      ...
      const position = {
        ...
        tradeScore: sig.tradeScore,
        expectedValueDollars: sig.expectedValueDollars,
        expectedRoi: sig.expectedRoi,
        breakEvenConfidence: sig.breakEvenConfidence,
        feeDollars: parseDollars(orderObj.taker_fees_dollars) + parseDollars(orderObj.maker_fees_dollars),
        reasoning: sig.reasoning,
        openedAt: Date.now(),
      ...
      if (/fill_or_kill_insufficient_resting_volume|insufficient resting volume/i.test(e.message || '')) {
        recordSkip(sig, 'Orderbook liquidity moved before fill', { liquidity: sig.liquidityContracts || 0, edge: effectiveEdge });
        recordRisk('Liquidity moved before order', `${sig.ticker} ${sig.side}: ${contracts} @ ${price}c was no longer executable`);
        log(`Order skipped after submit ${sig.ticker}: resting volume disappeared before IOC/FOK fill`, 'WARN');
        tg(`⚠️ <b>Order Skipped — Liquidity Moved</b>\n${sig.ticker} ${sig.side} @ ${price}c\nResting volume disappeared before Kalshi could fill it. No position opened.`);
        return false;
      }
...
4. Bot auto-restarts when cash is available`);
    ...
    if (S.scanCount % 3 === 0) {
      ...
      const phase = getPhase();
      const unrealizedSum = verifiedUnrealizedPnl();
...
function sendHeartbeat() {
  ...
  // Headline performance only uses verified real bot trades and verified open marks.
  const tradeStats = verifiedTradeStats();
  const settled = tradeStats.wins + tradeStats.losses;
  const unrealized = verifiedUnrealizedPnl();
...
📈 Verified open P&L: ${unrealized >= 0 ? '+' : ''}$${unrealized.toFixed(2)}
Today: ${tradeStats.todayPnl >= 0 ? '+' : ''}$${tradeStats.todayPnl.toFixed(2)} | Total: ${tradeStats.totalPnl >= 0 ? '+' : ''}$${tradeStats.totalPnl.toFixed(2)}
🎯 ${tradeStats.wins}W/${tradeStats.losses}L (${settled >= 5 ? ((tradeStats.wins/settled)*100).toFixed(0) : 'learning'}% WR) | ${doublingsNeeded()} doublings left${srcLine ? '\n' + srcLine : ''}
...
async function runPlumbingTest() {
  ...
  // 6. Telegram
  try {
    const r = await sendTelegramRaw(`🔬 KalshiBot ${CFG.version} plumbing test: Telegram ✅`, { timeoutMs: 8000 });
    check('Telegram', r.ok, r.ok ? 'Message delivered' : String(r.reason || 'not delivered').slice(0, 60));
  } catch(e) {
...
Scan: ${CFG.scanInterval/1000}s | Brain: ${(dynamicBrainInterval()/60000).toFixed(1)}min | Heartbeat: ${CFG.heartbeatInterval/60000}min | Watchdog: 3min${_startPosList}`);
  ...
  scan();
  if (CFG.telegramHeartbeatEnabled) heartbeatTimer = setInterval(sendHeartbeat, CFG.heartbeatInterval);
...
.mkt-card{border-radius:18px;padding:15px}.mkt-yes{font-size:24px}.mkt-track{height:5px;background:rgba(255,61,90,.23)}.mkt-fill{background:linear-gradient(90deg,#21ee8d,#49f5a7)}
	...
	.survival-card{margin:14px 20px 0;border-radius:28px;border:1px solid rgba(69,247,255,.28);background:radial-gradient(circle at 18% 12%,rgba(69,247,255,.16),transparent 34%),linear-gradient(145deg,rgba(8,28,42,.82),rgba(4,10,18,.78));box-shadow:inset 0 1px 0 rgba(255,255,255,.12),0 22px 60px rgba(0,0,0,.34);overflow:hidden}
	.surv-head{display:flex;align-items:center;justify-content:space-between;padding:16px 18px 10px}.surv-k{font-size:11px;font-weight:900;color:rgba(238,238,245,.42);text-transform:uppercase;letter-spacing:.18em}.surv-status{font-size:12px;font-weight:850;padding:6px 11px;border-radius:999px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.06)}
	.surv-main{display:grid;grid-template-columns:1fr auto;gap:12px;padding:0 18px 16px;align-items:end}.surv-v{font-size:42px;font-weight:900;letter-spacing:-1.6px;line-height:.95}.surv-sub{font-size:13px;color:var(--m);margin-top:7px}.surv-right{text-align:right}.surv-target{font-size:26px;font-weight:900;color:var(--gold)}.surv-small{font-size:11px;color:var(--m);margin-top:4px}
	.surv-track{height:10px;margin:0 18px 16px;background:rgba(255,255,255,.08);border-radius:999px;overflow:hidden}.surv-fill{height:100%;width:0%;border-radius:999px;background:linear-gradient(90deg,var(--r),var(--gold),var(--g));box-shadow:0 0 24px rgba(5,212,124,.22);transition:width .8s cubic-bezier(.2,.8,.2,1)}
	.surv-grid{display:grid;grid-template-columns:repeat(3,1fr);border-top:1px solid rgba(255,255,255,.08)}.surv-cell{padding:12px 10px;text-align:center;border-right:1px solid rgba(255,255,255,.07)}.surv-cell:last-child{border-right:0}.surv-l{font-size:9px;font-weight:850;letter-spacing:.10em;text-transform:uppercase;color:var(--m2)}.surv-c{font-size:16px;font-weight:850;margin-top:5px}
	.radar-card{margin:14px 20px 0;border-radius:30px;border:1px solid rgba(156,102,255,.30);background:radial-gradient(circle at 50% 50%,rgba(156,102,255,.24),transparent 33%),linear-gradient(145deg,rgba(10,20,34,.78),rgba(3,8,16,.84));min-height:312px;display:flex;flex-direction:column;align-items:center;justify-content:center;position:relative;overflow:hidden;box-shadow:inset 0 1px 0 rgba(255,255,255,.12)}
	.radar-card:before{content:"";position:absolute;width:245px;height:245px;border-radius:50%;background:repeating-radial-gradient(circle,rgba(69,247,255,.22) 0 1px,transparent 1px 35px);animation:spinSlow 16s linear infinite;opacity:.7}.radar-card:after{content:"";position:absolute;width:250px;height:250px;border-radius:50%;background:conic-gradient(from 0deg,transparent,rgba(69,247,255,.30),transparent 38%,rgba(156,102,255,.24),transparent 70%);filter:blur(.5px);animation:spinSlow 9s linear infinite reverse}
	.radar-core{position:relative;z-index:1;width:126px;height:126px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:38px;font-weight:900;background:radial-gradient(circle,rgba(69,247,255,.22),rgba(156,102,255,.16) 54%,rgba(0,0,0,.22));border:1px solid rgba(69,247,255,.35);box-shadow:0 0 55px rgba(69,247,255,.18),inset 0 0 28px rgba(156,102,255,.22)}.radar-title{position:relative;z-index:1;margin-top:22px;font-size:24px;font-weight:850}.radar-sub{position:relative;z-index:1;margin-top:8px;font-size:14px;color:var(--m);text-align:center;max-width:250px;line-height:1.35}@keyframes spinSlow{to{transform:rotate(360deg)}}
	.coach-card{margin:14px 20px 0;border-radius:24px;border:1px solid rgba(255,255,255,.14);background:linear-gradient(145deg,rgba(16,32,50,.76),rgba(5,12,20,.78));padding:14px 15px}.coach-row{display:flex;justify-content:space-between;gap:12px;margin-top:10px}.coach-score{font-size:31px;font-weight:900;letter-spacing:-1px}.coach-list{margin-top:10px;display:flex;flex-direction:column;gap:8px}.coach-item{font-size:12px;color:rgba(238,238,245,.70);line-height:1.35;padding-left:12px;border-left:2px solid rgba(69,247,255,.35)}
  ...
  <div class="chart-wrap">
    ...
    <svg id="CH" viewBox="0 0 430 140" preserveAspectRatio="none"></svg>
	  ...
	  <div class="survival-card">
	    <div class="surv-head"><div class="surv-k">Daily Survival Target</div><div id="targetStatus" class="surv-status">Learning</div></div>
	    <div class="surv-main">
	      <div><div id="targetProgress" class="surv-v">$0.00</div><div id="targetSub" class="surv-sub">verified progress today</div></div>
	      <div class="surv-right"><div id="targetGoal" class="surv-target">$100</div><div id="targetPct" class="surv-small">0.0%</div></div>
	    </div>
	    <div class="surv-track"><div id="targetFill" class="surv-fill"></div></div>
	    <div class="surv-grid">
	      <div class="surv-cell"><div class="surv-l">Need Left</div><div id="targetNeed" class="surv-c cgold">$--</div></div>
	      <div class="surv-cell"><div class="surv-l">Req ROI</div><div id="targetRoi" class="surv-c cr">--</div></div>
	      <div class="surv-cell"><div class="surv-l">Loss Limit</div><div id="targetLoss" class="surv-c cbl">$--</div></div>
	    </div>
	  </div>
	  <div class="risk-grid">
...
<div id="pg-trade" class="pg">
  <div class="ph"><div class="pt">Positions</div><div id="tradeSub" class="psub">0 open</div><div id="posBrainPill" class="pill pill-live" style="position:absolute;right:20px;top:calc(var(--sat) + 18px);height:46px"><div class="pill-dot"></div><span>Scanning</span></div></div>
	  ...
	  <div class="radar-card"><div class="radar-core">AI</div><div class="radar-title">Scanner Online</div><div id="radarSub" class="radar-sub">Watching live markets and waiting for verified edge.</div></div>
      ...
      <div class="sgrp">
	    ...
	    <div class="ssec"><div class="ssec-lbl">Health</div>
	      <div class="sgrp">
	        ...
	        <div class="srow"><div class="sn">Bot Rating</div><div id="cfgAuditRating" class="sv sv-g">--</div></div>
	        <div class="srow"><div class="sn">$100 Target</div><div id="cfgTarget" class="sv sv-gold">--</div></div>
	        <div class="srow"><div class="sn">Required Daily ROI</div><div id="cfgReqRoi" class="sv sv-r">--</div></div>
	    ...
	    <div class="ssec"><div class="ssec-lbl">Coach Audit</div>
	      <div class="coach-card"><div class="coach-row"><div><div class="surv-k">Current Grade</div><div id="coachRating" class="coach-score">--</div></div><div style="text-align:right"><div class="surv-k">Evidence</div><div id="coachEvidence" class="surv-c cbl">--</div></div></div><div id="coachList" class="coach-list"><div class="coach-item">Waiting for live audit.</div></div></div>
	    </div>
...
function applyState(s) {
  ...
  try {
    ...
    var wrReliable=!!s.winRateReliable;
    ...
    if(wr!=null&&tt>0&&wrReliable){
      ...
      if(wrEl){wrEl.textContent=tt>0?'Learning':'--';wrEl.style.color='var(--m)';}
      if(wrBar)wrBar.style.background='var(--m2)';
    }
    st('statWRsub',tt>0?(tt+' settled'+(wrReliable?'':' · need 5')):'no trades yet');
    ...
    if(unrealized!==0)subParts.push((unrealized>=0?'+':'-')+fm(Math.abs(unrealized))+' open');
	    ...
	    var audit=s.audit||{};
	    var target=audit.dailyTarget||s.dailyTarget||{};
	    var targetLive=_n(target.liveProgress);
	    var targetGoal=_n(target.target||100);
	    var targetPct=_n(target.progressPct);
	    st('targetProgress',(targetLive>=0?'+':'-')+fm(Math.abs(targetLive)));
	    st('targetGoal',targetGoal>0?'$'+targetGoal.toFixed(0):'Off');
	    st('targetPct',targetPct.toFixed(1)+'%');
	    st('targetNeed',fm(_n(target.shortfall)));
	    st('targetRoi',target.requiredRoiPct!=null?_n(target.requiredRoiPct).toFixed(0)+'%':'--');
	    st('targetLoss',fm(_n(target.dailyLossLimit)));
	    st('targetStatus',target.status||'Learning');
	    var targetOpen=_n(target.verifiedOpen);
	    st('targetSub','realized '+fm(_n(target.realized))+' + open '+(targetOpen>=0?'+':'-')+fm(Math.abs(targetOpen)));
	    var tf=g('targetFill');if(tf)tf.style.width=Math.min(100,Math.max(0,targetPct))+'%';
	    var ts=g('targetStatus');if(ts){var req=_n(target.requiredRoiPct);ts.style.color=targetLive>=targetGoal&&targetGoal>0?'var(--g)':req>100?'var(--r)':req>25?'var(--gold)':'var(--g)';}
    ...
    var pbp=g('posBrainPill');
	    ...
	    st('radarSub',running?('Scan #'+(s.scanCount||0)+' - '+openCnt+'/'+mp+' positions - '+((s.topMarkets||[]).length)+' markets live'):'Engine stopped. Start bot to scan.');
    ...
    var wrCfg=g('cfgWR');if(wrCfg){wrCfg.textContent=wr!=null&&tt>0&&wrReliable?wr.toFixed(1)+'%':tt>0?'Learning ('+tt+'/5)':'-- (no real trades yet)';wrCfg.className='sv '+(wr!=null&&tt>0&&wrReliable?(wr>=65?'sv-g':wr>=50?'sv-gold':'sv-r'):'');}
    ...
    var hEl=g('cfgHealth');
	    ...
	    var arating=_n(audit.rating);
	    st('cfgAuditRating',arating?arating.toFixed(1)+'/10':'--');
	    st('coachRating',arating?arating.toFixed(1)+'/10':'--');
	    st('coachEvidence',audit.evidenceCap?('cap '+_n(audit.evidenceCap).toFixed(1)):'learning');
	    st('cfgTarget',targetGoal>0?(targetPct.toFixed(1)+'% of $'+targetGoal.toFixed(0)):'Off');
	    st('cfgReqRoi',target.requiredRoiPct!=null?_n(target.requiredRoiPct).toFixed(1)+'%':'--');
	    var cl=g('coachList');
	    if(cl){
	      var recs=(audit.recommendations||[]).slice(0,5);
	      cl.innerHTML=recs.length?recs.map(function(r){return '<div class="coach-item">'+esc(r)+'</div>';}).join(''):'<div class="coach-item">No urgent recommendations. Keep collecting evidence.</div>';
	    }
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/api/coach' || url.pathname === '/api/coach-audit') {
    const force = url.searchParams.get('send') === '1' || req.method === 'POST';
    const snapshot = recordAuditSnapshot();
    if (force) tg(formatAuditForTelegram(snapshot));
    saveState();
    res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store, max-age=0' });
    return res.end(JSON.stringify({ ok: true, sent: force, audit: snapshot, message: formatAuditForTelegram(snapshot).replace(/<[^>]+>/g, '') }));
  }

  if (url.pathname === '/api/telegram-test') {
    sendTelegramRaw(`✅ KalshiBot ${CFG.version} Telegram test\n${new Date().toLocaleString('en-US', { timeZone: 'America/New_York' })} ET`, { timeoutMs: 10000 })
      .then(r => {
        saveState();
        res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store, max-age=0' });
        res.end(JSON.stringify({ ok: !!r.ok, result: r, configured: telegramConfigured(), telegram: S.telegramStatus }));
      })
      .catch(e => {
        if (!S.telegramStatus) S.telegramStatus = {};
        S.telegramStatus.ok = false;
        ...
        saveState();
        res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store, max-age=0' });
        res.end(JSON.stringify({ ok: false, error: S.telegramStatus.lastError, configured: telegramConfigured(), telegram: S.telegramStatus }));
      });
    return;
  }
  ...
  if (url.pathname === '/api/state') {
    ...
    try {
      ...
      const slim = {
        ...
        todayPnl: _nn(tradeStats.todayPnl),
        totalPnl: _nn(tradeStats.totalPnl),
        wins: _nn(tradeStats.wins),
        losses: _nn(tradeStats.losses),
        ...
        winRate: total > 0 ? parseFloat((_nn(tradeStats.wins) / total * 100).toFixed(1)) : null,
        ...
        // Unrealized P&L only includes bot-opened positions with verified cost basis.
        // Synced/manual Kalshi positions are shown, but not mixed into headline P&L.
        unrealizedPnl: parseFloat(verifiedUnrealizedPnl().toFixed(2)),
        winRateReliable: total >= 5,
        phase: phase.name,
        ...
        startingBankroll: _nn(CFG.bankroll),
	        cfg: { kellyFrac: phase.kelly, minEdge: phase.minEdge, minProb: phase.minProb,
               ...
               directionFastPathEnabled: CFG.directionFastPathEnabled,
               directionMinMinutes: CFG.directionMinMinutes, directionMaxSpread: CFG.directionMaxSpread,
               premiumEvGateEnabled: CFG.premiumEvGateEnabled,
               premiumMinExpectedRoi: CFG.premiumMinExpectedRoi,
	               premiumMinEvPerHour: CFG.premiumMinEvPerHour,
	               favoriteMinEdgeOverBreakeven: CFG.favoriteMinEdgeOverBreakeven },
	        dailyTarget: audit.dailyTarget,
          ...
          expectedValueDollars: _nn(p.expectedValueDollars), expectedRoi: _nn(p.expectedRoi),
          breakEvenConfidence: _nn(p.breakEvenConfidence), feeDollars: _nn(p.feeDollars),
        ...
        signals: (S.signals||[]).slice(0,5).map(s => ({
          ...
          expectedValueDollars: _nn(s.expectedValueDollars), expectedRoi: _nn(s.expectedRoi),
          breakEvenConfidence: _nn(s.breakEvenConfidence),
          reasoning: _ss(s.reasoning, 150), hoursToClose: _nn(s.hoursToClose),
        })),
        trades: tradeStats.trades.slice(0,30).map(t => ({
        ...
        audit,
        auditSnapshots: (S.auditSnapshots||[]).slice(0,8).map(x => ({
          ts: _nn(x.ts), rating: _nn(x.rating),
          verifiedTrades: _nn(x.verified && x.verified.trades),
          recommendations: (x.recommendations || []).slice(0,4).map(r => _ss(r, 140)),
        })),
        telegramStatus: S.telegramStatus || {},
      };
  ...
  if (url.pathname === '/api/reset') {
    ...
    S = {
      ...
      auditSnapshots: [], lastAuditAt: 0,
      telegramStatus: { configured: telegramConfigured(), ok: false, lastOkAt: 0, lastErrorAt: 0, lastError: '', sent: 0, failed: 0 },
    };
    saveState();
    res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store, max-age=0' });
...
fetch('/health').then(r=>r.json()).then(d=>{if(d.version)document.getElementById('ver').textContent=d.version;}).catch(()=>{});
    ...
    Promise.allSettled([
      ...
      const tradeStats = verifiedTradeStats();
      const total = _n(tradeStats.wins) + _n(tradeStats.losses);
      res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store, max-age=0' });
      res.end(JSON.stringify({
        isRunning: !!S.isRunning,
        balance: _n(S.balance), realBalance: _n(S.realBalance),
        peakBalance: _n(S.peakBalance), availableCash: _n(S.availableCash),
        todayPnl: _n(tradeStats.todayPnl), totalPnl: _n(tradeStats.totalPnl),
        wins: _n(tradeStats.wins), losses: _n(tradeStats.losses),
        scanCount: _n(S.scanCount), brainCount: _n(S.brainCount),
        dryRun: !!CFG.dryRun, maxPos: dynamicMaxPos(),
        winRate: total > 0 ? parseFloat((_n(tradeStats.wins) / total * 100).toFixed(1)) : null,
        winRateReliable: total >= 5,
        unrealizedPnl: parseFloat(verifiedUnrealizedPnl().toFixed(2)),
        openPositions: (S.openPositions||[]).slice(0,10),
        signals: (S.signals||[]).slice(0,5),
        trades: tradeStats.trades.slice(0,20),
  ...
  if (url.pathname === '/health' || url.pathname === '/api/ping') {
    ...
    return res.end(JSON.stringify({
      ...
      telegram: S.telegramStatus || {},
      auditRating: auditSnapshot().rating,
      version: CFG.version,
      dailyTarget: auditSnapshot().dailyTarget,
    }));
  }

...
async function scan(){
  ...
  try{
    ...
    // Kalshi returns KXMV parlays from default /markets endpoint.
    // Real liquid markets (sports, politics, economics) live under specific event series.
    // Fetch multiple pages + try known liquid event series.
    
    let allMarkets=[];
    
    // Fetch 1: default markets endpoint, page 1
    const r1=await kalshi('GET','/markets',{status:'open',limit:'200'});
    if(r1.status===401||r1.status===403){await onAuthFail(r1.status,'scan');return;}
    if(r1.status===200){authOk();allMarkets.push(...(r1.data.markets||[]));}
    
    // Fetch 2: try cursor pagination for page 2 if cursor exists
    if(r1.data&&r1.data.cursor){
      const r2=await kalshi('GET','/markets',{status:'open',limit:'200',cursor:r1.data.cursor});
      if(r2.status===200)allMarkets.push(...(r2.data.markets||[]));
    }
    
    // Fetch 3: try /markets sorted by volume to surface liquid markets
    const r3=await kalshi('GET','/markets',{status:'open',limit:'200',order_by:'volume'});
    if(r3.status===200){
      const existing=new Set(allMarkets.map(m=>m.ticker||m.ticker_name));
      const newOnes=(r3.data.markets||[]).filter(m=>!existing.has(m.ticker||m.ticker_name));
      allMarkets.push(...newOnes);
      log('Volume-sorted fetch: '+newOnes.length+' new markets');
    }
    ...
    if(!allMarkets.length){log('Scan: 0 markets returned from all endpoints');return;}

    // Volume distribution
    const withVol=allMarkets.filter(m=>getVol(m)>0).length;
    log('SCAN TOTAL: '+allMarkets.length+' markets | '+withVol+' with volume>0');

    // Log top 5 by volume — this shows us what real liquid markets look like
    const byVol=[...allMarkets].sort((a,b)=>getVol(b)-getVol(a)).slice(0,5);
    byVol.forEach((m,i)=>{
      const t=m.ticker||m.ticker_name||'?';
      log('VOL'+i+': '+t.slice(0,30)+' vol=$'+getVol(m).toFixed(0)+' price='+getPrice(m)+'¢ "'+String(m.title||'').slice(0,35)+'"');
    });

    const held=new Set(S.positions.map(p=>p.ticker));
    const now=Date.now();

    // Only skip pure KXMV multivariate composites — NOT individual game markets
    // KXNBA, KXNHL etc are individual game markets that CAN have real volume
    const skipPrefixes=['KXMVESPORTSMULTIGAME'];

    const scored=allMarkets.filter(m=>{
      const ticker=m.ticker||m.ticker_name||'';
      if(!ticker||held.has(ticker))return false;
      // Only skip the multivariate composite parlays
      if(skipPrefixes.some(p=>ticker.startsWith(p)))return false;
      // Require real volume
      const vol=getVol(m);
    ...
    log('Candidates: '+scored.length+' liquid → '+S.cands.length+' top');
    if(S.cands.length>0){
      log('Best: "'+S.cands[0].title+'" yes='+S.cands[0].yes+'¢ vol=$'+S.cands[0].vol+' '+S.cands[0].hrs+'h');
    }else{
      log('Still zero candidates. withVol='+withVol+'. Markets are KXMV composites only.');
      log('These markets exist on your account website but not via API default endpoint.');
      log('Next step: contact Kalshi support — API may need market access enabled.');
    }

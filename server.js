...
function pushFeed(msg, type) {
  ...
  KB.feed.unshift({msg: String(msg).slice(0, 180), type: type || 'info', ts: ts});
  if (KB.feed.length > 60) KB.feed.pop();
  renderFeed();
}
function renderFeed() {
  var w = g('liveFeed');
...
function sseConnect() {
  try {
    ...
    es.onerror = function() {
      ...
      startPoll(3000);
      setTimeout(sseConnect, 15000);
    };
  } catch(ex) { startPoll(3000); }
}

/* ---- POLLING ---- */
function startPoll(ms) {
  if (KB.pollTimer) clearInterval(KB.pollTimer);
  KB.pollTimer = setInterval(loadState, ms || 5000);
}
...
function pushPt(v) {
  ...
  if (KB.chart.length > 600) KB.chart.shift();
  drawChart();
}
function drawChart() {
  ...
  svg.innerHTML = '<defs><linearGradient id="cg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="'+col+'" stop-opacity="0.18"/><stop offset="90%" stop-color="'+col+'" stop-opacity="0"/></linearGradient></defs>'
    ...
    + '<path d="'+line+'" fill="none" stroke="'+col+'" stroke-width="1.8" stroke-linecap="round"/>'
...
function ico(t) {
  ...
  if (/CPI|KXCPI|FED|FEDRATE|INFL/i.test(t)) return {e:'CPI', c:'ico-cpi'};
  ...
  if (/SOL/i.test(t)) return {e:'SOL', c:'ico-eth'};
  return {e:'~', c:''};
}
...
function renderPos(positions) {
  ...
  w.innerHTML = positions.map(function(p) {
    ...
    return '<div class="pos-card"><div class="pos-inner">'
      + '<div class="pos-ico ' + ic.c + '">' + ic.e + '</div>'
      + '<div class="pos-info"><div class="pos-tick">' + esc(p.ticker || '--') + '</div>'
...
function renderSigs(sigs) {
  ...
  w.innerHTML = sigs.slice(0, 5).map(function(s) {
    ...
    var edgeStr = s.feeAdjustedEdge ? 'Fee-adj ' + Math.round(s.feeAdjustedEdge * 100) + '%' : 'Edge ' + Math.round((s.edge || 0) * 100) + '%';
    return '<div class="sig-card ' + (isY ? '' : 'sig-no') + '">'
      + '<div class="sig-top"><div class="sig-tick">' + esc(s.ticker || '--') + '</div>'
...
function renderMkts() {
  ...
  if (!KB.state || !KB.state.topMarkets || !KB.state.topMarkets.length) {
    w.innerHTML = '<div class="empty">' + (KB.state ? 'Scanner running...' : 'Loading...') + '</div>'; return;
  }
  w.innerHTML = KB.state.topMarkets.slice(0, 12).map(function(m) {
    var yp = Math.round((m.yesAsk || 0) * 100), np = 100 - yp;
    var hl = +(m.hoursLeft || 999), soon = hl < 24;
    var hlStr = hl < 999 ? (hl < 1 ? '<1h' : Math.round(hl) + 'h') : '--';
    var edge = +(m.edge || 0);
    return '<div class="mkt-card">'
      + '<div class="mkt-top"><div class="mkt-ttl">' + esc(m.title || m.ticker) + '</div>'
      + '<div class="mkt-exp' + (soon ? ' soon' : '') + '">' + hlStr + '</div></div>'
      + '<div class="mkt-row"><div class="mkt-yes">' + yp + 'c</div>'
      + '<div class="mkt-nr"><div class="mkt-no">' + np + 'c</div>'
...
function renderTx() {
  ...
  var fl = KB.allTx;
  if (KB.txf === 'win') fl = KB.allTx.filter(function(t) { return t.type === 'win'; });
  else if (KB.txf === 'loss') fl = KB.allTx.filter(function(t) { return t.type === 'loss'; });
  else if (KB.txf === 'open') fl = KB.allTx.filter(function(t) { return t.type === 'open'; });
  ...
  w.innerHTML = Object.keys(groups).map(function(mon) {
    return '<div class="tx-mon">' + mon + '</div>' + groups[mon].map(function(tx, i) {
      ...
      return '<div class="tx-row">'
        + '<div class="tx-ico ' + ic + '">' + (tx.type === 'win' ? 'W' : tx.type === 'loss' ? 'L' : '~') + '</div>'
        + '<div class="tx-info"><div class="tx-ttl">' + lbl + ' - ' + esc(tx.ticker) + '</div>'
...
function applyState(s) {
  ...
  try {
    ...
    if (g('todayBar')) g('todayBar').style.background = today > 0 ? 'var(--g)' : today < 0 ? 'var(--r)' : 'var(--m2)';
    ...
    if (s.phaseLabel) {
      ...
      st('homePhase', _s(s.phaseLabel));
      st('jPhase', _s(s.phaseLabel));
      st('jDesc', _s(s.phaseDesc));
    ...
    if (lw && s.logs && s.logs.length) {
      lw.innerHTML = s.logs.slice(0, 20).map(function(l) {
        ...
        return '<div' + (cls ? ' class="' + cls + '"' : '') + '><span class="log-ts">' + esc(l.ts || '') + '</span>' + esc(l.msg || '') + '</div>';
      }).join('');
      if (s.logs[0]) {
    ...
    if (s.brainCount && s.brainCount > KB.lastBrain && KB.lastBrain > 0) {
      ...
      toast('[BRAIN] #' + s.brainCount + (bs ? ' -> ' + bs.ticker + ' ' + bs.side : ''), bs ? 'signal' : 'info');
    }
    ...
    KB.allTx = (s.trades || []).map(function(t) {
      return {
        ...
        side: _s(t.side) || 'YES', contracts: _n(t.contracts) || 1, won: !!t.won
      };
    });
    renderMkts();
    KB.loaded = true;
    ...
    /* Show error visibly on dashboard for debugging */
    var hb = g('heroBal');
...
/* ---- LOAD STATE ---- */
function loadState() {
  fetch(KB.api + '/api/state?v=' + KB.VERSION + '&_=' + Date.now(), {cache: 'no-store'})
    .then(function(r) {
    ...
    .then(function(txt) {
      ...
      try { s = JSON.parse(txt); }
        ...
        var hb = g('heroBal');
        if (hb) hb.textContent = 'Parse error';
    ...
    .catch(function(e) {
      KB.retries++;
      var hb = g('heroBal');
      if (hb) hb.textContent = 'Retry ' + KB.retries + ': ' + e.message.slice(0, 30);
      var hc = g('heroChg');
      if (hc) hc.textContent = 'retry ' + KB.retries;
    });
}
...
function doSync() {
  ...
  fetch(KB.api + '/api/sync', {method: 'POST'})
    ...
    .then(function(t) {
      try { var s = JSON.parse(t); KB.state = s; applyState(s); } catch(e) { loadState(); }
      if (btn) btn.style.opacity = '1';
    })
    .catch(function() { loadState(); if (btn) btn.style.opacity = '1'; });
}
function doPurge() {
  if (!confirm('Remove stale synced positions? Real Kalshi positions unaffected.')) return;

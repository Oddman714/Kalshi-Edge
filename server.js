...
async function runBrain(topMarkets) {
  ...
  const _slots = dynamicMaxPos() - S.openPositions.length;
  const phase = getPhase();
  const _bal = S.realBalance || S.balance;

  const prompt = `{"task":"generate_trade_signals","rules":["respond ONLY with valid JSON","no preamble","start with {","end with }"],"context":{"MISSION":"Double $${_bal.toFixed(2)} to $500K FAST. We have ${_slots} OPEN SLOT(S). Each bet must have a payout ratio that can actually double money.","ALREADY_HELD":"${_heldStr} — DO NOT signal any of these. Find DIFFERENT markets.","BALANCE":"$${_bal.toFixed(2)} | Cash: $${S.availableCash.toFixed(2)} | Open: ${S.openPositions.length}/${dynamicMaxPos()} | Phase: ${phase.name}","PAYOUT_MATH":"Net payout=(1-price)/price*0.955. At 45c=1.05x(REJECT). At 35c=1.77x(OK). At 25c=2.87x(GOOD). At 15c=5.37x(GREAT). MINIMUM 1.8x net payout. This means YES price must be <=38c OR NO price must be <=38c.","HARD_RULES":["1. YES side: market price MUST be <=45c. If above 45c, BET NO instead or skip entirely.","2. NO side: market price for NO MUST be <=45c (meaning YES price >=55c).","3. Net payout ratio MUST be >=1.8x. Calculate: ((1-price)/price)*0.955. Reject if below 1.8.","4. Min confidence: ${(phase.minProb*100).toFixed(0)}%. Min edge: ${(phase.minEdge*100).toFixed(0)}% net after fee.","5. PREFER markets resolving TODAY or TOMORROW for fast capital recycling."]},"markets":${JSON.stringify(topMarkets.slice(0,10).map(m=>({ticker:m.ticker,title:m.title,yesAsk:+(m.yesAsk*100).toFixed(1),yesBid:+((m.yesBid||0)*100).toFixed(1),netPayoutRatio:+(((1-m.yesAsk)/m.yesAsk*0.955)).toFixed(2),volume:Math.round(m.volume),hoursLeft:+m.hoursLeft.toFixed(1),alreadyHeld:_heldTickers.includes(m.ticker)})))},"search_strategy":{"SEARCHES":10,"ORDER":["1. Live sports scores NOW — NBA/NHL/MLB/MMA games in progress or tonight","2. Crypto price vs strike — BTC/ETH current price vs each market threshold","3. Economic data today — CPI/jobs/GDP actual vs consensus (already know CPI was released today)","4. Weather today for KXHIGH markets — actual forecast vs market strike","5. Any market with netPayoutRatio>=2.0 and hoursLeft<24 — these are the best bets","6. High volume, tight spread markets with clear directional edge"]},"execution":{"MIN_NET_PAYOUT":"1.8x — HARD REJECT anything below this","MIN_EDGE":"${(phase.minEdge*100).toFixed(0)}% after 4.5% fee","MIN_CONF":"${(phase.minProb*100).toFixed(0)}%","SLOTS":"Signal up to ${_slots} trades — empty slots = missed compounding"},"required_output":{"signals":[{"ticker":"NOT in ALREADY_HELD","side":"YES or NO","confidence":0.68,"edge":0.12,"marketPrice":0.30,"feeAdjustedEdge":0.09,"netPayoutRatio":2.19,"reasoning":"SPECIFIC: [exact score/price/data found]. Payout=(1-0.30)/0.30*0.955=2.2x. Conf=68%>30% price=38% edge.","hoursToClose":8}],"marketSummary":"what searched, what found","skipReason":"only if zero edge found after all searches"}}`;
...
async function executeTrade(sig) {
  ...
  const minPayout = 1.5; // absolute floor — anything below is useless for doubling
...
Deposit funds and restart manually when ready.`);
    ...
    const cashUtilization = effectiveCash / Math.max((S.realBalance || S.balance), 0.01);
    // Also treat as "has slots" if we have significant cash to deploy (>30% of balance)
    const hasCash = effectiveCash >= 0.75 && cashUtilization > 0.30;
    if (brainReady && (hasSlots || hasCash) && hasMarkets) {
      log(`🧠 Brain firing: ${meaningfulPositions.length}/${maxPos} meaningful positions, $${effectiveCash.toFixed(2)} cash (${(cashUtilization*100).toFixed(0)}% avail)`);
      await runBrain(markets);
    } else if (brainReady && !hasSlots && !hasCash) {
      log(`Brain: slots full (${meaningfulPositions.length}/${maxPos} meaningful) and cash depleted -- running position monitor`);
      S.lastBrainAt = Date.now();

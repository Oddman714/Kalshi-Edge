...
function loadState() {
  ...
  S.availableCash = 0;
  S.restingOrders = [];
  // Keep stored balance until Kalshi sync overwrites it (~7s after boot)
  // This ensures dashboard shows last known value immediately instead of 0
...
var API='', POLL=3000, state=null, chartData=[], allTx=[], currentTF='1D', txFilter='all';
...
function _applyStateInner(s){
  ...
  var _lastBal=bal>0?bal:(state&&state.realBalance>0?state.realBalance:0);
  setText('heroBal',_lastBal>0?fm(_lastBal):(running?'Syncing...':'$0.00'));
...
server.listen(CFG.port, () => {
  ...
  }, 1000); // 1s — get balance ASAP for dashboard

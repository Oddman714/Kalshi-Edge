...
<div id="page-home" class="page active">
  ...
  <div style="text-align:center;font-size:9px;color:rgba(238,238,245,0.15);padding:6px;font-family:monospace">v9.4 — KalshiBot</div>
  <div class="gap"></div>
</div>
...
<script>
// Detect correct API base — handles PWA home screen installs where origin may differ
var _loc=window.location;
var API=(_loc.protocol==='file:'||_loc.protocol==='blob:')?'https://kalshi-edge-production.up.railway.app':'';
// If served from a path (e.g. /app), strip the path so API calls go to origin root
if(API===''&&_loc.pathname!=='/'&&_loc.pathname!=='/app'){API=_loc.protocol+'//'+_loc.host;}
var POLL=8000,state=null,chartData=[],allTx=[],currentTF='1D',txFilter='all';

// Kill any stale service worker that might be intercepting and caching responses
...
function loadState(){
  ...
  // Show what URL we're hitting so user can diagnose on mobile
  if(_firstLoad&&cb){
    cb.style.display='block';
    cb.textContent='⏳ Connecting to '+(_loc.host||'server')+'...';
  }
  ...
  fetch(url,{cache:'no-store',signal:ctrl?ctrl.signal:undefined})
    ...
    .then(function(s){
      ...
      _retries=0;
      state=s;
      applyState(s); // applyState hides banners as first action
    })
    .catch(function(e){
      if(timer)clearTimeout(timer);
      _retries++;
      var delay=Math.min(2000*_retries,12000);
      var msg=e.name==='AbortError'?'Timeout (9s)':e.message;
      console.error('loadState #'+_retries+':',msg,'url:',url);
      if(_firstLoad){
        if(cb){cb.style.display='block';cb.textContent='⏳ Attempt '+_retries+': '+msg;}
      } else {
        if(eb){eb.style.display='block';eb.textContent='⚠️ Reconnecting ('+_retries+')...';}
      }
...
function applyState(s){
  // Step 1: always hide banners and mark connected — we have data
  _firstLoad=false;
  var eb=document.getElementById('errBanner');
  var cb=document.getElementById('connBanner');

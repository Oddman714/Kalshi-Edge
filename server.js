...
async function runCryptoFastPath(topMarkets) {
  ...
  for (const m of cryptoMarkets) {
    ...
    // v18.2 Strategy 5: B-prefix format (KXSOLE-26APR2017-B86, KXXRPE-...-B1.4199)
    // Kalshi uses B-prefix for certain strike-tied markets. Extract number after B.
    if (!strike) {
      const bMatch = m.ticker.match(/[-_]B([\d]+\.?[\d]*)(?:[-_]|$)/i);
      if (bMatch) strike = parseFloat(bMatch[1]);
    }

    // v18.2 Strategy 6: subtitle / rules_primary / yes_sub_title fields from Kalshi
    // These fields often contain the strike as human-readable text ("SOL at $86.99 or above")
    if (!strike) {
      const textSources = [m.subtitle, m.rules_primary, m.yes_sub_title, m.no_sub_title].filter(Boolean);
      for (const txt of textSources) {
        const clean = txt.replace(/,/g, '');
        const dollarMatch = clean.match(/\$([\d]+\.?[\d]*)/);
        if (dollarMatch) {
          const parsed = parseFloat(dollarMatch[1]);
          if (parsed > 0) { strike = parsed; break; }
        }
      }
    }
    ...
    // v18.2 SAFETY: if parsed strike is more than 30% from live price, we probably parsed
    // the wrong field (e.g. bucket index instead of dollar strike). Skip as precaution.
    if (livePrice && strike > 0) {
      const gapSanity = Math.abs(livePrice - strike) / livePrice;
      if (gapSanity > 0.30) {
        log(`FastPath: ${m.ticker} parsed strike=$${strike} is ${(gapSanity*100).toFixed(0)}% from live=${livePrice} — likely parser error, skip`);
        continue;
      }
    }

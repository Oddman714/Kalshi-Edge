...
async function scan(){
  ...
  try{
    ...
    if(r.status!==200){log('Scan HTTP '+r.status+': '+JSON.stringify(r.data).slice(0,120));return;}
    authOk();
    const markets=r.data.markets||[];
    if(markets.length===0){log('Scan: API returned 0 markets');return;}

    // Always log first market fields until we have candidates (helps debugging)
    if(S.cands.length===0&&markets.length>0){
      const m0=markets[0];
      const allFields=Object.keys(m0).join(', ');
      log('RAW FIELDS: '+allFields);
      log('RAW SAMPLE: '+JSON.stringify(m0).slice(0,300));
    }

    const held=new Set(S.positions.map(p=>p.ticker));
    const now=Date.now();
    let rej={ticker:0,vol:0,time:0,price:0,pass:0};

    const scored=markets.filter(m=>{
      // Ticker — try all known field names
      const ticker=m.ticker_name||m.ticker||m.market_ticker||'';
      if(!ticker){rej.ticker++;return false;}
      if(held.has(ticker))return false;

      // Volume — NO minimum, accept everything
      // (we'll add a filter back once we confirm data is flowing)

      // Close time — try all known Kalshi field names
      const closeRaw=m.close_time||m.expiration_time||m.expiry_time||m.expiry||m.end_date||null;
      if(!closeRaw){rej.time++;return false;}
      const closeMs=new Date(closeRaw).getTime();
      if(isNaN(closeMs)||closeMs<now){rej.time++;return false;}
      // Accept markets closing in up to 30 days
      const hrs=(closeMs-now)/3.6e6;
      if(hrs>720){rej.time++;return false;}

      // Price — accept any valid price, default 50 if missing
      const y=m.last_price!=null?m.last_price:m.yes_ask!=null?m.yes_ask:m.yes_bid!=null?m.yes_bid:50;
      if(y<1||y>99){rej.price++;return false;}

      rej.pass++;
      return true;
    }).map(m=>{
      const ticker=m.ticker_name||m.ticker||m.market_ticker;
      const y=Math.round(m.last_price!=null?m.last_price:m.yes_ask!=null?m.yes_ask:m.yes_bid!=null?m.yes_bid:50);
      const closeRaw=m.close_time||m.expiration_time||m.expiry_time||m.expiry||m.end_date;
      const closeMs=new Date(closeRaw).getTime();
      ...
      const score=(1-Math.abs(y-50)/50)*0.4+Math.min((m.volume||0)/5000,1)*0.3+Math.max(0,1-parseFloat(hrs)/48)*0.3;
      return{
        ticker,
        title:(()=>{
        const raw=m.title||m.subtitle||ticker;
        // Multi-leg markets: extract event name from ticker or use shortened title
        if(raw.length>70){
          // Try to get event ticker (cleaner name)
          const evTicker=m.event_ticker||m.event_title||'';
          if(evTicker&&evTicker.length<60)return evTicker;
        ...
        vol:m.volume||0,
        hrs,
        cat:m.category||m.event_category||m.event_ticker||'market',
        mom,score
      };
    });

    // Sort: first by has-volume, then by score
    scored.sort((a,b)=>{
      const aHasVol=a.vol>0?1:0,bHasVol=b.vol>0?1:0;
      if(bHasVol!==aHasVol)return bHasVol-aHasVol;
      return b.score-a.score;
    });
    S.cands=scored.slice(0,25);
    log('Scan: '+markets.length+' fetched | pass='+rej.pass+' rej_ticker='+rej.ticker+' rej_time='+rej.time+' rej_price='+rej.price+' | candidates='+S.cands.length);
    if(S.cands.length>0){
      const t=S.cands[0];
      log('Top: '+t.ticker+' "'+t.title.slice(0,50)+'" yes='+t.yes+'¢ '+t.hrs+'h vol='+t.vol);
    }
...
✓ SKIP markets with vol=0 — no liquidity, can't fill limit orders
✓ SKIP multi-leg parlay titles (comma-separated player props) — too complex to price
✓ PREFER: Fed/rates, elections, economic data, crypto prices, single-event markets
...
function renderFeed(){
  ...
  el.innerHTML=list.slice(0,18).map(c=>'<div class="item"><div class="ihead"><div class="iname" style="font-size:12px">'+esc(c.title)+'</div><span style="font-size:11px;font-family:var(--m);color:var(--t3)">'+c.yes+'¢ '+c.mom+'</span></div><div class="imeta">'+(c.vol===0?'<span style="color:var(--r)">No liquidity</span>':'<span>Vol '+(c.vol||0).toLocaleString()+'</span>')+''<span>'+c.hrs+'h</span><span>'+c.cat+'</span>'+(sigSet.has(c.ticker)?'<span class="cb">SIGNAL</span>':'')+'</div></div>').join('');
}

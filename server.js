...
<div id="page-home" class="page active">
  ...
  <div style="text-align:center;font-size:9px;color:rgba(238,238,245,.12);padding:8px;font-family:monospace">v9.7</div>
  <div class="gap"></div>
</div>
...
function showBanner(msg,type){
  ...
  b.textContent=msg;
  b.className=type||'yellow';
  b.style.display='block';
  b.style.position='fixed';b.style.top='0';b.style.left='0';b.style.right='0';
  b.style.padding='12px 16px';b.style.textAlign='center';
  b.style.fontWeight='600';b.style.fontSize='12px';b.style.zIndex='9999';
  b.style.background=type==='red'?'#ff3d5a':'rgba(245,166,35,0.95)';
  b.style.color=type==='red'?'#fff':'#000';
}
function hideBanner(){var b=$('banner');if(b)b.style.display='none';}
  ...
  fetch(url,{cache:'no-store'})
    ...
    .then(function(text){
      ...
      try{ s=JSON.parse(text); }
        ...
        // JSON failed — log raw response and retry
        console.error('JSON FAIL len='+text.length+' start='+text.slice(0,100));
    ...
    .catch(function(e){
      ...
      if(!_loaded){
        showBanner('Connecting... ('+_retries+') '+e.message,'yellow');
      } else {
        showBanner('Lost connection — retrying...','red');
      }
...
const server = http.createServer((req, res) => {
    ...
    try {
      ...
      const slim = {
        ...
        logs: (S.logs||[]).slice(0,10).map(l => ({
          ts: ss(l.ts,8), level: ss(l.level,4), msg: ss(l.msg, 80),
        })),
        topMarkets: (S.topMarkets||[]).slice(0,8).map(m => ({
          ticker: ss(m.ticker,20), title: ss(m.title, 60),
          yesAsk: nn(m.yesAsk), yesBid: nn(m.yesBid),
          volume: nn(m.volume), score: nn(m.score),
          hoursLeft: nn(m.hoursLeft), edge: nn(m.edge),
        })),
        trades: (S.trades||[]).slice(0,10).map(t => ({
          ticker: ss(t.ticker,20), side: ss(t.side,4),
          contracts: nn(t.contracts), cost: nn(t.cost), pnl: nn(t.pnl),
          won: !!t.won, status: ss(t.status,8),
          openedAt: nn(t.openedAt), resolvedAt: nn(t.resolvedAt),
        })),
      };
  ...
  // Ultra-fast ping — just returns ok:true with minimal data
  // Used by dashboard to test if server is reachable before loading full state
  if (url.pathname === '/ping') {
    res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' });
    return res.end(JSON.stringify({ ok: true, running: S.isRunning, bal: S.realBalance||S.balance, scans: S.scanCount }));
  }

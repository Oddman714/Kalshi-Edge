...
// =============================================================================
// KalshiBot v14.0 — FLYWHEEL MAXIMIZER
// =============================================================================
// KEY FIXES vs v13:
// 1. BRAIN DECOUPLED: Brain fires every 60s when slots open, regardless of FastPath.
//    Old: brain skipped if FastPath filled even 1 slot. New: always fire both.
// 2. FASTER SCANS: 10s hungry mode (slots open), 15s full mode. Was 20s always.
// 3. FASTPATH GAP LOWERED: 15min needs 3% gap (was 5%). Hourly needs 2% (was 1.5%).
//    Old 8% favorite threshold was only achievable during extreme volatility.
// 4. BRAIN ALLOWS 15-MIN: Old hard ban removed. Brain can now signal 12min+ markets.
//    This means brain contributes to the fastest-resolving trades, not just hourly+.
// 5. CASH CALC FIXED: Resting order sync failure no longer zeros available cash.
//    Proper reserved cash calculation: balance - reserved orders - buffer.
// 6. POSITION SYNC ON EVERY SCAN when slots open. Old: every 3 scans (60s gap).
// 7. SLOT FREED = IMMEDIATE RESCAN: syncPositions now detects Kalshi-side resolution
//    and triggers brain reset + 3s rescan instead of waiting for next cycle.
// 8. CIRCUIT BREAKER: 4 losses → 15min pause (was 5 losses → 20min).
// 9. PAYOUT FLOORS LOWERED: FastPath 1.2x (was 1.5x), Brain crypto 1.5x (was 1.8x).
//    More trades fire without sacrificing EV.
// 10. BRAIN MIN CONF: 75% (was 80%). Allows more real-edge trades to surface.
// =============================================================================
...
3. NEVER trade anything with market-implied probability <12% (price <12c = skip)
...
8. Only signal trades with 75%+ true confidence

STEP 1 - CRYPTO (no search needed):
- For each crypto market: find the strike in the title
- YES ONLY if price > strike with gap >= 3%. NO ONLY if price < strike with gap >= 3%
- Signal if gap >= 3% AND netPayout >= 1.2 AND hoursLeft >= 0.20 (12+ minutes)
- For 15-min markets (hoursLeft < 0.30): gap must be >= 4% and confidence >= 82%
- Confidence: gap<3%=SKIP, gap 3-5%=78%, gap 5-8%=84%, gap 8-12%=90%, gap>12%=94%
- A 3% gap means price must move 3%+ against you to lose — that is real edge

STEP 2 - SPORTS (1 search per game): Search "[team] vs [team] live score". Signal team leading by 10+ points in 4th quarter or 2 goals in final period. netPayout>=2.0. 70%+ confidence.

STEP 3 - FALLBACK: If still under ${_slots} signals, best crypto with gap>=2.5% and hoursLeft>=0.5h.

Confidence floors: ALL brain signals need 75%+ minimum. No exceptions.
...
Output ONLY valid JSON. Start with { end with }. No markdown. No explanation outside JSON.`,
    ...
    if (S.signals.length > 0) {
      ...
      for (const sig of S.signals) {
        ...
        const meetsConf = sig.confidence >= Math.max(execPhase.minProb, 0.75);
...
async function executeTrade(sig) {
  ...
  const minPayout = isFavoriteExec ? 0.12  // favorites: any positive EV
    : isCryptoTicker
    ? (isFastPath ? 1.2 : 1.5)   // FastPath: 1.2x min. Brain crypto: 1.5x min.
    : 1.8;                        // Non-crypto brain: 1.8x min
  ...
  if (sidePriceActual < 0.12 && sig.confidence < 0.82) {
    log(`Skip ${sig.ticker}: extreme longshot ${sig.side}@${(sidePriceActual*100).toFixed(0)}c needs 82%+ conf, got ${(sig.confidence*100).toFixed(0)}%`);
    return;
  }
  if (sidePriceActual < 0.20 && sig.confidence < 0.72) {
    log(`Skip ${sig.ticker}: longshot ${sig.side}@${(sidePriceActual*100).toFixed(0)}c needs 72%+ conf, got ${(sig.confidence*100).toFixed(0)}%`);
    return;
  }
  if (sidePriceActual < 0.30 && sig.confidence < 0.68) {
    log(`Skip ${sig.ticker}: underdog ${sig.side}@${(sidePriceActual*100).toFixed(0)}c needs 68%+ conf, got ${(sig.confidence*100).toFixed(0)}%`);
    return;
  }
...
async function startBot() {
  ...
  _botStarted = true;
  S.isRunning = true;
  log('[M] KalshiBot v14.0 started -- running pre-flight sync...');

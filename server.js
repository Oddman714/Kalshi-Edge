...
const PHASES = [
  { name: 'SEED',     min: 0,      max: 50,     kelly: 0.40, minEdge: 0.10, minProb: 0.70, maxPos: 3,  label: '🌱 Seed',     desc: '3 slots max. 70%+ conf, 2.5x+ payout, <24h resolution only. Crypto + verified sports only.' },
...
async function getTopMarkets() {
  try {
    ...
    const seriesList = [
      ...
      // Hourly BTC/ETH: resolve every hour 24/7. Price is known. Edge = BTC vs strike.
      'KXBTCH','KXBTC15M','KXBTCUSDH','KXETHUSDH','KXETH15M',
...
async function executeTrade(sig) {
  ...
  // Tiered longshot guards — lower price = higher confidence required.

...
async function runBrain(topMarkets) {
  ...
  // Only skip if ALL slots are full — otherwise always hunt for trades
  if (S.openPositions.length >= dynamicMaxPos()) {
    log(`Brain: all ${S.openPositions.length} slots full — running monitor instead`);
    return;
  }
...
Deposit funds and restart manually when ready.`);
    ...
    if (brainReady && hasSlots && hasMarkets) {
      log(`Brain ready: ${S.openPositions.length}/${maxPos} positions, ${markets.length} markets`);
      ...
      if (S.scanCount % 10 === 0) log(`Brain: ${secsToNext}s to next fire | slots: ${S.openPositions.length}/${maxPos}`);
    }
...
<title>KalshiBot 20260408160740</title>
...
<div id="page-home" class="page active">
  ...
  <div style="text-align:center;font-size:9px;color:rgba(238,238,245,0.15);padding:6px;font-family:monospace">v20260408160740</div>
  <div class="gap"></div>
</div>
...
function loadState(){
  fetch(API+'/api/state?_='+Date.now(),{cache:'no-store'})
    ...
    .catch(function(e){
      ...
      var eb=document.getElementById('errBanner');
      if(eb){eb.textContent='Connecting... attempt '+_retries+' ('+e.message+')';eb.style.display='block';}
...
function applyState(s){
  try{
    ...
    setText('heroBalance',fm(bal));
    setText('heroReal',fm(real));
    setText('heroPeak',fm(peak));

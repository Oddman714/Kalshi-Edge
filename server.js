...
async function scan(){
  ...
  try{
    // Fetch markets sorted by volume descending to get liquid markets first.
    // Also fetch with min_volume filter to skip zero-volume test markets.
    // Kalshi KXMV prefixed tickers are internal test/parlay markets — skip them.
    const params={status:'open',limit:'200'};
    const r=await kalshi('GET','/markets',params);
    ...
    authOk();
    const allMarkets=r.data.markets||[];
    if(!allMarkets.length){log('Scan: 0 markets returned');return;}

    // Log first market fields for diagnostics
    if(allMarkets.length>0){
      const m0=allMarkets[0];
      log('SCAN FIELDS: '+Object.keys(m0).join(', '));
      const vols=allMarkets.map(m=>m.volume||0);
      const withVol=vols.filter(v=>v>0).length;
      log('SCAN: '+allMarkets.length+' total | '+withVol+' with volume>0');
      // Log top 3 by volume
      const byVol=[...allMarkets].sort((a,b)=>(b.volume||0)-(a.volume||0)).slice(0,3);
      byVol.forEach((m,i)=>log('TOP_VOL_'+i+': '+JSON.stringify({ticker:m.ticker_name||m.ticker,vol:m.volume,title:(m.title||'').slice(0,50),cat:m.category})));
    }

    const held=new Set(S.positions.map(p=>p.ticker));
    const now=Date.now();

    // JUNK TICKER PREFIXES — Kalshi internal test/parlay markets, no real liquidity
    const junkPrefixes=['KXMV','KXMEVESPORTS','KXMLT'];
    ...
    const scored=allMarkets.filter(m=>{
      ...
      // Skip Kalshi internal test markets
      if(junkPrefixes.some(p=>ticker.startsWith(p)))return false;

      // Require real volume — zero volume = no liquidity, can't fill orders
      ...
      if(hrs>720)return false; // skip beyond 30 days
      ...
      // Score: prefer mid-range prices + high volume + urgency
      const score=(1-Math.abs(y-50)/50)*0.35+Math.min(vol/50000,1)*0.4+Math.max(0,1-parseFloat(hrs)/48)*0.25;
      return{ticker,title,yes:y,no:100-y,vol,hrs,cat:m.category||m.event_category||'market',mom,score};
    });

    scored.sort((a,b)=>b.score-a.score);
    S.cands=scored.slice(0,25);
    log('Scan complete: '+allMarkets.length+' raw | '+scored.length+' liquid+valid | '+S.cands.length+' candidates');
    if(S.cands.length>0){const t=S.cands[0];log('Top candidate: '+t.ticker+' "'+t.title.slice(0,50)+'" yes='+t.yes+'¢ '+t.hrs+'h vol='+t.vol);}
    else{
      const withVol=allMarkets.filter(m=>(m.volume||0)>0&&!junkPrefixes.some(p=>(m.ticker_name||m.ticker||'').startsWith(p)));
      log('Zero liquid candidates. Liquid non-junk markets: '+withVol.length);
      if(withVol.length>0)log('Sample liquid: '+JSON.stringify(withVol[0]).slice(0,300));
    }
...
app.get('/debug',async(_,res)=>{try{const r=await kalshi('GET','/markets',{status:'open',limit:'3'});const m=r.data.markets||[];res.json({status:r.status,count:m.length,fields:m[0]?Object.keys(m[0]):[],sample:m.slice(0,1),cands:S.cands.length,lastErr:S.lastErr});}catch(e){res.json({error:e.message});}});

...
async function runCryptoFastPath(topMarkets) {
  ...
  for (const m of cryptoMarkets) {
    ...
    // Direction markets: "price up/down in next X mins?" OR Up/Down 15min markets
    // Also catches _isUpDownMarket flag set during normalization above
    const isDirectionMarket = (
      m._isUpDownMarket === true ||
      (m.title && /price (up|down) in (next|the next)/i.test(m.title) && !m.title.match(/\$[\d]/))
    );
    if (isDirectionMarket) {
      ...
      try {
        ...
        if (kr.status === 200 && Array.isArray(kr.body) && kr.body.length >= 3) {
          ...
          // Uptrend: candles trending up. currentVsClose >= -0.0005 = within noise band.
          // Fixing false "mixed" when cur=0.000% (live price exactly equals last close).
          if (trendPct > 0.001 && currentVsClose >= -0.0005) {
            trendDir = 'up';
            trendConf = trendPct > 0.003 ? 0.82 : 0.76; // stronger trend = higher conf
          } else if (trendPct < -0.001 && currentVsClose <= 0.0005) {
            trendDir = 'down';
            trendConf = trendPct < -0.003 ? 0.82 : 0.76;
          } else {
            // Genuinely conflicting signals — no edge
            log(`FastPath dir: ${m.ticker} mixed trend (trend=${(trendPct*100).toFixed(3)}% cur=${(currentVsClose*100).toFixed(3)}%) — skip`);
            continue;
          }
        }
      } catch(ke) {
      ...
      if (dirPayout < 1.5) {
        log(`FastPath dir: ${m.ticker} payout ${dirPayout.toFixed(2)}x < 1.5x floor — skip`);
        continue;
      }
    ...
    // (A) Underdog: price 5-35c, payout 2x+, confidence 89%+ → full Kelly sizing
    ...
    // (B) Favorite: price 55-88c, payout 0.2-0.8x, confidence 89%+
    //     THE WINNING TRADE: BTC $83k vs strike $71.5k (13.8% gap), YES @88c = 96% conf
    //     For 15min: ONLY allowed when gap >= 8% (large enough to survive volatility)
    //     For 1h+: allowed with gap >= 5% (more time = smaller gap acceptable)
    const isUnderdog = sidePrice <= 0.38;
    const isFavorite15min = is15Min && sidePrice >= 0.55 && sidePrice <= 0.92 && confidence >= 0.93 && gapPct >= 0.08;
    const isFavoriteHourly = !is15Min && sidePrice >= 0.55 && sidePrice <= 0.88 && confidence >= 0.89;
    const isFavorite = isFavorite15min || isFavoriteHourly;

    // --- GATE CHECKS ---
    const fastPathMinConf = isUnderdog ? Math.max(phase.minProb, 0.72) : 0.85;

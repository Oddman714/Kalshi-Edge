...
async function syncBalance() {
  try {
    ...
    if (!lastPt || lastPt.v !== bal || (Date.now() - lastPt.t) > 300000) {
...
Deposit funds and restart manually when ready.`);
    ...
    if (S.scanCount % 5 === 0) {
      const posStr = S.openPositions.length > 0
        ? '\n📋 ' + S.openPositions.map(p => p.ticker.split('-').slice(-1)[0] + ' ' + p.side).join(' | ')
        : '';
      const overLimit = S.openPositions.length > maxPos ? ` ⚠️ ${S.openPositions.length - maxPos} over limit` : '';
      tg(`📡 <b>Scan #${S.scanCount}</b>
💰 $${(S.realBalance||S.balance).toFixed(2)} | ${S.openPositions.length}/${maxPos} slots${overLimit} | ${markets.length} mkts
...
function pushPt(v) {
  ...
  if (last && last.v === +v && (Date.now() - last.t) < 300000) { drawChart(); return; }
...
function drawChart() {
  ...
  if (d.length < 2) {
    svg.innerHTML = '<text x="215" y="75" text-anchor="middle" fill="rgba(238,238,245,0.15)" font-size="12" font-family="DM Mono">No chart data yet</text>';
    return;
  }

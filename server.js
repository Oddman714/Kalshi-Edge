...
async function runCryptoFastPath(topMarkets) {
  ...
  for (const m of cryptoMarkets) {
    ...
    if (is15Min) {
      // v17: tightened. 15min BTC 1-sigma ~1.5%. Need 5x+ buffer for genuine edge.
      if (gapPct >= 0.15) confidence = 0.95;       // 10x vol buffer
      else if (gapPct >= 0.12) confidence = 0.92;  // 8x buffer
      else if (gapPct >= 0.09) confidence = 0.87;  // 6x buffer
      else { log(`FastPath 15min: ${m.ticker} gap ${(gapPct*100).toFixed(2)}% < 9% v17 floor — skip`); continue; }
    } else if (m.hoursLeft < 1.5) {
      // v17: tightened. 1h BTC 1-sigma ~2.5%. Need 2x+ buffer.
      if (gapPct >= 0.12) confidence = 0.93;
      else if (gapPct >= 0.08) confidence = 0.88;
      else if (gapPct >= 0.06) confidence = 0.82;
      else { log(`FastPath hourly: ${m.ticker} gap ${(gapPct*100).toFixed(2)}% < 6% v17 floor — skip`); continue; }
    } else {
...
async function startBot() {
  ...
  _botStarted = true;
  S.isRunning = true;
  log('[M] KalshiBot v17 started -- running pre-flight sync...');
...
server.listen(CFG.port, () => {
  log(`KalshiBot v17 listening on port ${CFG.port}`);

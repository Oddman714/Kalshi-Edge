...
function loadState() {
  ...
  S.availableCash = 0; // always recompute from Kalshi on boot
  S.restingOrders = []; // always recompute from Kalshi on boot
  // If state.json has a suspiciously high balance (stale from old run), clear it
  // Real balance will be synced from Kalshi in pre-flight
  if (S.balance > 100 && S.realBalance === 0) {
    log(`Stale balance $${S.balance.toFixed(2)} detected in state.json — clearing, will sync from Kalshi`);
    S.balance = CFG.bankroll;
    S.peakBalance = CFG.bankroll;
  }

...
<script>
// ── State ──────────────────────────────────────────────────────────────────
var S=null, chartData=[], allTx=[], txFilter='all', currentTF='1D';
var POLL=8000, retries=0, connected=false;
...
function set(id,v){var e=$(id);if(e)e.textContent=String(v==null?'':v);}
function col(id,c){var e=$(id);if(e)e.style.color=c;}
function $$(q){return document.querySelectorAll(q);}
function fm(n){return '$'+(Math.abs(n||0)).toFixed(2);}
function sgn(n){return(n>=0?'+':'')+fm(n);}
function dt(ts){
  if(!ts)return '';
  var d=new Date(ts);
...
function banner(msg,cls){
  ...
  b.textContent=msg; b.className=cls||'yellow'; b.style.display='block';
}

// ── Chart ──────────────────────────────────────────────────────────────────
function setTF(el,tf){
  currentTF=tf;
  $$('.tf-btn').forEach(function(b){b.classList.remove('active');});
...
function drawChart(){
  var svg=$('mchart'); if(!svg||chartData.length<2)return;
  ...
  if(d.length<2)d=chartData;
  if(d.length<2)return;
  var W=430,H=140,pad=8;
  var vals=d.map(function(p){return p.v;});
  var mn=Math.min.apply(null,vals),mx=Math.max.apply(null,vals),rng=mx-mn||0.01;
  function tx(i){return pad+(i/(d.length-1))*(W-pad*2);}
  function ty(v){return H-pad-((v-mn)/rng)*(H-pad*2-12);}
  var up=d[d.length-1].v>=d[0].v,c=up?'#05d47c':'#ff3d5a';
  ...
  svg.innerHTML='<defs><linearGradient id="cg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="'+c+'" stop-opacity="0.18"/><stop offset="90%" stop-color="'+c+'" stop-opacity="0"/></linearGradient></defs>'
    +'<path d="'+area+'" fill="url(#cg)"/>'
    +'<path d="'+line+'" fill="none" stroke="'+c+'" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>'
...
function renderPos(){
  if(!S)return;
  var pos=S.openPositions||[];
  var wrap=$('posList'),sub=$('tradeSub');
  if(sub)sub.textContent=pos.length+' open';
  if(!wrap)return;
  if(!pos.length){wrap.innerHTML='<div class="empty">No open positions</div>';return;}
  wrap.innerHTML=pos.map(function(p){
    var isY=(p.side||'').toUpperCase()==='YES';
    var ep=p.entryPrice||50, ct=p.contracts||1, cost=p.cost||(ep*ct/100);
    var hrs=p.openedAt?Math.round((Date.now()-p.openedAt)/3600000):0;
    var conf=p.confidence?Math.round(p.confidence*100)+'%':'--';
    var t=p.ticker||'--';
    var abbr=t.length>12?t.slice(0,12)+'…':t;
    return '<div class="pos-card">'
      +'<div class="pos-inner">'
      +'<div class="pos-icon">'+t.slice(0,4)+'</div>'
      +'<div class="pos-info"><div class="pos-ticker">'+t+'</div>'
      +'<div class="pos-tags"><span class="stag '+(isY?'stag-y':'stag-n')+'">'+(isY?'YES':'NO')+'</span>'
      +'<span class="pos-meta">'+ct+' ct</span></div></div>'
      +'<div class="pos-right"><div class="pos-price">'+ep+'¢</div>'
      +'<div class="pos-cost">'+fm(cost)+'</div></div></div>'
      +'<div class="pos-foot">'
      +'<div class="pf-item"><div class="pf-lbl">Conf</div><div class="pf-val">'+conf+'</div></div>'
      +'<div class="pf-item"><div class="pf-lbl">Open</div><div class="pf-val">'+hrs+'h</div></div>'
      +'<div class="pf-item"><div class="pf-lbl">Value</div><div class="pf-val">'+fm(ep*ct/100)+'</div></div>'
      +'</div></div>';
  }).join('');
}
...
function renderSigs(){
  ...
  if(!sigs.length){wrap.innerHTML='<div class="empty" style="padding:16px 20px">No signals yet</div>';return;}
  wrap.innerHTML=sigs.map(function(s){
    var isY=(s.side||'').toUpperCase()==='YES';
    return '<div style="background:var(--s1);border-radius:14px;margin:0 20px 8px;padding:14px;border:1px solid var(--border);border-left:3px solid '+(isY?'var(--green)':'var(--red)')+';">'
      +'<div style="display:flex;justify-content:space-between;margin-bottom:6px">'
      +'<span style="font-size:14px;font-weight:700;font-family:var(--mono)">'+(s.ticker||'--')+'</span>'
      +'<span class="stag '+(isY?'stag-y':'stag-n')+'">'+(s.side||'?')+'</span></div>'
      +'<div style="font-size:12px;color:var(--muted);line-height:1.45;margin-bottom:8px">'+(s.reasoning||'--')+'</div>'
      +'<div style="display:flex;gap:12px;flex-wrap:wrap;font-size:11px;color:var(--muted2)">'
      +'Conf <strong style="color:var(--text)">'+Math.round((s.confidence||0)*100)+'%</strong> &nbsp;'
      +'Edge <strong style="color:var(--text)">'+Math.round((s.edge||0)*100)+'%</strong> &nbsp;'
      +'Size <strong style="color:var(--text)">'+fm(s.kellySize||0)+'</strong>'
      +'</div></div>';
  }).join('');
}
...
function filt(f,el){
  txFilter=f;
  $$('.f-chip').forEach(function(c){c.classList.remove('active');});
  if(el)el.classList.add('active');
  renderTx();
}
function renderTx(){
  var wrap=$('txFeed'); if(!wrap)return;
  var fl=allTx;
  if(txFilter==='win')fl=fl.filter(function(t){return t.type==='win';});
  else if(txFilter==='loss')fl=fl.filter(function(t){return t.type==='loss';});
  else if(txFilter==='open')fl=fl.filter(function(t){return t.type==='open';});
  if(!fl.length){wrap.innerHTML='<div class="empty">No transactions yet</div>';return;}
  var groups={};
  fl.forEach(function(tx){
    var k=new Date(tx.ts).toLocaleString('en-US',{month:'long',year:'numeric'});
    if(!groups[k])groups[k]=[];
    groups[k].push(tx);
  });
  wrap.innerHTML=Object.keys(groups).map(function(month){
    return '<div class="tx-month">'+month+'</div>'+groups[month].map(function(tx,i,arr){
      var lbl=tx.type==='win'?'Win':tx.type==='loss'?'Loss':'Open';
      var icls='ti-'+(tx.type==='win'?'win':tx.type==='loss'?'loss':'open');
      var ic=tx.type==='win'?'W':tx.type==='loss'?'L':'~';
      var pnlStr=tx.type==='open'?'-'+fm(tx.cost||0):(tx.pnl>=0?'+'+fm(tx.pnl):'-'+fm(Math.abs(tx.pnl)));
      var pnlCls=tx.type==='win'?'amt-pos':tx.type==='loss'?'amt-neg':'amt-zero';
      return '<div class="tx-row">'
        +'<div class="tx-icon '+icls+'">'+ic+'</div>'
        +'<div class="tx-info"><div class="tx-title">'+lbl+' · '+(tx.ticker||'--')+'</div>'
        +'<div class="tx-sub">'+(tx.side||'YES')+' · '+dt(tx.ts)+'</div></div>'
        +'<div class="tx-right"><div class="tx-amt '+pnlCls+'">'+pnlStr+'</div>'
        +'<div class="tx-time">'+new Date(tx.ts).toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit'})+'</div></div>'
        +'</div>'+(i<arr.length-1?'<div class="tx-div"></div>':'');
    }).join('');
  }).join('');
}
...
function renderMkts(){
  ...
  if(!S||!S.topMarkets||!S.topMarkets.length){
    wrap.innerHTML='<div class="empty">'+(S?'Scanner running...':'Loading...')+'</div>';
    return;
  }
  wrap.innerHTML=S.topMarkets.slice(0,12).map(function(m){
    var yp=Math.round((m.yesAsk||0)*100),np=100-yp,hl=m.hoursLeft||999,soon=hl<24;
    return '<div class="mkt-card">'
      +'<div class="mkt-top"><div class="mkt-title">'+(m.title||m.ticker||'--')+'</div>'
      +'<div class="mkt-exp'+(soon?' soon':'')+'">'+( hl<999?(hl<1?'<1h':Math.round(hl)+'h'):'--')+'</div></div>'
      +'<div class="mkt-row"><div class="mkt-yes">'+yp+'¢ YES</div>'
      +'<div class="mkt-no-block"><div class="mkt-no">'+np+'¢ NO</div>'
      +'<div class="mkt-vol">vol '+(m.volume||0).toLocaleString()+'</div></div></div>'
...
function applyState(s){
  ...
  var w=+(s.wins||0),l=+(s.losses||0),tot=w+l;
  var openCnt=(s.openPositions||[]).length,mp=+(s.maxPos||5);
  var wr=tot>0?(w/tot*100):null;
  var dryRun=s.dryRun!==false,running=!!s.isRunning;

  // hero
  var hb=$('heroBal');
  if(hb){hb.style.color='';hb.textContent=bal>0?fm(bal):(running?'Syncing...':'$0.00');}
  set('heroReal',real>0?fm(real):(running?'...':'$0.00'));
  set('heroPeak',peak>0?fm(peak):'$0.00');
  var pnlEl=$('heroPnl');
  if(pnlEl){pnlEl.textContent=sgn(total);pnlEl.style.color=total>=0?'var(--green)':'var(--red)';}
  var chip=$('heroChip');
  if(chip)chip.className='hero-chip '+(today>0?'chip-up':today<0?'chip-dn':'chip-flat');
  set('heroArr',today>=0?'▲':'▼');
  set('heroChange',fm(today)+' today');

  // mode pill
  set('pillText',dryRun?'PAPER':'LIVE');
  var pill=$('modePill');if(pill)pill.className='mode-pill '+(dryRun?'pill-paper':'pill-live');
  var dot=$('pillDot');if(dot)dot.style.background=dryRun?'var(--gold)':'var(--green)';
  ...
  set('statWRsub',tot+' trades');
  var td=$('statToday');
  if(td){td.textContent=(today>=0?'+':'')+fm(today);td.style.color=today>0?'var(--green)':today<0?'var(--red)':'var(--muted)';}
  var todayBar=$('todayBar');if(todayBar)todayBar.style.background=today>0?'var(--green)':today<0?'var(--red)':'var(--muted2)';
  set('statTodaySub',sgn(total)+' all-time');
  set('statPos',openCnt+'/'+mp);
  set('statPosSub','of '+mp+' max');

  // engine
  var ed=$('engDot');if(ed)ed.className='eng-dot'+(running?' on':'');
  set('engName',running?'Engine Running':'Engine Stopped');
  set('engSub',running?'Scan #'+(s.scanCount||0)+' · '+openCnt+' open · '+fm(avail)+' cash':'Tap Start to begin');
  ...
  var ageStr=brainAge!==null?' · '+brainAge+'m ago':'';
  var sig0=s.signals&&s.signals[0];
  if(sig0)set('brainText',sig0.reasoning||'Analyzing...');
  else if(running)set('brainText','Scanner active'+ageStr+' — awaiting next analysis...');
  else set('brainText','Start the bot to begin signal generation');
  set('brainBadge','Brain #'+(s.brainCount||0)+ageStr);

  // journey
  if(s.phaseLabel){
    var dm=+(s.doublingsMade||0),dn=+(s.doublingsNeeded||0);
    var pct=+(s.doublingProgress||0),nxt=+(s.nextTarget||0);
    set('homePhase',s.phaseLabel);set('jPhase',s.phaseLabel);
    set('jDesc',s.phaseDesc||'');
    set('jNext','$'+nxt.toLocaleString('en-US',{maximumFractionDigits:0}));
    var jf=$('jFill');if(jf)jf.style.width=Math.min(100,pct)+'%';
    set('jPct',pct.toFixed(1)+'%');
    set('jLeft',dn+' doubling'+(dn!==1?'s':'')+' to $500K');
    set('jDone',dm+'x');
    if(s.cfg){set('jKelly',Math.round((s.cfg.kellyFrac||.55)*100)+'%');set('jEdge',Math.round((s.cfg.minEdge||.04)*100)+'%');}
  }

  // settings
  set('cfgReal',fm(real));set('cfgAvail',fm(avail));set('cfgPeak',fm(peak));
  var pnlSet=$('cfgPnl');if(pnlSet){pnlSet.textContent=sgn(total);pnlSet.className='s-val '+(total>=0?'sv-green':'sv-red');}
  set('cfgWR',wr!==null?wr.toFixed(1)+'%':'--');
  set('cfgWL',w+'W / '+l+'L');
  var ct2=$('cfgToday');if(ct2){ct2.textContent=sgn(today);ct2.className='s-val '+(today>=0?'sv-green':'sv-red');}
  set('cfgScans',(s.scanCount||0).toLocaleString());
  set('cfgBrain',(s.brainCount||0).toLocaleString());
  set('cfgSpend','$'+parseFloat(s.estimatedApiSpend||0).toFixed(2));
  set('cfgMode',dryRun?'Paper':'LIVE');
  set('cfgPhase',(s.phaseLabel||'--'));
  set('cfgMax',mp);
  set('cfgOpen',openCnt+' of '+mp);
  set('cfgBrainInt',s.cfg?((s.cfg.brainInterval||120000)/60000).toFixed(1)+'min':'2.0min');

  // chart push
  if(bal>0)pushChart(bal);

  // lists
  renderPos();renderSigs();

  // build tx list for activity
  allTx=(s.trades||[]).map(function(t){
    return{type:t.status==='open'?'open':(t.won?'win':'loss'),ticker:t.ticker||'--',
      pnl:+(t.pnl||0),cost:+(t.cost||0),ts:+(t.resolvedAt||t.openedAt||Date.now()),
      side:t.side||'YES',contracts:+(t.contracts||1),won:!!t.won,status:t.status||''};
  });

  // logs
  if(s.logs&&s.logs.length){
    var lw=$('logWrap');
    if(lw)lw.innerHTML=s.logs.slice(0,20).map(function(l){
      var lv=(l.level||'').toLowerCase();
      var cls=lv==='error'||lv==='err'?' class="log-err"':lv==='warn'?' class="log-warn"':'';
      return '<div'+cls+'><span class="log-ts">'+(l.ts||'')+'</span>'+(l.msg||'')+'</div>';
    }).join('');
  }
}
...
function loadState(){
  ...
  fetch(url,{cache:'no-store'})
    .then(function(r){
      ...
      return r.text(); // get raw text first — parse manually to catch bad JSON
    })
    .then(function(txt){
      var s;
      try{s=JSON.parse(txt);}
      catch(je){
        console.error('JSON parse fail, len='+txt.length+' preview='+txt.slice(0,120));
        banner('JSON error — check logs','red');
        return;
      }
      retries=0;connected=true;
      banner(null);
      applyState(s);
    })
    .catch(function(e){
      ...
      console.error('loadState #'+retries+': '+msg);
      if(!connected){
        banner('⏳ Connecting... ('+retries+') '+msg,'yellow');
      } else {
        banner('⚠️ Reconnecting... '+msg,'red');
      }
      setTimeout(loadState,Math.min(3000*retries,15000));
      return;
    });
}
...
// ── Actions ────────────────────────────────────────────────────────────────
function toggleBot(){
  fetch('/api/toggle',{method:'POST'}).then(function(){setTimeout(loadState,800);}).catch(console.error);
}
function doSync(){
  var b=document.querySelector('.abtn-sync');if(b){b.style.opacity='.4';b.disabled=true;}
  fetch('/api/sync',{method:'POST'})
    .then(function(r){return r.text();})
    .then(function(txt){
      if(b){b.style.opacity='';b.disabled=false;}
      var s;try{s=JSON.parse(txt);}catch(e){banner('Sync JSON error','red');return;}
      applyState(s);banner('✓ Synced','green');setTimeout(function(){banner(null);},2000);
    })
    .catch(function(e){
      if(b){b.style.opacity='';b.disabled=false;}
      banner('Sync failed: '+e.message,'red');
    });
}

// ── Init ───────────────────────────────────────────────────────────────────
for(var _i=16;_i>=0;_i--)chartData.push({t:Date.now()-_i*300000,v:12});
drawChart();
banner('⏳ Connecting...','yellow');
loadState();

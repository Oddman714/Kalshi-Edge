...
function loadState() {
  ...
  S.availableCash = 0;
  S.restingOrders = [];
  // Keep stored balance until Kalshi sync completes — shows last known value in dashboard
  // syncBalance() will overwrite with real value within seconds of boot
  // S.balance and S.realBalance are left as-is from state.json
...
.chip-flat{background:var(--s3);color:var(--text);opacity:0.7}
...
<div id="page-home" class="page active">
  ...
  <div class="hero">
    ...
    <div class="hero-bal" id="heroBal" style="font-size:32px;color:var(--muted)">Syncing...</div>
    <div class="hero-chip chip-flat" id="heroChip">
      <span id="heroArr">↑</span>&nbsp;<span id="heroChange">loading...</span>
    </div>
    <div class="hero-row">
      <div class="hero-cell"><div class="hc-lbl">Real</div><div class="hc-val" id="heroReal" style="color:var(--muted)">...</div></div>
      <div class="hero-cell"><div class="hc-lbl">Peak</div><div class="hc-val" id="heroPeak" style="color:var(--muted)">...</div></div>
      <div class="hero-cell"><div class="hc-lbl">P&L</div><div class="hc-val" id="heroPnl" style="color:var(--muted)">...</div></div>
    </div>
  </div>
...
function drawChart(){
  var svg=$('mchart'); if(!svg||chartData.length<2){if(svg)svg.innerHTML='';return;}
...
function renderPositions(positions){
  var wrap=$('posList'),sub=$('tradeSub'); if(!wrap)return;
...
function renderSignals(signals){
  var wrap=$('sigList'); if(!wrap)return;
...
function filt(type,el){filterTx(type,el);}
...
function renderMarkets(){
  var wrap=$('mktList');if(!wrap)return;
...
function applyState(s){
  if(!s)return;
  try{ _applyStateInner(s); }
  catch(err){
    console.error('applyState CRASH:', err.message, err.stack);
    var b=$('banner');
    if(b){
      b.textContent='applyState error: '+err.message;
      b.style.cssText='display:block;position:fixed;top:0;left:0;right:0;background:#ff3d5a;color:#fff;padding:12px;font-size:11px;font-weight:600;z-index:9999;word-break:break-all';
    }
    // Still try to show balance even if full render fails
    var hb=$('heroBal');
    if(hb && s.realBalance) hb.textContent='$'+parseFloat(s.realBalance).toFixed(2);
    else if(hb && s.balance) hb.textContent='$'+parseFloat(s.balance).toFixed(2);
  }
}
function _applyStateInner(s){
  if(!s)return;
  ...
  /* hero balance */
  var hb=$('heroBal');if(hb){hb.style.color='';}
  setText('heroBal',bal>0?fm(bal):(running?'Syncing...':'$0.00'));
  setText('heroReal',real>0?fm(real):(running?'...':'$0.00'));
  setText('heroPeak',peak>0?fm(peak):'$0.00');
  var pnlEl=$('heroPnl');if(pnlEl){pnlEl.textContent=fs(total);pnlEl.style.color=total>=0?'var(--green)':'var(--red)';}
  setText('heroArr',today>=0?'+':'-');
  setText('heroChange',fm(today)+' today');
  ...
  }else{if(wrEl){wrEl.textContent='--';wrEl.style.color='var(--muted)';}if(wrBar)wrBar.style.background='var(--muted2)';}
  setText('statWRsub',totalTrades+' trades');
  var tdEl=$('statToday');
  ...
  setText('statPosSub','of '+mp+' max');

  /* engine card */
  var edot=$('engDot');if(edot)edot.className='eng-dot'+(running?' on':'');
  setText('engName',running?'Engine Running':'Engine Stopped');
  setText('engSub',running?'Scan #'+(s.scanCount||0)+' · '+openCnt+'/'+mp+' · $'+avail.toFixed(2)+(brainAge!==null?' · brain '+brainAge+'m ago':''):'Tap Start to begin');
  var sb=$('startBtn'),stb=$('stopBtn');
  ...
  setText('brainBadge','Signal #'+(s.brainCount||0));

  /* journey / road to 500K */
  if(s.phaseLabel){
    var dm=s.doublingsMade||0,dn=s.doublingsNeeded||0;
    var pct=parseFloat(s.doublingProgress||0),nxt=parseFloat(s.nextTarget||0);
    setText('homePhase',s.phaseLabel);
    setText('jPhase',s.phaseLabel);
    setText('jDesc',s.phaseDesc||'');
    setText('jNext','$'+nxt.toLocaleString('en-US',{maximumFractionDigits:0}));
  ...
  var spEl=$('cfgPnl');if(spEl){spEl.textContent=fs(total);spEl.className='s-val '+(total>=0?'sv-green':'sv-red');}
  ...
  var cfgTdEl=$('cfgToday');if(cfgTdEl){cfgTdEl.textContent=fs(today);cfgTdEl.className='s-val '+(today>=0?'sv-green':'sv-red');}
  setText('cfgScans',(s.scanCount||0).toLocaleString());
  setText('cfgBrain',(s.brainCount||0).toLocaleString());
  var apiSpend=parseFloat(s.estimatedApiSpend||0);
  setText('cfgSpend','$'+apiSpend.toFixed(2));
  setText('cfgMode',dryRun?'Paper':'LIVE');
  setText('cfgPhase',s.phaseLabel||'--');
  setText('cfgMax',mp);
  setText('cfgOpen',openCnt+' / '+mp);
  ...
  if(lw&&s.logs&&s.logs.length){
    lw.innerHTML=s.logs.slice(0,20).map(function(l){
...
function loadState(){
  ...
  fetch(url,{cache:'no-store'})
    ...
    .then(function(text){
      ...
      var cb=$('banner');
      if(cb)cb.style.cssText='display:none';
      // Direct balance injection BEFORE applyState — guarantees hero shows data
      var _bal=(s.realBalance||s.balance||0);
      var _hb=document.getElementById('heroBal');
      if(_hb&&_bal>0){_hb.textContent='$'+_bal.toFixed(2);_hb.style.color='';}
      var _hc=document.getElementById('heroChange');
      if(_hc){_hc.textContent=(s.todayPnl>=0?'+':'')+Math.abs(s.todayPnl||0).toFixed(2)+' today';}
      var _hr=document.getElementById('heroReal');
      if(_hr&&s.realBalance>0){_hr.textContent='$'+s.realBalance.toFixed(2);}
      applyState(s);
    })
    .catch(function(e){
      _retries++;
      ...
      var b=$('banner');
      if(b){
        b.textContent=(!_loaded?'Connecting ('+_retries+'): ':'Reconnecting: ')+e.message;
        b.style.cssText='display:block;position:fixed;top:0;left:0;right:0;'+
          'background:'+((!_loaded)?'rgba(245,166,35,0.95)':'#ff3d5a')+';'+
          'color:'+((!_loaded)?'#000':'#fff')+';padding:12px 16px;text-align:center;'+
          'font-weight:600;font-size:12px;z-index:9999';
      }
      setTimeout(loadState,Math.min(3000*_retries,15000));
    });
}

function doSync(){forceSync();}
function forceSync(){
  var btn=document.querySelector('.abtn-sync');if(btn){btn.style.opacity='0.5';btn.disabled=true;}

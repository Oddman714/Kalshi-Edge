...
// ─── GROWTH PHASE ENGINE ───────────────────────────────────────────────────
// Research insight (Bürgi et al. 2026): High-price contracts (55¢+) win more
// than their price implies — small positive expected return. LOW-price contracts
// (<40¢) display favorite-longshot bias — lose more than priced. Bot targets 50-85¢.
...
const PHASES = [
  { name: 'SEED',     min: 0,      max: 50,     kelly: 0.50, minEdge: 0.06, minProb: 0.58, maxPos: 5,  label: '[SEED]',     desc: 'Aggressive compounding — target high-confidence fast markets' },
  { name: 'SPROUT',   min: 50,     max: 200,    kelly: 0.45, minEdge: 0.06, minProb: 0.59, maxPos: 6,  label: '[SPROUT]',   desc: 'Building — stay aggressive, favor 55-80¢ YES contracts' },
  { name: 'GROWTH',   min: 200,    max: 1000,   kelly: 0.40, minEdge: 0.07, minProb: 0.60, maxPos: 7,  label: '[GROWTH]',   desc: 'Scaling — diversify, edge bar rising, fee awareness critical' },
  { name: 'MOMENTUM', min: 1000,   max: 5000,   kelly: 0.35, minEdge: 0.07, minProb: 0.61, maxPos: 8,  label: '[MOMENTUM]', desc: 'Momentum — protect gains, press only clear edges' },
  { name: 'SCALE',    min: 5000,   max: 25000,  kelly: 0.30, minEdge: 0.08, minProb: 0.62, maxPos: 9,  label: '[SCALE]',    desc: 'Scaling — larger positions, only cleanest signals' },
  { name: 'HARVEST',  min: 25000,  max: 100000, kelly: 0.25, minEdge: 0.08, minProb: 0.63, maxPos: 10, label: '[HARVEST]',  desc: 'Harvest — preservation + steady compounding' },
  { name: 'ENDGAME',  min: 100000, max: 500000, kelly: 0.20, minEdge: 0.09, minProb: 0.64, maxPos: 12, label: '[ENDGAME]',  desc: 'Endgame — controlled march to $500K' },
];
...
async function getTopMarkets() {
  try {
    ...
    // Bürgi et al. 2026: High-price contracts (50-85¢) have best expected return.
    // Low-price (<40¢) shows favorite-longshot bias — avoid as primary targets.
    ...
    const markets = allMarkets.filter(m => {
      ...
      // SWEET SPOT: 18¢-88¢ — avoid extreme longshots (fee kills edge)
    ...
    log(`After filter: ${markets.length} tradeable (18¢-88¢, vol≥25, spread≤28¢)`);
    ...
    const scored = markets.map(m => {
      ...
      // Research-backed: high-price contracts outperform — favor 55-80¢
    ...
    if (S.topMarkets[0]) {
      log(`Top: ${S.topMarkets[0].ticker} YES=${(S.topMarkets[0].yesAsk*100).toFixed(1)}¢ vol=${S.topMarkets[0].volume} score=${S.topMarkets[0].score.toFixed(3)}`);
    }
    return S.topMarkets;
  } catch(e) {
  ...
  try {
    ...
    if (S.signals.length > 0) {
      const sigText = S.signals.map(s =>
        `[CHART] ${s.ticker} ${s.side} | ${(s.confidence*100).toFixed(0)}% confidence | edge ${(s.edge*100).toFixed(1)}% | $${s.kellySize?.toFixed(2)} | ${s.reasoning}`
      ).join('\n');
...
async function runPositionMonitor() {
  ...
  try {
    ...
    if (parsed.exitNow && parsed.exitNow.length > 0) {
      ...
      log(`[SIGNAL] Monitor: all ${S.openPositions.length} positions holding — ${parsed.summary || ''}`);
    }
  } catch(e) {
...
async function executeTrade(sig) {
  ...
  if (CFG.dryRun) {
    ...
    log(`[NOTE] PAPER TRADE: ${sig.ticker} ${sig.side} x${contracts} @ ${price}¢ = $${position.cost.toFixed(2)}`);
    ...
    try {
      ...
      S.openPositions.push(position);
      log(`[OK] LIVE ORDER: ${sig.ticker} ${sig.side} x${contracts} @ ${price}¢`);
...
async function scan() {
  ...
  try {
    ...
    if (effectiveBal <= FLOOR && !CFG.dryRun) {
      if (S.isRunning) {
        log('[HALT] FLOOR HIT: balance $' + S.balance.toFixed(2) + ' ≤ $' + FLOOR + ' — halting live trading', 'ERROR');
        tg(`[HALT] <b>Floor Hit — Trading Halted</b>
...
function sendHeartbeat() {
  ...
  tg(`[CHART] <b>Heartbeat — ${phase.label}</b>
...
[SPEND] API: $${apiSpend.toFixed(2)} total (~$${dailyBurn.toFixed(2)}/day) | ROI vs spend: ${roiVsSpend}%`);
}
...
async function startBot() {
  ...
  _botStarted = true;
  S.isRunning = true;
  log('[GO] KalshiBot v10 started — running pre-flight sync...');
  ...
  tg(`[GO] <b>KalshiBot v10 Online</b>
  ...
  try{
    ...
    es.addEventListener('notify',function(e){
      try{
        ...
        pushFeed(m,type);
        if(/[WIN]|[LOSS]|[OK]|[ERR]|[TARGET]|[GO]|[BRAIN]|WIN|LOSS|Order|Brain/i.test(m))toast(m.slice(0,80),type);
      }catch(ex){}
    });
    es.onopen=function(){
...
/* ── ICO ── */
function ico(t){
  if(/BTC|KXBTCD/i.test(t))return{e:'₿',c:'ico-btc'};
  if(/ETH|KXETH/i.test(t))return{e:'Ξ',c:'ico-eth'};
  if(/NBA|KXNBA/i.test(t))return{e:'B',c:'ico-nba'};
  if(/CPI|KXCPI|FED|FEDRATE|INFL/i.test(t))return{e:'[CHART]',c:'ico-cpi'};
  if(/NFL|KXNFL/i.test(t))return{e:'FB',c:'ico-nba'};
  if(/MLB|KXMLB/i.test(t))return{e:'BB',c:'ico-nba'};
  if(/NHL|KXNHL/i.test(t))return{e:'HC',c:'ico-nba'};
  if(/SOL|KXSOL/i.test(t))return{e:'S',c:'ico-eth'};
  return{e:'~',c:''};
}
...
function renderPos(positions){
  ...
  w.innerHTML=positions.map(function(p){
    ...
    return '<div class="pos-card"><div class="pos-inner"><div class="pos-ico '+ic.c+'">'+ic.e+'</div>'
      ...
      +'<div class="pos-r"><div class="pos-price">'+ep+'¢</div><div class="pos-cost">'+fm(cost)+'</div>'
...
function renderMkts(){
  ...
  w.innerHTML=state.topMarkets.slice(0,12).map(function(m){
    ...
    return '<div class="mkt-card"><div class="mkt-top"><div class="mkt-ttl">'+(m.title||m.ticker)+'</div>'
      ...
      +'<div class="mkt-row"><div class="mkt-yes">'+yp+'¢</div>'
      +'<div class="mkt-nr"><div class="mkt-no">'+np+'¢</div><div class="mkt-vol">vol '+(m.volume||0).toLocaleString()+'</div></div></div>'

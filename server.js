...
async function executeTrade(sig) {
  ...
  // Slot cap logic:
  // - 4th slot always available for sub-1.5h trades (hourly/15min) — fast recycling
  // - 4th slot available for any trade when balance > $20 (safe floor buffer)
  // - At very low balance (<$20), 3rd slot is safe max for daily trades
  const isVeryFast = (sig.hoursToClose || 999) < 1.5;
  const currentBal = S.realBalance || S.balance;
  const balanceSafe = currentBal >= 20; // enough above $5 floor for 4 simultaneous bets
  const effectiveMax = (isVeryFast || balanceSafe) ? maxPos : Math.max(3, maxPos - 1);
  if (totalPosCount >= effectiveMax) {
    log(`Skip ${sig.ticker}: at position cap (${totalPosCount}/${effectiveMax}) [bal=$${currentBal.toFixed(2)}, fast=${isVeryFast}]`);
    return;
  }
...
4. Restart — position caps now hard-enforced`);
    ...
    const _balSafe = _currentBal >= 20;
    // 4th slot open when balance is safe or during active hourly window
    const _etMinNow = new Date().getUTCMinutes();
    const _inHourlyWindow = (_etMinNow >= 10 && _etMinNow <= 52); // between 10-52 min of hour
    const _effectiveMaxSlots = (_balSafe || _inHourlyWindow) ? maxPos : Math.max(3, maxPos - 1);

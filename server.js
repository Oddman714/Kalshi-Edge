...
async function getTopMarkets() {
  try {
    // Kalshi's generic /markets endpoint returns MVE composites first.
    // Instead: fetch specific series tickers that are real individual markets.
    const seriesTickers = [
      'KXNBAGAME', 'KXNFLGAME', 'KXMLBGAME', 'KXNHLGAME',   // Sports
      'INXD', 'INXW', 'NASDAQ100D', 'NASDAQ100W',             // Finance
      'KXBTCD', 'KXETHUSD',                                   // Crypto
      'HIGHNY', 'HIGHMIA', 'HIGHCHI', 'HIGHLA',              // Weather
      'KXPREZ', 'KXSENATE', 'KXHOUSE',                       // Politics
      'FED', 'FEDRATE',                                       // Macro
    ];

    // Fetch events for each series in parallel (events contain real markets)
    const eventFetches = await Promise.allSettled(
      seriesTickers.map(s => kalshi('GET', '/events', null, { series_ticker: s, status: 'open', limit: '20' }))
    );

    // Also fetch the generic endpoint as fallback
    const genericFetch = await kalshi('GET', '/markets', null, { status: 'open', limit: '200' }).catch(() => ({ markets: [] }));

    let allMarkets = [...(genericFetch.markets || [])];

    for (const result of eventFetches) {
      if (result.status === 'fulfilled') {
        const events = result.value.events || [];
        for (const ev of events) {
          if (ev.markets) allMarkets.push(...ev.markets);
          // Some events nest markets differently
          if (ev.market) allMarkets.push(ev.market);
        }
      }
    }
    ...
    log(`Raw fetch: ${allMarkets.length} total, ${allMarkets.filter(m=>m.mve_collection_ticker).length} MVE composites`);

    const markets = allMarkets.filter(m => {
      // Drop composites
      if (m.mve_collection_ticker) return false;
      // Must have a price in either format
    ...
    log(`After filter: ${markets.length} real tradeable markets`);
    ...
    log(`Scanner: top market = ${S.topMarkets[0]?.ticker || 'none'} @ score ${(S.topMarkets[0]?.score || 0).toFixed(3)}`);
    return S.topMarkets;
  } catch(e) {
...
async function runBrain(topMarkets) {
  ...
  const prompt = `You are KalshiBot, an expert prediction market trader. Your ONLY goal: double the portfolio as quickly and safely as possible using Kelly criterion sizing.
...
async function runPlumbingTest() {
  ...
  // 2. Kalshi Markets — use series-based fetch same as scanner
  try {
    // Fetch via events for a known active series
    const testSeries = ['INXD', 'KXBTCD', 'KXNBAGAME', 'FED'];
    const fetches = await Promise.allSettled(
      testSeries.map(s => kalshi('GET', '/events', null, { series_ticker: s, status: 'open', limit: '10' }))
    );
    let testMarkets = [];
    for (const f of fetches) {
      if (f.status === 'fulfilled') {
        for (const ev of (f.value.events || [])) {
          if (ev.markets) testMarkets.push(...ev.markets);
        }
      }
    }
    // Also try generic endpoint
    const gen = await kalshi('GET', '/markets', null, { status: 'open', limit: '100' }).catch(() => ({ markets: [] }));
    testMarkets.push(...(gen.markets || []).filter(m => !m.mve_collection_ticker));

    const seen = new Set();
    testMarkets = testMarkets.filter(m => { if(!m||seen.has(m.ticker)) return false; seen.add(m.ticker); return true; });
    ...
    if (tradeable[0]) {
      log(`DEBUG tradeable[0]: ticker=${tradeable[0].ticker} yes_ask=${tradeable[0].yes_ask} yes_ask_dollars=${tradeable[0].yes_ask_dollars}`);
    }
    check('Market Access', testMarkets.length > 0, `${testMarkets.length} fetched via series+generic`);
    ...
    check('MVE Filter', false, 'Cannot test — ' + e.message.slice(0, 40));
  }
...
<div class="app">
  ...
  <div id="tab-home">
    <div class="balance-card">
      <div class="bal-label">Sim Balance</div>
...
function render(s) {
  ...
  document.getElementById('positionList').innerHTML = positions.length === 0

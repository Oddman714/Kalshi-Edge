...
const CFG = {
  ...
  version: 'v20.1.0-recovery-autopsy-ios',
};
...
function riskModeSnapshot() {
  ...
  const peak = Math.max(S.peakBalance || 0, S.sessionPeakBalance || 0, bal);
  ...
  const active = (S.recoveryUntil || 0) > now || streak >= CFG.lossRecoveryTrigger || drawdownPct >= 0.12 || (recent.length >= 2 && recent.slice(-2).every(t => t.won === false));
  const emergency = streak >= 3 || drawdownPct >= 0.20 || recentPnl < -bal * 0.14;
  return {
    active,
    emergency,
    label: active ? (emergency ? 'Recovery+' : 'Recovery') : 'Normal',
    sizeMultiplier: active ? (emergency ? Math.min(0.22, CFG.recoverySizeMultiplier) : CFG.recoverySizeMultiplier) : 1,
    minProbBump: active ? (emergency ? CFG.recoveryMinProbBump + 0.04 : CFG.recoveryMinProbBump) : 0,
    minEdgeBump: active ? (emergency ? CFG.recoveryMinEdgeBump + 0.02 : CFG.recoveryMinEdgeBump) : 0,
    pauseBrain: active,
    blockLongshots: active,
    drawdownPct,
    streak,
    recentPnl,
...
<div id="pg-home" class="pg on">
  ...
  <div style="text-align:center;font-size:9px;color:rgba(238,238,245,.16);padding:8px;font-family:monospace">KalshiBot v20.1 recovery autopsy</div>
  <div class="gap"></div>
</div>
    ...
    try {
      ...
      const slim = {
        ...
        openPositions: (S.openPositions||[]).slice(0,20).map(p => ({
          ...
          liquidityContracts: _nn(p.liquidityContracts),

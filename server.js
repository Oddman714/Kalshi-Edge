...
async function getTopMarkets() {
  try {
    ...
    const markets = (r.markets || []).filter(m => {
      ...
      if ((m.volume || 0) < 100) return false;
      // Must have valid yes ask price
      const yesAsk = (m.yes_ask || 0) / 100;
      if (yesAsk <= 0.02 || yesAsk >= 0.98) return false;
      return true;
    });
...
async function executeTrade(sig) {
  ...
  const price = sig.side === 'YES'
    ? Math.round(sig.confidence * 100)
    : Math.round((1 - sig.confidence) * 100);

  const size = kellySize(sig.confidence, price / 100);
  const contracts = Math.max(1, Math.floor(size / (price / 100)));
...
async function runPlumbingTest() {
  ...
  try {
    ...
    const all = r.markets || [];
    const tradeable = all.filter(m => !m.mve_collection_ticker && (m.volume||0) >= 100);

...
#errBanner{display:none;position:fixed;top:0;left:0;right:0;background:#ff3d5a;color:#fff;font-size:12px;font-weight:600;padding:12px 16px;text-align:center;z-index:9999}
...
<body>
<div id="errBanner" style="display:none;position:fixed;top:0;left:0;right:0;background:#ff3d5a;color:#fff;font-size:12px;font-weight:600;padding:12px 16px;text-align:center;z-index:9999;padding-top:max(12px,env(safe-area-inset-top))"></div>
<div id="connBanner" style="display:none;position:fixed;top:0;left:0;right:0;background:rgba(245,166,35,0.95);color:#000;font-size:12px;font-weight:600;padding:12px 16px;text-align:center;z-index:9998;padding-top:max(12px,env(safe-area-inset-top))">⏳ Connecting to KalshiBot server...</div>
<div class="app">
...
<div id="page-home" class="page active">
  ...
  <div class="hero">
    ...
    <div class="hero-amount" id="heroBalance" style="font-size:40px;color:var(--muted)">Loading...</div>
    <div class="hero-change-row">
      <div class="hero-chip chip-flat" id="heroChip">
        <span id="heroArr">↑</span> <span id="heroChangeText">connecting...</span>
      </div>
    </div>
    <div class="hero-row">
      <div class="hero-cell"><div class="hc-label">Real</div><div class="hc-val" id="heroReal" style="color:var(--muted)">...</div></div>
      <div class="hero-cell"><div class="hc-label">Peak</div><div class="hc-val" id="heroPeak" style="color:var(--muted)">...</div></div>
      <div class="hero-cell"><div class="hc-label">All-time P&L</div><div class="hc-val" id="heroTotal" style="color:var(--muted)">...</div></div>
    </div>
  </div>
  ...
  <div style="text-align:center;font-size:9px;color:rgba(238,238,245,0.15);padding:6px;font-family:monospace">v9.5</div>
  <div class="gap"></div>
</div>
...
var _retries=0,_firstLoad=true;
function loadState(){
  // Show connecting banner on first load
  if(_firstLoad){
    var cb=document.getElementById('connBanner');
    if(cb)cb.style.display='block';
  }
  // 8-second timeout on mobile
  ...
  fetch(url,{cache:'no-store',signal:ctrl?ctrl.signal:undefined})
    ...
    .catch(function(e){
      ...
      var msg=e.name==='AbortError'?'Server timeout':'Connection error: '+e.message;
      console.error('loadState #'+_retries+':',msg);
      var eb=document.getElementById('errBanner');
      var cb=document.getElementById('connBanner');
      if(_firstLoad){
        // Still on first load — show connecting banner with retry count
        if(cb)cb.textContent='⏳ Connecting... (attempt '+_retries+') '+msg;
        if(cb)cb.style.display='block';
      } else {
        // We had data before — show red error
        if(eb){eb.textContent='⚠️ Lost connection — retrying ('+_retries+')';eb.style.display='block';}
        if(cb)cb.style.display='none';
      }
...
function applyState(s){
  try{
    // Immediately hide both banners — we have data
    var eb=document.getElementById('errBanner');
    var cb=document.getElementById('connBanner');
    if(eb)eb.style.display='none';
    if(cb)cb.style.display='none';
    ...
    // Reset loading-state styles, show real values
    var hb=document.getElementById('heroBalance');
    if(hb){hb.style.fontSize='';hb.style.color='';}
    var balDisplay=bal>0?fm(bal):(s.isRunning?'Syncing...':'$0.00');
    var realDisplay=real>0?fm(real):(s.isRunning?'...':'$0.00');
    var peakDisplay=peak>0?fm(peak):'$0.00';
    setText('heroBalance',balDisplay);
    setText('heroReal',realDisplay);
    setText('heroPeak',peakDisplay);
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/api/state') {
    try {
      ...
      // Safe string: strip any chars that can break JSON.stringify in transit
      const ss = v => typeof v === 'string' ? v.replace(/[\u0000-\u001F\u007F]/g, ' ').slice(0, 200) : (v || '');

      // Lean position objects only — no raw Kalshi API blobs
      const safePositions = (S.openPositions || []).slice(0, 10).map(p => ({
        ticker: ss(p.ticker), side: ss(p.side),
        ...
        reasoning: ss(p.reasoning),
        fromKalshi: !!p.fromKalshi,
        status: ss(p.status) || 'open',
      }));

      // Lean signal objects
      const safeSignals = (S.signals || []).slice(0, 5).map(s => ({
        ticker: ss(s.ticker), side: ss(s.side),
        confidence: Number(s.confidence) || 0,
        edge: Number(s.edge) || 0,
        marketPrice: Number(s.marketPrice) || 0,
        kellySize: Number(s.kellySize) || 0,
        reasoning: ss(s.reasoning),
        hoursToClose: Number(s.hoursToClose) || 0,
      }));

      // Lean trade objects — only what the activity feed needs
      const safeTrades = (S.trades || []).slice(0, 20).map(t => ({
        ticker: ss(t.ticker), side: ss(t.side),
        contracts: Number(t.contracts) || 0,
        cost: Number(t.cost) || 0,
        pnl: Number(t.pnl) || 0,
        won: !!t.won, status: ss(t.status),
        openedAt: Number(t.openedAt) || 0,
        resolvedAt: Number(t.resolvedAt) || 0,
      }));

      // Lean log objects — last 20 only
      const safeLogs = (S.logs || []).slice(0, 20).map(l => ({
        ts: ss(l.ts), level: ss(l.level), msg: ss(l.msg),
      }));

      // Lean market objects
      const safeMarkets = (S.topMarkets || []).slice(0, 12).map(m => ({
        ticker: ss(m.ticker), title: ss(m.title),
        yesAsk: Number(m.yesAsk) || 0, yesBid: Number(m.yesBid) || 0,
        volume: Number(m.volume) || 0, score: Number(m.score) || 0,
        hoursLeft: Number(m.hoursLeft) || 999,
      ...
      const slim = {
        ...
        realBalance: Number(S.realBalance) || 0,
        peakBalance: Number(S.peakBalance) || 0,
        availableCash: Number(S.availableCash) || 0,
        todayPnl: Number(S.todayPnl) || 0,
        totalPnl: Number(S.totalPnl) || 0,
        wins: Number(S.wins) || 0,
        losses: Number(S.losses) || 0,
        scanCount: Number(S.scanCount) || 0,
        brainCount: Number(S.brainCount) || 0,
        startedAt: Number(S.startedAt) || 0,
        lastBrainAt: Number(S.lastBrainAt) || 0,
        lastErr: ss(S.lastErr),
        estimatedApiSpend: parseFloat((S.estimatedApiSpend || 0).toFixed(2)),
        openPositions: safePositions,
        signals: safeSignals,
        trades: safeTrades,
        logs: safeLogs,
        topMarkets: safeMarkets,
        dryRun: !!CFG.dryRun,
        maxPos: dynamicMaxPos(),
        winRate,
        drawdown,
        openCount: safePositions.length,
        phase: phase.name,
        phaseLabel: phase.label,
        phaseDesc: ss(phase.desc),
      ...
      log('api/state error: ' + stateErr.message, 'ERROR');
      // Return minimal valid JSON so dashboard at least shows running state
      const fallback = JSON.stringify({
        isRunning: !!S.isRunning, balance: Number(S.balance)||0,
        realBalance: Number(S.realBalance)||0, error: String(stateErr.message).slice(0,100),
        openPositions: [], signals: [], trades: [], logs: [], topMarkets: [],
        wins: S.wins||0, losses: S.losses||0, scanCount: S.scanCount||0,
      });
      res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store, max-age=0' });

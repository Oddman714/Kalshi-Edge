...
// Fetches real fills + settlements from Kalshi and merges into S.trades
// so History tab shows everything, not just bot-placed trades
async function backfillKalshiHistory() {
  try {
    // Fetch fills (executed orders)
    const fillsRes = await kalshi('GET', '/portfolio/fills', null, { limit: '100' });
    const fills = fillsRes.fills || [];

    // Fetch settlements (resolved positions)
    ...
    if (fills.length === 0 && settlements.length === 0) {
      log('Kalshi history: no fills or settlements found');
      return;
    }

    // Build a map of ticker -> settlement for revenue lookup
    const settleMap = {};
    for (const s of settlements) {
      const ticker = s.ticker || s.market_ticker;
      if (ticker) settleMap[ticker] = s;
    }

    // Convert fills to internal trade format
    const existingTickers = new Set(S.trades.map(t => t.ticker + '_' + t.openedAt));
    let added = 0;
    ...
    for (const fill of fills) {
      ...
      const side = fill.side === 'yes' ? 'YES' : 'NO';
      const contracts = Math.abs(parseFloat(fill.count_fp || fill.count || 1));
      const price = parsePrice(fill.yes_price_dollars || fill.no_price_dollars || fill.price_dollars || '0.5');
      const cost = contracts * price;
      const filledAt = fill.created_time ? new Date(fill.created_time).getTime() : Date.now();

      // Check if we already have this trade
      const key = ticker + '_' + filledAt;
      if (existingTickers.has(key)) continue;

      // Check if there's a settlement for this ticker
      const settled = settleMap[ticker];
      let won = null;
      let pnl = null;
      let revenue = 0;

      if (settled) {
        revenue = parsePrice(settled.revenue_dollars || settled.revenue || '0');
        pnl = revenue - cost;
        won = pnl > 0;
      }
      ...
      const trade = {
        ...
        side,
        contracts,
        entryPrice: Math.round(price * 100),
        cost,
        openedAt: filledAt,
        resolvedAt: settled ? Date.now() : null,
        status: settled ? 'resolved' : 'open',
        won,
        pnl,
        revenue,
        reasoning: 'Imported from Kalshi history',
        fromKalshi: true,
      };

      S.trades.push(trade);
      existingTickers.add(key);
      added++;

      // Update win/loss counters if resolved
      if (settled && won !== null) {
    ...
    if (added > 0) {
      // Sort trades by openedAt descending
      S.trades.sort((a, b) => (b.openedAt || 0) - (a.openedAt || 0));
      // Rebuild W/L/PnL from full trade list to avoid double-counting
      S.wins = S.trades.filter(t => t.won === true).length;
      S.losses = S.trades.filter(t => t.won === false).length;
      S.totalPnl = S.trades.reduce((sum, t) => sum + (t.pnl || 0), 0);
      saveState();
      log(`Kalshi history backfill: imported ${added} trades (${fills.length} fills, ${settlements.length} settlements)`);
    } else {
      log(`Kalshi history: ${fills.length} fills checked, all already tracked`);
    }
  } catch(e) {

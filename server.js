...
:root{
  ...
  --text:#eeeef5;--muted:rgba(238,238,245,0.45);--muted2:rgba(238,238,245,0.22);--muted3:rgba(238,238,245,0.12);
  ...
  --purple:#9b7ef8;--purple-d:rgba(155,126,248,0.12);
...
/* NAV */
.nav{position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:430px;background:rgba(6,6,10,0.92);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);border-top:1px solid var(--border);display:flex;padding-bottom:var(--safe-bot);z-index:200}
...
.nav-pip{position:absolute;top:0;left:50%;transform:translateX(-50%);width:20px;height:2px;background:var(--green);border-radius:0 0 2px 2px;opacity:0;transition:opacity 0.15s}
...
.home-brand{font-size:14px;font-weight:600;color:var(--muted);letter-spacing:-0.2px}
.mode-pill{display:flex;align-items:center;gap:5px;padding:5px 11px;border-radius:20px;font-size:11px;font-weight:700;letter-spacing:0.04em}
...
.pill-dot-live{background:var(--green);animation:glow 2s infinite}
@keyframes glow{0%,100%{box-shadow:0 0 0 0 rgba(5,212,124,0.5)}50%{box-shadow:0 0 0 5px rgba(5,212,124,0)}}
...
.hero-amount{font-size:60px;font-weight:700;letter-spacing:-3.5px;line-height:1;color:var(--text)}
...
.hc-val{font-size:15px;font-weight:700;letter-spacing:-0.3px}

/* CHART */
.chart-wrap{padding:14px 0 0}
.tf-row{display:flex;gap:2px;padding:0 20px;margin-bottom:8px}
.tf-btn{flex:1;padding:7px;text-align:center;font-size:12px;font-weight:600;color:var(--muted);background:none;border:none;border-radius:8px;cursor:pointer;transition:all 0.15s;font-family:var(--font)}
.tf-btn.active{background:var(--s3);color:var(--text)}
.chart-svg-wrap{width:100%;height:164px;overflow:hidden}
svg.main-chart{width:100%;height:164px;display:block}
...
.stat-top-bar{position:absolute;top:0;left:0;right:0;height:2px;border-radius:2px 2px 0 0}
...
.abtn{flex:1;padding:16px;border-radius:14px;border:none;font-family:var(--font);font-size:15px;font-weight:600;cursor:pointer;transition:all 0.14s;display:flex;align-items:center;justify-content:center;gap:8px;letter-spacing:-0.2px}
...
.engine-dot.on{background:var(--green);box-shadow:0 0 0 0 var(--green-g);animation:pulse 2s infinite}
...
.journey-phase-desc{font-size:12px;color:var(--muted);margin-top:3px;line-height:1.35;max-width:200px}
...
.j-track{height:8px;background:var(--s3);border-radius:4px;overflow:hidden;margin-bottom:8px;position:relative}
.j-fill{height:100%;background:linear-gradient(90deg,var(--green),#00ff88);border-radius:4px;transition:width 0.9s cubic-bezier(.4,0,.2,1);position:relative}
.j-fill::after{content:'';position:absolute;right:0;top:0;bottom:0;width:16px;background:rgba(255,255,255,0.35);border-radius:4px;filter:blur(4px)}
...
.j-500k{font-size:11px;font-weight:700;color:var(--muted2);text-align:right}
/* milestone pips */
.j-pips{display:flex;gap:0;overflow-x:auto;scrollbar-width:none;margin-top:2px;padding-bottom:2px}
...
.pip-node{width:10px;height:10px;border-radius:50%;background:var(--s3);border:2px solid var(--border);transition:all 0.4s;flex-shrink:0}
.pip-node.done{background:var(--green);border-color:var(--green)}
.pip-node.now{background:transparent;border-color:var(--gold);box-shadow:0 0 0 3px rgba(245,166,35,0.2);animation:nowpip 1.8s infinite}
@keyframes nowpip{0%,100%{box-shadow:0 0 0 3px rgba(245,166,35,0.15)}50%{box-shadow:0 0 0 6px rgba(245,166,35,0.3)}}
.pip-lbl{font-size:8px;font-weight:600;color:var(--muted2);text-align:center;font-family:var(--mono);line-height:1.2}
...
.j-stat-val{font-size:14px;font-weight:700;letter-spacing:-0.3px}
...
/* POSITION CARDS — Coinbase style */
.pos-card{background:var(--s1);border-radius:16px;margin:0 20px 8px;padding:0;border:1px solid var(--border);overflow:hidden}
.pos-card-inner{display:flex;align-items:center;gap:12px;padding:14px 14px 10px}
.pos-icon{width:44px;height:44px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;border:1px solid var(--border2)}
...
.pos-icon.nba{background:rgba(29,66,138,0.2);border-color:rgba(29,66,138,0.3)}
.pos-icon.nfl{background:rgba(1,81,36,0.2);border-color:rgba(1,81,36,0.3)}
.pos-icon.cpi{background:rgba(77,138,240,0.12);border-color:rgba(77,138,240,0.2)}
.pos-icon.fed{background:rgba(155,126,248,0.12);border-color:rgba(155,126,248,0.2)}
.pos-icon.gen{background:var(--s3);border-color:var(--border)}
.pos-info{flex:1;min-width:0}
.pos-ticker{font-size:13px;font-weight:700;font-family:var(--mono);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--text)}
.pos-tags{display:flex;align-items:center;gap:6px;margin-top:5px;flex-wrap:wrap}
.side-tag{padding:2px 8px;border-radius:5px;font-size:10px;font-weight:700;letter-spacing:0.04em}
.tag-yes{background:var(--green-d);color:var(--green)}
.tag-no{background:var(--red-d);color:var(--red)}
.pos-meta-tag{font-size:10px;color:var(--muted);font-weight:500}
.pos-right{text-align:right;flex-shrink:0;min-width:72px}
...
/* pos footer bar */
.pos-footer{display:flex;align-items:center;justify-content:space-between;padding:8px 14px;border-top:1px solid var(--border2);background:var(--s2)}
.pos-footer-item{font-size:10px;color:var(--muted2);display:flex;flex-direction:column;gap:2px}
.pos-footer-val{font-size:12px;font-weight:600;color:var(--muted)}
...
.f-chip{display:flex;align-items:center;gap:5px;padding:7px 14px;border-radius:20px;background:var(--s2);font-size:13px;font-weight:500;color:var(--muted);white-space:nowrap;cursor:pointer;border:1px solid transparent;font-family:var(--font);transition:all 0.15s}
...
.mkt-exp{font-size:11px;color:var(--muted);flex-shrink:0;text-align:right}
...
.mkt-badge{display:inline-flex;align-items:center;gap:4px;font-size:10px;font-weight:700;padding:2px 8px;border-radius:5px;margin-top:8px}
.mb-high{background:var(--green-d);color:var(--green)}
.mb-med{background:var(--gold-d);color:var(--gold)}
...
.s-name{font-size:15px;font-weight:500;letter-spacing:-0.2px}
...
/* win rate accuracy note */
.wr-note{font-size:10px;color:var(--muted2);font-style:italic;margin-top:4px}

/* dd meter */
.dd-wrap{padding:0 16px 14px}
.dd-head{display:flex;justify-content:space-between;margin-bottom:6px}
.dd-lbl{font-size:10px;font-weight:700;color:var(--muted2);text-transform:uppercase;letter-spacing:0.08em}
.dd-pct{font-size:12px;font-weight:700}
...
<div id="page-home" class="page active">
  ...
  <div class="hero">
    ...
    <div class="hero-row">
      <div class="hero-cell">
        <div class="hc-label">Real</div>
        <div class="hc-val" id="heroReal">$—</div>
      </div>
      <div class="hero-cell">
        <div class="hc-label">Peak</div>
        <div class="hc-val" id="heroPeak">$—</div>
      </div>
      <div class="hero-cell">
        <div class="hc-label">All-time P&amp;L</div>
        <div class="hc-val" id="heroTotal" style="color:var(--green)">$—</div>
      </div>
    </div>
  </div>
  ...
  <div class="chart-wrap">
    ...
    <div class="chart-svg-wrap">
      <svg class="main-chart" id="mainChart" viewBox="0 0 430 164" preserveAspectRatio="none"></svg>
    </div>
  </div>

  <div class="stat-row">
    <div class="stat-card">
      <div class="stat-top-bar" id="wrBar" style="background:var(--green)"></div>
      <div class="sc-label">Win Rate</div>
      <div class="sc-val" id="statWR" style="color:var(--green)">—</div>
      <div class="sc-sub" id="statWRsub">— trades</div>
    </div>
    <div class="stat-card" id="todayCard">
      ...
      <div class="sc-val" id="statToday">$0.00</div>
  ...
  <div class="action-row">
    ...
    <button class="abtn abtn-icon" onclick="loadState()">↻</button>
  </div>

  <div class="card">
    <div class="card-head">
      <div class="card-title">Trading Engine</div>
      <div id="modeBadge" class="card-badge badge-muted">PAPER</div>
    </div>
  ...
  <div class="card">
    ...
    <div class="journey-body">
      <div class="journey-top">
        <div>
          <div class="journey-phase-name" id="jPhaseName">—</div>
          <div class="journey-phase-desc" id="jPhaseDesc">—</div>
        </div>
        <div class="journey-target">
          <div class="journey-target-lbl">Next Target</div>
          <div class="journey-target-val" id="jNextTarget">$—</div>
        </div>
      </div>
      <div class="j-track"><div class="j-fill" id="jFill" style="width:0%"></div></div>
      <div class="j-meta">
        <div class="j-pct" id="jPct">0.0%</div>
        <div class="j-left" id="jLeft">— doublings left</div>
      </div>
      <div class="j-pips" id="jPips"></div>
      <div class="j-stats">
        <div class="j-stat">
          <div class="j-stat-lbl">Doublings</div>
          <div class="j-stat-val" id="jDone" style="color:var(--green)">0×</div>
        </div>
        <div class="j-stat">
          <div class="j-stat-lbl">Kelly Frac</div>
          <div class="j-stat-val" id="jKelly" style="color:var(--gold)">—</div>
        </div>
        <div class="j-stat">
          <div class="j-stat-lbl">Min Edge</div>
          <div class="j-stat-val" id="jEdge" style="color:var(--blue)">—</div>
        </div>
      </div>
    </div>
  </div>
...
<div id="page-trade" class="page">
  <div class="page-hdr">
    ...
    <div class="page-sub" id="tradeSubtitle">0 open</div>
  </div>
...
<div id="page-activity" class="page">
  <div class="page-hdr" style="padding-bottom:4px">
...
<div id="page-predict" class="page">
  <div class="page-hdr">
    ...
    <div class="page-sub">Ranked by edge · Short-duration priority</div>
  </div>
  ...
  <div class="settings-wrap">
    ...
    <div class="s-section">
      ...
      <div class="s-group">
        <div class="j-settings-block">
          <div class="jsb-top">
            <div class="jsb-name" id="cfgPhase">—</div>
            <div class="jsb-pct" id="cfgPct">0.0%</div>
          </div>
          <div class="jsb-desc" id="cfgPhaseDesc">—</div>
          <div class="jsb-track"><div class="jsb-fill" id="cfgFill" style="width:0%"></div></div>
          <div class="jsb-labels">
            <span id="cfgCurBal">$—</span>
            <span id="cfgNextTarget">→ $—</span>
          </div>
        </div>
        <div class="s-row"><div><div class="s-name">Doublings Completed</div></div><div class="s-val sv-green" id="cfgDoublings">0× done</div></div>
        <div class="s-row"><div><div class="s-name">Doublings Left to $500K</div></div><div class="s-val" id="cfgNeeded">— left</div></div>
        <div class="s-row"><div><div class="s-name">Starting Bankroll</div><div class="s-sub">Initial capital when bot launched</div></div><div class="s-val" id="cfgBankroll">$—</div></div>
        <div class="s-row"><div><div class="s-name">Current Phase Kelly</div></div><div class="s-val sv-gold" id="cfgKelly">—</div></div>
      </div>
    </div>
    ...
    <div class="s-section">
      ...
      <div class="s-group">
        <div class="s-row">
          <div>
            <div class="s-name">Win Rate</div>
            <div class="wr-note" id="wrNote">— resolved trades · — open</div>
          </div>
          <div>
            <div class="s-val sv-green" id="cfgWR">—</div>
          </div>
        </div>
        <div class="s-row"><div><div class="s-name">Win / Loss Record</div></div><div class="s-val sv-green" id="cfgWL">—</div></div>
        ...
        <div class="s-row"><div><div class="s-name">Total Scans Run</div></div><div class="s-val" id="cfgScans">—</div></div>
        <div class="s-row"><div><div class="s-name">Brain Signals Fired</div></div><div class="s-val" id="cfgBrainCount">—</div></div>
      </div>
    </div>
    ...
    <div class="s-section">
      ...
      <div class="s-group">
        ...
        <div>
          <div class="s-row" style="border-bottom:none;padding-bottom:6px">
            <div><div class="s-name">Drawdown from Peak</div></div>
            <div class="s-val sv-red" id="cfgDD">0.0%</div>
          </div>
          <div class="dd-wrap">
    ...
    <div class="s-section">
      <div class="s-section-lbl">Bot Configuration</div>
      <div class="s-group">
        <div class="s-row"><div><div class="s-name">Mode</div></div><div class="s-val" id="cfgMode">—</div></div>
        <div class="s-row"><div><div class="s-name">Max Positions</div><div class="s-sub">Scales with phase</div></div><div class="s-val" id="cfgMaxPos">—</div></div>
        <div class="s-row"><div><div class="s-name">Brain Interval</div></div><div class="s-val" id="cfgBrainInt">—</div></div>
        <div class="s-row"><div><div class="s-name">Open Positions</div></div><div class="s-val" id="cfgOpen">—</div></div>
      </div>
    </div>
...
const API='';
const POLL=12000;
let state=null,chartData=[],allTx=[],currentTF='1D',txFilter='all';
...
/* CHART */
function setTF(tf,el){currentTF=tf;document.querySelectorAll('.tf-btn').forEach(b=>b.classList.remove('active'));el.classList.add('active');drawChart();}
function pushChartPoint(v){chartData.push({t:Date.now(),v});if(chartData.length>600)chartData.shift();drawChart();}
function drawChart(){
  const svg=document.getElementById('mainChart');
  if(chartData.length<2){svg.innerHTML='';return;}
  const now=Date.now(),cuts={1H:3600000,1D:86400000,1W:604800000,ALL:Infinity};
  const d=chartData.filter(p=>now-p.t<(cuts[currentTF]||Infinity));
  const data=d.length>1?d:chartData;
  if(data.length<2){svg.innerHTML='';return;}
  const W=430,H=164,pad=10;
  const vals=data.map(p=>p.v),mn=Math.min(...vals),mx=Math.max(...vals),rng=mx-mn||0.01;
  const tx=i=>pad+(i/(data.length-1))*(W-pad*2);
  const ty=v=>H-pad-((v-mn)/rng)*(H-pad*2-16);
  const up=data[data.length-1].v>=data[0].v,col=up?'#05d47c':'#ff3d5a';
  const area='M '+pad+','+(H+4)+' L '+pad+','+ty(data[0].v)+' '+data.map((p,i)=>'L '+tx(i)+','+ty(p.v)).join(' ')+' L '+(W-pad)+','+(H+4)+' Z';
  const line='M '+data.map((p,i)=>tx(i)+','+ty(p.v)).join(' L ');
  const lx=tx(data.length-1),ly=ty(data[data.length-1].v);
  svg.innerHTML='<defs><linearGradient id="g" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="'+col+'" stop-opacity="0.18"/><stop offset="90%" stop-color="'+col+'" stop-opacity="0"/></linearGradient></defs><path d="'+area+'" fill="url(#g)"/><path d="'+line+'" fill="none" stroke="'+col+'" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/><circle cx="'+lx+'" cy="'+ly+'" r="4" fill="'+col+'"/><circle cx="'+lx+'" cy="'+ly+'" r="8" fill="'+col+'" opacity="0.15"/>';
}

/* WL BARS */
function renderWLBars(trades){
  const wrap=document.getElementById('wlBars');
  const now=Date.now(),buckets=Array(24).fill(null).map(()=>({w:0,l:0}));
  (trades||[]).forEach(t=>{const hr=Math.floor((now-(t.resolvedAt||t.openedAt||now))/3600000);if(hr<24){const b=buckets[23-hr];(t.won?b.w++:b.l++);}});
  const mx=Math.max(...buckets.map(b=>b.w+b.l),1);
  wrap.innerHTML=buckets.map(b=>{if(!b.w&&!b.l)return '<div class="wl-bar wl-e" style="height:5px"></div>';const h=Math.max(7,((b.w+b.l)/mx)*50);return '<div class="wl-bar '+(b.w>=b.l?'wl-w':'wl-l')+'" style="height:'+h+'px"></div>';}).join('');
  const wins=(trades||[]).filter(t=>t.won).length,losses=(trades||[]).filter(t=>t.status==='resolved'&&!t.won).length;
  const el=document.getElementById('wlSummary');el.textContent=wins+'W / '+losses+'L';el.style.color=wins>=losses?'var(--green)':'var(--red)';
}

/* market icon */
function mktIcon(ticker){
  if(/BTC|KXBTCD/i.test(ticker))return{emoji:'₿',cls:'btc'};
  if(/ETH|KXETH/i.test(ticker))return{emoji:'Ξ',cls:'eth'};
  if(/NBA|KXNBA/i.test(ticker))return{emoji:'🏀',cls:'nba'};
  if(/NFL|KXNFL/i.test(ticker))return{emoji:'🏈',cls:'nfl'};
  if(/CPI|KXCPI/i.test(ticker))return{emoji:'📊',cls:'cpi'};
  if(/FED|KXFED/i.test(ticker))return{emoji:'🏛',cls:'fed'};
  return{emoji:'📈',cls:'gen'};
}
...
function renderPositions(positions){
  ...
  wrap.innerHTML=positions.map(p=>{
    ...
    const pnl=p.pnl||0;
    const ep=p.entryPrice||50;
    const contracts=p.contracts||1;
    const cost=p.cost||(ep*contracts/100);
    const hoursOpen=p.openedAt?Math.round((Date.now()-p.openedAt)/3600000):0;
    const conf=p.confidence?Math.round(p.confidence*100)+'%':'—';
    const {emoji,cls}=mktIcon(p.ticker||'');
    // determine category label
    const cat=cls==='btc'?'BTC':cls==='eth'?'ETH':cls==='nba'?'NBA':cls==='nfl'?'NFL':cls==='cpi'?'CPI':cls==='fed'?'FED':'MKTX';
    return '<div class="pos-card">'+
      '<div class="pos-card-inner">'+
        '<div class="pos-icon '+cls+'">'+emoji+'</div>'+
        '<div class="pos-info">'+
          '<div class="pos-ticker">'+p.ticker+'</div>'+
          '<div class="pos-tags">'+
            '<span class="side-tag '+(isYes?'tag-yes':'tag-no')+'">'+(isYes?'YES':'NO')+'</span>'+
            '<span class="pos-meta-tag">'+contracts+' contract'+(contracts>1?'s':'')+' · '+cat+'</span>'+
          '</div>'+
        '</div>'+
        '<div class="pos-right">'+
          '<div class="pos-price-big">'+ep+'¢</div>'+
          '<div class="pos-cost-line">Cost $'+cost.toFixed(2)+'</div>'+
          '<div class="pos-pnl-line '+(pnl>0?'pnl-pos':pnl<0?'pnl-neg':'pnl-zero')+'">'+(pnl>=0?'+':'')+'$'+Math.abs(pnl).toFixed(2)+'</div>'+
        '</div>'+
      '</div>'+
      '<div class="pos-footer">'+
        '<div class="pos-footer-item"><span style="font-size:9px;color:var(--muted2);text-transform:uppercase;letter-spacing:0.08em">Confidence</span><span class="pos-footer-val">'+conf+'</span></div>'+
        '<div class="pos-footer-item" style="align-items:center"><span style="font-size:9px;color:var(--muted2);text-transform:uppercase;letter-spacing:0.08em">Open</span><span class="pos-footer-val">'+hoursOpen+'h</span></div>'+
        '<div class="pos-footer-item" style="align-items:flex-end"><span style="font-size:9px;color:var(--muted2);text-transform:uppercase;letter-spacing:0.08em">Value</span><span class="pos-footer-val">$'+(ep*contracts/100).toFixed(2)+'</span></div>'+
      '</div>'+
    '</div>';
  }).join('');
}
...
function renderSignals(signals){
  ...
  wrap.innerHTML=signals.slice(0,5).map(s=>{
    ...
    return '<div class="sig-card '+(isYes?'':'sig-no')+'">'+
      '<div class="sig-top"><div class="sig-ticker">'+s.ticker+'</div><span class="side-tag '+(isYes?'tag-yes':'tag-no')+'">'+s.side+'</span></div>'+
...
function buildTx(trades){
  return (trades||[]).map(t=>({
    type:t.status==='open'?'open':(t.won?'win':'loss'),
    ticker:t.ticker||'—',pnl:t.pnl||0,cost:t.cost||0,
    ts:t.resolvedAt||t.openedAt||Date.now(),
    side:t.side||'YES',contracts:t.contracts||1,won:t.won,status:t.status,revenue:t.revenue||0
  }));
}
function renderTxFeed(txList,filter){
  ...
  else if(txFilter==='open')fl=txList.filter(t=>t.type==='open');
  if(!fl.length){wrap.innerHTML='<div class="empty-state">No transactions yet</div>';return;}
  const groups={};
  fl.forEach(tx=>{const k=new Date(tx.ts).toLocaleString('en-US',{month:'long',year:'numeric'});if(!groups[k])groups[k]=[];groups[k].push(tx);});
  wrap.innerHTML=Object.entries(groups).map(([month,txs])=>
    '<div class="tx-month">'+month+'</div>'+
    txs.map((tx,i)=>{
      const typeLabel=tx.type==='win'?'Win':tx.type==='loss'?'Loss':'Open';
      const iconCls='ti-'+(tx.type==='win'?'win':tx.type==='loss'?'loss':'open');
      const iconChar=tx.type==='win'?'✓':tx.type==='loss'?'✗':mktIcon(tx.ticker).emoji;
      // For wins: show pnl if > 0, else show revenue context
      const pnlDisplay=tx.type==='win'&&tx.pnl>0?'+$'+tx.pnl.toFixed(2):
                       tx.type==='win'&&tx.pnl===0?'+$0.00':
                       tx.type==='loss'?'-$'+Math.abs(tx.pnl||tx.cost||0).toFixed(2):
                       '-$'+Math.abs(tx.cost||0).toFixed(2);
      const pnlCls=tx.type==='win'?'tx-amt-pos':tx.type==='loss'?'tx-amt-neg':'tx-amt-zero';
      return '<div class="tx-row">'+
        '<div class="tx-icon '+iconCls+'">'+iconChar+'</div>'+
        '<div class="tx-info">'+
        ...
        '</div>'+
        '<div class="tx-right">'+
          '<div class="tx-amt '+pnlCls+'">'+pnlDisplay+'</div>'+
          '<div class="tx-time">'+new Date(tx.ts).toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit'})+'</div>'+
        '</div>'+
      '</div>'+
      (i<txs.length-1?'<div class="tx-div"></div>':'');
    }).join('')
  ).join('');
}
function filterTx(type,el){document.querySelectorAll('.f-chip').forEach(c=>c.classList.remove('active'));el.classList.add('active');renderTxFeed(allTx,type);}
...
function renderMarkets(){
  ...
  wrap.innerHTML=state.topMarkets.slice(0,15).map(m=>{
    const yp=Math.round((m.yesAsk||0)*100),np=100-yp,hl=m.hoursLeft||999,soon=hl<24;
    const edge=m.edge||0,badgeCls=edge>0.08?'mb-high':'mb-med';
    return '<div class="mkt-card">'+
      '<div class="mkt-top"><div class="mkt-title">'+(m.title||m.ticker)+'</div><div class="mkt-exp'+(soon?' soon':'')+'">⏱ '+(hl<999?(hl<1?'<1h':hl.toFixed(0)+'h'):'—')+'</div></div>'+
      '<div class="mkt-row"><div class="mkt-yes">'+yp+'¢ YES</div><div class="mkt-no-block"><div class="mkt-no">'+np+'¢ NO</div><div class="mkt-vol">vol '+(m.volume||0).toLocaleString()+'</div></div></div>'+
      '<div class="mkt-track"><div class="mkt-fill" style="width:'+yp+'%"></div></div>'+
      (edge>0?'<span class="mkt-badge '+badgeCls+'">⚡ Edge '+(edge*100).toFixed(0)+'%</span>':'')+
    '</div>';
  }).join('');
}
...
function renderPips(done,needed,bankroll){
  const wrap=document.getElementById('jPips');if(!wrap)return;
  const total=Math.min(done+(needed||16),16);
  let html='';
  for(let i=0;i<total;i++){
    const amt=bankroll*Math.pow(2,i+1);
    const lbl=amt>=1000?'$'+(amt/1000).toFixed(0)+'K':'$'+amt.toFixed(0);
    const isDone=i<done,isCurrent=i===done;
    html+='<div class="j-pip"><div class="pip-node'+(isDone?' done':isCurrent?' now':'')+'"></div><div class="pip-lbl'+(isDone?' done':isCurrent?' now':'')+'">'+lbl+'</div></div>';
  }
  wrap.innerHTML=html;
}
...
async function loadState(){
  try{const r=await fetch(API+'/api/state');state=await r.json();applyState(state);}catch(e){console.error(e);}
}
...
function applyState(s){
  ...
  const w=s.wins||0,l=s.losses||0,totalTrades=w+l;
  const openCnt=(s.openPositions||[]).length;
  const mp=s.maxPos||5;
  // Use server-provided winRate (only from resolved trades)
  const wr=s.winRate!==null&&s.winRate!==undefined?parseFloat(s.winRate):null;
  const bankroll=s.startingBankroll||50;
  const dryRun=s.dryRun!==false;

  /* HERO */
  document.getElementById('heroBalance').textContent=fmtBal(bal);
  document.getElementById('heroReal').textContent=fmtBal(real);
  document.getElementById('heroPeak').textContent=fmtBal(peak);
  ...
  const absT=Math.abs(today).toFixed(2);
  document.getElementById('heroArr').textContent=today>=0?'▲':'▼';
  document.getElementById('heroChangeText').textContent=(today>=0?'+':'-')+'\$'+absT+' today';
  chip.className='hero-chip '+(today>0?'chip-up':today<0?'chip-dn':'chip-flat');
  ...
  document.getElementById('pillText').textContent=dryRun?'PAPER':'LIVE';
  if(dryRun){pill.className='mode-pill pill-paper';dot.style.cssText='background:var(--gold)';}
  else{pill.className='mode-pill pill-live';dot.style.cssText='background:var(--green)';dot.className='pill-dot pill-dot-live';}

  /* STATS — win rate uses server winRate field which is only resolved trades */
  const wrEl=document.getElementById('statWR');
  const wrBar=document.getElementById('wrBar');
  if(wr!==null){
    wrEl.textContent=wr.toFixed(0)+'%';
    wrEl.style.color=wr>=70?'var(--green)':wr>=50?'var(--gold)':'var(--red)';
    wrBar.style.background=wr>=70?'var(--green)':wr>=50?'var(--gold)':'var(--red)';
  } else {
    wrEl.textContent='—';wrEl.style.color='var(--muted)';wrBar.style.background='var(--muted2)';
  }
  document.getElementById('statWRsub').textContent=totalTrades+' resolved trade'+(totalTrades!==1?'s':'');

  const td=document.getElementById('statToday');
  td.textContent=(today>=0?'+':'')+'\$'+Math.abs(today).toFixed(2);
  td.style.color=today>0?'var(--green)':today<0?'var(--red)':'var(--muted)';
  document.getElementById('todayBar').style.background=today>0?'var(--green)':today<0?'var(--red)':'var(--muted2)';
  document.getElementById('statTodaySub').textContent=(total>=0?'+':'')+'\$'+Math.abs(total).toFixed(2)+' all-time';

  document.getElementById('statPos').textContent=openCnt+'/'+mp;
  document.getElementById('statPosSub').textContent='of '+mp+' max';

  /* ENGINE */
  const running=s.isRunning;
  document.getElementById('engineDot').className='engine-dot'+(running?' on':'');
  ...
  /* JOURNEY */
  if(s.phaseLabel){
    const dm=s.doublingsMade||0,dn=s.doublingsNeeded||0;
    ...
    document.getElementById('jPhaseName').textContent=s.phaseLabel;
    document.getElementById('jPhaseDesc').textContent=s.phaseDesc||'';
    document.getElementById('jNextTarget').textContent='\$'+nxt.toLocaleString('en-US',{maximumFractionDigits:0});
    document.getElementById('jFill').style.width=Math.min(100,pct)+'%';
    document.getElementById('jPct').textContent=pct.toFixed(1)+'%';
    document.getElementById('jLeft').textContent=dn+' doubling'+(dn!==1?'s':'')+' to \$500K';
    document.getElementById('jDone').textContent=dm+'×';
    if(s.cfg){
      document.getElementById('jKelly').textContent=Math.round((s.cfg.kellyFrac||0.55)*100)+'%';
      document.getElementById('jEdge').textContent=Math.round((s.cfg.minEdge||0.04)*100)+'%';
    }
    ...
    document.getElementById('cfgPct').textContent=pct.toFixed(1)+'%';
    document.getElementById('cfgFill').style.width=Math.min(100,pct)+'%';
    document.getElementById('cfgCurBal').textContent=fmtBal(bal);
    document.getElementById('cfgNextTarget').textContent='→ \$'+nxt.toLocaleString('en-US',{maximumFractionDigits:0});
    document.getElementById('cfgDoublings').textContent=dm+'× done';
    document.getElementById('cfgNeeded').textContent=dn+' left';
    if(s.cfg) document.getElementById('cfgKelly').textContent=Math.round((s.cfg.kellyFrac||0.55)*100)+'%';
  }
  // Starting bankroll — read from API, not hardcoded
  document.getElementById('cfgBankroll').textContent=fmtBal(bankroll);

  /* SETTINGS performance */
  document.getElementById('cfgWR').textContent=wr!==null?wr.toFixed(1)+'%':'—';
  document.getElementById('cfgWR').className='s-val '+(wr>=70?'sv-green':wr>=50?'sv-gold':wr!==null?'sv-red':'');
  document.getElementById('wrNote').textContent=totalTrades+' resolved · '+openCnt+' open';
  document.getElementById('cfgWL').textContent=w+'W / '+l+'L';
  document.getElementById('cfgWL').className='s-val '+(w>l?'sv-green':w<l?'sv-red':'');
  const tpEl=document.getElementById('cfgTotalPnl');
  tpEl.textContent=fmt$(total);tpEl.className='s-val '+(total>=0?'sv-green':'sv-red');
  const tdEl=document.getElementById('cfgTodayPnl');
  tdEl.textContent=fmt$(today);tdEl.className='s-val '+(today>=0?'sv-green':'sv-red');
  document.getElementById('cfgScans').textContent=(s.scanCount||0).toLocaleString();
  document.getElementById('cfgBrainCount').textContent=(s.brainCount||0).toLocaleString();

  /* SETTINGS portfolio */
  document.getElementById('cfgReal').textContent=fmtBal(real);
  document.getElementById('cfgAvail').textContent=fmtBal(avail);
  document.getElementById('cfgPeak').textContent=fmtBal(peak);
  const dd=parseFloat(s.drawdown||0);
  document.getElementById('cfgDD').textContent=dd.toFixed(1)+'%';
  document.getElementById('cfgDDFill').style.width=Math.min(100,dd)+'%';

  /* SETTINGS bot config */
  document.getElementById('cfgMode').textContent=dryRun?'📝 Paper':'🔴 Live';
  document.getElementById('cfgMaxPos').textContent=mp;
  document.getElementById('cfgBrainInt').textContent=s.cfg?((s.cfg.brainInterval||90000)/60000).toFixed(1)+'min':'1.5min';
  document.getElementById('cfgOpen').textContent=openCnt+' of '+mp;
  ...
  if(s.logs&&s.logs.length){
    ...
    lw.innerHTML=s.logs.slice(0,60).map(l=>{
      ...
      const cls=lvl==='error'||lvl==='err'?'log-err':lvl==='warn'?'log-warn':'';
      return '<div'+(cls?' class="'+cls+'"':'')+'><span class="log-ts">'+(l.ts||'')+'</span>'+(l.msg||l)+'</div>';
    }).join('');
  }
}

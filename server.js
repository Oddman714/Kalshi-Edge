...
// Smart brain interval: fast during active windows, slow overnight to save API cost
function dynamicBrainInterval() {
  const etH = (new Date().getUTCHours() - 4 + 24) % 24; // approx Eastern Time
  if (etH >= 2 && etH < 9)   return 300000; // dead zone 2-9am ET: 5 min
  if ((etH >= 19 && etH <= 23) || (etH >= 9 && etH <= 16)) return 60000; // games+market: 1 min
...
async function getTopMarkets() {
  try {
    ...
    const scored = filteredMarkets.map(m => {
      ...
      // Duration scoring — faster resolution = faster compounding = priority.
      // Sweet spot: 3-12 hours (tonight's games, same-day data, crypto thresholds).
      const durationScore = hoursLeft < 0.5  ? 0.05  // too imminent
        : hoursLeft < 1   ? 0.70              // very soon
        : hoursLeft < 3   ? 1.10              // excellent: resolves in hours
        : hoursLeft < 8   ? 1.40              // BEST: tonight/today
...
async function runBrain(topMarkets) {
  ...
  let _priceCtx = '';
  const _hasCrypto = topMarkets.some(m => /kxbtc|kxeth|kxsol/i.test(m.ticker));
  if (_hasCrypto) {
    try {
  ...
  const prompt = `{"task":"generate_trade_signals","rules":["respond ONLY with valid JSON","no preamble","start with {","end with }"],"context":{"MISSION":"Double $${_bal.toFixed(2)} to $500K. ${_slots} OPEN SLOT(S). ${_priceCtx || ''} | TIME: ${new Date().toLocaleString('en-US',{timeZone:'America/New_York',hour:'2-digit',minute:'2-digit',weekday:'short'})}","ALREADY_HELD":"${_heldStr} — DO NOT signal any of these. Find DIFFERENT markets.","BALANCE":"$${_bal.toFixed(2)} | Cash: $${S.availableCash.toFixed(2)} | Open: ${S.openPositions.length}/${dynamicMaxPos()} | Phase: ${phase.name}","PAYOUT_MATH":"Net payout=(1-price)/price*0.955. At 28c=2.5x(TARGET). At 22c=3.5x(GREAT). At 17c=4.6x(EXCELLENT). Min 2.5x in SEED/SPROUT phase — anything below is a waste of a slot.","HARD_RULES":["1. PAYOUT FLOOR: >=2.5x in SEED/SPROUT, >=2.0x otherwise. Hard reject below floor.","2. CRYPTO: Prices are in LIVE_PRICES_NOW — use directly. Compare to each strike. >5% away + payout>=2.5x = signal. Do NOT search for crypto prices again.","3. NO-SIDE: Each market has a suggestedSide field. Markets with suggestedSide=NO mean betting AGAINST the outcome (buy NO contracts). Calculate NO payout = ((1-(1-yesBid))/(1-yesBid))*0.955. If confident event won't happen and payout>=2.5x: signal NO.","4. WEATHER: SKIP unless NWS places >=65% on that exact band. Low prices = market maker has the forecast.","5. SPORTS: Only with official injury/lineup data from ESPN/team site shifting win prob >20%. Never signal on narrative alone.","6. CONFIDENCE: <12c=85%+. 12-20c=78%+. 20-30c=72%+. 30c+=${(phase.minProb*100).toFixed(0)}%+.","7. Signal EXACTLY ${_slots} trade(s). ZERO is correct when nothing meets the bar.","8. Prefer <8h resolution. Reject >36h unless 4x+ payout."]},"markets":${JSON.stringify(topMarkets.slice(0,10).map(m=>({ticker:m.ticker,title:m.title,yesAsk:+(m.yesAsk*100).toFixed(1),yesBid:+((m.yesBid||0)*100).toFixed(1),netPayoutRatio:+(m.netPayoutRatio||((1-m.yesAsk)/m.yesAsk*0.955)).toFixed(2),suggestedSide:m.suggestedSide||'YES',volume:Math.round(m.volume),hoursLeft:+m.hoursLeft.toFixed(1),alreadyHeld:_heldTickers.includes(m.ticker)})))},"search_strategy":{"SEARCHES":10,"ORDER":["1. CRYPTO: Use LIVE_PRICES_NOW directly. For each BTC/ETH market: price vs strike. Edge = abs(price-strike)/strike. If edge>5% and payout>=2.5x: signal the correct side (YES if price>strike and YES is cheap, NO if price<strike and NO is cheap).","2. NO-SIDE SCAN: Review markets with suggestedSide=NO. These are markets where YES is overpriced — calculate NO payout and confidence. If you have 65%+ conf the outcome WON'T happen: signal NO.","3. SPORTS INJURY CHECK: For games at 8c-30c: search '[team] injury report [today's date]'. Only signal with hard evidence of key player out from official source.","4. WEATHER: Search 'NWS [city] high temperature [date]'. Only signal if >=65% probability on that exact band from official forecast.","5. MACRO: Any unreleased data today? Only if clear skew vs market.","6. RULE: Only include signals where you found SPECIFIC DATA. Zero signals is correct when nothing qualifies."]},"execution":{"MIN_NET_PAYOUT":"2.5x in SEED/SPROUT phase, 2.0x otherwise — hard reject below","MIN_EDGE":"${(phase.minEdge*100).toFixed(0)}% after 4.5% fee","MIN_CONF":"${(phase.minProb*100).toFixed(0)}%","SLOTS":"Signal EXACTLY ${_slots} trade(s) max — the number of open slots available right now"},"required_output":{"signals":[{"ticker":"NOT in ALREADY_HELD","side":"YES or NO","confidence":0.68,"edge":0.12,"marketPrice":0.30,"feeAdjustedEdge":0.09,"netPayoutRatio":2.19,"reasoning":"SPECIFIC: [exact score/price/data found]. Payout=(1-0.30)/0.30*0.955=2.2x. Conf=68%>30% price=38% edge.","hoursToClose":8}],"marketSummary":"what searched, what found","skipReason":"only if zero edge found after all searches"}}`;
...
WHAT ACTUALLY WORKS on Kalshi (ranked by reliability):
1. CRYPTO THRESHOLDS: BTC/ETH above/below a price resolving within 2-8 hours. Check current price vs strike — pure math, no opinion error. Highest reliability.
2. SPORTS MISPRICING: Only when Kalshi implied probability diverges 20%+ from actual win probability AND you have HARD EVIDENCE: confirmed injury report, official starting lineup, season record. Never bet on 'motivation' or 'playoff implications' alone.
3. WEATHER (only the dominant band): ONLY bet the temperature band the NWS forecast ACTUALLY lands on. If the forecast is 82F and the market is '82-83 YES at 16c' — DO NOT BET. The crowd knows. Only signal weather when official forecast gives that band 65%+ probability.
4. ECONOMIC DATA: Only before release. Skip any market already priced >55c — payout too small.

WHAT DESTROYS PORTFOLIOS:
- Betting 15-25c weather longshots. Market maker has live NWS data. 15c means the forecast is NOT there.
- Sports underdogs without confirmed injury/lineup evidence from official sources.
- Any trade under 2.5x net payout at low balance — need each win to actually move the needle.

HARD RULES:
1. Net payout MUST be >=2.5x in SEED/SPROUT phase, >=2.0x otherwise: ((1-price)/price)*0.955.
2. Minimum confidence: ${(getPhase().minProb*100).toFixed(0)}%. No guessing.
3. ONLY signal ${_slots} trade(s) — exactly the open slots available.

...
// Timing
const SCAN_SEC = 30;
const BRAIN_SEC = C.dryRun ? 120 : 300;  // 2min paper, 5min live
const HEARTBEAT_SEC = 1800;
...
let S = {
  ...
  claudeDown: false, lastCreditCheck: 0, lastBrainReasoning: '',
  pnlHistory: [],   // [{time, pnl, balance}] for chart
  brainNotes: [],    // Claude's learning notes
};
...
load();
// Timers don't survive restarts — always reset running state
S.isRunning = false;
setInterval(save, 15000);

// ─── HTTP ───────────────────────────────────────────────────

// Fast HTTP — 15s timeout for Kalshi API, orderbooks, etc
...
// Slow HTTP — 120s timeout for Claude API with web search (can take 60-90s)
function httpSlow(url, opts, body) {
  return new Promise((resolve, reject) => {
    const u = new URL(url);
    const r = https.request({ hostname: u.hostname, port: 443, path: u.pathname + u.search, ...opts }, res => {
      let d = ''; res.on('data', c => d += c);
    ...
    r.setTimeout(120000, () => { r.destroy(); reject(new Error('brain timeout')); });
    r.on('error', reject);
    if (body) r.write(body);
    r.end();
  });
...
async function brain() {
  ...
  const body = JSON.stringify({
    ...
    system: `You are EDGE — an elite autonomous AI prediction market trader on Kalshi managing $${Math.max(S.balance/100,C.bankroll).toFixed(2)}.

MISSION: Double the portfolio as quickly and safely as possible.

═══ YOUR PROCESS (follow EVERY time) ═══
1. SEARCH THE WEB for breaking news: Fed/economic data, political events, sports scores/injuries, weather — anything affecting these markets RIGHT NOW
2. For each market, estimate true probability based on what you found
3. SIGNAL trades where your informed probability differs from market price by ${C.dryRun ? '3' : (C.edgeMin*100).toFixed(0)}+ cents
4. Include a "brainNote" — one learning to remember for next session

═══ YOUR EDGE (what makes real money) ═══
1. NEWS SPEED: Search for current events the market hasn't priced in yet. A Fed statement, injury report, or political development can create 10-30¢ mispricing for minutes.
2. FAVOURITE-LONGSHOT BIAS: Contracts >85¢ win MORE than implied (buy YES). Contracts <15¢ win LESS (buy NO/sell). Proven by academic research on 300k+ Kalshi contracts.
3. TEMPORAL EDGE: Markets closing in 1-24h are most predictable.
4. MOMENTUM + NEWS: If price is moving AND your research confirms the direction, ride it.

═══ WHAT LOSES MONEY (NEVER DO) ═══
- Multi-leg parlays: compound probability makes them structurally overpriced. ALWAYS skip these.
- Sports props without specific news: the line is already efficient
- Cheap longshots (<15¢): they win far less than price implies
- Markets weeks from close: too much uncertainty

═══ CONFIDENCE CALIBRATION ═══
- HIGH: You found specific current news that directly affects this market AND the market hasn't repriced yet. Big bet.
- MEDIUM: Your knowledge strongly suggests mispricing but you didn't find confirming breaking news. Normal bet.
- LOW: Skip unless edge >15%.
${C.dryRun ? '\nPAPER MODE: Be more aggressive — signal 3%+ edge to generate data for the learning system.' : ''}

═══ YOUR MEMORY ═══
${mem}

═══ OUTPUT ═══
After searching news, output a JSON object. Your FINAL text MUST be valid JSON:
{"signals":[{"ticker":"X","title":"name","side":"yes"|"no","marketPrice":0.XX,"trueProb":0.XX,"edge":0.XX,"confidence":"high"|"medium"|"low","reasoning":"what news/knowledge supports this, max 25 words"}],"brainNote":"one sentence learning for next session"}
No edge found? {"signals":[],"brainNote":"what you observed for next time"}`,
    messages: [{role:'user',content:`TIMESTAMP: ${new Date().toISOString()}

STEP 1: Search the web for current news in these areas:
- US economic data, Fed policy, tariffs, trade policy
- Political events, congressional activity, executive orders  
- Sports scores, injuries, lineup changes (for any sports markets below)
- Weather forecasts (for any weather markets below)
- Any breaking news relevant to these specific markets

STEP 2: Analyze these pre-filtered candidates against what you find:

${mBlock}

STEP 3: Output your JSON trading signals. Be aggressive on high-conviction, news-backed calls.`}],
  });

  try {
    log('L2 🧠 Calling Claude with web search...');
    const r = await httpSlow('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': C.claudeKey, 'anthropic-version': '2023-06-01' } }, body);
...
async function runScan() {
  ...
  try {
    ...
    // Record P&L history every 10 scans (~5min) for chart
    if (S.scanCount % 10 === 0) {
      S.pnlHistory.push({ t: Date.now(), p: S.totalPnL, b: S.balance });
      if (S.pnlHistory.length > 500) S.pnlHistory = S.pnlHistory.slice(-500);
    }
    S.lastError = null;
    save();
...
function start() {
  ...
  // Reset stale daily P&L on start
  const today = new Date().toISOString().slice(0,10);
  if (S.dailyDate !== today) { S.dailyPnL = 0; S.dailyDate = today; }

  log('🚀 v5 | ' + (C.dryRun ? 'PAPER' : 'LIVE'));
  log('🚀 Starting loops...');

  // TG notification — completely detached, cannot block anything
  setTimeout(() => tg(`🚀 <b>Kalshi Edge v5</b>\n${C.dryRun?'📋 Paper':'⚡ LIVE'}\n${portfolioLine()}`), 100);

  // Scan loop — self-healing setTimeout chain
  function doScan() {
    if (!S.isRunning) return;
    runScan()
      .catch(e => log('s: ' + e.message))
      .finally(() => { if (S.isRunning) scanT = setTimeout(doScan, SCAN_SEC * 1000); });
  }
  ...
  function doBrain() {
    ...
    runBrain()
      ...
      .finally(() => { if (S.isRunning) brainT = setTimeout(doBrain, BRAIN_SEC * 1000); });
  }

  // Start scan immediately on next tick
  setTimeout(doScan, 10);
  // Start brain after 30s
  setTimeout(doBrain, 30000);
  // Heartbeat
  heartT = setInterval(() => heartbeat().catch(() => {}), HEARTBEAT_SEC * 1000);
  setTimeout(() => heartbeat().catch(() => {}), 60000);
...
app.get('/api/brain', (_,r) => r.json({ notes: S.brainNotes || [], reasoning: S.lastBrainReasoning || '', perfCat: S.perfCat, perfConf: S.perfConf, perfSide: S.perfSide, pnl: (S.totalPnL/100).toFixed(2) }));
app.get('/api/pnl', (_,r) => r.json(S.pnlHistory || []));
...
app.get('/api/reset', (_,r) => {
  // Reset all state — accessible via browser URL
  S.trades = []; S.positions = []; S.signals = []; S.candidates = [];
  S.resolved = []; S.priceHistory = {}; S.marketMeta = {};
  S.balance = C.bankroll * 100; S.dailyPnL = 0; S.totalPnL = 0;
  S.wins = 0; S.losses = 0; S.paperWins = 0; S.paperLosses = 0;
  S.scanCount = 0; S.brainCount = 0; S.totalClaude = 0;
  S.perfCat = {}; S.perfConf = {high:{w:0,l:0},medium:{w:0,l:0},low:{w:0,l:0}};
  S.perfSide = {yes:{w:0,l:0},no:{w:0,l:0}};
  S.lastError = null; S.claudeDown = false;
  save();
  log('🔄 STATE RESET');
  r.json({ok:true, message:'State reset complete'});
});

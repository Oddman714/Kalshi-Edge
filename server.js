...
function saveState() {
  try {
    ...
    // Notify SSE clients with a lightweight "state changed" ping — they'll re-poll /api/state
    sseNotify('update', { ts: Date.now(), running: S.isRunning, balance: S.realBalance || S.balance });
...
// ─── TELEGRAM ──────────────────────────────────────────────────────────────
function tg(msg) {
  // Strip HTML tags for SSE/live feed display
  const plain = msg.replace(/<[^>]+>/g, '').trim();
  // Push to live dashboard feed regardless of Telegram config
  sseNotify('notify', { msg: plain.slice(0, 200), type: 'tg', ts: Date.now() });
...
async function syncBalance() {
  try {
    ...
    // Kalshi ALWAYS returns balance in cents as an integer (e.g. 4832 = $48.32)
    // balance_dollars is a legacy field that may also exist as float dollars
    let rawBal = r.balance != null ? Number(r.balance) : (r.balance_dollars != null ? Number(r.balance_dollars) : 0);
    let bal;
    if (rawBal === 0) { log('Balance sync returned $0 — skipping', 'WARN'); return; }
    // Heuristic: if value looks like cents (>= 100, whole number-ish), divide by 100
    // if value < 100, treat as dollars already (balance_dollars path)
    if (rawBal >= 100 && Number.isInteger(rawBal)) {
      bal = rawBal / 100;
    } else if (rawBal >= 1) {
      // Could be dollars already (e.g. 48.32) or a small cent value — use parsePrice logic
      bal = rawBal > 1 ? rawBal : rawBal * 100; // shouldn't happen but safe
      // If still looks like cents after parsePrice (> 100), re-divide
      if (bal > 100000) bal = bal / 100;
    } else {
      bal = rawBal; // fractional dollar — accept as-is
    }
    bal = parseFloat(bal.toFixed(2));
    log(`Balance raw=${rawBal} → $${bal.toFixed(2)}`);
    if (bal <= 0) { log('Balance computed to $0 — skipping', 'WARN'); return; }
...
/* LIVE FEED */
.live-feed{margin:0 20px 10px;background:var(--s1);border-radius:14px;border:1px solid var(--b);overflow:hidden;max-height:200px;overflow-y:auto}
.lf-row{display:flex;align-items:flex-start;gap:9px;padding:9px 13px;border-bottom:1px solid var(--b2)}
.lf-row:last-child{border-bottom:none}
.lf-dot{width:7px;height:7px;border-radius:50%;flex-shrink:0;margin-top:5px;background:var(--m2)}
.lf-dot-g{background:var(--g)}.lf-dot-r{background:var(--r)}.lf-dot-gold{background:var(--gold)}
.lf-msg{font-size:12px;color:var(--m);line-height:1.4;flex:1}
.lf-ts{font-size:10px;color:var(--m2);flex-shrink:0;font-family:var(--mono);margin-top:2px}
...
/* TOASTS */
.toast{position:fixed;top:calc(var(--sat)+12px);left:50%;transform:translateX(-50%) translateY(-80px);
  max-width:380px;width:calc(100% - 40px);padding:11px 16px;border-radius:13px;
  font-size:13px;font-weight:500;line-height:1.4;
  background:var(--s2);color:var(--txt);border:1px solid var(--b);
  box-shadow:0 8px 32px rgba(0,0,0,.5);transition:transform .3s cubic-bezier(.34,1.56,.64,1),opacity .3s;
  opacity:0;z-index:999;text-align:center}
.toast.show{transform:translateX(-50%) translateY(0);opacity:1}
.toast-error{background:var(--rd);border-color:rgba(255,61,90,.3);color:var(--r)}
.toast-warn{background:var(--goldd);border-color:rgba(245,166,35,.3);color:var(--gold)}
.toast-signal{background:var(--gd);border-color:rgba(5,212,124,.3);color:var(--g)}
.toast-info{background:var(--s2);color:var(--m)}
</style>
</head>
<body>
<div class="app">
...
<div id="pg-home" class="pg on">
  <div class="top">
    <div class="brand">KalshiBot <span id="connDot" title="Live connection" style="display:inline-block;width:6px;height:6px;border-radius:50%;background:var(--m2);vertical-align:middle;margin-left:4px;transition:background .4s"></span></div>
  ...
  <div style="text-align:center;font-size:9px;color:rgba(238,238,245,.1);padding:8px;font-family:monospace">KalshiBot v9.9</div>
  <div class="gap"></div>
</div>
...
<div id="pg-activity" class="pg">
  ...
  <div class="sec-lbl" style="margin-top:0;padding-top:6px">Live Feed</div>
...
<script>
/* KalshiBot v9.9 — SSE live feed + balance fix */
...
/* ── TOAST NOTIFICATIONS ── */
var _lastLogTs=0,_lastScanCount=0,_lastBrainCount=0;
function toast(msg,type){
  ...
  setTimeout(function(){t.classList.remove('show');setTimeout(function(){if(t.parentNode)t.parentNode.removeChild(t);},400);},3500);
}
function checkNotifications(s){
  // New log entries since last check
  if(s.logs&&s.logs.length&&s.logs[0]&&s.logs[0].ts&&s.logs[0].ts!==_lastLogTs){
    var newest=s.logs[0];
    _lastLogTs=newest.ts;
    var lvl=(newest.level||'').toLowerCase();
    if(lvl==='error')toast(newest.msg,'error');
    else if(lvl==='warn')toast(newest.msg,'warn');
  }
  // Brain fired
  if(s.brainCount&&s.brainCount>_lastBrainCount&&_lastBrainCount>0){
    var sig0=s.signals&&s.signals[0];
    toast('🧠 Brain #'+s.brainCount+(sig0?' → '+sig0.ticker+' '+sig0.side:''),sig0?'signal':'info');
  }
  _lastBrainCount=s.brainCount||0;
  // Scan milestone
  if(s.scanCount&&s.scanCount>_lastScanCount&&s.scanCount%10===0){
    toast('Scan #'+s.scanCount+' complete','info');
  }
  _lastScanCount=s.scanCount||0;
}
...
function pushFeed(msg,type){
  type=type||'info';
  var ts=new Date().toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit',second:'2-digit'});
  _feedItems.unshift({msg:msg,type:type,ts:ts});
  if(_feedItems.length>60)_feedItems.pop();
  renderFeed();
}
function renderFeed(){
  var w=g('liveFeed');if(!w)return;
  if(!_feedItems.length){w.innerHTML='<div class="lf-row"><div class="lf-msg" style="color:var(--m2)">Waiting for bot events...</div></div>';return;}
  w.innerHTML=_feedItems.slice(0,25).map(function(f){
    var dc=f.type==='win'?'lf-dot-g':f.type==='loss'?'lf-dot-r':f.type==='brain'||f.type==='signal'?'lf-dot-g':f.type==='warn'||f.type==='error'?'lf-dot-r':f.type==='trade'?'lf-dot-gold':'';
    return '<div class="lf-row"><div class="lf-dot '+dc+'"></div><div class="lf-msg">'+f.msg+'</div><div class="lf-ts">'+f.ts+'</div></div>';
  }).join('');
}
var _prevLogs=[];
function syncFeedFromLogs(logs){
  if(!logs||!logs.length)return;
  var newest=logs[0];
  if(!newest||!newest.msg)return;
  // Only add entries we haven't seen before (compare by ts+msg)
  var key=newest.ts+'|'+newest.msg;
  if(_prevLogs[0]===key)return;
  _prevLogs.unshift(key);if(_prevLogs.length>60)_prevLogs.pop();
  var lvl=(newest.level||'').toLowerCase();
  var type=lvl==='error'?'error':lvl==='warn'?'warn':
    /WIN|🟢/.test(newest.msg)?'win':/LOSS|🔴/.test(newest.msg)?'loss':
    /Brain|🧠/.test(newest.msg)?'brain':/Order|✅|trade/i.test(newest.msg)?'trade':'info';
  pushFeed(newest.msg,type);
}
...
function applyState(s){
  try{
    ...
    // Show real balance if available, $0.00 if confirmed $0, only show Syncing if running and never received data
    var heroTxt = bal>0 ? fm(bal) : (running && !_loaded ? 'Syncing...' : '$0.00');
    st('heroBal', heroTxt);
    ...
    /* live feed sync */
    if(s.logs&&s.logs.length)syncFeedFromLogs(s.logs);
    /* notifications */
...
/* ── LOAD STATE ── */
var _loaded=false,_retries=0,_sseConnected=false,_pollTimer=null,_sseObj=null;
...
function connectSSE(){
  if(_sseObj)try{_sseObj.close();}catch(e){}
  try{
    var es=new EventSource(API+'/api/events');
    _sseObj=es;
    es.addEventListener('update',function(e){
      // SSE fired — immediately grab fresh state
      loadState();
    });
    es.addEventListener('notify',function(e){
      try{
        ...
        // Classify for toast
        var toastType=/win|🟢/i.test(msg)?'signal':/loss|🔴|error/i.test(msg)?'error':/warn/i.test(msg)?'warn':/brain|🧠|signal/i.test(msg)?'signal':'info';
        // Show toast for important events
        if(/win|loss|error|brain|order|🟢|🔴|✅|❌|🎯|🚀/i.test(msg)) toast(msg.slice(0,80),toastType);
        // Always push to live feed
        pushFeed(msg,'tg');
      }catch(ex){}
    });
    es.onopen=function(){
      _sseConnected=true;
      _startPoll(8000);
      var cd=document.getElementById('connDot');if(cd)cd.style.background='var(--g)';
    };
    es.onerror=function(){
      _sseConnected=false;
      _startPoll(3000);
      var cd=document.getElementById('connDot');if(cd)cd.style.background='var(--r)';
    };
  }catch(e){
    _startPoll(3000);
  }
...
function loadState(){
  fetch(API+'/api/state?_='+Date.now(),{cache:'no-store'})
    ...
    .catch(function(e){
      ...
      if(!_loaded){
        var hb=g('heroBal');
        if(hb){hb.textContent=_retries>3?'Reconnecting...':'Connecting...';hb.style.fontSize='28px';hb.style.color='var(--m)';}
        var hc=g('heroChg');if(hc)hc.textContent=_retries>1?'retry '+_retries:'loading...';
      }
    });
}
function doSync(){
  fetch(API+'/api/sync',{method:'POST'}).then(function(r){
    if(!r.ok)throw new Error('HTTP '+r.status);return r.text();
  }).then(function(t){
    try{var s=JSON.parse(t);state=s;applyState(s);}
    catch(e){console.error('sync parse:',e.message);loadState();}
  }).catch(function(e){console.error('sync err:',e.message);loadState();});
}
function toggleBot(){
...
connectSSE();          // SSE for instant push
_startPoll(3000);      // fallback poll (SSE will slow this to 8s on connect)
</script>
</body>
</html>
`;
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/api/notify' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        sseNotify('notify', { msg: d.msg || '', type: d.type || 'info', ts: Date.now() });
  ...
  if (url.pathname === '/api/events') {
    ...
    res.write(`event: update\ndata: ${JSON.stringify({ ts: Date.now(), running: S.isRunning, balance: S.realBalance || S.balance })}\n\n`);
    sseClients.add(res);
...
server.listen(CFG.port, () => {
  log(`KalshiBot v9.9 listening on port ${CFG.port}`);

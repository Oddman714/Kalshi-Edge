...
async function runCryptoFastPath(topMarkets) {
  ...
  for (const m of cryptoMarkets) {
    ...
    // Payout floors — calibrated to balance trade frequency vs edge quality
    // Hourly/sub-2h: 1.5x min (was 2.5x — was blocking good 1.9x trades)
    // 15min: 1.8x min (faster turnover compensates for lower payout)
    const minPayout = isUnderdog
      ? (is15Min ? 1.8 : (m.hoursLeft < 2.0 ? 1.5 : 2.0))
      : 0.15; // favorites: any positive EV
...
async function executeTrade(sig) {
  ...
  // HARD GATE: brain cannot trade 15min markets — requires mathematical precision
  // FastPath IS allowed for 15min (math-based, gap threshold enforced above)
  if (/15M|15MIN/i.test(sig.ticker) && sig.source === 'brain') {
    log("Hard gate: brain BANNED from 15min market: " + sig.ticker + " — FastPath only");
    return;
  }
  // HARD GATE: brain signals need 45min+ to close — short windows = variance not edge
  // FastPath signals are exempt — they have mathematical gap enforcement
  if ((sig.hoursToClose || 999) < 0.40 && sig.source === 'brain') {
    log("Hard gate: brain signal rejected — " + (sig.hoursToClose*60).toFixed(0) + "min too short (need 45min+)");
    return;
  }
  ...
  const minPayout = isFavoriteExec ? 0.15  // favorites: any positive EV
    : isCryptoTicker
    ? (isFastPath ? 1.5 : 1.8)   // FastPath: 1.5x min. Brain crypto: 1.8x min.
    : 2.0;                        // Non-crypto brain: 2.0x min
...
4. Bot auto-restarts when cash is available`);
    ...
    if (brainReady && slotsAfterFastPath > 0 && hasMarkets && !fastPathFoundTrade) {
...
async function startBot() {
  ...
  _botStarted = true;
  S.isRunning = true;
  log('[M] KalshiBot v13.1 started -- running pre-flight sync...');
...
server.listen(CFG.port, () => {
  log(`KalshiBot v13.1 listening on port ${CFG.port}`);

...
const PHASES = [
  { name: 'SEED',     min: 0,      max: 50,     kelly: 0.40, minEdge: 0.08, minProb: 0.70, maxPos: 3,  label: '🌱 Seed',     desc: '3 slots max. 70%+ conf, 2.5x+ payout, <24h resolution only. Crypto + verified sports only.' },
...
function dynamicBrainInterval() {
  const etH = (new Date().getUTCHours() - 4 + 24) % 24;
  const etM = new Date().getUTCMinutes();

  // Check if hourly crypto markets are live and near resolution
  // Hourly markets close at :00 each hour — sweet spot is 15-45min before close
  // i.e. minutes 15-45 of each hour
  const minsToHour = 60 - etM;  // minutes until next hour
  const inCryptoWindow = minsToHour >= 15 && minsToHour <= 50;

  if (etH >= 2 && etH < 9 && !inCryptoWindow) return 300000; // dead zone: 5min unless crypto window
  if (inCryptoWindow)  return 90000;    // 90s — hourly crypto window (don't over-fire, use good entries)
  if ((etH >= 19 && etH <= 23) || (etH >= 9 && etH <= 16)) return 60000;  // games+market: 1min
  return 120000; // default 2min
}

// Smart brain interval: faster during active trading windows, off overnight
function dynamicBrainInterval() {
  const utcH = new Date().getUTCHours();
  const etH  = (utcH - 4 + 24) % 24; // approx Eastern (EDT)
  // Dead zone 2am-8am ET: almost no live events worth trading
  if (etH >= 2 && etH < 9)   return 300000; // 5 min — save API cost
  // Active: evening games (7-11pm ET) and market hours (9:30am-4pm ET)
  if ((etH >= 19 && etH <= 23) || (etH >= 9 && etH <= 16)) return 60000;  // 1 min
...
async function syncRestingOrders() {
  try {
    ...
    // Alert when orders sit unfilled 2h+ — market too thin or price too low
    const _now_ms = Date.now();
    for (const _ord of S.restingOrders) {
      if (!_ord.placedAt || _ord.alertedStale) continue;
      if ((_now_ms - _ord.placedAt) > 2 * 3600000) {
        _ord.alertedStale = true;
        log(`⚠️ Stale order: ${_ord.ticker} ${(_ord.contracts||0).toFixed(0)}ct @ ${((_ord.pricePerContract||0)*100).toFixed(0)}c — unfilled ${((_now_ms-_ord.placedAt)/3600000).toFixed(1)}h`, 'WARN');
        tg(`⚠️ <b>Unfilled Order ${((_now_ms-_ord.placedAt)/3600000).toFixed(1)}h</b>\n${_ord.ticker} — ${(_ord.contracts||0).toFixed(0)} contracts @ ${((_ord.pricePerContract||0)*100).toFixed(0)}c\nReserving $${(_ord.reservedCash||0).toFixed(2)} — consider cancelling if market too thin`);
      }
    }
  } catch(e) {
...
async function getTopMarkets() {
  try {
    ...
    const seriesList = [
      // ── TIER 1: HOURLY + 15-MIN CRYPTO (fastest compounding, pure math) ──
      // Hourly BTC markets (various Kalshi ticker patterns — all tried to maximize coverage)
      'KXBTCH','KXBTCUSDH','KXBTCHOUR','KXBITCOINH',
      // 15-min BTC (screenshot shows "BTC 15 min" markets)
      'KXBTC15M','KXBTCUP','KXBTCDOWN','KXBTC15','KXBTCMIN15',
      // Hourly ETH
      'KXETHUSDH','KXETHH','KXETHHOUR',
      // Daily crypto (next best — same math, longer window)
      'KXBTCD','KXBTCW','KXETHUSD','KXETH','KXSOL','KXXRP','KXDOGE',
      'KXBNB','KXAVAX','KXADA','KXLINK','KXMATIC',
      // Sports -- dominant volume, clear live scores
      'KXNBAGAME','KXNBAPLAYOFFS','KXNBAPLAYOFF','KXMLBGAME','KXNHLGAME','KXNFLGAME',
      'KXMMA','KXGOLF','KXTENNIS','KXSOCCER','KXNCAAMB',
      // Esports — screenshot shows 3 live esports markets, often good payouts
      'KXESPORTS','KXLOL','KXCS2','KXVALORANT','KXDOTA',
    ...
    const fetches = await Promise.allSettled(
      seriesList.map(s =>
        kalshi('GET', '/markets', null, { series_ticker: s, status: 'open', limit: '100' })
      )
    );
    ...
    // Log which series returned markets to diagnose coverage
    const _returnedSeries = fetches
      .map((f, i) => f.status === 'fulfilled' && (f.value.markets||[]).length > 0 ? seriesList[i] : null)
      .filter(Boolean);
    log(`Series fetch: ${successCount}/${seriesList.length} returned markets | Active: ${_returnedSeries.slice(0,10).join(',')}`);
    ...
    // Additional pass: fetch fast-expiring markets directly (catches hourly not in named series)
    // These are the highest-value trades — always include them regardless of series name
    try {
      const fastRes = await kalshi('GET', '/markets', null, {
        status: 'open', limit: '200',
        max_close_ts: Math.floor((Date.now() + 2 * 3600000) / 1000).toString() // closes within 2h
      });
      const fastMkts = (fastRes.markets || []).filter(m =>
        !m.mve_collection_ticker &&
        /btc|eth|bitcoin|ethereum|crypto|sol/i.test(m.ticker + ' ' + (m.series_ticker||''))
      );
      if (fastMkts.length > 0) {
        const existing = new Set(allMarkets.map(m => m.ticker));
        const newFast = fastMkts.filter(m => !existing.has(m.ticker));
        allMarkets.push(...newFast);
        log(`Fast-expiry crypto fetch: added ${newFast.length} markets resolving within 2h`);
      }
    } catch(e) { log('Fast-expiry fetch failed: ' + e.message.slice(0,60), 'WARN'); }
    ...
    log(`After filter: ${markets.length} tradeable (8c-42c YES, vol>=50, spread<=18c)`);

    // Pre-filters: remove markets that are structurally bad opportunities
    const preFilterCount = markets.length;
    const filteredMarkets = markets.filter(m => {
      const yesAsk = parsePrice(m.yes_ask_dollars);
      // 1. Hard duration cap: never show markets >36h to brain in SEED/SPROUT
      //    They distract brain and lead to "sports resolve too far out" skips
      const hoursLeftFilter = m.close_time
        ? Math.max(0, (new Date(m.close_time) - Date.now()) / 3600000) : 999;
      const phase = getPhase();
      const maxHours = (phase.name === 'SEED' || phase.name === 'SPROUT') ? 36 : 72;
      if (hoursLeftFilter > maxHours) return false;

      // 2. Weather: remove mid-probability bands (20-35c) — market maker has NWS data
    ...
    if (removedCount > 0) {
      log(`Pre-filter removed ${removedCount} markets (>36h or mid-prob weather)`);
    }
    ...
    const scored = filteredMarkets.map(m => {
      ...
      // Category multipliers: hourly crypto = highest (pure math, 24/7, fast)
      const isCrypto = /btc|eth|sol|xrp|doge|crypto|inxd|inxw|spx|ndx/i.test(m.ticker);
      const isHourlyCrypto = isCrypto && (/H$|15M$|HOURLY/i.test(m.ticker) || (m.series_ticker && /H$|15M/i.test(m.series_ticker)));
      const isWeather = /kxhigh|kxlow/i.test(m.ticker);
      const categoryMult = isHourlyCrypto ? 1.65  // hourly crypto: best — math-only, 24/7
        : isCrypto ? 1.30                           // daily/weekly crypto: very good
        : isWeather ? 0.65                          // weather: penalize (market maker has NWS)
        : 1.0;
...
async function runBrain(topMarkets) {
  ...
  S.estimatedApiSpend = (S.estimatedApiSpend || 0) + 0.28; // Sonnet ~$0.28/call
  ...
  const prompt = `{"task":"generate_trade_signals","rules":["respond ONLY with valid JSON","no preamble","start with {","end with }"],"context":{"MISSION":"Double $${_bal.toFixed(2)} to $500K. ${_slots} OPEN SLOT(S). ${_priceCtx || ''} | TIME: ${new Date().toLocaleString('en-US',{timeZone:'America/New_York',hour:'2-digit',minute:'2-digit',weekday:'short'})}","ALREADY_HELD":"${_heldStr} — DO NOT signal any of these. Find DIFFERENT markets.","BALANCE":"$${_bal.toFixed(2)} | Cash: $${S.availableCash.toFixed(2)} | Open: ${S.openPositions.length}/${dynamicMaxPos()} | Phase: ${phase.name}","MARKET_NOTE":"All markets below resolve within 36 hours. hoursLeft<1.5=THIS HOUR. hoursLeft<8=TONIGHT. Check hoursLeft field for each market.","PAYOUT_MATH":"Net payout=(1-price)/price*0.955. At 28c=2.5x. At 22c=3.5x. At 17c=4.6x. At 12c=7.0x. EDGE FORMULA: For YES: edge=confidence-yesAsk. For NO: edge=confidence-noAsk where noAsk=(1-yesBid). Example: BTC NO at 12c with 90% conf -> edge=0.90-0.12=0.78 (78%). Always report feeAdjustedEdge as edge for the SIDE you are betting.","HARD_RULES":["1. PAYOUT FLOOR: CRYPTO (BTC/ETH/SOL): >=2.0x in SEED/SPROUT. SPORTS/WEATHER: >=2.5x in SEED/SPROUT. GROWTH+: >=1.8x all types. The 2.0x floor for crypto is because these are pure math trades with high certainty — a 2.13x payout on BTC with 85%+ confidence is still a strong positive-EV trade.","2. HOURLY CRYPTO FIRST (hoursLeft<1.5): Check BTC/ETH price from LIVE_PRICES_NOW vs each strike. Edge = abs(price-strike)/strike*100. If edge>2% AND payout>=2.0x: strong signal. If edge>1% AND payout>=3.0x: also strong. Even a 1% gap with 5x payout is positive EV. Use YES if price ABOVE strike. Use NO if price BELOW strike.","3. DAILY CRYPTO (hoursLeft 1-8h): Same logic. Edge>3% + payout>=2.0x = signal. Check all BTC/ETH strikes — find the one where current price gives clearest edge.","4. NO-SIDE: Markets with suggestedSide=NO — calculate NO payout. If confident event won't happen and payout>=2.5x: signal NO.","5. SPORTS (hoursLeft<6): For tonight's games, you can signal based on: (a) LIVE score check — if a team is losing by 15+ points with <30min left, their YES side is likely overpriced. Search '[game] live score' to verify. (b) Pre-game: only with confirmed injury data shifting odds >20%. Look for clear mismatches like playoff contender vs eliminated team.","6. WEATHER: SKIP unless NWS gives >=65% to that specific band.","7. CONFIDENCE: <12c=85%+. 12-20c=78%+. 20-30c=72%+. 30c+=${(phase.minProb*100).toFixed(0)}%+. EXCEPTION: hourly crypto where math clearly shows price far from strike — confidence can be 90%+ from the math alone.","8. Every market in this list already passes the 36h filter — sports markets shown ARE tonight's games. Signal EXACTLY ${_slots} trade(s). Zero is correct only when payout/confidence bars aren't met."]},"markets":${JSON.stringify(topMarkets.slice(0,12).map(m=>({ticker:m.ticker,title:m.title,yesAsk:+(m.yesAsk*100).toFixed(1),yesBid:+((m.yesBid||0)*100).toFixed(1),netPayoutRatio:+(m.netPayoutRatio||((1-m.yesAsk)/m.yesAsk*0.955)).toFixed(2),suggestedSide:m.suggestedSide||'YES',volume:Math.round(m.volume),hoursLeft:+m.hoursLeft.toFixed(2),isHourly:m.hoursLeft<1.5,alreadyHeld:_heldTickers.includes(m.ticker)})))},"search_strategy":{"SEARCHES":10,"ORDER":["1. HOURLY CRYPTO (hoursLeft<1.5): Use LIVE_PRICES_NOW. For each market: Edge% = abs(price-strike)/strike*100. Signal when: edge>2% + payout>=2.0x, OR edge>1% + payout>=3.0x. Use YES if price>strike (>2%), NO if price<strike (>2%). No web search needed.","2. DAILY CRYPTO (hoursLeft 1.5-8h): Same logic for BTC/ETH daily markets. Use LIVE_PRICES_NOW, compute edge, signal if qualifies.","3. NO-SIDE SCAN: Markets with suggestedSide=NO where you have 65%+ conf event won't happen. Calculate NO payout.","4. SPORTS TONIGHT (hoursLeft<6): First search '[game] live score' for each sport market. If game is in progress and team is losing big (15+ pts basketball, 3+ runs baseball in late innings), their YES side may be overpriced — signal NO. If pre-game: search injury report. Either way: verify market price vs actual win probability.","5. WEATHER: Search 'NWS [city] forecast [date]'. Only signal if >=65% on that band.","6. ZERO SIGNALS is correct if nothing passes the bar — never fill slots with weak trades."]},"execution":{"MIN_NET_PAYOUT":"CRYPTO: 2.0x in SEED/SPROUT. SPORTS/WEATHER: 2.5x in SEED/SPROUT. GROWTH+: 1.8x all.","MIN_EDGE":"${(phase.minEdge*100).toFixed(0)}% after 4.5% fee","MIN_CONF":"${(phase.minProb*100).toFixed(0)}%","SLOTS":"Signal EXACTLY ${_slots} trade(s) max — the number of open slots available right now"},"required_output":{"signals":[{"ticker":"NOT in ALREADY_HELD","side":"YES or NO","confidence":0.75,"edge":0.63,"marketPrice":0.127,"feeAdjustedEdge":0.60,"netPayoutRatio":6.56,"reasoning":"SPECIFIC DATA: BTC=$72900 vs strike $73999 — BTC is $1100 BELOW strike. NO wins if BTC stays below $74000 at 7pm. Conf=90%. NO at 12.7c = 6.56x net payout. Edge=conf(0.75)-NOPrice(0.127)=0.623","hoursToClose":1}],"marketSummary":"what searched, what found","skipReason":"only if zero edge found after all searches"}}`;
...
WHAT ACTUALLY WORKS on Kalshi (ranked by reliability and speed):

1. ⚡ HOURLY CRYPTO (BEST — hoursLeft<1.5): BTC/ETH hourly threshold markets. These resolve EVERY HOUR. The math is binary: current price vs strike. If BTC=$72,900 and strike is $73,000, NO wins (price below strike). Payout for NO at 10% = 8.6x. These are the highest-value trades — pure math, no opinion, fast resolution, maximum compounding speed. LIVE_PRICES_NOW gives you the data instantly.

2. 📊 DAILY CRYPTO (hoursLeft 1.5-8h): Same BTC/ETH threshold logic, longer window. Still pure math.

3. 🏀 SPORTS MISPRICING: Only with CONFIRMED injury/lineup data from official sources shifting probability >20%. Never bet on narrative.

4. 🌡️ WEATHER: Only the exact band NWS forecast shows at >=65% probability. Low-priced bands mean market maker has the forecast.

5. 📈 MACRO: Only unreleased data with clear skew vs market price.

WHAT DESTROYS PORTFOLIOS:
- Ignoring hourly crypto markets — these are the fastest path to doubling.
- Betting weather longshots. Market maker has live NWS data.
- Sports without confirmed injury evidence.
- Any trade under 2.5x payout at low balance.

HARD RULES:
1. Payout >=2.5x in SEED/SPROUT, >=2.0x in GROWTH+.
2. Confidence >= ${(getPhase().minProb*100).toFixed(0)}%. For hourly crypto: math confidence from price vs strike is valid.
3. ONLY signal ${_slots} trade(s).
4. Prefer markets resolving within 8 hours. Reject >36 hours unless payout is 4x+.
5. Output ONLY valid JSON. Start with { end with }. No markdown.`,
        tools: [{ type: 'web_search_20250305', name: 'web_search', max_uses: 12 }], // 12 searches for thorough verification
...
async function executeTrade(sig) {
  ...
  // For sub-1.5h trades (hourly crypto), allow 1 extra slot above cap
  // These resolve so fast they don't create real concurrent risk
  const isVeryFast = (sig.hoursToClose || 999) < 1.5;
  const effectiveMax = isVeryFast ? maxPos + 1 : maxPos;
  if (totalPosCount >= effectiveMax) {
    log(`Skip ${sig.ticker}: at position cap (${totalPosCount}/${effectiveMax}${isVeryFast?' incl. hourly bonus':''})`);
    return;
  }
  ...
  const mktP_range = sig.marketPrice || 0.5;
  const sidePriceCheck = sig.side === 'NO' ? (1 - mktP_range) : mktP_range;
  if(sidePriceCheck <= 0.05 || sidePriceCheck >= 0.95){
    log('Skip '+sig.ticker+': '+sig.side+' @'+(sidePriceCheck*100).toFixed(0)+'c outside 5-95c range'); return;
  }
  ...
  // Compute payout from the ACTUAL side price (not YES-equivalent)
  // For YES: sidePrice = yesAsk. For NO: sidePrice = noAsk = 1 - yesBid.
  const mktP_pay = sig.marketPrice || 0.5;
  const sidePriceForPayout = sig.side === 'NO' ? (1 - mktP_pay) : mktP_pay;
  ...
  const mktP_ls = sig.marketPrice || 0.5;
  const sidePrice_ls = sig.side === 'NO' ? (1 - mktP_ls) : mktP_ls;
  if (sidePrice_ls < 0.12 && sig.confidence < 0.85) {
    log(`Skip ${sig.ticker}: extreme longshot ${sig.side}@${(sidePrice_ls*100).toFixed(0)}c needs 85%+ conf, got ${(sig.confidence*100).toFixed(0)}%`);
    return;
  }
  if (sidePrice_ls < 0.20 && sig.confidence < 0.78) {
    log(`Skip ${sig.ticker}: longshot ${sig.side}@${(sidePrice_ls*100).toFixed(0)}c needs 78%+ conf, got ${(sig.confidence*100).toFixed(0)}%`);
    return;
  }
  if (sidePrice_ls < 0.30 && sig.confidence < 0.72) {
    log(`Skip ${sig.ticker}: underdog ${sig.side}@${(sidePrice_ls*100).toFixed(0)}c needs 72%+ conf, got ${(sig.confidence*100).toFixed(0)}%`);
    return;
  }
  ...
  const priceForKelly = sig.side === 'NO' ? sidePriceForPayout : tradePrice;
  const serverKelly = kellySize(sig.confidence, priceForKelly);
  ...
  // Order price: use correct side price for both YES and NO orders
  // Brain may report marketPrice as YES price even for NO signals — normalize here
  const rawOrderP = sig.marketPrice || (sig.side === 'YES' ? sig.confidence : 1 - sig.confidence);
  // For NO: if brain reported YES price (>0.5 for a NO bet), convert to NO price
  const orderP = (sig.side === 'NO' && rawOrderP > 0.5) ? (1 - rawOrderP) : rawOrderP;
...
📈 ${_dblProg}% to $${_dblNext} | ${doublingsNeeded()} doublings left to $500K`);
        saveState();
...
function sendHeartbeat() {
  ...
  const _openList = S.openPositions.length > 0
    ? '\n📋 ' + S.openPositions.slice(0,3).map(p => p.ticker.split('-').slice(-2).join('-') + ' ' + p.side).join(' | ') + (S.openPositions.length > 3 ? ` +${S.openPositions.length-3}` : '')
    : '';
  tg(`📊 <b>Heartbeat — ${phase.label}</b>
${mode} | Scans: ${S.scanCount} | Brain: #${S.brainCount} | API: $${apiSpend.toFixed(2)}
💰 $${bal.toFixed(2)} → $${nextTarget().toFixed(2)} next target
${progressBar} ${progress}% | DD: ${drawdown}%
Today: ${S.todayPnl >= 0 ? '+' : ''}$${S.todayPnl.toFixed(2)} | All-time: ${S.totalPnl >= 0 ? '+' : ''}$${S.totalPnl.toFixed(2)}
🎯 ${S.wins}W/${S.losses}L (${S.wins+S.losses > 0 ? ((S.wins/(S.wins+S.losses||1))*100).toFixed(0) : '--'}% WR) | ${doublingsNeeded()} doublings to $500K${_openList}`);
}
...
Brain: ${(dynamicBrainInterval()/60000).toFixed(1)}min (smart schedule) | Scan: ${CFG.scanInterval/1000}s${_startPosList}`);

...
:root{
  ...
  --green:#05d47c;--green-d:rgba(5,212,124,0.12);
...
html,body{background:var(--bg);color:var(--text);font-family:var(--font);min-height:100dvh;-webkit-font-smoothing:antialiased}
.app{max-width:430px;margin:0 auto;padding-bottom:calc(72px + var(--sab))}

/* STATUS BANNER */
#banner{position:fixed;top:0;left:50%;transform:translateX(-50%);width:100%;max-width:430px;padding:10px 16px;padding-top:max(10px,var(--sat));font-size:12px;font-weight:600;text-align:center;z-index:999;display:none}
#banner.yellow{background:rgba(245,166,35,0.95);color:#000}
#banner.red{background:#ff3d5a;color:#fff}
#banner.green{background:var(--green);color:#000}

/* NAV */
.nav{position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:430px;background:rgba(6,6,10,0.95);backdrop-filter:blur(24px);border-top:1px solid var(--border);display:flex;padding-bottom:var(--sab);z-index:200}
.nav-item{flex:1;display:flex;flex-direction:column;align-items:center;padding:10px 0 6px;cursor:pointer;color:var(--muted);gap:3px}
.nav-item.active{color:var(--text)}
.nav-label{font-size:10px;font-weight:500}
.page{display:none}.page.active{display:block}

/* HOME */
.home-top{padding:calc(var(--sat) + 16px) 20px 0;display:flex;align-items:center;justify-content:space-between}
...
.hero-eye{font-size:11px;font-weight:600;color:var(--muted2);text-transform:uppercase;letter-spacing:.12em;margin-bottom:8px}
.hero-bal{font-size:52px;font-weight:700;letter-spacing:-3px;line-height:1}
.hero-chip{display:inline-flex;align-items:center;gap:6px;font-size:14px;font-weight:600;padding:6px 16px;border-radius:22px;margin:10px 0 16px}
...
.hc-lbl{font-size:9px;font-weight:700;color:var(--muted2);text-transform:uppercase;letter-spacing:.1em;margin-bottom:4px}
...
.chart-svg-wrap{width:100%;height:140px;overflow:hidden}
svg.mchart{width:100%;height:140px;display:block}
...
.sc-lbl{font-size:9px;font-weight:700;color:var(--muted2);text-transform:uppercase;letter-spacing:.1em}
.sc-val{font-size:19px;font-weight:700;letter-spacing:-.5px;line-height:1.1}
.sc-sub{font-size:10px;color:var(--muted2)}
.act-row{display:flex;gap:10px;padding:14px 20px 0}
...
.abtn-go{background:var(--green);color:#000}
.abtn-stop{background:rgba(255,61,90,.15);color:var(--red);border:1px solid rgba(255,61,90,.3)}
.abtn-sync{background:var(--s2);color:var(--text);flex:0;padding:16px 18px;border:1px solid var(--border)}
...
.card-title{font-size:11px;font-weight:700;color:var(--muted2);text-transform:uppercase;letter-spacing:.1em}
.card-badge{font-size:10px;font-weight:700;padding:3px 9px;border-radius:20px}
.badge-green{background:var(--green-d);color:var(--green);border:1px solid rgba(5,212,124,.2)}
.badge-gold{background:var(--gold-d);color:var(--gold)}
.badge-muted{background:var(--s3);color:var(--muted)}
.eng-body{display:flex;align-items:center;gap:12px;padding:12px 16px 14px}
.eng-dot{width:9px;height:9px;border-radius:50%;background:var(--muted2);flex-shrink:0}
.eng-dot.on{background:var(--green);animation:pulse 2s infinite}
@keyframes pulse{0%{box-shadow:0 0 0 0 rgba(5,212,124,.3)}70%{box-shadow:0 0 0 10px transparent}100%{box-shadow:0 0 0 0 transparent}}
.eng-name{font-size:15px;font-weight:600}
.eng-sub{font-size:12px;color:var(--muted);margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.brain-body{padding:10px 16px 14px}
.brain-text{font-size:13px;line-height:1.55;color:rgba(238,238,245,.75)}
.j-body{padding:12px 16px 16px}
.j-top{display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:14px}
.j-phase{font-size:20px;font-weight:700;letter-spacing:-.5px}
.j-desc{font-size:12px;color:var(--muted);margin-top:3px;line-height:1.35;max-width:190px}
.j-target{text-align:right;flex-shrink:0}
.j-target-lbl{font-size:9px;font-weight:700;color:var(--muted2);text-transform:uppercase;letter-spacing:.1em;margin-bottom:4px}
.j-target-val{font-size:22px;font-weight:700;color:var(--gold);letter-spacing:-.5px}
.j-track{height:8px;background:var(--s3);border-radius:4px;overflow:hidden;margin-bottom:8px}
.j-fill{height:100%;background:linear-gradient(90deg,var(--green),#00ff88);border-radius:4px;transition:width .8s ease}
...
.j-stat-lbl{font-size:9px;font-weight:700;color:var(--muted2);text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px}
.j-stat-val{font-size:14px;font-weight:700}
.gap{height:12px}

/* TRADE PAGE */
.page-hdr{padding:calc(var(--sat)+18px) 20px 12px}
.page-title{font-size:30px;font-weight:700;letter-spacing:-1px}
.page-sub{font-size:13px;color:var(--muted);margin-top:2px}
.sec-lbl{padding:4px 20px 6px;font-size:11px;font-weight:700;color:var(--muted2);text-transform:uppercase;letter-spacing:.12em}
...
.pos-icon{width:44px;height:44px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;flex-shrink:0;background:var(--s3);border:1px solid var(--border2);font-family:var(--mono)}
...
.stag{padding:2px 8px;border-radius:5px;font-size:10px;font-weight:700}
.stag-y{background:var(--green-d);color:var(--green)}
.stag-n{background:var(--red-d);color:var(--red)}
.pos-meta{font-size:10px;color:var(--muted)}
.pos-right{text-align:right;flex-shrink:0;min-width:70px}
.pos-price{font-size:20px;font-weight:700;letter-spacing:-.5px}
.pos-cost{font-size:11px;color:var(--muted);margin-top:2px}
.pos-foot{display:flex;justify-content:space-between;padding:8px 14px;border-top:1px solid var(--border2);background:var(--s2)}
.pf-item{display:flex;flex-direction:column;gap:2px}
.pf-lbl{font-size:9px;font-weight:700;color:var(--muted2);text-transform:uppercase;letter-spacing:.08em}
.pf-val{font-size:12px;font-weight:600;color:var(--muted)}
.empty{text-align:center;padding:40px 20px;color:var(--muted);font-size:14px}

/* ACTIVITY */
.f-row{display:flex;gap:8px;padding:0 20px 14px;overflow-x:auto;scrollbar-width:none}
...
.f-chip.active{background:rgba(255,255,255,.07);color:var(--text);border-color:var(--border)}
...
.amt-pos{color:var(--green)}.amt-neg{color:var(--red)}.amt-zero{color:var(--muted)}
...
.mkt-track{height:5px;background:rgba(255,61,90,.2);border-radius:3px;overflow:hidden}
.mkt-fill{height:100%;background:var(--green);border-radius:3px}

/* SETTINGS */
.set-wrap{padding:calc(var(--sat)+18px) 20px 0}
.set-title{font-size:30px;font-weight:700;letter-spacing:-1px;margin-bottom:20px}
.s-sec{margin-bottom:18px}
.s-sec-lbl{font-size:11px;font-weight:700;color:var(--muted2);text-transform:uppercase;letter-spacing:.12em;margin-bottom:8px;padding:0 4px}
...
.log-wrap{background:var(--s1);border-radius:18px;padding:14px;border:1px solid var(--border);max-height:240px;overflow-y:auto;font-family:var(--mono);font-size:11px;color:rgba(238,238,245,.45);line-height:1.65}
...
<div id="page-home" class="page active">
  ...
  <div class="hero">
    ...
    <div class="hero-bal" id="heroBal" style="color:var(--muted)">---</div>
    <div class="hero-chip chip-flat" id="heroChip">
      <span id="heroArr">↑</span> <span id="heroChange">connecting...</span>
    </div>
    <div class="hero-row">
      <div class="hero-cell"><div class="hc-lbl">Real</div><div class="hc-val" id="heroReal">---</div></div>
      <div class="hero-cell"><div class="hc-lbl">Peak</div><div class="hc-val" id="heroPeak">---</div></div>
      <div class="hero-cell"><div class="hc-lbl">P&L</div><div class="hc-val" id="heroPnl" style="color:var(--green)">---</div></div>
    </div>
  </div>
  <div class="chart-wrap">
    ...
    <div class="chart-svg-wrap"><svg class="mchart" id="mchart" viewBox="0 0 430 140" preserveAspectRatio="none"></svg></div>
  </div>
  <div class="stat-row">
    <div class="stat-card">
      <div class="stat-bar" id="wrBar" style="background:var(--muted2)"></div>
    ...
    <div class="stat-card">
      <div class="stat-bar" id="todayBar" style="background:var(--muted2)"></div>
      <div class="sc-lbl">Today P&L</div>
      <div class="sc-val" id="statToday" style="color:var(--muted)">$0.00</div>
      <div class="sc-sub" id="statTodaySub">all-time</div>
    </div>
    <div class="stat-card">
      <div class="stat-bar" style="background:var(--blue)"></div>
      <div class="sc-lbl">Positions</div>
  ...
  <div class="act-row">
    ...
    <button class="abtn abtn-sync" onclick="doSync()" title="Force sync">&#8635;</button>
  </div>
  <div class="card">
    <div class="card-head"><div class="card-title">Engine</div><div id="modeBadge" class="card-badge badge-muted">PAPER</div></div>
    <div class="eng-body">
      <div class="eng-dot" id="engDot"></div>
      <div style="flex:1;min-width:0">
        <div class="eng-name" id="engName">Stopped</div>
        <div class="eng-sub" id="engSub">Tap Start</div>
      </div>
    </div>
  </div>
  <div class="card">
    <div class="card-head"><div class="card-title">Road to $500K</div><div class="card-badge badge-gold" id="homePhase">--</div></div>
    <div class="j-body">
      <div class="j-top">
        <div><div class="j-phase" id="jPhase">--</div><div class="j-desc" id="jDesc">--</div></div>
        <div class="j-target"><div class="j-target-lbl">Next Target</div><div class="j-target-val" id="jNext">$--</div></div>
      </div>
  ...
  <div class="card" style="margin-bottom:0">
    <div class="card-head"><div class="card-title">AI Brain</div><div class="card-badge badge-muted" id="brainBadge">Signal #0</div></div>
    <div class="brain-body"><div class="brain-text" id="brainText">Waiting for first signal...</div></div>
  </div>
  <div style="text-align:center;font-size:9px;color:rgba(238,238,245,.12);padding:8px;font-family:monospace">v9.6</div>
  <div class="gap"></div>
</div>

<!-- TRADE -->
<div id="page-trade" class="page">
  <div class="page-hdr"><div class="page-title">Positions</div><div class="page-sub" id="tradeSub">0 open</div></div>
  <div id="posList"><div class="empty">No open positions</div></div>
  <div class="sec-lbl" style="margin-top:8px">Recent Signals</div>
  <div id="sigList"><div class="empty" style="padding:16px 20px">No signals yet</div></div>
  <div class="gap"></div>
</div>
...
<div id="page-activity" class="page">
  ...
  <div class="f-row">
    <button class="f-chip active" onclick="filt('all',this)">All</button>
    <button class="f-chip" onclick="filt('win',this)">Wins</button>
    <button class="f-chip" onclick="filt('loss',this)">Losses</button>
    <button class="f-chip" onclick="filt('open',this)">Open</button>
  </div>
  <div id="txFeed"></div>
</div>
...
<div id="page-settings" class="page">
  <div class="set-wrap">
    ...
    <div class="s-sec">
      <div class="s-sec-lbl">Portfolio</div>
      <div class="s-group">
        ...
        <div class="s-row"><div><div class="s-name">All-time P&L</div></div><div class="s-val" id="cfgPnl">$0.00</div></div>
      </div>
    </div>
    <div class="s-sec">
      <div class="s-sec-lbl">Performance</div>
      <div class="s-group">
        <div class="s-row"><div><div class="s-name">Win Rate</div></div><div class="s-val sv-green" id="cfgWR">--</div></div>
        <div class="s-row"><div><div class="s-name">Win / Loss</div></div><div class="s-val" id="cfgWL">--</div></div>
        <div class="s-row"><div><div class="s-name">Today P&L</div></div><div class="s-val" id="cfgToday">$0.00</div></div>
        <div class="s-row"><div><div class="s-name">Total Scans</div></div><div class="s-val" id="cfgScans">--</div></div>
        <div class="s-row"><div><div class="s-name">Brain Cycles</div></div><div class="s-val" id="cfgBrain">--</div></div>
        <div class="s-row"><div><div class="s-name">API Spend</div></div><div class="s-val sv-gold" id="cfgSpend">$0.00</div></div>
      </div>
    </div>
    <div class="s-sec">
      <div class="s-sec-lbl">Config</div>
      <div class="s-group">
        <div class="s-row"><div><div class="s-name">Mode</div></div><div class="s-val" id="cfgMode">--</div></div>
        <div class="s-row"><div><div class="s-name">Phase</div></div><div class="s-val sv-gold" id="cfgPhase">--</div></div>
        <div class="s-row"><div><div class="s-name">Max Positions</div></div><div class="s-val" id="cfgMax">--</div></div>
    ...
    <div class="s-sec">
      <div class="s-sec-lbl">Live Log</div>
...
<nav class="nav">
  <div class="nav-item active" onclick="goPage('home',this)">
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M3 12L5 10M5 10L12 3L19 10M5 10V20C5 20.6 5.4 21 6 21H9M19 10L21 12M19 10V20C19 20.6 18.6 21 18 21H15M9 21V15C9 14.4 9.4 14 10 14H14C14.6 14 15 14.4 15 15V21M9 21H15" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>
    <div class="nav-label">Home</div>
  </div>
  <div class="nav-item" onclick="goPage('trade',this)">
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/><polyline points="16 7 22 7 22 13" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>
    <div class="nav-label">Trade</div>
  </div>
  <div class="nav-item" onclick="goPage('activity',this)">
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="18" height="18" rx="3" stroke="currentColor" stroke-width="1.8"/><line x1="8" y1="9" x2="16" y2="9" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/><line x1="8" y1="13" x2="16" y2="13" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/><line x1="8" y1="17" x2="12" y2="17" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>
...
<script>
/* ── KalshiBot Dashboard v9.6 ── */
var API='', POLL=8000, state=null, chartData=[], allTx=[], currentTF='1D', txFilter='all';
var _retries=0, _loaded=false;

function $(id){return document.getElementById(id);}
function setText(id,v){var e=$(id);if(e)e.textContent=v;}
function setStyle(id,p,v){var e=$(id);if(e)e.style[p]=v;}
function fm(n){return '$'+(Math.abs(isFinite(n)?n:0)).toFixed(2);}
function fs(n){var x=isFinite(n)?n:0;return(x>=0?'+':'')+fm(x);}

function goPage(id,el){
  document.querySelectorAll('.page').forEach(function(p){p.classList.remove('active');});
  document.querySelectorAll('.nav-item').forEach(function(n){n.classList.remove('active');});
  var pg=$(('page-'+id)); if(pg)pg.classList.add('active');
...
function drawChart(){
  var svg=$('mainChart'); if(!svg||chartData.length<2){if(svg)svg.innerHTML='';return;}
  var now=Date.now(),cuts={1H:3600000,1D:86400000,1W:604800000,ALL:1e15};
  var d=chartData.filter(function(p){return p.v>0&&now-p.t<(cuts[currentTF]||1e15);});
  var data=d.length>1?d:chartData.filter(function(p){return p.v>0;});
...
function mktIcon(t){
  if(/BTC|KXBTCD/i.test(t))return{e:'B',cls:'btc'};
  if(/ETH|KXETH/i.test(t))return{e:'E',cls:'eth'};
  if(/NBA|KXNBA/i.test(t))return{e:'B',cls:'nba'};
  if(/NFL|KXNFL/i.test(t))return{e:'F',cls:'nfl'};
  if(/CPI|KXCPI/i.test(t))return{e:'C',cls:'cpi'};
  if(/FED/i.test(t))return{e:'F',cls:'fed'};
  return{e:'~',cls:'gen'};
}

function renderPositions(positions){
  var wrap=$('positionsList'),sub=$('tradeSubtitle'); if(!wrap)return;
  ...
  wrap.innerHTML=positions.map(function(p){
    ...
    var ep=p.entryPrice||50,contracts=p.contracts||1,cost=p.cost||(ep*contracts/100),pnl=p.pnl||0;
    ...
    return '<div class="pos-card"><div class="pos-inner"><div class="pos-icon '+icon.cls+'">'+icon.e+'</div><div class="pos-info"><div class="pos-ticker">'+(p.ticker||'--')+'</div><div class="pos-tags"><span class="side-tag '+(isYes?'tag-yes':'tag-no')+'">'+(isYes?'YES':'NO')+'</span><span class="pos-meta-tag">'+contracts+' ct</span></div></div><div class="pos-right"><div class="pos-price-big">'+ep+'c</div><div class="pos-cost-line">Cost '+fm(cost)+'</div><div class="pos-pnl-line '+pnlCls+'">'+(pnl>=0?'+':'')+fm(pnl)+'</div></div></div><div class="pos-footer"><div class="pf-item"><div class="pf-lbl">Conf</div><div class="pf-val">'+conf+'</div></div><div class="pf-item" style="align-items:center"><div class="pf-lbl">Open</div><div class="pf-val">'+hoursOpen+'h</div></div><div class="pf-item" style="align-items:flex-end"><div class="pf-lbl">Value</div><div class="pf-val">'+fm(ep*contracts/100)+'</div></div></div></div>';
  }).join('');
}

function renderSignals(signals){
  var wrap=$('signalsList'); if(!wrap)return;
...
function renderWLBars(trades){
  var wrap=$('wlBars'); if(!wrap)return;
  var now=Date.now(),buckets=[];
  for(var i=0;i<24;i++)buckets.push({w:0,l:0});
  (trades||[]).forEach(function(t){var hr=Math.floor((now-(t.resolvedAt||t.openedAt||now))/3600000);if(hr<24){if(t.won)buckets[23-hr].w++;else buckets[23-hr].l++;}});
  var mx=Math.max.apply(null,buckets.map(function(b){return b.w+b.l;}));if(mx<1)mx=1;
  wrap.innerHTML=buckets.map(function(b){if(!b.w&&!b.l)return'<div class="wl-bar wl-e" style="height:5px"></div>';return'<div class="wl-bar '+(b.w>=b.l?'wl-w':'wl-l')+'" style="height:'+Math.max(7,((b.w+b.l)/mx)*50)+'px"></div>';}).join('');
  ...
  var el=$('wlSummary');if(el){el.textContent=wins+'W / '+losses+'L';el.style.color=wins>=losses?'var(--green)':'var(--red)';}
}

function buildTx(trades){return(trades||[]).map(function(t){return{type:t.status==='open'?'open':(t.won?'win':'loss'),ticker:t.ticker||'--',pnl:t.pnl||0,cost:t.cost||0,ts:t.resolvedAt||t.openedAt||Date.now(),side:t.side||'YES',contracts:t.contracts||1,won:t.won,status:t.status};});}

function renderTxFeed(txList,filter){
  txFilter=filter||txFilter;var wrap=$('txFeed');if(!wrap)return;
  var fl=txList;
  ...
  wrap.innerHTML=Object.keys(groups).map(function(month){var txs=groups[month];return'<div class="tx-month">'+month+'</div>'+txs.map(function(tx,i){var lbl=tx.type==='win'?'Win':tx.type==='loss'?'Loss':'Open';var iconCls='ti-'+(tx.type==='win'?'win':tx.type==='loss'?'loss':'open');var pnlD=tx.type==='win'&&tx.pnl>0?'+'+fm(tx.pnl):tx.type==='win'?'+$0.00':tx.type==='loss'?'-'+fm(Math.abs(tx.pnl||0)):'-'+fm(Math.abs(tx.cost||0));var pnlC=tx.type==='win'?'tx-amt-pos':tx.type==='loss'?'tx-amt-neg':'tx-amt-zero';return'<div class="tx-row"><div class="tx-icon '+iconCls+'">'+(tx.type==='win'?'W':tx.type==='loss'?'L':'~')+'</div><div class="tx-info"><div class="tx-title">'+lbl+' \xB7 '+tx.ticker+'</div><div class="tx-sub">'+tx.side+' \xB7 '+(tx.contracts||1)+' ct \xB7 '+new Date(tx.ts).toLocaleDateString('en-US',{month:'short',day:'numeric'})+'</div></div><div class="tx-right"><div class="tx-amt '+pnlC+'">'+pnlD+'</div><div class="tx-time">'+new Date(tx.ts).toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit'})+'</div></div></div>'+(i<txs.length-1?'<div class="tx-div"></div>':'');}).join('');}).join('');
}
function filterTx(type,el){document.querySelectorAll('.f-chip').forEach(function(c){c.classList.remove('active');});el.classList.add('active');renderTxFeed(allTx,type);}

function renderMarkets(){
  var wrap=$('marketsList');if(!wrap)return;
  if(!state||!state.topMarkets||!state.topMarkets.length){wrap.innerHTML='<div class="empty-state">'+(state?'Scanner running...':'Waiting for data...')+'</div>';return;}
  wrap.innerHTML=state.topMarkets.slice(0,12).map(function(m){var yp=Math.round((m.yesAsk||0)*100),np=100-yp,hl=m.hoursLeft||999,soon=hl<24,edge=m.edge||0;return'<div class="mkt-card"><div class="mkt-top"><div class="mkt-title">'+(m.title||m.ticker)+'</div><div class="mkt-exp'+(soon?' soon':'')+'">'+( hl<999?(hl<1?'<1h':Math.round(hl)+'h'):'--')+'</div></div><div class="mkt-row"><div class="mkt-yes">'+yp+'c YES</div><div class="mkt-no-block"><div class="mkt-no">'+np+'c NO</div><div class="mkt-vol">vol '+(m.volume||0).toLocaleString()+'</div></div></div><div class="mkt-track"><div class="mkt-fill" style="width:'+yp+'%"></div></div>'+(edge>0?'<span class="mkt-badge '+(edge>0.08?'mb-high':'mb-med')+'">Edge '+(edge*100).toFixed(0)+'%</span>':'')+'</div>';}).join('');
}

function renderPips(done,needed,bankroll){
  var wrap=$('jPips');if(!wrap)return;
  var total=Math.min((done||0)+(needed||16),16),html='';
  for(var i=0;i<total;i++){var amt=(bankroll||50)*Math.pow(2,i+1);var lbl=amt>=1000?'$'+(amt/1000).toFixed(0)+'K':'$'+Math.round(amt);var isDone=i<done,isCur=i===done;html+='<div class="j-pip"><div class="pip-node'+(isDone?' done':isCur?' now':'')+'"></div><div class="pip-lbl'+(isDone?' done':isCur?' now':'')+'">'+lbl+'</div></div>';}
  wrap.innerHTML=html;
}
...
function applyState(s){
  ...
  var hb=$('heroBalance');if(hb){hb.style.fontSize='';hb.style.color='';}
  setText('heroBalance',bal>0?fm(bal):(s.isRunning?'Syncing...':'$0.00'));
  setText('heroReal',real>0?fm(real):(s.isRunning?'...':'$0.00'));
  setText('heroPeak',peak>0?fm(peak):'$0.00');
  var totEl=$('heroTotal');if(totEl){totEl.textContent=fs(total);totEl.style.color=total>=0?'var(--green)':'var(--red)';}
  setText('heroArr',today>=0?'+':'-');setText('heroChangeText',fm(today)+' today');
  var chip=$('heroChip');if(chip)chip.className='hero-chip '+(today>0?'chip-up':today<0?'chip-dn':'chip-flat');
  pushChartPoint(bal);
  var pill=$('modePill'),dot=$('pillDot');
  ...
  var wrEl=$('statWR'),wrBar=$('wrBar');
  if(wr!==null){if(wrEl){wrEl.textContent=wr.toFixed(0)+'%';wrEl.style.color=wr>=70?'var(--green)':wr>=50?'var(--gold)':'var(--red)';}if(wrBar)wrBar.style.background=wr>=70?'var(--green)':wr>=50?'var(--gold)':'var(--red)';}else{if(wrEl){wrEl.textContent='--';wrEl.style.color='var(--muted)';}if(wrBar)wrBar.style.background='var(--muted2)';}
  setText('statWRsub',totalTrades+' resolved');
  var tdEl=$('statToday');if(tdEl){tdEl.textContent=(today>=0?'+':'')+fm(today);tdEl.style.color=today>0?'var(--green)':today<0?'var(--red)':'var(--muted)';}
  setStyle('todayBar','background',today>0?'var(--green)':today<0?'var(--red)':'var(--muted2)');
  setText('statTodaySub',fs(total)+' all-time');
  setText('statPos',openCnt+'/'+mp);setText('statPosSub','of '+mp+' max');
  var running=s.isRunning;
  var edot=$('engineDot');if(edot)edot.className='engine-dot'+(running?' on':'');
  ...
  setText('engineSub',running?'Scan #'+(s.scanCount||0)+' - '+openCnt+' open - $'+avail.toFixed(2)+' avail'+(brainAge!==null?' - brain '+brainAge+'m ago':''):'Tap Start to begin');
  var sb=$('startBtn'),stb=$('stopBtn');if(sb)sb.style.display=running?'none':'flex';if(stb)stb.style.display=running?'flex':'none';
  var badge=$('modeBadge');if(badge){badge.textContent=dryRun?'PAPER':'LIVE';badge.className='card-badge '+(dryRun?'badge-gold':'badge-green');}
  var sig0=s.signals&&s.signals[0];
  if(sig0)setText('brainText',sig0.reasoning||'Analyzing...');
  else if(running)setText('brainText','Scanner active'+(brainAge!==null?' - brain '+brainAge+'m ago':'')+' - hunting for edge...');
  else setText('brainText','Start the bot to begin');
  setText('brainCount','Signal #'+(s.brainCount||0));
  if(s.phaseLabel){
    var dm=s.doublingsMade||0,dn=s.doublingsNeeded||0,pct=parseFloat(s.doublingProgress||0),nxt=parseFloat(s.nextTarget||0);
    setText('homePhaseLabel',s.phaseLabel);setText('jPhaseName',s.phaseLabel);setText('jPhaseDesc',s.phaseDesc||'');
    setText('jNextTarget','$'+nxt.toLocaleString('en-US',{maximumFractionDigits:0}));
    setStyle('jFill','width',Math.min(100,pct)+'%');setText('jPct',pct.toFixed(1)+'%');setText('jLeft',dn+' doubling'+(dn!==1?'s':'')+' to $500K');setText('jDone',dm+'x');
    ...
    setText('cfgPhase',s.phaseLabel);setText('cfgPhaseDesc',s.phaseDesc||'');setText('cfgPct',pct.toFixed(1)+'%');setStyle('cfgFill','width',Math.min(100,pct)+'%');
    setText('cfgCurBal',fm(bal));setText('cfgNextTarget','-> $'+nxt.toLocaleString('en-US',{maximumFractionDigits:0}));setText('cfgDoublings',dm+'x done');setText('cfgNeeded',dn+' left');
    if(s.cfg)setText('cfgKelly',Math.round((s.cfg.kellyFrac||0.55)*100)+'%');
  }
  setText('cfgBankroll',fm(bankroll));
  var cfgWrEl=$('cfgWR');if(cfgWrEl){cfgWrEl.textContent=wr!==null?wr.toFixed(1)+'%':'--';cfgWrEl.className='s-val '+(wr>=70?'sv-green':wr>=50?'sv-gold':wr!==null?'sv-red':'');}
  setText('wrNote',totalTrades+' resolved - '+openCnt+' open');
  var cfgWlEl=$('cfgWL');if(cfgWlEl){cfgWlEl.textContent=w+'W / '+l+'L';cfgWlEl.className='s-val '+(w>l?'sv-green':w<l?'sv-red':'');}
  var tpEl=$('cfgTotalPnl');if(tpEl){tpEl.textContent=fs(total);tpEl.className='s-val '+(total>=0?'sv-green':'sv-red');}
  var tdEl2=$('cfgTodayPnl');if(tdEl2){tdEl2.textContent=fs(today);tdEl2.className='s-val '+(today>=0?'sv-green':'sv-red');}
  setText('cfgScans',(s.scanCount||0).toLocaleString());setText('cfgBrainCount',(s.brainCount||0).toLocaleString());
  setText('cfgReal',fm(real));setText('cfgAvail',fm(avail));setText('cfgPeak',fm(peak));
  var dd=parseFloat(s.drawdown||0);setText('cfgDD',dd.toFixed(1)+'%');setStyle('cfgDDFill','width',Math.min(100,dd)+'%');
  setText('cfgMode',dryRun?'Paper':'LIVE');setText('cfgMaxPos',mp);setText('cfgBrainInt',s.cfg?((s.cfg.brainInterval||120000)/60000).toFixed(1)+'min':'2.0min');setText('cfgOpen',openCnt+' of '+mp);
  var apiSpend=parseFloat(s.estimatedApiSpend||0),uptimeH=s.startedAt?(Date.now()-s.startedAt)/3600000:0,burn=uptimeH>0.5?(apiSpend/uptimeH)*24:0;
  setText('cfgApiSpend','$'+apiSpend.toFixed(2));
  var burnEl=$('cfgDailyBurn');if(burnEl){burnEl.textContent='$'+burn.toFixed(2)+'/day';burnEl.style.color=burn>bal?'var(--red)':'var(--gold)';}
  renderPositions(s.openPositions||[]);renderSignals(s.signals||[]);renderWLBars(s.trades||[]);
  allTx=buildTx(s.trades||[]);renderMarkets();
  var lw=$('logWrap');
  if(lw&&s.logs&&s.logs.length){lw.innerHTML=s.logs.slice(0,20).map(function(l){var lvl=(l.level||'').toLowerCase();var cls=lvl==='error'||lvl==='err'?' class="log-err"':lvl==='warn'?' class="log-warn"':'';return'<div'+cls+'><span class="log-ts">'+(l.ts||'')+'</span>'+(l.msg||l)+'</div>';}).join('');}
}
...
function loadState(){
  ...
  fetch(url,{cache:'no-store'})
    ...
    .then(function(text){
      var s;
      try{s=JSON.parse(text);}catch(e){console.error('JSON parse fail:',text.slice(0,300));throw new Error('Bad JSON');}
      _retries=0;_loaded=true;state=s;
      var eb=$('errBanner'),cb=$('connBanner');
      if(eb)eb.style.display='none';if(cb)cb.style.display='none';
      applyState(s);
    })
    .catch(function(e){
      _retries++;
      console.error('loadState #'+_retries,e.message);
      var cb=$('connBanner'),eb=$('errBanner');
      if(!_loaded){if(cb){cb.style.display='block';cb.textContent='Connecting ('+_retries+'): '+e.message;}}
      else{if(eb){eb.style.display='block';eb.textContent='Lost connection - retrying...';}}
      setTimeout(loadState,Math.min(3000*_retries,15000));
    });
}

function forceSync(){
  var btn=document.querySelector('.abtn-icon');if(btn){btn.style.opacity='0.5';btn.disabled=true;}
  fetch(API+'/api/sync',{method:'POST'}).then(function(r){return r.text();})
    ...
    .finally(function(){if(btn){btn.style.opacity='1';btn.disabled=false;}});
}
function toggleBot(){fetch(API+'/api/toggle',{method:'POST'}).then(function(){setTimeout(loadState,800);});}

for(var _i=15;_i>=0;_i--)chartData.push({t:Date.now()-_i*300000,v:0});
drawChart();
loadState();

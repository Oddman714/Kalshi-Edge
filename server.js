...
/* ERROR BANNER */
#errBanner{display:none;position:fixed;top:0;left:0;right:0;background:var(--red);color:#fff;font-size:12px;font-weight:600;padding:8px 16px;text-align:center;z-index:999}
/* NAV */
...
.hero-row{display:grid;grid-template-columns:1fr 1fr 1fr;background:var(--s1);border-radius:16px;border:1px solid var(--border);overflow:hidden;margin-top:0}
...
.pos-icon{width:44px;height:44px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;border:1px solid var(--border2);background:var(--s3)}
...
.tx-icon{width:42px;height:42px;border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:16px;font-weight:700}
...
<div id="page-home" class="page active">
  ...
  <div class="chart-wrap">
    ...
    <div class="chart-svg-wrap">
      <svg class="main-chart" id="mainChart" viewBox="0 0 430 160" preserveAspectRatio="none"></svg>
    </div>
  </div>
  <div class="stat-row">
    <div class="stat-card">
  ...
  <div class="settings-wrap">
    ...
    <div class="s-section">
      ...
      <div class="s-group">
        ...
        <div class="s-row"><div><div class="s-name">Starting Bankroll</div><div class="s-sub">Actual initial capital from env</div></div><div class="s-val" id="cfgBankroll">$--</div></div>
    ...
    <div class="s-section">
      ...
      <div class="s-group">
        <div class="s-row">
          <div><div class="s-name">Win Rate</div><div class="wr-note s-sub" id="wrNote">-- resolved trades</div></div>
...
// safe get — never throws null ref
...
function setCls(id, cls) { var el = g(id); if (el) el.className = cls; }
...
function drawChart() {
  ...
  var col = up ? '#05d47c' : '#ff3d5a';
  var area = 'M ' + pad + ',' + (H + 4) + ' L ' + pad + ',' + ty(data[0].v) + ' ' +
    data.map(function(p, i) { return 'L ' + tx(i) + ',' + ty(p.v); }).join(' ') +
    ' L ' + (W - pad) + ',' + (H + 4) + ' Z';
  ...
  svg.innerHTML =
    ...
    '<stop offset="90%" stop-color="' + col + '" stop-opacity="0"/>' +
    '</linearGradient></defs>' +
...
function renderWLBars(trades) {
  ...
  var mx = Math.max.apply(null, buckets.map(function(b) { return b.w + b.l; }));
  if (mx < 1) mx = 1;
...
function fmtMoney(n) { return '$' + (Math.abs(n || 0)).toFixed(2); }
function fmtSigned(n) { return (n >= 0 ? '+' : '-') + fmtMoney(n); }

function renderPositions(positions) {
  var wrap = g('positionsList');
  var sub = g('tradeSubtitle');
  if (!wrap) return;
  ...
  wrap.innerHTML = positions.map(function(p) {
    ...
    return '<div class="pos-card">' +
      '<div class="pos-inner">' +
        '<div class="pos-icon ' + icon.cls + '">' + icon.e + '</div>' +
        '<div class="pos-info">' +
          '<div class="pos-ticker">' + (p.ticker || '--') + '</div>' +
          '<div class="pos-tags">' +
            '<span class="side-tag ' + (isYes ? 'tag-yes' : 'tag-no') + '">' + (isYes ? 'YES' : 'NO') + '</span>' +
            '<span class="pos-meta-tag">' + contracts + ' contract' + (contracts !== 1 ? 's' : '') + ' &middot; ' + icon.cls.toUpperCase() + '</span>' +
          '</div>' +
        '</div>' +
        '<div class="pos-right">' +
          '<div class="pos-price-big">' + ep + 'c</div>' +
          '<div class="pos-cost-line">Cost ' + fmtMoney(cost) + '</div>' +
          '<div class="pos-pnl-line ' + pnlCls + '">' + (pnl >= 0 ? '+' : '') + fmtMoney(pnl) + '</div>' +
        '</div>' +
      '</div>' +
      '<div class="pos-footer">' +
        ...
        '<div class="pf-item" style="align-items:flex-end"><div class="pf-lbl">Value</div><div class="pf-val">' + fmtMoney(ep * contracts / 100) + '</div></div>' +
      '</div>' +
    '</div>';
  }).join('');
}
...
function renderSignals(signals) {
  ...
  wrap.innerHTML = signals.slice(0, 5).map(function(s) {
    ...
    return '<div class="sig-card ' + (isYes ? '' : 'sig-no') + '">' +
      '<div class="sig-top"><div class="sig-ticker">' + (s.ticker || '--') + '</div>' +
      '<span class="side-tag ' + (isYes ? 'tag-yes' : 'tag-no') + '">' + (s.side || 'YES') + '</span></div>' +
      ...
      '<div class="sig-stats-row">' +
        ...
        '<div class="sig-stat">Size <strong>' + fmtMoney(s.kellySize || 0) + '</strong></div>' +
  ...
  wrap.innerHTML = Object.keys(groups).map(function(month) {
    ...
    return '<div class="tx-month">' + month + '</div>' +
      txs.map(function(tx, i) {
        ...
        var iconChar = tx.type === 'win' ? 'W' : tx.type === 'loss' ? 'L' : '~';
        var pnlDisp = tx.type === 'win' && tx.pnl > 0 ? '+' + fmtMoney(tx.pnl) :
                      tx.type === 'win' ? '+$0.00' :
                      tx.type === 'loss' ? '-' + fmtMoney(Math.abs(tx.pnl || 0)) :
                      '-' + fmtMoney(Math.abs(tx.cost || 0));
        ...
        return '<div class="tx-row">' +
          ...
          '<div class="tx-info">' +
            ...
            '<div class="tx-sub">' + tx.side + ' &middot; ' + (tx.contracts || 1) + ' contract &middot; ' +
              new Date(tx.ts).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) + '</div>' +
          '</div>' +
          '<div class="tx-right">' +
...
function renderMarkets() {
  var wrap = g('marketsList');
  if (!wrap) return;
  if (!state || !state.topMarkets || !state.topMarkets.length) {
    wrap.innerHTML = '<div class="empty-state">No markets loaded</div>'; return;
  }
  wrap.innerHTML = state.topMarkets.slice(0, 15).map(function(m) {
    ...
    return '<div class="mkt-card">' +
      ...
      '<div class="mkt-exp' + (soon ? ' soon' : '') + '">' +
      (hl < 999 ? (hl < 1 ? '<1h' : Math.round(hl) + 'h') : '--') + '</div></div>' +
      '<div class="mkt-row"><div class="mkt-yes">' + yp + 'c YES</div>' +
      '<div class="mkt-no-block"><div class="mkt-no">' + np + 'c NO</div>' +
      '<div class="mkt-vol">vol ' + (m.volume || 0).toLocaleString() + '</div></div></div>' +
      '<div class="mkt-track"><div class="mkt-fill" style="width:' + yp + '%"></div></div>' +
      (edge > 0 ? '<span class="mkt-badge ' + (edge > 0.08 ? 'mb-high' : 'mb-med') + '">Edge ' + (edge * 100).toFixed(0) + '%</span>' : '') +
    '</div>';
  }).join('');
}
...
function renderPips(done, needed, bankroll) {
  var wrap = g('jPips');
  if (!wrap) return;
  var total = Math.min((done || 0) + (needed || 16), 16);
  var html = '';
  for (var i = 0; i < total; i++) {
    ...
    html += '<div class="j-pip">' +
      '<div class="pip-node' + (isDone ? ' done' : isCurrent ? ' now' : '') + '"></div>' +
      '<div class="pip-lbl' + (isDone ? ' done' : isCurrent ? ' now' : '') + '">' + lbl + '</div>' +
    '</div>';
  }
  wrap.innerHTML = html;
}
...
function loadState() {
  fetch(API + '/api/state')
    ...
    .catch(function(e) {
      ...
      var eb = g('errBanner');
      if (eb) { eb.textContent = 'API error: ' + e.message + ' - retrying...'; eb.style.display = 'block'; }
    });
}
...
function applyState(s) {
  try {
    ...
    // HERO
    set('heroBalance', fmtMoney(bal));
    set('heroReal', fmtMoney(real));
    set('heroPeak', fmtMoney(peak));
    var totEl = g('heroTotal');
    if (totEl) { totEl.textContent = fmtSigned(total); totEl.style.color = total >= 0 ? 'var(--green)' : 'var(--red)'; }
    set('heroArr', today >= 0 ? '+' : '-');
    set('heroChangeText', fmtMoney(today) + ' today');
    var chip = g('heroChip');
    ...
    set('statWRsub', totalTrades + ' resolved');

    // TODAY P&L
    var tdEl = g('statToday');
    if (tdEl) { tdEl.textContent = (today >= 0 ? '+' : '') + fmtMoney(today); tdEl.style.color = today > 0 ? 'var(--green)' : today < 0 ? 'var(--red)' : 'var(--muted)'; }
    setStyle('todayBar', 'background', today > 0 ? 'var(--green)' : today < 0 ? 'var(--red)' : 'var(--muted2)');
    set('statTodaySub', fmtSigned(total) + ' all-time');
    ...
    var edot = g('engineDot');
    if (edot) edot.className = 'engine-dot' + (running ? ' on' : '');
    set('engineName', running ? 'Engine Running' : 'Engine Stopped');
    set('engineSub', running ? ('Scan #' + (s.scanCount || 0) + ' - ' + openCnt + ' open - ' + fmtMoney(avail) + ' avail') : 'Tap Start to begin');
    var sb = g('startBtn'), stopb = g('stopBtn');
    if (sb) sb.style.display = running ? 'none' : 'flex';
    if (stopb) stopb.style.display = running ? 'flex' : 'none';
    ...
    if (sig0) set('brainText', sig0.reasoning || 'Analyzing markets...');
    else if (s.lastBrainReason) set('brainText', s.lastBrainReason);
    set('brainCount', 'Signal #' + (s.brainCount || 0));
    ...
    if (s.phaseLabel) {
      ...
      set('homePhaseLabel', s.phaseLabel);
      set('jPhaseName', s.phaseLabel);
      set('jPhaseDesc', s.phaseDesc || '');
      set('jNextTarget', '$' + nxt.toLocaleString('en-US', { maximumFractionDigits: 0 }));
      setStyle('jFill', 'width', Math.min(100, pct) + '%');
      set('jPct', pct.toFixed(1) + '%');
      set('jLeft', dn + ' doubling' + (dn !== 1 ? 's' : '') + ' to $500K');
      set('jDone', dm + 'x');
      if (s.cfg) {
        set('jKelly', Math.round((s.cfg.kellyFrac || 0.55) * 100) + '%');
        set('jEdge', Math.round((s.cfg.minEdge || 0.04) * 100) + '%');
      }
      renderPips(dm, dn, bankroll);
      // Settings journey
      set('cfgPhase', s.phaseLabel);
      set('cfgPhaseDesc', s.phaseDesc || '');
      set('cfgPct', pct.toFixed(1) + '%');
      setStyle('cfgFill', 'width', Math.min(100, pct) + '%');
      set('cfgCurBal', fmtMoney(bal));
      set('cfgNextTarget', '-> $' + nxt.toLocaleString('en-US', { maximumFractionDigits: 0 }));
      set('cfgDoublings', dm + 'x done');
      set('cfgNeeded', dn + ' left');
      if (s.cfg) set('cfgKelly', Math.round((s.cfg.kellyFrac || 0.55) * 100) + '%');
    }
    set('cfgBankroll', fmtMoney(bankroll));
    ...
    set('wrNote', totalTrades + ' resolved - ' + openCnt + ' open');
    var cfgWlEl = g('cfgWL');
    if (cfgWlEl) { cfgWlEl.textContent = w + 'W / ' + l + 'L'; cfgWlEl.className = 's-val ' + (w > l ? 'sv-green' : w < l ? 'sv-red' : ''); }
    var tpEl2 = g('cfgTotalPnl');
    if (tpEl2) { tpEl2.textContent = fmtSigned(total); tpEl2.className = 's-val ' + (total >= 0 ? 'sv-green' : 'sv-red'); }
    var tdEl2 = g('cfgTodayPnl');
    if (tdEl2) { tdEl2.textContent = fmtSigned(today); tdEl2.className = 's-val ' + (today >= 0 ? 'sv-green' : 'sv-red'); }
    set('cfgScans', (s.scanCount || 0).toLocaleString());
    set('cfgBrainCount', (s.brainCount || 0).toLocaleString());

    // Settings portfolio
    set('cfgReal', fmtMoney(real));
    set('cfgAvail', fmtMoney(avail));
    set('cfgPeak', fmtMoney(peak));
    var dd = parseFloat(s.drawdown || 0);
    set('cfgDD', dd.toFixed(1) + '%');
    setStyle('cfgDDFill', 'width', Math.min(100, dd) + '%');

    // Settings bot
    set('cfgMode', dryRun ? 'Paper' : 'LIVE');
    set('cfgMaxPos', mp);
    set('cfgBrainInt', s.cfg ? ((s.cfg.brainInterval || 90000) / 60000).toFixed(1) + 'min' : '1.5min');
...
function toggleBot() {
  fetch(API + '/api/toggle', { method: 'POST' })
    .then(function() { setTimeout(loadState, 600); })

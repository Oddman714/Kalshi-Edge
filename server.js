...
async function scan(){
  ...
  try{
    // Fetch from multiple Kalshi endpoints to find real liquid markets
    // Primary: /markets with high limit
    // Also try: /markets with event_ticker for known liquid series
    ...
    // DEBUG: log field names and volume distribution every scan
    const m0=allMarkets[0];
    // Kalshi API uses volume_fp (string) not volume
    const getVol=m=>parseFloat(m.volume_fp||m.volume||'0')||0;
    // Kalshi prices are in dollars (0.0-1.0), multiply by 100 for cents
    const getPrice=m=>{
      const raw=m.last_price_dollars||m.last_price||m.yes_ask_dollars||m.yes_bid_dollars||null;
      if(raw==null)return 50;
      const f=parseFloat(raw);
      // If value is 0.0-1.0 range, it's in dollars → convert to cents
      if(f>=0&&f<=1.0)return Math.round(f*100);
      // If already in cents range (0-100), use directly
      return Math.round(f);
    };
    const withVol=allMarkets.filter(m=>getVol(m)>0).length;
    log('SCAN: '+allMarkets.length+' total | '+withVol+' with volume>0');

    // Log top 3 by volume for diagnosis
    const byVol=[...allMarkets].sort((a,b)=>getVol(b)-getVol(a)).slice(0,3);
    byVol.forEach((m,i)=>log('TOP'+i+': ticker='+(m.ticker||m.ticker_name)+' vol='+getVol(m).toFixed(0)+' price='+getPrice(m)+'¢ "'+String(m.title||'').slice(0,40)+'"'));

    const held=new Set(S.positions.map(p=>p.ticker));
    const now=Date.now();

    // Kalshi internal test/parlay market prefixes — no real trading volume
    const junk=['KXMV','KXNHL','KXNBA','KXMLB','KXMVESPORTS','KXMLT'];

    const scored=allMarkets.filter(m=>{
      // Correct ticker field name
      const ticker=m.ticker||m.ticker_name||m.market_ticker||'';
      if(!ticker||held.has(ticker))return false;

      // Skip internal test/parlay markets
      if(junk.some(p=>ticker.startsWith(p)))return false;

      // Require real trading volume (volume_fp is a string in Kalshi API)
      const vol=getVol(m);
      if(vol<100)return false; // minimum $100 volume
      ...
      if(hrs<0.5||hrs>720)return false;

      // Price sanity (in cents after conversion)
      const y=getPrice(m);
      return y>=3&&y<=97;
    }).map(m=>{
      const ticker=m.ticker||m.ticker_name||m.market_ticker;
      ...
      const score=(1-Math.abs(y-50)/50)*0.35+Math.min(vol/100000,1)*0.4+Math.max(0,1-parseFloat(hrs)/48)*0.25;
      const cat=m.event_ticker||m.category||'market';
      return{ticker,title,yes:y,no:100-y,vol,hrs,cat,mom,score};
    });

    scored.sort((a,b)=>b.score-a.score);
    S.cands=scored.slice(0,25);
    log('Scan: '+scored.length+' liquid non-junk → '+S.cands.length+' candidates');
    if(S.cands.length>0){
      const t=S.cands[0];
      log('Top: "'+t.title+'" yes='+t.yes+'¢ vol='+t.vol+' '+t.hrs+'h ['+t.ticker+']');
    }else{
      log('No candidates found. Check TOP0/TOP1/TOP2 logs above for market structure.');
      log('If all markets are KXNHL/KXNBA/KXMLB — your API key only returns sports parlays.');
      log('Solution: Go to kalshi.com → Account → API Keys → delete and recreate key.');
    }
...
app.get('/debug',async(_,res)=>{try{const r=await kalshi('GET','/markets',{status:'open',limit:'3'});const m=r.data.markets||[];const withVol=m.filter(x=>parseFloat(x.volume_fp||x.volume||'0')>0).length;res.json({status:r.status,count:m.length,withVol,fields:m[0]?Object.keys(m[0]):[],sample:m.slice(0,1),cands:S.cands.length,lastErr:S.lastErr?S.lastErr.slice(0,100):null});}catch(e){res.json({error:e.message});}});
